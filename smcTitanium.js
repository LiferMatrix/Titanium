...
const cciCache = new Map(); // NOVO CACHE PARA CCI
...
async function checkDivergenceOtherTF(symbol, type) {
    try {
        ...
        for (const tf of timeframes) {
            ...
            if (hasDiv) {
                console.log(`   ✅ Divergência ${type === 'BUY' ? 'BULLISH' : 'BEARISH'} detectada em ${tf} (segundo TF)`);
                return true;
            }
        }
        return false;
...
async function calculateSmartStop(symbol, entryPrice, tradeType, structureData) {
    ...
    let atrStopPercent = atrData ? Math.min(Math.max(atrData.atrPercent * 1.5, 1.0), 3.0) : 2.0;
    let structureStopPrice = null;
    if (tradeType === 'SELL' && structureData) structureStopPrice = (structureData.sweepPrice || structureData.obPrice) * 1.002;
    if (tradeType === 'BUY' && structureData) structureStopPrice = (structureData.sweepPrice || structureData.obPrice) * 0.998;
    ...
    if (structureStopPrice) {
        ...
        if (structureStopPercent > atrStopPercent) { finalStopPercent = Math.min(structureStopPercent, 4.0); stopType = "ESTRUTURA"; }
    }
...
function isExtremeOversold(rsi, stoch15m) {
    return (rsi && rsi.value <= CONFIG.MONITOR.SMC.EXTREME_OVERSOLD_RSI) || (stoch15m && stoch15m.k <= CONFIG.MONITOR.SMC.EXTREME_OVERSOLD_STOCH);
}

function isExtremeOverbought(rsi, stoch15m) {
    return (rsi && rsi.value >= CONFIG.MONITOR.SMC.EXTREME_OVERBOUGHT_RSI) || (stoch15m && stoch15m.k >= CONFIG.MONITOR.SMC.EXTREME_OVERBOUGHT_STOCH);
}
...
function calculateWeightedScore(analysis) {
    const weights = {
        ...
        divergenceConvergence: 16,
        higherTimeframe: 5,
        cciSignal: 5,      // NOVO: CCI crossover
        lsrSignal: 5,       // NOVO: LSR
        fundingSignal: 5    // NOVO: Funding Rate
    };
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        const [stoch15m, stoch1h] = await Promise.all([
            calculateStochastic(fullSymbol, '15m', 5, 3, 3),
            calculateStochastic(fullSymbol, '1h', 5, 3, 3)
        ]);
        
        // ANALISA CCI NO TIMEFRAME 4h
        ...
        if (isBuyTrendValid) {
            const isOverbought = isExtremeOverbought(rsi, stoch15m);
            const hasRequiredDivergence = hasDiv15mBuy && hasDivOtherBuy;
            
            // VERIFICA CONDIÇÕES ADICIONAIS PARA SCORE
            ...
            if (!isOverbought && hasRequiredDivergence) {
                ...
                // NOVOS PONTOS PARA CCI, LSR E FUNDING
                if (hasCCICrossover) {
                    score += 10;
        ...
        if (isSellTrendValid) {
            const isOversold = isExtremeOversold(rsi, stoch15m);
        ...
        // INFORMAÇÕES DO CCI PARA O ALERTA
        let cciText = '';
        if (cciAnalysis) {
            const cciStatus = tradeSignal === 'BUY' ? (cciAnalysis.crossoverUp ? ' ⤴️' : 'CCI Neutro') : (cciAnalysis.crossoverDown ? '⤵️' : 'CCI Neutro');
            cciText = `\n<i> CCI 4h: ${cciAnalysis.display} | ${cciStatus}</i>`;
        }
        
        // INFORMAÇÕES DE LSR E FUNDING
        let lsrFundText = '';
        if (lsr) {
            const lsrStatus = tradeSignal === 'BUY' ? (lsr.value < 2.5 ? '✅' : '⚠️') : (lsr.value > 2.5 ? '✅' : '⚠️');
            lsrFundText += `\n<i>📊 LSR: ${lsr.value.toFixed(2)} ${lsr.emoji} ${lsrStatus}</i>`;
        }
        if (funding) {
            const fundingPercent = (funding * 100).toFixed(3);
            const fundingStatus = tradeSignal === 'BUY' ? (funding < 0 ? '✅' : '⚠️') : (funding > 0 ? '✅' : '⚠️');
        ...
        const divInfo = tradeSignal === 'BUY' ? 
            `🟢 Divergência Bullish detectada em: ${divTimeframesList.join(', ')}` : 
            `🔴 Divergência Bearish detectada em: ${divTimeframesList.join(', ')}`;
        ...
        return {
            ...
            setupDescription, oiAnalysis, funding: funding ? funding * 100 : null, lsr, rsi, stoch15m, stoch1h,
...
async function sendSMCAlert(analysis) {
    ...
    message += analysis.cciText || '';
    message += analysis.lsrFundText || '';
    ...
    if (analysis.stoch15m) message += `\n<i>Stoch 15m (5,3,3): ${analysis.stoch15m.display}</i>`;
    if (analysis.stoch1h) message += `\n<i>Stoch 1h (5,3,3): ${analysis.stoch1h.display}</i>`;
...
async function scanAndAlert() {
    ...
    try {
        ...
        console.log(`📊 DIVERGÊNCIAS VERIFICADAS EM: 15m (obrigatório) + 30m,1h,2h,4h,12h,1d,3d,1w`);
