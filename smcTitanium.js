...
// =====================================================================
const TELEGRAM_BOT_TOKEN = '7708427979:AAg';
...
// =====================================================================
// === SISTEMA DE MEMÓRIA DE ESTUDO (NOVO) ===
// =====================================================================
class StudyMemory {
    constructor() {
        this.memory = new Map();
    ...
    loadFromFile() {
        try {
            if (fs.existsSync(MEMORY_FILE)) {
                ...
                console.log(`📚 Carregados ${this.memory.size} estudos da memória`);
            }
        } catch (error) {
    ...
    saveToFile() {
        try {
            ...
            console.log(`⚠️ Erro ao salvar memória: ${error.message}`);
        }
    }

    addStudy(symbol, direction, price, analysisData) {
        ...
        this.saveToFile();
        console.log(`📝 MEMÓRIA: ${symbol} ${direction} registrado para estudo (válido por ${expiryHours}h)`);
    }
    ...
    cleanupExpired() {
        ...
        if (removed > 0) {
            this.saveToFile();
            console.log(`🧹 Limpeza de memória: ${removed} estudos expirados removidos`);
        }
    }

    getStats() {
        const stats = { total: this.memory.size, byDirection: { BUY: 0, SELL: 0 } };
        for (const [key, study] of this.memory) {
            if (study.direction === 'BUY') stats.byDirection.BUY++;
            else if (study.direction === 'SELL') stats.byDirection.SELL++;
        }
        return stats;
    }
}
...
function loadDailyCounters() {
    try {
        if (fs.existsSync(DAILY_COUNTER_FILE)) {
            ...
            console.log(`📊 Carregados ${dailyCounter.size} contadores diários`);
        }
    } catch (error) {
        console.log(`⚠️ Erro ao carregar contadores diários: ${error.message}`);
    }
}
...
function saveDailyCounters() {
    try {
        ...
        console.log(`⚠️ Erro ao salvar contadores diários: ${error.message}`);
    }
}
...
function getDailyLimit(symbol) {
    ...
    if (top10.includes(symbol)) return 55;
    
    const top50 = [...top10, 'MATICUSDT', 'DOTUSDT', 'TRXUSDT', 'UNIUSDT', 'ATOMUSDT',
                   'ETCUSDT', 'ICPUSDT', 'FILUSDT', 'NEARUSDT', 'APTUSDT',
                   'LTCUSDT', 'BCHUSDT', 'ARBUSDT', 'OPUSDT', 'INJUSDT',
                   'IMXUSDT', 'STXUSDT', 'HBARUSDT', 'VETUSDT', 'RUNEUSDT',
                   'ALGOUSDT', 'EGLDUSDT', 'MANAUSDT', 'SANDUSDT', 'AXSUSDT',
                   'APEUSDT', 'CHZUSDT', 'GALAUSDT', 'FLOWUSDT', 'THETAUSDT',
                   'AAVEUSDT', 'SUSHIUSDT', 'COMPUSDT', 'SNXUSDT', 'YFIUSDT',
                   'CRVUSDT', 'BALUSDT', '1INCHUSDT', 'ENJUSDT', 'ZILUSDT'];
    
    if (top50.includes(symbol)) return 60;
    return 75;
}

function isDailyLimitReached(symbol) {
    const count = dailyCounter.get(symbol) || 0;
    const limit = getDailyLimit(symbol);
    if (count >= limit) {
        console.log(`📅 Limite diário atingido para ${symbol}: ${count}/${limit}`);
        return true;
    }
    return false;
}

function incrementDailyCounter(symbol) {
    const newCount = (dailyCounter.get(symbol) || 0) + 1;
    dailyCounter.set(symbol, newCount);
    saveDailyCounters();
    return newCount;
}

// =====================================================================
// === COOLDOWN POR TIMEFRAME (NOVO) ===
// =====================================================================
const COOLDOWN_BY_TIMEFRAME = {
    '15m': 10,
    '30m': 10,
    '1h': 10,
    '2h': 10,
...
function getTimeframeCooldown(tradeSignal, timeframe) {
    return COOLDOWN_BY_TIMEFRAME[timeframe] || 30;
}

// =====================================================================
// === CONFIGURAÇÃO COMPLETA COM MELHORIAS ===
// =====================================================================
const CONFIG = {
    TELEGRAM: {
...
// =====================================================================
// === FUNÇÃO PARA DETECTAR REJEIÇÃO DE WICK (NOVA) ===
// =====================================================================
function detectWickRejection(candles, isBuySignal) {
    ...
    try {
        ...
        const timeframes = CONFIG.WICK_REJECTION.CHECK_TIMEFRAMES;
        const minBodyToWickRatio = CONFIG.WICK_REJECTION.MIN_BODY_TO_WICK_RATIO;
        const minWickPercent = CONFIG.WICK_REJECTION.MIN_WICK_PERCENT;
        const maxBodyPercent = CONFIG.WICK_REJECTION.MAX_BODY_PERCENT;
        ...
        for (const tf of timeframes) {
            ...
            // COMPRA (isBuySignal = true) → procura rejeição de BAIXA (wick inferior)
            // VENDA (isBuySignal = false) → procura rejeição de ALTA (wick superior)
            ...
            if (isBuySignal) {
                ...
                if (hasLowerRejection) {
                    const scoreBonus = CONFIG.WICK_REJECTION.SCORE_BONUS;
                    return {
                        hasRejection: true,
        ...
        console.log(`⚠️ Erro ao detectar rejeição de wick: ${error.message}`);
        return { hasRejection: false, scoreBonus: 0, message: '', timeframe: null };
    }
}

// =====================================================================
// === FUNÇÃO DE EMA PARA CONFIRMAÇÃO NO GRÁFICO 3 MINUTOS (NOVA) ===
// =====================================================================
...
async function checkEMAConfirmation(symbol, direction) {
    try {
        const timeframe = CONFIG.MEMORY.CHECK_TIMEFRAME || '3m';
        ...
        const ema13Series = calculateEMASeries(closes, CONFIG.MEMORY.EMA_FAST);
        const ema34Series = calculateEMASeries(closes, CONFIG.MEMORY.EMA_SLOW);
        const ema55Series = calculateEMASeries(closes, CONFIG.MEMORY.EMA_TREND);
        ...
        console.log(`⚠️ Erro ao verificar EMA para ${symbol}: ${error.message}`);
        return { confirmed: false, message: 'Erro na verificação EMA', ema13: null, ema34: null, ema55: null };
    }
}

// =====================================================================
// === FUNÇÃO DINÂMICA DE LSR (NOVO) ===
// =====================================================================
function calculateDynamicLSRThreshold(score, timeframe, hasWickRejection, hasPivotDivergence) {
    let threshold = 2.5; // base para COMPRA
    
    if (score >= 80) threshold -= 0.5;
    if (score >= 65) threshold -= 0.3;
    if (score >= 50) threshold -= 0.2;
    
    if (timeframe === '4h') threshold -= 0.3;
    if (timeframe === '1d') threshold -= 0.5;
    if (timeframe === '1w') threshold -= 0.8;
    
    if (hasWickRejection) threshold -= 0.3;
    if (hasPivotDivergence) threshold -= 0.3;
    
    return Math.max(threshold, 1.5);
}

// =====================================================================
// === FUNÇÃO DINÂMICA DE VOLUME (NOVO) ===
// =====================================================================
function calculateDynamicVolumeThreshold(score, timeframe, isBuy) {
    let threshold = isBuy ? 60 : 40;
    
    if (score >= 80) threshold -= 10;
    if (score >= 65) threshold -= 5;
    if (score >= 50) threshold -= 3;
    
    if (timeframe === '4h') threshold -= 8;
    if (timeframe === '1d') threshold -= 12;
    if (timeframe === '1w') threshold -= 15;
    
    if (isBuy) {
        return Math.max(threshold, 35);
    } else {
        return Math.min(threshold, 65);
    }
}
...
function getClusterStrength(touches) {
    if (touches >= 10) return { text: 'LENDÁRIO', emoji: '💎' };
    if (touches >= 7) return { text: 'EXTREMO', emoji: '🔥' };
...
// =====================================================================
// === SCORE PONDERADO COM NOVAS MELHORIAS ===
// =====================================================================
function calculateWeightedScore(analysis) {
    const weights = {
        ...
        fundingSignal: 5,
        macdDivergence: 10,
        wickRejection: 12,        // NOVO
        studyConfirmation: 15      // NOVO - peso para confirmação EMA
    };
    ...
    if (analysis.hasWickRejection) {
        totalScore += weights.wickRejection;
    }
    maxPossible += weights.wickRejection;
    
    if (analysis.hasStudyConfirmation) {
        totalScore += weights.studyConfirmation;
    }
    maxPossible += weights.studyConfirmation;
...
// =====================================================================
// === FUNÇÃO PARA BUSCAR TODOS OS CANDLES POR TIMEFRAME ===
// =====================================================================
...
// =====================================================================
// === ANÁLISE SMC COMPLETA COM MELHORIAS ===
// =====================================================================
async function analyzeSMC(symbolData) {
    ...
    try {
        const allCandles = await fetchAllCandles(fullSymbol);
        ...
        // =============================================================
        // DETECÇÃO DE WICK (NOVO)
        // =============================================================
        const wickRejection = detectWickRejection(allCandles, false); // temporário, será definido após definir tradeSignal
        ...
        if (isBuyTrendValid) {
            ...
            // Detectar wick para COMPRA
            const wickCheck = detectWickRejection(allCandles, true);
            if (wickCheck.hasRejection) {
                hasWickRejection = true;
                wickBonusAmount = wickCheck.scoreBonus;
                wickMessage = wickCheck.message;
            }
            
            if (cciAnalysis && cciAnalysis.crossoverUp) hasCCICrossover = true;
            if (lsr && lsr.value < calculateDynamicLSRThreshold(0, '15m', hasWickRejection, false)) lsrConfirmed = true;
            ...
            if (!isOverbought && hasRequiredDivergence) {
                ...
                if (hasWickRejection) { 
                    score += wickBonusAmount; 
                    reasons.push(`${wickMessage} (+${wickBonusAmount})`);
                }
        ...
        if (isSellTrendValid) {
            ...
            // Detectar wick para VENDA
            const wickCheck = detectWickRejection(allCandles, false);
            if (wickCheck.hasRejection) {
                hasWickRejection = true;
                wickBonusAmount = wickCheck.scoreBonus;
                wickMessage = wickCheck.message;
        ...
        // SISTEMA DE MEMÓRIA DE ESTUDO (NOVO)
        // =============================================================
        const directionStr = tradeSignal === 'BUY' ? 'BUY' : 'SELL';
        const existingStudy = studyMemory.getStudy(fullSymbol, directionStr);
        
        if (existingStudy) {
            console.log(`📚 ${fullSymbol} - Estudo ativo encontrado para ${directionStr}`);
            
            const emaConfirmation = await checkEMAConfirmation(fullSymbol, directionStr);
            
            if (emaConfirmation.confirmed) {
                console.log(`✅ ${fullSymbol} - CONFIRMAÇÃO OBTIDA! Enviando alerta...`);
                studyMemory.removeStudy(fullSymbol, directionStr);
            } else {
                console.log(`⏳ ${fullSymbol} - Aguardando confirmação EMA: ${emaConfirmation.message}`);
                return null;
            }
        } else {
            // Verificar se precisa registrar estudo (falta confirmação de candle)
            const isConfirmed = await getCandleConfirmation(fullSymbol, '15m', tradeSignal);
            
            if (!isConfirmed && CONFIG.MEMORY.ENABLED) {
                const studyData = {
                    symbol: fullSymbol,
                    direction: directionStr,
                    price: price,
                    analysisData: { setupDescription, score: 0 }
                };
                studyMemory.addStudy(fullSymbol, directionStr, price, null, null, 0, studyData);
                console.log(`📚 ${fullSymbol} - NOVO ESTUDO registrado para ${directionStr}. Aguardando confirmação EMA...`);
                return null;
            }
        }
        ...
        // VERIFICAÇÕES FINAIS
        // =============================================================
        const isConfirmedCandle = await getCandleConfirmation(fullSymbol, '15m', tradeSignal);
        if (!isConfirmedCandle) return null;
        
        // Limite diário
        if (CONFIG.DAILY_LIMITS.ENABLED && isDailyLimitReached(fullSymbol)) return null;
        
        // Cooldown
        if (!canAlert(fullSymbol, tradeSignal, 0)) return null;
        ...
        const weightedScore = calculateWeightedScore({
            ...
            hasMACDDivergence,
            hasWickRejection,
            hasStudyConfirmation: existingStudy ? true : false
        });
        ...
        if (CONFIG.DAILY_LIMITS.ENABLED) {
            incrementDailyCounter(fullSymbol);
        }
        
        // Marcar alerta para cooldown
        markAlerted(fullSymbol, tradeSignal, weightedScore);
        ...
        return {
            ...
            wickDetected: hasWickRejection,
            wickMessage: wickMessage,
            wickBonus: wickBonusAmount
        };
...
// =====================================================================
// === ENVIAR ALERTA COM MENSAGEM OTIMIZADA ===
// =====================================================================
async function sendSMCAlert(analysis) {
    ...
    // Linha 1: Título
    messageLines.push(`<b>${tradeEmoji} ${tradeText} ${analysis.symbol}</b> - 💲 ${formatPrice(analysis.price)}`);
    
    // Linha 2: Score
    messageLines.push(`<i>${dt.date.slice(0,5)} ${dt.time.slice(0,5)}hs</i> ${tvLinkText}`);
    messageLines.push(`🔍 <b>#SCORE: ${analysis.score}</b> <i>${scoreMedal}</i>`);
    
    // Linha 3: Divergências
    if (analysis.divergenceInfo) {
        messageLines.push(`<i>• Divergência RSI de ${analysis.tradeSignal === 'SELL' ? 'BAIXA' : 'ALTA'} (${analysis.divergenceInfo})</i>`);
    }
    
    // Linha 4: MACD
    if (analysis.hasMACDDivergence && analysis.macdDivergenceInfo) {
        messageLines.push(`<i>• ${analysis.macdDivergenceInfo}</i>`);
    }
    
    // Linha 5: Wick (NOVO)
    if (analysis.wickDetected && analysis.wickMessage) {
        messageLines.push(`<i>• ${analysis.wickMessage} (+${analysis.wickBonus})</i>`);
    }
    
    // Linha 6: FVG
    if (analysis.bullishFVG || analysis.bearishFVG) {
        const fvgPrice = analysis.tradeSignal === 'SELL' ? analysis.bearishFVG?.targetPrice : analysis.bullishFVG?.targetPrice;
        const fvgType = analysis.tradeSignal === 'SELL' ? 'Bearish' : 'Bullish';
        if (fvgPrice) {
            messageLines.push(`<i>• FVG ${fvgType} a ${formatPrice(fvgPrice)}</i>`);
        }
    }
    
    // Linha 7: Estrutura
    if (analysis.structureBreaks && analysis.structureBreaks.description) {
        messageLines.push(`<i>• ${analysis.structureBreaks.description}</i>`);
    }
    
    // Linha 8: CCI
    messageLines.push(`<i>• CCI 4h: ${analysis.cci4hDisplay}</i>`);
    messageLines.push(`<i>• CCI 1D: ${analysis.cciDailyDisplay}</i>`);
    
    // Linha 9: Entrada
    messageLines.push(`<b>📊 Entrada:</b> <i>${formatPrice(analysis.entryPrice)}</i>`);
    messageLines.push(`<b>📈 Fibo_OTE:</b> <i>${analysis.oteText}</i>`);
    
    // Linha 10: Alvos
    messageLines.push(`<b>🎯 Alvos:</b> <i>${analysis.targetsText}</i>`);
    
    // Linha 11: Stop
    messageLines.push(`<b>🛑 Stop:</b> <i>${formatPrice(analysis.stopPrice)} (${analysis.stopPercent.toFixed(0)}%) [${analysis.stopType}]</i>`);
    messageLines.push(`<i>   └─ ${analysis.stopComponents}</i>`);
    
    // Linha 12: Zona
    if (analysis.premiumDiscount) {
        messageLines.push(`<b>📍 Zona:</b> <i>${analysis.premiumDiscount.emoji} ${analysis.premiumDiscount.zone}</i>`);
    }
    
    // Linha 13: Indicadores
    messageLines.push(`<b>📊 Indicadores:</b>`);
    messageLines.push(`<i>• RSI ${analysis.rsiValue} (${analysis.rsiStatus}) ${analysis.rsiEmoji}</i>`);
    if (analysis.stoch4hFormatted && analysis.stochDailyFormatted) {
        messageLines.push(`<i>• Stoch 4h: ${analysis.stoch4hFormatted} | Diário: ${analysis.stochDailyFormatted}</i>`);
    }
    messageLines.push(`<i>• #LSR ${analysis.lsrValue} ${analysis.lsrCheck}</i>`);
    messageLines.push(`<i>• Funding ${analysis.fundingValue}% (${analysis.fundingStatus}) ${analysis.fundingCheck}</i>`);
    
    // Linha 14: Suporte/Resistência
    messageLines.push(`<b>🔺 RESISTÊNCIA:</b> <i>${analysis.resistanceShort}</i>`);
    messageLines.push(`<b>🔻 SUPORTE:</b> <i>${analysis.supportShort}</i>`);
    
    // Linha 15: Rodapé
    messageLines.push(`<i>✨ Titanium SMC by @J4Rviz</i>`);
    
    // Filtrar linhas vazias e juntar
    const finalMessage = messageLines.filter(line => line && line.trim() !== '').join('\n');
...
async function scanAndAlert() {
    ...
    try {
        ...
        console.log(`📚 Memória: ${memoryStats.total} estudos ativos (BUY: ${memoryStats.byDirection.BUY}, SELL: ${memoryStats.byDirection.SELL})`);
...
async function startMonitorWithReconnect() {
    ...
    console.log('\n' + '='.repeat(70));
    console.log('⚡ TITANIUM SMC - VERSÃO MELHORADA ⚡');
    console.log('='.repeat(70));
    console.log(`✅ Monitor 15m ativo | SCORE MÍNIMO: ${CONFIG.MONITOR.MIN_SCORE_ACCEPT}`);
    console.log(`🕯️ Rejeição de Wick: ATIVADA | Bônus: +${CONFIG.WICK_REJECTION.SCORE_BONUS}`);
    console.log(`📚 Memória de estudo: ATIVADA | Expira em ${CONFIG.MEMORY.EXPIRY_HOURS}h`);
    console.log(`📅 Limite diário: ATIVADO | Top10:55 | Top50:60 | Outros:75`);
    console.log('='.repeat(70));
...
// =====================================================================
process.on('SIGINT', () => {
    console.log('🛑 Desligando Titanium SMC...');
    studyMemory.saveToFile();
    saveDailyCounters();
    process.exit(0);
});
