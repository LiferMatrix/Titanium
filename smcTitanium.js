...
// =====================================================================
// === DETECÇÃO DE DIVERGÊNCIAS (MULTI-TIMEFRAME COMPLETO) ===
// =====================================================================
...
// FUNÇÃO ATUALIZADA: Verifica divergência em timeframe específico
...
// FUNÇÃO ATUALIZADA: Verifica divergência em MÚLTIPLOS TIMEFRAMES (15m + 2h + 12h + 1d + 3d + 1w)
async function checkDivergenceMultipleTimeframes(symbol, type) {
    try {
        // Lista completa de timeframes para verificar
        const timeframesToCheck = ['15m', '2h', '12h', '1d', '3d', '1w'];
        const results = {};
        ...
        for (const tf of timeframesToCheck) {
            try {
                ...
                results[tf] = hasDiv;
                if (hasDiv) {
                    console.log(`   ✅ Divergência ${type === 'BUY' ? 'BULLISH' : 'BEARISH'} detectada em ${tf}`);
                }
            } catch (error) {
                results[tf] = false;
            }
        ...
        return { '15m': false, '2h': false, '12h': false, '1d': false, '3d': false, '1w': false };
    }
}

// FUNÇÃO ATUALIZADA: Verifica se tem divergência no 15m
async function checkDivergence15m(symbol, type) {
    return await checkDivergenceOnTimeframe(symbol, '15m', type);
}

// FUNÇÃO ATUALIZADA: Verifica se tem divergência em OUTROS TIMEFRAMES (2h, 12h, 1d, 3d, 1w)
async function checkDivergenceOtherTF(symbol, type) {
    try {
        const timeframes = ['2h', '12h', '1d', '3d', '1w'];
...
// FUNÇÃO: Retorna todos os timeframes com divergência detectada
async function getDivergenceTimeframesList(symbol, type) {
    try {
        const timeframes = ['15m', '2h', '12h', '1d', '3d', '1w'];
        const detected = [];
...
function calculateWeightedScore(analysis) {
    const weights = {
        timeframeAlignment: 25,
        fvgValidity: 20,
        liquiditySweep: 15,
        orderBlockMitigation: 15,
        volumeConfirmation: 10,
        divergenceConvergence: 10,
        higherTimeframe: 5
    };
...
// =====================================================================
// === ANÁLISE SMC COMPLETA COM DIVERGÊNCIAS MÚLTIPLAS ===
// =====================================================================
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        const bullishFVGValidation = fvgAnalysis4h?.bullishValidation || fvgAnalysis1h?.bullishValidation;
        const bearishFVGValidation = fvgAnalysis4h?.bearishValidation || fvgAnalysis1h?.bearishValidation;
        ...
        const valueArea = getValueArea(oneHourCandles);
        const premiumDiscount = calculatePremiumDiscount(price, valueArea.low, valueArea.high);
        
        const recentHigh = Math.max(...oneHourCandles.slice(-20).map(c => c.high));
        const recentLow = Math.min(...oneHourCandles.slice(-20).map(c => c.low));
        ...
        const [stochDaily, stoch4h] = await Promise.all([
        ...
        // VERIFICA DIVERGÊNCIAS OBRIGATÓRIAS EM MÚLTIPLOS TIMEFRAMES
        ...
        // LISTA DE TIMEFRAMES COM DIVERGÊNCIA
        ...
        // =============================================================
        // VENDA (SELL) - Com divergência de baixa obrigatória
        // =============================================================
        if (dailyEMA.trend === 'BAIXA' && !dailyEMA.isAboveEMA13 && !dailyEMA.isAboveEMA34 && !dailyEMA.isAboveEMA55) {
            const isOversold = isExtremeOversold(rsi, stoch4h);
            
            // CONDIÇÃO OBRIGATÓRIA: Divergência de baixa no 15m E em outro timeframe (2h,12h,1d,3d,1w)
            const hasRequiredDivergence = hasDiv15mSell && hasDivOtherSell;
            
            if (!isOversold && hasRequiredDivergence) {
                let score = 0, reasons = [];
                const divergenciaTFs = sellDivTimeframes.join(', ');
                ...
                if (divergences.hasBearish) { score += 15; reasons.push(`${divergences.bearish[0]?.emoji || '🔴'} Divergência Bearish`); hasDivergence = true; }
        ...
        // =============================================================
        // COMPRA (BUY) - Com divergência de alta obrigatória
        // =============================================================
        if (dailyEMA.trend === 'ALTA' && dailyEMA.isAboveEMA13 && dailyEMA.isAboveEMA34 && dailyEMA.isAboveEMA55) {
            const isOverbought = isExtremeOverbought(rsi, stoch4h);
            
            // CONDIÇÃO OBRIGATÓRIA: Divergência de alta no 15m E em outro timeframe (2h,12h,1d,3d,1w)
            const hasRequiredDivergence = hasDiv15mBuy && hasDivOtherBuy;
            
            if (!isOverbought && hasRequiredDivergence) {
                let score = 0, reasons = [];
                const divergenciaTFs = buyDivTimeframes.join(', ');
                ...
                if (divergences.hasBullish) { score += 15; reasons.push(`${divergences.bullish[0]?.emoji || '🟢'} Divergência Bullish`); hasDivergence = true; }
        ...
        // INFORMAÇÃO DAS DIVERGÊNCIAS PARA O ALERTA
        const divTimeframesList = tradeSignal === 'BUY' ? buyDivTimeframes : sellDivTimeframes;
        const divInfo = tradeSignal === 'BUY' ? 
            `🟢 Divergência Bullish detectada em: 15m, ${divTimeframesList.filter(tf => tf !== '15m').join(', ')}` : 
            `🔴 Divergência Bearish detectada em: 15m, ${divTimeframesList.filter(tf => tf !== '15m').join(', ')}`;
        ...
        return {
            ...
            oteText,
            volumeConfirmed,
            fvgValidation: tradeSignal === 'BUY' ? bullishFVGValidation : bearishFVGValidation,
...
async function sendSMCAlert(analysis) {
    ...
    message += `<i>${analysis.higherTimeframe.marketPhase}</i>\n`;
    message += `<i>${analysis.conflictAnalysis.message}</i>\n`;
    message += `<i>${analysis.divergenceInfo}</i>\n`;
    ...
    message += analysis.oteText;
    message += analysis.premiumDiscount;
    message += analysis.breakerText;
    
    if (analysis.fvgValidation?.confirmed) {
        message += `\n<i> FVG Validado em ${analysis.fvgValidation.multipleTimeframes} timeframes (${analysis.fvgValidation.strength})</i>`;
    }
    
    message += `\n<i>🔺#Resistência: ${analysis.resistancesText || 'N/A'}</i>`;
    message += `\n<i>🔻#Suporte: ${analysis.supportsText || 'N/A'}</i>`;
    
    if (analysis.bullishFVG) {
        message += `\n<i>📍FVG Bull: ${formatPrice(analysis.bullishFVG.targetPrice)} (${analysis.bullishFVG.displayDistance}) | Gap: ${analysis.bullishFVG.gapSize.toFixed(2)}%</i>`;
    }
    if (analysis.bearishFVG) {
        message += `\n<i>📍FVG Bear: ${formatPrice(analysis.bearishFVG.targetPrice)} (${analysis.bearishFVG.displayDistance}) | Gap: ${analysis.bearishFVG.gapSize.toFixed(2)}%</i>`;
    }
    if (analysis.structureBreaks && analysis.structureBreaks.description !== '❌ Sem quebra recente') {
        message += `\n<i>   ${analysis.structureBreaks.description}</i>`;
    }
    
    if (analysis.divergences && analysis.divergences.summary !== '❌ Sem divergências') {
        message += `\n<i>   ${analysis.divergences.summary}</i>`;
    }
    
    if (analysis.volumeConfirmed) {
        message += `\n<i>   ✅ Volume Confirmado</i>`;
    }
    ...
    message += `\n<i>LSR ${lsrEmoji} ${lsrValue} | Funding ${fundingValue}%</i>`;
    if (analysis.oiAnalysis) message += `\n<i>RSI ${rsiValue}${rsiEmoji} | OI: ${analysis.oiAnalysis.display}</i>`;
    
    if (analysis.stoch4h) message += `\n<i>Stoch 4h (5,3,3): ${analysis.stoch4h.display}</i>`;
    if (analysis.stochDaily) message += `\n<i>Stoch Diário (5,3,3): ${analysis.stochDaily.display}</i>`;
    
    message += `\n<i>✨Titanium by @J4Rviz - Divergências Multi-TF (15m,2h,12h,1d,3d,1w)</i>`;
...
async function scanAndAlert() {
    ...
    try {
        ...
        console.log(`📊 DIVERGÊNCIAS VERIFICADAS EM: 15m, 2h, 12h, 1d, 3d, 1w`);
        let alertsSent = 0;
...
// =====================================================================
process.on('SIGINT', () => {
    console.log('🛑 Desligando Titanium PRO Monitor...');
    process.exit(0);
});
