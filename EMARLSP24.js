...
const TELEGRAM_BOT_TOKEN = '7708427979:AAF7vVx6g';
...
const ZONE_SETTINGS = {
    timeframe: '15m',
    lookbackPeriod: 100,
    proximityThreshold: 1.0, // ✅ ALTERADO PARA 1%
    checkInterval: 30000,
    bidAskZoneSize: 0.25,
...
const EMA_ZONE_SETTINGS = {
    ...
    alertGroups: 20,
    // Configurações para zona de suporte/resistência - ✅ ALTERADO PARA 1%
    zoneProximity: 1.0, // ✅ ALTERADO PARA 1%
    zoneTimeframe: '15m',
    minZoneStrength: 1,
    ...
    minStopDistancePercent: 1.0, // ✅ AUMENTADO PARA 1%
...
// =====================================================================
// 🕯️ CONFIGURAÇÕES PARA PADRÕES DE CANDLES ===
// =====================================================================
...
// =====================================================================
// 🕯️ FUNÇÃO PARA DETECTAR PADRÕES DE CANDLES (3M)
// =====================================================================
...
async function detectCandlePatterns(symbol) {
    try {
        ...
        // Verificar padrões em candles individuais
        for (let i = 1; i < recentCandles.length; i++) {
            const current = recentCandles[i];
            const previous = recentCandles[i-1];
            
            // Calcular características da vela
            ...
            // 4. ENGULFING (só se tiver candle anterior)
            if (i >= 1) {
            ...
            if (i >= 2) {
                const first = recentCandles[i-2];
                const second = recentCandles[i-1];
                const third = current;
                
                const firstIsBearish = first.close < first.open;
                const secondIsSmall = Math.abs(second.close - second.open) < first.bodySize * 0.5;
                const thirdIsBullish = third.close > third.open;
                const thirdClosesAboveFirstMid = third.close > (first.open + first.close) / 2;
                
                if (firstIsBearish && secondIsSmall && thirdIsBullish && thirdClosesAboveFirstMid) {
                    patterns.push('MORNING_STAR');
                    details.push({
                        type: 'MORNING_STAR',
                        emoji: '🌅',
                        confidence: 'ALTA',
                        description: 'Estrela da Manhã - Reversão alcista forte'
                    });
                }
            }
            
            // 6. ESTRELA DA TARDE (EVENING STAR) - precisa de 3 candles
            if (i >= 2) {
                const first = recentCandles[i-2];
                const second = recentCandles[i-1];
                const third = current;
                ...
                const thirdClosesBelowFirstMid = third.close < (first.open + first.close) / 2;
                
                if (firstIsBullish && secondIsSmall && thirdIsBearish && thirdClosesBelowFirstMid) {
                    patterns.push('EVENING_STAR');
                    details.push({
                        type: 'EVENING_STAR',
                        emoji: '🌇',
                        confidence: 'ALTA',
                        description: 'Estrela da Tarde - Reversão baixista forte'
                    });
                }
            }
            
            // 7. MARUBOZU (corpo grande, pavios pequenos)
            if (bodyPercent >= 80 && totalRange > 0) {
                const isBullishMarubozu = current.close > current.open;
                patterns.push(isBullishMarubozu ? 'MARUBOZU_BULLISH' : 'MARUBOZU_BEARISH');
                details.push({
                    type: isBullishMarubozu ? 'MARUBOZU_BULLISH' : 'MARUBOZU_BEARISH',
                    emoji: isBullishMarubozu ? '📈' : '📉',
                    confidence: 'ALTA',
                    description: `Marubozu ${isBullishMarubozu ? 'Alcista' : 'Baixista'} (corpo ${bodyPercent.toFixed(0)}% do range)`
                });
            }
        }
        ...
        if (patterns.length > 0) {
            ...
            // Encontrar padrão mais frequente
            let maxCount = 0;
            let mostFrequent = '';
            ...
            // Mapear padrão para descrição amigável
            const patternMap = {
                'DOJI': 'Doji (Indecisão)',
                ...
                'MARUBOZU_BULLISH': 'Marubozu Alcista',
                'MARUBOZU_BEARISH': 'Marubozu Baixista'
            };
            
            dominantPattern = patternMap[mostFrequent] || 'Padrão Detectado';
            
            // Definir emoji baseado no tipo de padrão
            if (mostFrequent.includes('BULLISH') || mostFrequent === 'MORNING_STAR' || mostFrequent === 'HAMMER_BULLISH') {
                patternEmoji = '🟢';
                confidence = 75;
            } else if (mostFrequent.includes('BEARISH') || mostFrequent === 'EVENING_STAR' || mostFrequent === 'SHOOTING_STAR' || mostFrequent === 'HAMMER_BEARISH') {
                patternEmoji = '🔴';
                confidence = 75;
        ...
        const result = {
            ...
            details: details.slice(-3), // Últimos 3 padrões detectados
            totalPatterns: patterns.length,
            lastCandle: recentCandles[recentCandles.length - 1]
        };
...
async function checkZoneThenEMA(symbol) {
    try {
        ...
        const allCriteriaMet = 
            atrData.meetsVolatilityCriteria &&
            (!EMA_ZONE_SETTINGS.requireVolumeSpike || volumeAnalysis.meetsVolumeSpikeCriteria) &&
            finalConfidence >= 70;
...
// =====================================================================
// 🆕 FUNÇÃO PARA ENVIAR ALERTA COM PADRÕES DE CANDLES
// =====================================================================
...
async function sendZoneEMAAlert(setupData) {
    try {
        ...
        const [rsiData, lsrData, fundingData, btcStrength, atrTargets, candlePatterns] = await Promise.allSettled([
            getRSI(symbol, '1h'),
        ...
        const candlePatternInfo = candlePatterns.status === 'fulfilled' && candlePatterns.value ? 
            `${candlePatterns.value.patternEmoji} ${candlePatterns.value.dominantPattern}` : 'N/A';
        
        let candlePatternDetails = '';
        if (candlePatterns.status === 'fulfilled' && candlePatterns.value && candlePatterns.value.details.length > 0) {
            const lastPatterns = candlePatterns.value.details.slice(-2);
        ...
        // MENSAGEM COMPLETA COM PADRÕES DE CANDLES
        const message = `
...
• Reação/Toques: ${zone.strength || 1}/5
...
• Volatilidade (ATR 15m): ${atrData?.volatilityEmoji || '⚪'} ${atrData?.volatilityLevel || 'N/A'} (${atrData?.atrPercent?.toFixed(2) || '0.00'}%)

<i>Padrões de Candles:</i>
...
class ZoneEMAMonitor {
    ...
    async initializeSymbols() {
        try {
            ...
            console.log(`   • Zona de preço: ≤ ${EMA_ZONE_SETTINGS.zoneProximity}% do suporte/resistência (15m)`); // ✅ ATUALIZADO
...
async function mainZoneEMAMonitorLoop() {
    ...
    console.log(`🕯️  Padrões de Candles: Análise em tempo real (3m)`);
...
async function startZoneEMABot() {
    try {
        ...
        console.log(`🕯️  Padrões de Candles: Análise em tempo real (Doji, Martelo, Engulfing, etc.)`);
        console.log(`💰 Volume mínimo: $${(EMA_ZONE_SETTINGS.minVolumeUSD/1000).toFixed(0)}k 24h`);
        console.log(`📍 Proximidade: ${EMA_ZONE_SETTINGS.zoneProximity}% da zona`); // ✅ ATUALIZADO
        console.log(`⚡ Rate Limit: 2400/min, 100/seg - Sistema adaptativo inteligente`);
        console.log('⚠️  Alerta só após setup completo (Zona → EMA → Volume → Volatilidade → Preço/EMA55 → Padrões Candles)');
