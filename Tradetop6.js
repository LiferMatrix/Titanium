...
// =====================================================================
const TELEGRAM_BOT_TOKEN = '7633398974:';
...
// =====================================================================
// === FUNÇÃO PARA VERIFICAR REJEIÇÃO DE WICK NO TIMEFRAME 15m ===
// =====================================================================
async function check15mWickRejection(symbol, expectedDirection) {
    try {
        const candles = await getCandles(symbol, '15m', 5);
        if (!candles || candles.length < 2) return { hasRejection: false, description: 'Sem candles suficientes' };
        
        const lastCandle = candles[candles.length - 1];
        const bodySize = Math.abs(lastCandle.close - lastCandle.open);
        
        if (bodySize === 0) return { hasRejection: false, description: 'Doji - sem corpo' };
        ...
        if (expectedDirection === 'BUY') {
            ...
            if (hasRejection) {
                const strength = lowerWick >= (bodySize * 3) ? 'EXTREMA' : 'FORTE';
                return { 
                    hasRejection: true, 
                    description: `🕯️ REJEIÇÃO DE BAIXA (martelo) 15m | Wick ${(lowerWick/bodySize).toFixed(1)}x corpo ${strength}`,
                    strength: strength
                };
            }
            return { hasRejection: false, description: `Sem rejeição de baixa 15m | Wick ${(lowerWick/bodySize).toFixed(1)}x corpo` };
        }
        
        if (expectedDirection === 'SELL') {
            const hasRejection = upperWick >= (bodySize * 2);
            if (hasRejection) {
                const strength = upperWick >= (bodySize * 3) ? 'EXTREMA' : 'FORTE';
                return { 
                    hasRejection: true, 
                    description: `🕯️ REJEIÇÃO DE ALTA (estrela) 15m | Wick ${(upperWick/bodySize).toFixed(1)}x corpo ${strength}`,
                    strength: strength
                };
            }
            return { hasRejection: false, description: `Sem rejeição de alta 15m | Wick ${(upperWick/bodySize).toFixed(1)}x corpo` };
        }
        
        return { hasRejection: false, description: 'Direção inválida' };
    } catch (error) {
        return { hasRejection: false, description: `Erro: ${error.message}` };
    }
}

// =====================================================================
// === FUNÇÃO PARA VERIFICAR SE É RETESTE DE FVG ===
// =====================================================================
function isFVGRetest(fvgData, currentPrice, direction) {
    if (direction === 'bullish' && fvgData.bullish.length > 0) {
        // Verifica se o preço está dentro de algum FVG Bullish
        for (const fvg of fvgData.bullish) {
            if (currentPrice >= fvg.bottom && currentPrice <= fvg.top) {
                return { isRetest: true, timeframe: fvg.timeframe, type: 'bullish', description: `🔄 RETESTE DE FVG BULLISH (${fvg.timeframe}) - Preço dentro do gap` };
            }
        }
        // Verifica se o preço está muito próximo do FVG (até 0.5% acima)
        for (const fvg of fvgData.bullish) {
            if (currentPrice > fvg.top) {
                const distance = ((currentPrice - fvg.top) / currentPrice) * 100;
                if (distance <= 0.5) {
                    return { isRetest: true, timeframe: fvg.timeframe, type: 'bullish', description: `🔄 RETESTE DE FVG BULLISH (${fvg.timeframe}) - Preço ${distance.toFixed(2)}% acima do gap` };
                }
            }
        }
    }
    
    if (direction === 'bearish' && fvgData.bearish.length > 0) {
        // Verifica se o preço está dentro de algum FVG Bearish
        for (const fvg of fvgData.bearish) {
            if (currentPrice >= fvg.bottom && currentPrice <= fvg.top) {
                return { isRetest: true, timeframe: fvg.timeframe, type: 'bearish', description: `🔄 RETESTE DE FVG BEARISH (${fvg.timeframe}) - Preço dentro do gap` };
            }
        }
        // Verifica se o preço está muito próximo do FVG (até 0.5% abaixo)
        for (const fvg of fvgData.bearish) {
            if (currentPrice < fvg.bottom) {
                const distance = ((fvg.bottom - currentPrice) / currentPrice) * 100;
                if (distance <= 0.5) {
                    return { isRetest: true, timeframe: fvg.timeframe, type: 'bearish', description: `🔄 RETESTE DE FVG BEARISH (${fvg.timeframe}) - Preço ${distance.toFixed(2)}% abaixo do gap` };
                }
            }
        }
    }
    
    return { isRetest: false, description: '' };
}

// =====================================================================
// === FUNÇÃO PARA VERIFICAR VOLUME DE 3 MINUTOS ===
// =====================================================================
async function check3MinVolume(symbol, direction) {
    try {
        const candles = await getCandles(symbol, '3m', 10);
        if (!candles || candles.length < 5) return false;
        
        const lastCandle = candles[candles.length - 1];
        const avgVolume = candles.slice(-6, -1).reduce((a, b) => a + b.volume, 0) / 5;
        const volumeRatio = lastCandle.volume / avgVolume;
        
        if (volumeRatio < 1.5) return false;
        
        const isBullishCandle = lastCandle.close > lastCandle.open;
        const isBearishCandle = lastCandle.close < lastCandle.open;
        
        if (direction === 'BUY' && isBullishCandle && volumeRatio >= 1.5) {
            return { confirmed: true, volumeRatio: volumeRatio.toFixed(1), direction: 'comprador' };
        }
        if (direction === 'SELL' && isBearishCandle && volumeRatio >= 1.5) {
            return { confirmed: true, volumeRatio: volumeRatio.toFixed(1), direction: 'vendedor' };
        }
...
// =====================================================================
function gerarInterpretacaoBearish(symbol, currentPrice, fvgData, additionalData) {
    let interpretacao = "";
...
function gerarInterpretacaoBullish(symbol, currentPrice, fvgData, additionalData) {
    let interpretacao = "";
...
// =====================================================================
// === MONITOR ESPECÍFICO PARA BTC E ETH COM FVG + VOLUME 3min ===
// =====================================================================
const fvgAlertedCache = new Map();

async function monitorBTCAndETHFVG() {
    const symbolsToMonitor = ['BTCUSDT','XRPUSDT','GALAUSDT','FILUSDT','LINKUSDT','AVAXUSDT','ADAXUSDT','SUSHIUSDT','ICPUSDT','DOTUSDT','ETHUSDT'];
    
    console.log('\n' + '='.repeat(70));
    console.log('🎯 MONITOR FVG + VOLUME 3min - BTC e ETH');
    console.log('='.repeat(70));
    console.log(`📊 Timeframes monitorados: 15min, 1h, 4h, Diário, Semanal`);
    console.log(`📊 Volume requerido: 1.5x média em candle de 3 minutos`);
    console.log('='.repeat(70));
        ...
        for (const symbol of symbolsToMonitor) {
            try {
                ...
                if (fvgMultiTimeframe.confirmedBullish) {
                    const volumeCheck = await check3MinVolume(symbol, 'BUY');
                    ...
                    if (volumeCheck && volumeCheck.confirmed) {
                        ...
                        if (!lastAlert || (Date.now() - lastAlert) > cooldownMinutes * 60 * 1000) {
                            await sendFVGBullAlertCompleto(symbol, currentPrice, fvgMultiTimeframe, volumeCheck, additionalData);
                            fvgAlertedCache.set(cacheKey, Date.now());
                            console.log(`✅ ${symbol} - FVG BULL CONFIRMADO enviado!`);
                        } else {
                ...
                if (fvgMultiTimeframe.confirmedBearish) {
                    const volumeCheck = await check3MinVolume(symbol, 'SELL');
                    ...
                    if (volumeCheck && volumeCheck.confirmed) {
                        ...
                        if (!lastAlert || (Date.now() - lastAlert) > cooldownMinutes * 60 * 1000) {
                            await sendFVGBearAlertCompleto(symbol, currentPrice, fvgMultiTimeframe, volumeCheck, additionalData);
                            fvgAlertedCache.set(cacheKey, Date.now());
                            console.log(`✅ ${symbol} - FVG BEAR CONFIRMADO enviado!`);
                        } else {
...
async function sendFVGBullAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData) {
    ...
    // Verifica rejeição de wick no timeframe 15m
    const wickRejection = await check15mWickRejection(symbol, 'BUY');
    
    // Verifica se é reteste de FVG
    const retestInfo = isFVGRetest(fvgData, currentPrice, 'bullish');
    ...
    // Gera interpretação dos cenários
    let interpretacao = gerarInterpretacaoBullish(symbolShort, currentPrice, fvgData, additionalData);
    ...
    // Destaque para rejeição de wick (contraste maior)
    let wickHighlight = '';
    if (wickRejection.hasRejection) {
        wickHighlight = `<b>🔥🔥🔥 ${wickRejection.description} 🔥🔥🔥</b>\n`;
    }
    
    // Informação de reteste de FVG
    let retestText = '';
    if (retestInfo.isRetest) {
        retestText = `<b>📌 ${retestInfo.description}</b>\n`;
    }
    ...
    const message = `<b>🟢 ${symbolShort} FVG Bull💹✔︎</b>\n` +
                    ...
                    `<b> Vol 3min:</b> ${volumeData.volumeRatio}x média (${volumeData.direction})\n` +
                    `${retestText}` +
                    `${wickHighlight}` +
                    `<i>${divergenciaText}</i>\n` +
                    `<i>${macdText}</i>\n` +
...
async function sendFVGBearAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData) {
    const dt = getBrazilianDateTime();
    const symbolShort = symbol.replace('USDT', '');
    const tvLink = `https://www.tradingview.com/chart/?symbol=BINANCE%3A${symbol}`;
    const tvLinkText = `<a href="${tvLink}"> 🔍Ver_Gráfico</a>`;
    ...
    // Verifica rejeição de wick no timeframe 15m
    const wickRejection = await check15mWickRejection(symbol, 'SELL');
    
    // Verifica se é reteste de FVG
    const retestInfo = isFVGRetest(fvgData, currentPrice, 'bearish');
    ...
    // Gera interpretação dos cenários
    let interpretacao = gerarInterpretacaoBearish(symbolShort, currentPrice, fvgData, additionalData);
...
async function sendInitMessage() {
    const msg = `<i>⚡ Titanium ⚡</i>\n\n<i> 🤖 Olá... espero que estejam bem... ja estou com monitoramento especial hoje em BTC, ETH, XRP, ADA, AVAX, LINK, DOT, GALA, VANRY, FIL, SUSHI, ICP... sou o agente IA do @J4rviz... </i>\n<i> bons trades a todos...!</i>`;
