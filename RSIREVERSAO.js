...
const config = {
  ...
  RSI_PERIOD: 14,
  ATR_PERIOD: 14,
  ADX_PERIOD: 14, // Período para o ADX
  EMA_FAST_PERIOD: 13, // EMA rápida
  EMA_SLOW_PERIOD: 21, // EMA lenta
...
const state = {
  ...
  rsiPeaks: new Map(),
  lastEmaValues: new Map() // Cache para valores anteriores de EMA
};
...
function calculateADX(data, symbol) {
  if (!data || data.length < config.ADX_PERIOD + 14) {
    logger.warn(`Dados insuficientes para calcular ADX para ${symbol}: ${data?.length || 0} velas disponíveis, necessário: ${config.ADX_PERIOD + 14}`);
    return null;
  }
  ...
  if (highs.every(h => h === highs[0]) || lows.every(l => l === lows[0]) || closes.every(c => c === closes[0])) {
    logger.warn(`Preços constantes detectados para ${symbol}, ADX inválido`);
    return null;
  }
  ...
  try {
    ...
    if (adxValue === null || isNaN(adxValue)) {
      logger.warn(`Falha ao calcular ADX para ${symbol}: resultado inválido`);
      return null;
    }
    logger.info(`ADX calculado para ${symbol}: ${adxValue}, dados: ${data.length} velas`);
    return adxValue;
  } catch (e) {
    logger.warn(`Erro ao calcular ADX para ${symbol}: ${e.message}`);
    return null;
  }
}
...
function calculateEMA(data, period, symbol) {
  ...
  const closes = data.map(d => d.close);
  if (closes.every(c => c === closes[0])) {
    logger.warn(`Preços constantes detectados para ${symbol}, EMA(${period}) inválido`);
    return null;
  }
  ...
  logger.info(`EMA(${period}) calculado para ${symbol}: ${emaValue}, dados: ${data.length} velas`);
  return emaValue;
}
...
function detectEMACrossover(symbol, emaFast, emaSlow) {
  ...
  state.lastEmaValues.set(cacheKey, { emaFast, emaSlow });
  logger.info(`EMA Crossover para ${symbol}: EMA13=${emaFast}, EMA21=${emaSlow}, Bullish=${isBullishCrossover}, Bearish=${isBearishCrossover}`);
...
async function sendMonitorAlert(coins) {
  ...
  if (topLowRsiWithBullishEMA.length > 0) {
    let emaBullishAlertText = `🟢*RSI Baixo + Tendência (EMA13⤴️EMA21 3m) 📈*\n\n`;
    ...
    logger.info(`Tamanho da mensagem de RSI baixo com cruzamento de alta: ${emaBullishAlertText.length} caracteres`);
    await sendTelegramMessage(emaBullishAlertText);
    logger.info('Alerta de moedas com RSI em sobrevenda e cruzamento de alta enviado com sucesso');
  }

  // Alerta para RSI alto com cruzamento de baixa
  if (topHighRsiWithBearishEMA.length > 0) {
    let emaBearishAlertText = `🔴*RSI Alto + Têndencia (EMA13⤵️EMA21 3m) 📉*\n\n`;
    ...
    logger.info(`Tamanho da mensagem de RSI alto com cruzamento de baixa: ${emaBearishAlertText.length} caracteres`);
    await sendTelegramMessage(emaBearishAlertText);
    logger.info('Alerta de moedas com RSI em sobrecompra e cruzamento de baixa enviado com sucesso');
  }
...
async function checkCoins() {
  try {
    ...
    const coinsData = await limitConcurrency(usdtPairs, async (symbol) => {
      try {
        ...
        const ohlcv15mRaw = getCachedData(`ohlcv_${symbol}_15m`) ||
          await withRetry(() => exchangeFutures.fetchOHLCV(symbol, '15m', undefined, Math.max(config.RSI_PERIOD, config.ATR_PERIOD, config.ADX_PERIOD + 14, config.EMA_SLOW_PERIOD) + 1));
        setCachedData(`ohlcv_${symbol}_15m`, ohlcv15mRaw);
        const ohlcv15m = normalizeOHLCV(ohlcv15mRaw, symbol);
        if (!ohlcv15m.length || ohlcv15m.length < config.ADX_PERIOD + 14) {
          logger.warn(`Dados OHLCV insuficientes para ${symbol} (15m): ${ohlcv15m.length} velas, necessário: ${config.ADX_PERIOD + 14}, pulando...`);
          return null;
        }
        ...
        logger.info(`Moeda processada: ${symbol}, RSI: ${rsi}, RSI1h: ${rsi1h}, RSI4h: ${rsi4h}, ATR: ${atr}, ATR%: ${atrPercent}, ADX: ${adx}, Stoch4h: K=${stoch4h.k}, D=${stoch4h.d}, Stoch1d: K=${stoch1d.k}, D=${stoch1d.d}, LSR: ${lsr}, EMA13: ${emaFast}, EMA21: ${emaSlow}`);

        return { symbol, price, rsi, rsi1h, rsi4h, atr, atrPercent, adx, lsr, funding, delta, oi5m, oi15m, volume, volumeSpike, fundingAnomaly, stoch4h, stoch1d, supportResistance, emaCrossover };
      } catch (e) {
    ...
    validCoins.forEach(coin => logger.info(`Moeda: ${coin.symbol}, RSI: ${coin.rsi}, RSI1h: ${coin.rsi1h}, RSI4h: ${coin.rsi4h}, ATR: ${coin.atr}, ATR%: ${coin.atrPercent}, ADX: ${coin.adx}, Stoch4h: K=${coin.stoch4h.k}, D=${coin.stoch4h.d}, Stoch1d: K=${coin.stoch1d.k}, D=${coin.stoch1d.d}, LSR: ${coin.lsr}, EMA13: ${coin.emaCrossover.emaFast}, EMA21: ${coin.emaCrossover.emaSlow}`));
