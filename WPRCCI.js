...
const config = {
  ...
  WPR_PERIOD: 26, // Novo parâmetro para WPR
};
...
function calculateWPR(ohlcv, period = config.WPR_PERIOD) {
  ...
  return wpr.length ? wpr[wpr.length - 1] : null;
}
...
async function sendAlertCCICross(symbol, price, rsi15m, rsi1h, lsr, fundingRate, support, resistance, atr, oi5m, oi15m) {
  ...
  if (rsi1h < 55 && state.lastSignals[symbol] !== 'COMPRA' && oi5m.isRising ) {
    ...
    logger.info(`Sinal de VENDA enviado para ${symbol} (RSI descendo, volume anormal, volatilidade mínima, OI 5m caindo, OI 15m caindo)`);
  }
  if (alertText) {
    try {
      await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, alertText, {
...
async function monitorCCICrossovers() {
  try {
    ...
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      const ohlcv2hRaw = getCachedData(`${cacheKeyPrefix}_2h`) || await withRetry(() => exchangeSpot.fetchOHLCV(symbolWithSlash, '2h', undefined, config.WPR_PERIOD));
      ...
      if (!ohlcv2hRaw || ohlcv2hRaw.length < config.WPR_PERIOD) {
        logger.warn(`Dados insuficientes para ${symbol} (2h)`);
        //await bot.api.sendMessage(config.TELEGRAM_CHAT_ID, `⚠️ Dados insuficientes para ${symbol} (2h).`);
        return;
      }
      ...
      const wpr2h = calculateWPR(ohlcv2hRaw);
      if (wpr2h === null) {
        logger.warn(`WPR não calculado para ${symbol} (2h)`);
        return;
      }
