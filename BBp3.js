...
const CONFIG = {
    ...
    ALERTS: {
        ...
        MAX_ALERTS_PER_SCAN: 50,
        IGNORE_LOW_VOLUME_SYMBOLS: true,
        TELEGRAM_DELAY_MS: 2000
    },
    VOLUME: {
        EMA_PERIOD: 9
    },
...
async function getTopSymbols() {
    try {
        ...
        return ['BTCUSDT','ETHUSDT','SOLUSDT','XRPUSDT','BNBUSDT','DOGEUSDT','ADAUSDT','AVAXUSDT','LINKUSDT','TONUSDT'];
    }
}
...
function canSendAlert(symbol, direction, price, score, volumeRatio, volumeUSDT, volume24hUSDT, trendStrength, rsi, volumeDirection, volumePercentage) {
    ...
    if (lastAlert) {
        ...
        if (minutesDiff < CONFIG.ALERTS.COOLDOWN_MINUTES) {
            log(`Cooldown ${symbol} ${direction}: ${minutesDiff.toFixed(1)}min < ${CONFIG.ALERTS.COOLDOWN_MINUTES}min necessários`, 'warning');
            return false;
        }
    }
    ...
    if (CONFIG.ALERTS.ALLOW_OPPOSITE_DIRECTION) {
        ...
        if (oppositeAlert) {
            ...
            log(`Direção oposta para ${symbol} - permitido (último alerta oposto há ${oppositeMinutesDiff.toFixed(1)}min)`, 'info');
        }
    }
...
function registerAlert(symbol, direction, price) {
    ...
    log(`Registrado alerta ${symbol} ${direction} #${dailyCount} do dia`, 'success');
}
...
async function analyzeSymbol(symbol) {
    try {
        ...
        if (candles1h.length >= CONFIG.BOLLINGER.PERIOD + 5) {
            await analyzeTimeframe(symbol, candles1h, '1h');
        }
...
async function analyzeTimeframe(symbol, candles, timeframe) {
    try {
        ...
        // Filtro de Bandwidth só é aplicado quando realmente tocou na banda
...
async function startScanner() {
    ...
    // Enviar mensagem de inicialização - VERSÃO CORRIGIDA
    try {
        const token = CONFIG.TELEGRAM.BOT_TOKEN;
        const chatId = CONFIG.TELEGRAM.CHAT_ID;
        
        // Mensagem simples sem formatação complexa
        const initMessage = `🚀 Titanium Prime Ativado\n\n` +
            ...
            `📈 Limites: TOP10=25 | TOP50=40 | OUTROS=55\n` +
        ...
        const response = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
            ...
            body: JSON.stringify({
                chat_id: chatId,
                text: initMessage,
                parse_mode: 'HTML' // Removendo parse_mode ou usando 'Markdown'
            })
        });
