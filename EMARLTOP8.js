...
🎯 Foco em: Volume, LSR, RSI ajustado, Pivot Points, Funding
🔧 by @J4Rviz.
        `;
...
async function checkFundingRate(symbol, isBullish) {
    try {
        ...
        return {
            isValid: isValid,
            raw: fundingRate,
            scorePoints: isValid ? 8 : 0,
            directionFavorable: isValid
        };
    } catch (error) {
        return { isValid: false, raw: 0, scorePoints: 0, directionFavorable: false };
    }
}
...
async function calculateSignalQuality(symbol, isBullish, marketData) {
    ...
    if (marketData.funding && marketData.funding.isValid) {
        ...
        score += fundingScore;
        const fundingPercent = (marketData.funding.raw * 100).toFixed(4);
        
        if (isBullish) {
            details.push(` Funding Rate: ${fundingScore}/${QUALITY_WEIGHTS.funding} (${fundingPercent}% NEGATIVO - FAVORÁVEL para COMPRA)`);
        } else {
            details.push(` Funding Rate: ${fundingScore}/${QUALITY_WEIGHTS.funding} (${fundingPercent}% POSITIVO - FAVORÁVEL para VENDA)`);
        }
    } else {
...
async function monitorSymbol(symbol) {
    try {
        ...
        const fundingInfo = fundingData.raw ? `${(fundingData.raw * 100).toFixed(4)}%` : 'N/A';
        ...
        console.log(`   💰 Funding: ${fundingInfo} ${fundingData.isValid ? '✅ FAVORÁVEL' : '❌'}`);
