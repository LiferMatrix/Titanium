...
function loadFVGMemory() {
    try {
        if (fs.existsSync(FVG_MEMORY_FILE)) {
            ...
            console.log(`📂 Memória de FVG carregada com sucesso`);
        } else {
            console.log(`📂 Nenhuma memória de FVG encontrada, iniciando nova`);
        }
    } catch (error) {
...
function saveFVGMemory() {
    try {
        ...
        console.log(`💾 Memória de FVG salva com sucesso`);
    } catch (error) {
...
function mergePivotsByConfluence(pivots, priceTolerancePercent = 0.5) {
    ...
    for (const pivot of sorted) {
        ...
        if (diffPercent < priceTolerancePercent) {
            ...
            if (pivot.timestamp) last.timestamp = pivot.timestamp;
        } else {
            merged.push({ ...pivot, strength: pivot.strength, confluenceCount: 1 });
        }
    }
...
function identifyPivotZones(candles, timeframe) {
    try {
        ...
        const strongResistances = mergedHighPivots.filter(p => p.strength >= CONFIG.MONITOR.PIVOT.STRENGTH_THRESHOLD);
        const strongSupports = mergedLowPivots.filter(p => p.strength >= CONFIG.MONITOR.PIVOT.STRENGTH_THRESHOLD);
        
        return {
            timeframe,
            resistances: strongResistances,
            supports: strongSupports,
            allHighPivots: mergedHighPivots,
            allLowPivots: mergedLowPivots
        };
    } catch (error) {
        return null;
    }
...
function findDivergencesWithPivots(prices, rsiValues, pivotZones) {
    ...
    const supports = pivotZones.supports;
    const resistances = pivotZones.resistances;
    
    for (const support of supports) {
        const idx = support.index;
        if (!idx || idx < 5 || idx >= prices.length - 5) continue;
        
        const priceLow = prices[idx];
        const rsiLow = rsiValues[idx];
        
        for (let i = idx + 5; i < prices.length - 5; i++) {
            const isPriceLower = prices[i] < priceLow;
            const isRsiHigher = rsiValues[i] > rsiLow;
            
            if (isPriceLower && isRsiHigher) {
                const strength = Math.abs((prices[i] - priceLow) / priceLow) * 100;
    ...
    for (const resistance of resistances) {
        const idx = resistance.index;
        if (!idx || idx < 5 || idx >= prices.length - 5) continue;
        
        const priceHigh = prices[idx];
        const rsiHigh = rsiValues[idx];
        
        for (let i = idx + 5; i < prices.length - 5; i++) {
            const isPriceHigher = prices[i] > priceHigh;
            const isRsiLower = rsiValues[i] < rsiHigh;
            
            if (isPriceHigher && isRsiLower) {
                const strength = Math.abs((prices[i] - priceHigh) / priceHigh) * 100;
                if (strength >= CONFIG.MONITOR.RSI.MIN_DIVERGENCE_STRENGTH) {
...
function findConvergencesWithPivots(prices, rsiValues, pivotZones) {
    ...
    for (const support of supports) {
        ...
        for (let i = idx + 5; i < prices.length - 5; i++) {
            const isPriceHigher = prices[i] > priceLow;
            const isRsiHigher = rsiValues[i] > rsiLow;
            
            if (isPriceHigher && isRsiHigher) {
                const strength = Math.abs((prices[i] - priceLow) / priceLow) * 100;
    ...
    for (const resistance of resistances) {
        ...
        for (let i = idx + 5; i < prices.length - 5; i++) {
            const isPriceLower = prices[i] < priceHigh;
            const isRsiLower = rsiValues[i] < rsiHigh;
            
            if (isPriceLower && isRsiLower) {
                const strength = Math.abs((priceHigh - prices[i]) / priceHigh) * 100;
                if (strength >= CONFIG.MONITOR.RSI.MIN_CONVERGENCE_STRENGTH) {
...
// =====================================================================
// === ANÁLISE COMPLETA COM PIVÔS E DIVERGÊNCIAS ===
// =====================================================================
async function analyzeDivergencesWithPivots(symbol) {
    const cacheKey = `${symbol}_divergences_pivots`;
    const now = Date.now();
    ...
    for (const timeframe of CONFIG.MONITOR.RSI.TIMEFRAMES) {
        try {
            ...
            const pivotZones = identifyPivotZones(candles, timeframe);
            if (pivotZones) {
                result.pivotZones[timeframe] = {
                    supports: pivotZones.supports,
                    resistances: pivotZones.resistances
                };
            }
            
            const divergences = findDivergencesWithPivots(prices, rsi, pivotZones);
            const convergences = findConvergencesWithPivots(prices, rsi, pivotZones);
...
async function checkAndAlert(symbolData, cvdManager) {
    ...
    try {
        ...
        const [rsiValue, divergenciasCompletas, lsrTrendData] = await Promise.all([
            ...
            analyzeDivergencesWithPivots(fullSymbol),
        ...
        const asset = {
            ...
            cvdDirection: cvd ? cvd.direction : '', divergencias,
...
// =====================================================================
// === ENVIAR ALERTA FVG COM INFORMAÇÕES DE DIVERGÊNCIA E PIVÔ ===
// =====================================================================
async function sendFVGAlert(asset, type) {
    ...
    // ===== INFORMAÇÕES DE DIVERGÊNCIA E PIVÔ =====
    let divergenciaInfo = '';
    let pivotInfo = '';
    
    if (type === 'BULL' && divergencias.bestBullishDivergence) {
        const div = divergencias.bestBullishDivergence;
        divergenciaInfo = `📈 Divergência Bullish ${div.timeframe}`;
        pivotInfo = `📍 Pivô Suporte ${div.timeframe} em ${formatPrice(div.pivotPrice)} USDT`;
    } else if (type === 'BEAR' && divergencias.bestBearishDivergence) {
        const div = divergencias.bestBearishDivergence;
        divergenciaInfo = `📉 Divergência Bearish ${div.timeframe}`;
        pivotInfo = `📍 Pivô Resistência ${div.timeframe} em ${formatPrice(div.pivotPrice)} USDT`;
    } else if (type === 'BULL' && divergencias.bestBullishConvergence) {
        const conv = divergencias.bestBullishConvergence;
        divergenciaInfo = `🔄 Convergência Bullish ${conv.timeframe}`;
        pivotInfo = `📍 Pivô Suporte ${conv.timeframe} em ${formatPrice(conv.pivotPrice)} USDT`;
    } else if (type === 'BEAR' && divergencias.bestBearishConvergence) {
        const conv = divergencias.bestBearishConvergence;
        divergenciaInfo = `🔄 Convergência Bearish ${conv.timeframe}`;
        pivotInfo = `📍 Pivô Resistência ${conv.timeframe} em ${formatPrice(conv.pivotPrice)} USDT`;
    }
    ...
    if (type === 'BULL') {
        ...
        if (possibleEntry && possibleEntry < currentPrice) {
            ...
            log(`⚠️ Ajuste entrada COMPRA ${asset.symbol}: ${formatPrice(currentPrice)} → ${formatPrice(entryPrice)}`, 'warning');
        }
    } else {
        ...
        if (possibleEntry && possibleEntry > currentPrice) {
            ...
            log(`⚠️ Ajuste entrada VENDA ${asset.symbol}: ${formatPrice(currentPrice)} → ${formatPrice(entryPrice)}`, 'warning');
        }
    }
    ...
    if (type === 'BULL') {
        ...
        if (volumeAlert.structure4h?.closestSupport) {
            message += `🟢 Suporte em ${formatPrice(volumeAlert.structure4h.closestSupport.targetPrice)} USDT (${volumeAlert.structure4h.closestSupport.distancePercent.toFixed(2)}%)\n`;
        }
        ...
        for (let i = 0; i < targets.targets.length; i++) {
            ...
            message += ` Alvo ${i+1} : ${formatPrice(t.price)} USDT (+${t.profitPercent.toFixed(2)}%) - Fechar ${t.partialClose}%\n`;
        }
        ...
        if (volumeAlert.structure1h?.zoneType === 'VENDA') {
        ...
        if (volumeAlert.structure4h?.closestResistance) {
            message += `🔴 Resistência em ${formatPrice(volumeAlert.structure4h.closestResistance.targetPrice)} USDT (${volumeAlert.structure4h.closestResistance.distancePercent.toFixed(2)}%)\n`;
        }
        ...
        if (volumeAlert.structure1h?.zoneType === 'COMPRA') {
