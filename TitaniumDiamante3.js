...
const config = {
  ...
  VOL_HIGH_THRESHOLD: 0.015, // ATR/price > 1.5% = alta volatilidade
  GAMMA_OI_MULTIPLIER: 1.5, // Multiplicador para OI crescente
  GAMMA_PRICE_SURGE: 0.05, // Aumento de preço > 5%
  GAMMA_LSR: 3.0, // LSR > 3.0 para longs dominantes
  GAMMA_FUNDING: 0.0005, // Funding rate > 0.05%
  RSI_PERIOD: 14,
  ...
  VOLUME_MULTIPLIER: 2.1, // Multiplicador para considerar volume "anormal" (ex: 1.5x o médio)
  ...
  ADX_MIN_TREND: process.env.ADX_MIN_TREND ? parseFloat(process.env.ADX_MIN_TREND) : 20, // Mínimo ADX para considerar tendência forte nos alertas
};
// Logger
...
// ================= DETECÇÃO DE GAMMA SQUEEZE ================= //
async function detectGammaSqueeze(symbol, data) {
  const { ohlcv15m, price, isAbnormalVol, lsr, fundingRate } = data;
  try {
    const oiHistory = await withRetry(() => exchangeFutures.fetchOpenInterestHistory(symbol, '15m', undefined, 10));
    if (oiHistory.length < 2) {
      logger.warn(`Dados insuficientes de OI para ${symbol}`);
      return false;
    }
    const avgOI = oiHistory.slice(0, -1).reduce((sum, o) => sum + o.openInterest, 0) / (oiHistory.length - 1);
    const currentOI = oiHistory[oiHistory.length - 1].openInterest;
    const isRisingOI = currentOI > avgOI * config.GAMMA_OI_MULTIPLIER;
    const prevPrice = ohlcv15m[ohlcv15m.length - 2].close;
    const priceChange = (price - prevPrice) / prevPrice;
    const isPriceSurge = priceChange > config.GAMMA_PRICE_SURGE;
    const isHighLSR = lsr.value > config.GAMMA_LSR;
    const isHighFunding = fundingRate.current > config.GAMMA_FUNDING;
    const isGamma = isRisingOI && isPriceSurge && isAbnormalVol && isHighLSR && isHighFunding;
    if (isGamma) logger.info(`💥Simetria Detectada💥 ${symbol}`);
    return isGamma;
  } catch (e) {
    logger.error(`Erro ao detectar Gamma Squeeze para ${symbol}: ${e.message}`);
    return false;
  }
}
// ================= FUNÇÕES DE ALERTAS ================= //
...
async function sendAlertStochasticCross(symbol, data) {
  ...
  let alertText = '';
  const isGammaSqueeze = await detectGammaSqueeze(symbol, data);
  if (isBuySignal) {
    const cooldown = (data.atr / price > config.VOL_HIGH_THRESHOLD) ? 15 * 60 * 1000 : config.TEMPO_COOLDOWN_MS;
    const ultimoBuy = state.ultimoAlertaPorAtivo[symbol].historico.filter(r => r.direcao === 'buy').slice(-1)[0];
    const foiAlertado = ultimoBuy && (agora - ultimoBuy.timestamp < cooldown) && 
                        (price >= ultimoBuy.entryZoneMin && price <= ultimoBuy.entryZoneMax);
    if (!foiAlertado) {
      const direcao = 'buy';
      ...
      state.ultimoAlertaPorAtivo[symbol].historico.push({ direcao: 'buy', timestamp: agora, entryZoneMin: buyEntryLow, entryZoneMax: price });
    ...
    const cooldown = (data.atr / price > config.VOL_HIGH_THRESHOLD) ? 15 * 60 * 1000 : config.TEMPO_COOLDOWN_MS;
    const ultimoSell = state.ultimoAlertaPorAtivo[symbol].historico.filter(r => r.direcao === 'sell').slice(-1)[0];
    const foiAlertado = ultimoSell && (agora - ultimoSell.timestamp < cooldown) && 
                        (price >= ultimoSell.entryZoneMin && price <= ultimoSell.entryZoneMax);
    if (!foiAlertado) {
      const direcao = 'sell';
      const count = state.ultimoAlertaPorAtivo[symbol].historico.filter(r => r.direcao === direcao).length + 1;
      ...
      state.ultimoAlertaPorAtivo[symbol]['4h'] = agora;
      state.ultimoAlertaPorAtivo[symbol].historico.push({ direcao: 'sell', timestamp: agora, entryZoneMin: price, entryZoneMax: sellEntryHigh });
      state.ultimoAlertaPorAtivo[symbol].historico = state.ultimoAlertaPorAtivo[symbol].historico.slice(-config.MAX_HISTORICO_ALERTAS);
...
async function main() {
  ...
  try {
    ...
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium Diamante 3 💎✨ ...'));
