...
// =====================================================================
// === BUSCAR CANDLES (SPOT BINANCE) ===
// =====================================================================
async function getCandles(symbol, interval, limit = 100) {
    try {
        // Usando API Spot da Binance (api.binance.com em vez de fapi.binance.com)
...
// =====================================================================
// === DADOS ADICIONAIS PARA O ALERTA (SPOT) ===
// =====================================================================
async function getAdditionalData(symbol, currentPrice, isGreenAlert) {
    try {
        // Buscar candles para diferentes timeframes usando API Spot
        ...
        // NOTA: LSR e Funding Rate não estão disponíveis na API Spot
        // Removemos essas chamadas de API
        ...
        return {
            ...
            // LSR e Funding não disponíveis em Spot
            lsr: 'N/A',
            funding: 'N/A'
        };
    } catch (error) {
...
function formatAlert(data) {
    ...
    // Formatação para Spot (sem funding rate)
    const lsr = data.lsr || 'N/A';
    const funding = data.funding || 'N/A';
    ...
    return `${data.direction} Convergência - ${symbolName} (SPOT)
...
async function sendTelegramAlert(message) {
    // Usando diretamente o token e chat ID da configuração
...
// =====================================================================
// === BUSCAR SÍMBOLOS (SPOT BINANCE) ===
// =====================================================================
async function fetchSymbols() {
    try {
        // Usando API Spot da Binance
        const data = await rateLimiter.makeRequest('https://api.binance.com/api/v3/exchangeInfo');
        
        return data.symbols
            .filter(s => 
                s.symbol.endsWith('USDT') && // Apenas pares USDT
                s.status === 'TRADING' && // Apenas em negociação
                s.quoteAsset === 'USDT' // Garantir que USDT é a moeda de cotação
            )
            .map(s => s.symbol)
            .sort(); // Ordenar alfabeticamente
    } catch (error) {
        console.log('⚠️ Erro ao buscar símbolos, usando lista fallback:', error.message);
        // Fallback para símbolos populares SPOT
        return ['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'SOLUSDT', 'XRPUSDT', 'DOGEUSDT', 
                ...
                'UNIUSDT', 'ATOMUSDT', 'ALGOUSDT', 'VETUSDT', 'FILUSDT', 'ICPUSDT',
                'ETCUSDT', 'EOSUSDT', 'XLMUSDT', 'TRXUSDT', 'SHIBUSDT', 'LTCUSDT'];
    }
}
...
async function startScanner() {
    console.log('\n' + '='.repeat(60));
    console.log('📊 Convergência - SPOT BINANCE');
    console.log('='.repeat(60));
    
    const symbols = await fetchSymbols();
    console.log(`📈 Monitorando ${symbols.length} pares SPOT USDT`);
    ...
    console.log(`📱 Telegram: Configurado com token e chat ID do código`);
    console.log(`💱 Mercado: SPOT (À Vista)`);
    
    await sendTelegramAlert(`🤖 Titanium Scanner SPOT Ativado!\nMonitorando ${symbols.length} pares SPOT USDT\nScan a cada ${CONFIG.SCAN.INTERVAL_MINUTES}min\nRSI: ${CONFIG.RSI.PERIOD} (Wilder)\nStoch Diário: K5/D3\nStoch 4H: K14/D3`);
        ...
        try {
            ...
            for (const alert of alerts) {
                ...
                if (sent) {
                    ...
                    sentCount++;
                    console.log(`✅ ${alert.direction} ${alert.symbol} - RSI: ${alert.rsi.toFixed(0)} | Stoch D: ${alert.stoch1d}`);
                }
...
// === INICIAR ===
// =====================================================================
console.log('🚀 Iniciando Scanner SPOT Binance...');
console.log('📱 Usando token e chat ID configurados diretamente no código');
console.log('💱 Buscando apenas pares SPOT USDT');
