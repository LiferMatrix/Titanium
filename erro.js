...
async function sendMonitorAlert(coins) {
  // Menores LSR com RSI 15m e 1h baixo
  const topLowLsrLowRsi = coins
    .filter(c => c.lsr !== null && c.rsi !== null && c.rsi1h !== null && 
                 c.rsi < config.RSI_OVERSOLD_THRESHOLD && c.rsi1h < config.RSI_OVERSOLD_THRESHOLD)
    .sort((a, b) => a.lsr - b.lsr || a.rsi - b.rsi)
    .slice(0, config.MAX_COINS_PER_ALERT);

  // Maiores LSR com RSI 15m e 1h alto
  const topHighLsrHighRsi = coins
    .filter(c => c.lsr !== null && c.rsi !== null && c.rsi1h !== null && 
                 c.rsi > config.RSI_OVERBOUGHT_THRESHOLD && c.rsi1h > config.RSI_OVERBOUGHT_THRESHOLD)
    .sort((a, b) => b.lsr - a.lsr || b.rsi - a.rsi)
    .slice(0, config.MAX_COINS_PER_ALERT);

  logger.info(`Menores LSR com RSI baixo: ${topLowLsrLowRsi.length}, Maiores LSR com RSI alto: ${topHighLsrHighRsi.length}`);

  // Alerta para menores LSR com RSI baixo
  if (topLowLsrLowRsi.length > 0) {
    let lowLsrLowRsiAlertText = `🟢*Menores LSR com RSI Baixo (15m e 1h) 🚀*\n\n`;
    lowLsrLowRsiAlertText += await Promise.all(topLowLsrLowRsi.map(async (coin, i) => {
    ...
    lowLsrLowRsiAlertText += `\n☑︎ 🤖 Monitor - @J4Rviz`;
    logger.info(`Tamanho da mensagem de Menores LSR com RSI baixo: ${lowLsrLowRsiAlertText.length} caracteres`);
    await sendTelegramMessage(lowLsrLowRsiAlertText);
    logger.info('Alerta de moedas com Menores LSR e RSI baixo enviado com sucesso');
  }

  // Alerta para maiores LSR com RSI alto
  if (topHighLsrHighRsi.length > 0) {
    let highLsrHighRsiAlertText = `🔴*Maiores LSR com RSI Alto (15m e 1h) 📉*\n\n`;
    highLsrHighRsiAlertText += await Promise.all(topHighLsrHighRsi.map(async (coin, i) => {
      const tradingViewLink = `https://www.tradingview.com/chart/?symbol=BINANCE:${coin.symbol.replace('/', '')}&interval=15`;
      let lsrSymbol = '';
      if (coin.lsr !== null && !isNaN(coin.lsr)) {
        if (coin.lsr <= 1.4) lsrSymbol = '✅ Baixo';
    ...
    highLsrHighRsiAlertText += `\n☑︎ 🤖 Monitor - @J4Rviz`;
    logger.info(`Tamanho da mensagem de Maiores LSR com RSI alto: ${highLsrHighRsiAlertText.length} caracteres`);
    await sendTelegramMessage(highLsrHighRsiAlertText);
    logger.info('Alerta de moedas com Maiores LSR e RSI alto enviado com sucesso');
  }

  if (topLowLsrLowRsi.length === 0 && topHighLsrHighRsi.length === 0) {
    ...
    const coinsData = await limitConcurrency(usdtPairs, async (symbol) => {
      try {
        ...
        const ohlcv15mRaw = getCachedData(`ohlcv_${symbol}_15m`) ||
          await withRetry(() => exchangeFutures.fetchOHLCV(symbol, '15m', undefined, config.RSI_PERIOD + 1));
        ...
        return { symbol, price, rsi, rsi1h, lsr, funding, delta, oi5m, oi15m, stoch4h, stoch1d, supportResistance };
      } catch (e) {
