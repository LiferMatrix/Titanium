...
async function sendAlert1h4h(symbol, data) {
  const { ohlcv15m, ohlcv3m, ohlcv1h, ohlcvDiario, ohlcv4h, price, wpr4h, wpr1h, rsi1h, atr, lsr, volumeProfile, orderBookLiquidity, isOIRising5m, estocasticoD, estocastico4h, fundingRate, oi15m, ema13_3m, ema34_3m, previousEma13_3m, previousEma34_3m } = data;
  const agora = Date.now();
  if (state.ultimoAlertaPorAtivo[symbol]?.['1h_4h'] && agora - state.ultimoAlertaPorAtivo[symbol]['1h_4h'] < config.TEMPO_COOLDOWN_MS) return;
  const aggressiveDelta = await calculateAggressiveDelta(symbol);
  const atrPercent = (atr / price) * 100;
  if (!state.wprTriggerState[symbol]) state.wprTriggerState[symbol] = { '1h_4h': { buyTriggered: false, sellTriggered: false } };
  if (wpr4h <= config.WPR_LOW_THRESHOLD && wpr1h <= config.WPR_LOW_THRESHOLD) {
    state.wprTriggerState[symbol]['1h_4h'].buyTriggered = true;
  } else if (wpr4h >= config.WPR_HIGH_THRESHOLD && wpr1h >= config.WPR_HIGH_THRESHOLD) {
    state.wprTriggerState[symbol]['1h_4h'].sellTriggered = true;
  }
  if (!state.ultimoEstocastico[symbol]) state.ultimoEstocastico[symbol] = {};
  const kAnteriorD = state.ultimoEstocastico[symbol].kD || estocasticoD?.k || 0;
  const kAnterior4h = state.ultimoEstocastico[symbol].k4h || estocastico4h?.k || 0;
  ...
  const entryLow = format(price - 0.3 * atr);
  const entryHigh = format(price + 0.5 * atr);
  const isBuySignal = state.wprTriggerState[symbol]['1h_4h'].buyTriggered && 
                      //isOIRising5m && 
                      ema13_3m > ema34_3m && 
                      previousEma13_3m <= previousEma34_3m;
  const isSellSignal = state.wprTriggerState[symbol]['1h_4h'].sellTriggered && 
                      //!isOIRising5m && 
                      ema13_3m < ema34_3m && 
                      previousEma13_3m >= previousEma34_3m;
  const targets = isSellSignal
  ...
  let alertText = `🔹Ativo: *${symbol}* [- TradingView](${tradingViewLink})\n` +
    ...
    `🔹 Stoch Diário %K: ${estocasticoD ? estocasticoD.k.toFixed(2) : '--'} ${stochDEmoji} ${direcaoD}\n` +
    `🔹 Stoch 4H %K: ${estocastico4h ? estocastico4h.k.toFixed(2) : '--'} ${stoch4hEmoji} ${direcao4h}\n` +
    `☑︎ Monitor WPR 4h 🤖 @J4Rviz\n`;
  if (isBuySignal) {
    try {
      await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, `🟢*Reversão WPR 4h -✳️Compra✳️*\n\n${alertText}`, {
        parse_mode: 'Markdown',
        disable_web_page_preview: true
      }));
      if (!state.ultimoAlertaPorAtivo[symbol]) state.ultimoAlertaPorAtivo[symbol] = {};
      state.ultimoAlertaPorAtivo[symbol]['1h_4h'] = agora;
      state.wprTriggerState[symbol]['1h_4h'].buyTriggered = false;
    } catch (e) {
      logger.error(`Erro ao enviar alerta de compra 1h/4h para ${symbol}: ${e.message}`);
    }
  } else if (isSellSignal) {
    try {
      await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, `🔴*Venda WPR 4h -🔻Correção/Exaustão🔻*\n\n${alertText}`, {
        parse_mode: 'Markdown',
        disable_web_page_preview: true
      }));
      if (!state.ultimoAlertaPorAtivo[symbol]) state.ultimoAlertaPorAtivo[symbol] = {};
      state.ultimoAlertaPorAtivo[symbol]['1h_4h'] = agora;
      state.wprTriggerState[symbol]['1h_4h'].sellTriggered = false;
    } catch (e) {
      logger.error(`Erro ao enviar alerta de correção 1h/4h para ${symbol}: ${e.message}`);
    }
  }
}
...
async function checkConditions() {
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      const ohlcv4hRaw = getCachedData(`${cacheKeyPrefix}_4h`) || await withRetry(() => exchangeSpot.fetchOHLCV(symbol, '4h', undefined, config.WPR_PERIOD + 1));
      ...
      const wpr4hValues = calculateWPR(ohlcv4h);
      ...
      if (!wpr2hValues.length || !wpr4hValues.length || !wpr1hValues.length || !rsi1hValues.length || !atrValues.length || !ema13_3mValues.length || !ema34_3mValues.length) {
      ...
      await sendAlert1h4h(symbol, {
        ohlcv15m, ohlcv3m, ohlcv1h, ohlcvDiario, ohlcv4h,
        price: currentPrice,
        wpr4h: wpr4hValues[wpr4hValues.length - 1],
        wpr1h: wpr1hValues[wpr1hValues.length - 1],
        rsi1h: rsi1hValues[rsi1hValues.length - 1],
        atr: atrValues[atrValues.length - 1],
        lsr,
...
async function main() {
  ...
  try {
    ...
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium WPR2 💹Start...'));
