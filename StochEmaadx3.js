...
async function detectRecentFVG(symbol) {
  try {
    const raw = await withRetry(() => exchangeSpot.fetchOHLCV(symbol, '3m', undefined, 20));
...
async function main() {
  ...
  try {
    ...
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium3 by J4Rviz...'));
