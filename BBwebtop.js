...
const CONFIG = {
    ...
    CVD: {
        ENABLED: true,
        TIMEFRAMES: ['15m', '3m'],  // Adicionado suporte para múltiplos timeframes
        LOOKBACK_CANDLES: 20,
        ...
        // Configurações do WebSocket Real
        WEBSOCKET: {
            ENABLED: true,
            MAX_SYMBOLS: 100,          // Máximo de símbolos para CVD real
...
// =====================================================================
// === CVD REAL VIA WEBSOCKET - VERSÃO COM MÚLTIPLOS TIMEFRAMES ===
// =====================================================================

class RealCVDManager {
    constructor() {
        this.connections = new Map(); // symbol -> WebSocket instance
        this.snapshots = new Map(); // symbol -> { timeframes, lastPrice, startPrice }
        this.listeners = new Map(); // symbol -> callbacks
    ...
    async initialize(symbols) {
        ...
        log(`Timeframes CVD: ${CONFIG.CVD.TIMEFRAMES.join(', ')}`, 'info');
        
        // Inicializa cada símbolo
    ...
    getMaxDeltas(timeframe) {
        // Aproximadamente 1 trade por segundo
        const intervals = {
            '3m': 180,      // 3 minutos = 180 trades
            '15m': 900,     // 15 minutos = 900 trades
            '30m': 1800,
            '1h': 3600
        };
    ...
    async connectSymbol(symbol) {
        ...
        log(`Conectando WebSocket para ${symbol}...`, 'info');
        
        const ws = new WebSocket(wsUrl);
        
        // Inicializa estrutura para múltiplos timeframes
        const timeframesData = {};
        for (const tf of CONFIG.CVD.TIMEFRAMES) {
            timeframesData[tf] = {
                cumulative: 0,
                deltas: [],
                lastUpdate: Date.now(),
    ...
    processTrade(symbol, trade) {
        ...
        // m = true: vendedor agressivo, m = false: comprador agressivo
        const isAggressiveBuy = !trade.m;
        const delta = isAggressiveBuy ? value : -value;
        
        // Atualiza cada timeframe separadamente
        for (const tf of CONFIG.CVD.TIMEFRAMES) {
            const tfData = snapshot.timeframes[tf];
            if (!tfData) continue;
            
            // Atualiza CVD acumulado
            tfData.cumulative += delta;
            
            // Armazena delta para análise de divergência
            tfData.deltas.push({
                delta: delta,
                price: price,
                timestamp: timestamp,
                cumulative: tfData.cumulative
            });
            
            tfData.lastPrice = price;
            if (!tfData.startPrice) tfData.startPrice = price;
            
            // Limita tamanho do array baseado no timeframe
            const maxDeltas = this.getMaxDeltas(tf);
            if (tfData.deltas.length > maxDeltas) {
                tfData.deltas = tfData.deltas.slice(-maxDeltas);
            }
        ...
        if (listeners) {
            listeners.forEach(cb => cb({
                ...
                price,
                delta,
                cumulative: snapshot.timeframes['15m']?.cumulative || 0,
                timestamp: trade.T
            }));
        }
    }
    ...
    getCVDAnalysisForTimeframe(symbol, currentPrice, isGreenAlert, timeframe) {
        const snapshot = this.snapshots.get(symbol);
        
        // Se não tem dados reais, retorna null
        if (!snapshot || !snapshot.timeframes[timeframe]) {
            return null;
        }
        
        const tfData = snapshot.timeframes[timeframe];
        if (!tfData.deltas || tfData.deltas.length < 10) {
            return null;
        }
        
        const deltas = tfData.deltas;
        const recentDeltas = deltas.slice(-50); // últimos ~50 trades para análise
        
        // Calcula tendência do CVD
        ...
        // Divergência de Alta (Bullish): preço caindo, CVD subindo
        ...
        // Divergência de Baixa (Bearish): preço subindo, CVD caindo
        ...
        // Tendência alinhada (sem divergência)
        ...
        return { cvdConfirmed: false, cvdScore: 0, cvdSignal: null, isReal: true, timeframe: timeframe };
    }

    getCVDAnalysis(symbol, currentPrice, isGreenAlert) {
        const results = {};
        for (const tf of CONFIG.CVD.TIMEFRAMES) {
            results[tf] = this.getCVDAnalysisForTimeframe(symbol, currentPrice, isGreenAlert, tf);
        }
        return results;
    }
    ...
    getStats() {
        ...
        for (const [symbol, snapshot] of this.snapshots) {
            let hasData = false;
            for (const tf of CONFIG.CVD.TIMEFRAMES) {
                if (snapshot.timeframes[tf]?.deltas?.length > 0) {
                    hasData = true;
                    break;
                }
            }
            if (hasData) {
                ...
                stats.symbols.push({
                    symbol,
                    timeframes: Object.keys(snapshot.timeframes).map(tf => ({
                        timeframe: tf,
                        deltas: snapshot.timeframes[tf]?.deltas?.length || 0,
                        cumulative: snapshot.timeframes[tf]?.cumulative?.toFixed(2) || 0
                    }))
                });
            }
        }
...
// Instância global do CVD Manager
const realCVD = new RealCVDManager();

// =====================================================================
// === FUNÇÃO CVD HÍBRIDA (REAL + SIMULADO) ===
// =====================================================================
async function analyzeCVDHybrid(symbol, currentPrice, isGreenAlert) {
    // Tenta usar CVD Real primeiro
    if (CONFIG.CVD.WEBSOCKET.ENABLED && realCVD.isInitialized) {
        return realCVD.getCVDAnalysis(symbol, currentPrice, isGreenAlert);
    }
    
    // Fallback para CVD Simulado (candle-based)
    const results = {};
    for (const tf of CONFIG.CVD.TIMEFRAMES) {
        results[tf] = await analyzeCVDSimulated(symbol, currentPrice, isGreenAlert, tf);
    }
    return results;
}

// Mantém o CVD Simulado como fallback
async function getCVDDataSimulated(symbol, interval) {
    try {
        ...
        return {
            ...
            deltas: deltas,
            lastCandle: deltas[deltas.length - 1],
            previousCandle: deltas.length >= 2 ? deltas[deltas.length - 2] : null
        };
    } catch (error) {
        return null;
    }
...
async function analyzeCVDSimulated(symbol, currentPrice, isGreenAlert, timeframe = '15m') {
    if (!CONFIG.CVD.ENABLED) return { cvdScore: 0, cvdSignal: null, cvdConfirmed: false, isReal: false };
    
    try {
        const candles = await getCandles(symbol, timeframe, 40);
        const cvdData = await getCVDDataSimulated(symbol, timeframe);
        ...
        if (cvdDivergence) {
            ...
            if (isAligned) {
                return {
                    ...
                    cvdSignal: {
                        type: cvdDivergence.type,
                        description: cvdDivergence.description,
                        strength: cvdDivergence.strength
                    },
                    cvdConfirmed: true,
                    isReal: false,
                    timeframe: timeframe
                ...
                return { cvdScore: 0, cvdSignal: cvdDivergence, cvdConfirmed: false, isReal: false, timeframe: timeframe };
            }
        }
        
        return { cvdScore: 0, cvdSignal: null, cvdConfirmed: false, isReal: false, timeframe: timeframe };
...
async function sendSingleAlert(alert) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const cvd15mType = alert.cvd15m?.isReal ? 'REAL' : 'SIM';
            const cvd3mType = alert.cvd3m?.isReal ? 'REAL' : 'SIM';
            log(`${penaltyIcon} ALERTA ENVIADO: ${alert.direction} ${alert.symbol} [${alert.timeframe}] - Score: ${alert.confirmationScore} | CVD15m: ${alert.cvd15m?.cvdConfirmed ? '✅' : '❌'} (${cvd15mType}) | CVD3m: ${alert.cvd3m?.cvdConfirmed ? '✅' : '❌'} (${cvd3mType}) | Volume: ${alert.volume1h.direction} (${alert.volume1h.percentage.toFixed(1)}%) | Penalidades: ${alert.totalPenalty}`, 'success');
        }
        return true;
    } catch (error) {
...
async function sendGroupedAlerts(alerts, priority) {
    ...
    if (buyAlerts.length > 0) {
        ...
        buyAlerts.slice(0, 5).forEach(a => {
            const cvd15mType = a.cvd15m?.isReal ? '🔴' : '🟡';
            const cvd3mType = a.cvd3m?.isReal ? '🔴' : '🟡';
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} | CVD15m: ${a.cvd15m?.cvdConfirmed ? '✅' : '❌'} ${cvd15mType} | CVD3m: ${a.cvd3m?.cvdConfirmed ? '✅' : '❌'} ${cvd3mType} | Vol: ${a.volume1h.direction} (${a.volume1h.percentage.toFixed(1)}%) | Penal: ${a.totalPenalty}\n`;
        });
    ...
    if (sellAlerts.length > 0) {
        ...
        sellAlerts.slice(0, 5).forEach(a => {
            const cvd15mType = a.cvd15m?.isReal ? '🔴' : '🟡';
            const cvd3mType = a.cvd3m?.isReal ? '🔴' : '🟡';
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} | CVD15m: ${a.cvd15m?.cvdConfirmed ? '✅' : '❌'} ${cvd15mType} | CVD3m: ${a.cvd3m?.cvdConfirmed ? '✅' : '❌'} ${cvd3mType} | Vol: ${(100 - a.volume1h.percentage).toFixed(1)}% vendedor | Penal: ${a.totalPenalty}\n`;
        });
...
async function analyzeSymbol(symbol) {
    try {
        ...
        // ANÁLISE DO CVD (HÍBRIDO: REAL + SIMULADO) - AGORA RETORNA PARA MÚLTIPLOS TIMEFRAMES
        const cvdAnalysisResults = await analyzeCVDHybrid(symbol, currentPrice, isGreenAlert);
        
        // Extrai CVD 15m e CVD 3m
        const cvd15m = cvdAnalysisResults['15m'] || { cvdConfirmed: false, cvdScore: 0, cvdSignal: null, isReal: false };
        const cvd3m = cvdAnalysisResults['3m'] || { cvdConfirmed: false, cvdScore: 0, cvdSignal: null, isReal: false };
        ...
        // ADICIONA O SCORE DO CVD 15m
        if (cvd15m.cvdConfirmed) {
            maxScore += cvd15m.cvdScore;
            const cvdType = cvd15m.isReal ? 'REAL' : 'SIM';
            log(`✅ ${symbol} - CVD 15m ${cvdType} confirmado! Bônus +${cvd15m.cvdScore} | Score total: ${maxScore.toFixed(1)}`, 'success');
        }
        
        // ADICIONA O SCORE DO CVD 3m
        if (cvd3m.cvdConfirmed) {
            maxScore += cvd3m.cvdScore;
            const cvdType = cvd3m.isReal ? 'REAL' : 'SIM';
            log(`✅ ${symbol} - CVD 3m ${cvdType} confirmado! Bônus +${cvd3m.cvdScore} | Score total: ${maxScore.toFixed(1)}`, 'success');
        }
        ...
        const consolidated = {
            ...
            // CVDs separados por timeframe
            cvd15m: {
                confirmed: cvd15m.cvdConfirmed,
                score: cvd15m.cvdScore,
                signal: cvd15m.cvdSignal,
                isReal: cvd15m.isReal || false
            },
            cvd3m: {
                confirmed: cvd3m.cvdConfirmed,
                score: cvd3m.cvdScore,
                signal: cvd3m.cvdSignal,
                isReal: cvd3m.isReal || false
            }
        };
...
function formatAlert(data) {
    ...
    // Linha do CVD 15m
    let cvd15mLine = '';
    const cvd15mIcon = data.cvd15m?.isReal ? '🔴' : '🟡';
    if (data.cvd15m?.confirmed) {
        const cvdEmoji = data.cvd15m?.signal?.type === 'bullish' ? '💹' : '🔻';
        cvd15mLine = `#CVD🐋 ${cvd15mIcon}: ${cvdEmoji} ${data.cvd15m?.signal?.type === 'bullish' ? '#BULL' : '#BEAR'} ${data.cvd15m?.signal?.type === 'bullish' ? '✅' : '⚠️'} (15m)`;
    } else if (data.cvd15m?.signal && !data.cvd15m?.confirmed) {
        cvd15mLine = `#CVD🐋 ${cvd15mIcon}: ⚠️ ${data.cvd15m?.signal?.type === 'bullish' ? '#BULL' : '#BEAR'} (não alinhado - 15m)`;
    } else {
        cvd15mLine = `#CVD🐋 ${cvd15mIcon}: ❌ Indefinido (15m)`;
    }
    
    // Linha do CVD 3m
    let cvd3mLine = '';
    const cvd3mIcon = data.cvd3m?.isReal ? '🔴' : '🟡';
    if (data.cvd3m?.confirmed) {
        const cvdEmoji = data.cvd3m?.signal?.type === 'bullish' ? '💹' : '🔻';
        cvd3mLine = `#CVD 3m🐋 ${cvd3mIcon}: ${cvdEmoji} ${data.cvd3m?.signal?.type === 'bullish' ? '#BULL' : '#BEAR'} ${data.cvd3m?.signal?.type === 'bullish' ? '✅' : '⚠️'} (3m)`;
    } else if (data.cvd3m?.signal && !data.cvd3m?.confirmed) {
        cvd3mLine = `#CVD 3m🐋 ${cvd3mIcon}: ⚠️ ${data.cvd3m?.signal?.type === 'bullish' ? '#BULL' : '#BEAR'} (não alinhado - 3m)`;
    } else {
        cvd3mLine = `#CVD 3m🐋 ${cvd3mIcon}: ❌ Indefinido (3m)`;
    }
...
async function startScanner() {
    ...
    log(`CVD (Cumulative Volume Delta) - TIMEFRAMES: ${CONFIG.CVD.TIMEFRAMES.join(', ')}`, 'success');
    
    try {
        const token = CONFIG.TELEGRAM.BOT_TOKEN;
        const chatId = CONFIG.TELEGRAM.CHAT_ID;
        const cvdStatus = CONFIG.CVD.WEBSOCKET.ENABLED ? 'CVD REAL via WebSocket' : 'CVD Simulado';
        const initMessage = `🚀 Titanium Prime v3.0 Ativado (CVD Real via WebSocket)\n` +
            `Monitorando: ${currentTopSymbols.length} símbolos\n` +
            `${cvdStatus}\n` +
            `Timeframes CVD: ${CONFIG.CVD.TIMEFRAMES.join(', ')}\n` +
    ...
    while (true) {
        try {
            ...
            // Mostra estatísticas do CVD Real a cada 10 scans
