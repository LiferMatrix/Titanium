...
const config = {
  ...
  EMA_FAST: 13, // Período da EMA rápida (alterado de 34 para 13)
  EMA_SLOW: 34, // Período da EMA lenta (alterado de 89 para 34)
  EMA_MEDIUM: 55, // Novo: Período da EMA média para filtro de preço
...
function getDirectionEmoji(current, previous) {
  if (current > previous) return "⬆️";
  if (current < previous) return "⬇️";
  return "➡️"; // Neutro se igual
}
...
function calculateEMACrossover(data, price) {
  if (!data || data.length < config.EMA_SLOW + 1) return { isBullish: false, isBearish: false, emaFast: null, emaSlow: null, emaMedium: null };
  ...
  if (candles.length < config.EMA_SLOW + 1) return { isBullish: false, isBearish: false, emaFast: null, emaSlow: null, emaMedium: null };
  ...
  const emaMedium = calculateEMA(candles, config.EMA_MEDIUM); // Novo: Calcular EMA 55

  if (emaFast.length < 2 || emaSlow.length < 2 || emaMedium.length < 1) return { isBullish: false, isBearish: false, emaFast: null, emaSlow: null, emaMedium: null };
  ...
  return { isBullish, isBearish, emaFast: currentFast, emaSlow: currentSlow, emaMedium: currentMedium };
}
...
async function sendAlertEMATrend(symbol, data) {
  ...
  const emaMedium = emaCrossover.emaMedium;

  logger.info(`EMA 13/34 (3m) para ${symbol}: Bullish=${isBullishCrossover}, Bearish=${isBearishCrossover}, EMA13=${emaCrossover.emaFast}, EMA34=${emaCrossover.emaSlow}, EMA55=${emaMedium}`);
  ...
  let stoch4h = 'N/A';
  let stoch4hDirection = '';
  let stoch1d = 'N/A';
  let stoch1dDirection = '';
  if (ohlcv4h.length >= 14) {
    ...
    if (stochResult4h.length >= 2) {
      const currentK4h = stochResult4h[stochResult4h.length - 1].k;
      const previousK4h = stochResult4h[stochResult4h.length - 2].k;
      stoch4h = currentK4h.toFixed(2);
      stoch4hDirection = getDirectionEmoji(currentK4h, previousK4h);
    }
  }
  if (ohlcv1d.length >= 14) {
    ...
    if (stochResult1d.length >= 2) {
      const currentK1d = stochResult1d[stochResult1d.length - 1].k;
      const previousK1d = stochResult1d[stochResult1d.length - 2].k;
      stoch1d = currentK1d.toFixed(2);
      stoch1dDirection = getDirectionEmoji(currentK1d, previousK1d);
    }
  }
  ...
  const stoch4hText = stoch4h !== 'N/A' ? `Stoch 4h: ${stoch4h} ${getStochasticEmoji(parseFloat(stoch4h))} ${stoch4hDirection}` : '🔹 Stoch 4h';
  const stoch1dText = stoch1d !== 'N/A' ? `Stoch 1d: ${stoch1d} ${getStochasticEmoji(parseFloat(stoch1d))} ${stoch1dDirection}` : '🔹 Stoch 1d';
  ...
  const isBuySignal = isBullishCrossover &&
                      rsi1h < 60 &&
                      (price > emaMedium);  // Novo: Preço de fechamento acima da EMA 55 para compra
                     
                      
  
  const isSellSignal = isBearishCrossover &&
                       rsi1h > 60 &&
                       (price < emaMedium);  // Novo: Preço de fechamento abaixo da EMA 55 para venda
  ...
  if (isBuySignal) {
    ...
    if (!foiAlertado) {
      ...
      logger.info(`Sinal de compra detectado para ${symbol}: Preço=${format(price)}, EMA 13/34(3m)=Bullish, RSI 1h=${rsi1h.toFixed(2)}, Preço > EMA55=${price > emaMedium}`);
    }
    ...
    if (!foiAlertado) {
      ...
      state.ultimoAlertaPorAtivo[symbol].historico = state.ultimoAlertaPorAtivo[symbol].historico.slice(-config.MAX_HISTORICO_ALERTAS);
      logger.info(`Sinal de venda detectado para ${symbol}: Preço=${format(price)}, EMA 13/34(3m)=Bearish, RSI 1h=${rsi1h.toFixed(2)}, Preço < EMA55=${price < emaMedium}`);
    }
  }
  ...
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      if (ohlcv3m.length < config.EMA_SLOW + 1) {
        logger.warn(`Candles insuficientes para EMA 13/34 (3m) em ${symbol}: ${ohlcv3m.length}`);
        return;
      }
...
async function main() {
  ...
  try {
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium 💹EMAbull...'));
