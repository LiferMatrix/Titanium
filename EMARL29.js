...
const TELEGRAM_BOT_TOKEN = '7708427979:AAF7vVx6AGg';
...
// === NOVAS CONFIGURAÇÕES PARA CCI ===
const CCI_SETTINGS = {
    period: 20,
    emaPeriod: 5,
    timeframes: ['1h', '12h', '1d'],
    buyScore: 6,           // Pontos quando CCI cruzar acima da EMA 5
    sellPenalty: -6,       // Pontos quando CCI cruzar abaixo da EMA 5
    volumeSensitivity: {
        timeframe: '1h',
        emaPeriod: 9,
        buyPoints: 8,      // 8 pontos quando volume estiver subindo
        sellPoints: -8     // 8 pontos quando volume estiver descendo
    }
};
...
const QUALITY_WEIGHTS = {
    ...
    cciAnalysis: 18               // NOVO: Peso para análise CCI (6 pontos por timeframe × 3 timeframes)
};
...
async function getTechnicalIndicatorWithFallback(symbol, indicatorType, params) {
    try {
        switch(indicatorType) {
            ...
            case 'CCI':
                return await getCCIWithFallback(symbol, params);
            default:
...
async function getCCIWithFallback(symbol, params = {}) {
    try {
        const timeframe = params.timeframe || '1h';
        const candles = await getCandlesWithFallback(symbol, timeframe, 50);
        
        if (candles.length < CCI_SETTINGS.period + 10) {
            throw new Error('Dados insuficientes para cálculo CCI');
        }
        ...
        if (!cciValues || cciValues.length < CCI_SETTINGS.emaPeriod) {
            throw new Error('Cálculo CCI falhou');
        }

        const recentCCI = cciValues.slice(-20);
        ...
        const currentEMA = cciEMA[cciEMA.length - 1];
        const previousCCI = cciValues[cciValues.length - 2];
        const previousEMA = cciEMA[cciEMA.length - 2];
        ...
        return {
            ...
            previousCCI,
            previousEMA,
            isAboveEMA: currentCCI > currentEMA,
            isBelowEMA: currentCCI < currentEMA,
            hasCrossedUp: previousCCI <= previousEMA && currentCCI > currentEMA,
            hasCrossedDown: previousCCI >= previousEMA && currentCCI < currentEMA,
            timeframe,
            rawData: {
                cci: cciValues.slice(-10),
                ema: cciEMA.slice(-10)
            }
        };
    } catch (error) {
        console.log(`⚠️ Fallback CCI ${symbol}: ${error.message}`);
        return {
            currentCCI: 0,
            currentEMA: 0,
            previousCCI: 0,
...
// =====================================================================
// 🆕 FUNÇÃO PARA ANALISAR CCI NOS TRÊS TIMEFRAMES
// =====================================================================

async function analyzeCCIForAllTimeframes(symbol, isBullish) {
    try {
        const results = {};
        let totalScore = 0;
        let details = [];

        for (const timeframe of CCI_SETTINGS.timeframes) {
            const cciData = await getCCIWithFallback(symbol, { timeframe });
            
            if (cciData.isFallback) {
                results[timeframe] = {
                    score: 0,
                    details: `CCI ${timeframe}: Fallback (dados indisponíveis)`
                };
                continue;
            }
            ...
            if (isBullish) {
                // Para COMPRA: CCI cruzando acima da EMA 5
                if (cciData.hasCrossedUp) {
                    timeframeScore = CCI_SETTINGS.buyScore;
                    timeframeDetails = `CCI ${timeframe}: +${CCI_SETTINGS.buyScore} pontos (CCI ${cciData.currentCCI.toFixed(1)} cruzou acima da EMA ${cciData.currentEMA.toFixed(1)})`;
                } else if (cciData.isAboveEMA) {
                    timeframeScore = 3; // Bônus menor se já estiver acima
                    timeframeDetails = `CCI ${timeframe}: +3 pontos (CCI ${cciData.currentCCI.toFixed(1)} acima da EMA ${cciData.currentEMA.toFixed(1)})`;
                } else {
                    timeframeScore = CCI_SETTINGS.sellPenalty;
                    timeframeDetails = `CCI ${timeframe}: ${CCI_SETTINGS.sellPenalty} pontos (CCI ${cciData.currentCCI.toFixed(1)} abaixo da EMA ${cciData.currentEMA.toFixed(1)})`;
                }
            } else {
                // Para VENDA: CCI cruzando abaixo da EMA 5
                if (cciData.hasCrossedDown) {
                    timeframeScore = CCI_SETTINGS.buyScore; // Mesmo score positivo para venda quando cruza para baixo
                    timeframeDetails = `CCI ${timeframe}: +${CCI_SETTINGS.buyScore} pontos (CCI ${cciData.currentCCI.toFixed(1)} cruzou abaixo da EMA ${cciData.currentEMA.toFixed(1)})`;
                } else if (cciData.isBelowEMA) {
                    timeframeScore = 3; // Bônus menor se já estiver abaixo
                    timeframeDetails = `CCI ${timeframe}: +3 pontos (CCI ${cciData.currentCCI.toFixed(1)} abaixo da EMA ${cciData.currentEMA.toFixed(1)})`;
                } else {
                    timeframeScore = CCI_SETTINGS.sellPenalty;
                    timeframeDetails = `CCI ${timeframe}: ${CCI_SETTINGS.sellPenalty} pontos (CCI ${cciData.currentCCI.toFixed(1)} acima da EMA ${cciData.currentEMA.toFixed(1)})`;
                }
            }

            results[timeframe] = {
                score: timeframeScore,
                details: timeframeDetails,
                data: cciData
            };

            totalScore += timeframeScore;
            details.push(timeframeDetails);
        }

        // Análise de volume 1h sensível
        const volumeScore = await analyzeSensitiveVolume1h(symbol, isBullish);
        totalScore += volumeScore.score;
        details.push(volumeScore.details);
        ...
        return {
            ...
            summary: `CCI Score Total: ${totalScore} pontos (${details.length} análises)`
        };

    } catch (error) {
        console.log(`⚠️ Erro análise CCI ${symbol}: ${error.message}`);
        return {
            totalScore: 0,
            details: ['CCI: Análise indisponível'],
            results: {},
            volumeAnalysis: { score: 0, details: 'Volume: Análise indisponível' },
            summary: 'CCI: Erro na análise'
        };
    }
}

// =====================================================================
// 🆕 FUNÇÃO PARA ANALISAR VOLUME 1H SENSÍVEL
// =====================================================================

async function analyzeSensitiveVolume1h(symbol, isBullish) {
    try {
        const candles = await getCandlesWithFallback(symbol, CCI_SETTINGS.volumeSensitivity.timeframe, 50);
        
        if (candles.length < CCI_SETTINGS.volumeSensitivity.emaPeriod + 10) {
            return {
                score: 0,
                details: 'Volume 1h: Dados insuficientes',
                trend: 'neutral'
            };
        }
        ...
        const volumeEMA = EMA.calculate({
            period: CCI_SETTINGS.volumeSensitivity.emaPeriod,
            values: volumes
        });

        if (!volumeEMA || volumeEMA.length < 2) {
            return {
                score: 0,
                details: 'Volume 1h: EMA indisponível',
                trend: 'neutral'
            };
        }
        ...
        const previousEMA = volumeEMA[volumeEMA.length - 2];

        const isVolumeRising = currentVolume > currentEMA && currentVolume > previousVolume;
        const isVolumeFalling = currentVolume < currentEMA && currentVolume < previousVolume;
        const volumeRatio = currentVolume / currentEMA;

        let score = 0;
        let details = '';
        let trend = 'neutral';

        if (isVolumeRising && volumeRatio > 1.2) {
            score = CCI_SETTINGS.volumeSensitivity.buyPoints;
            details = `Volume 1h: +${CCI_SETTINGS.volumeSensitivity.buyPoints} pontos (Volume ${volumeRatio.toFixed(2)}x acima da EMA 9 - FORTE ALTA)`;
            trend = 'rising';
        } else if (isVolumeRising) {
            score = 4; // Pontuação menor para alta moderada
            details = `Volume 1h: +4 pontos (Volume ${volumeRatio.toFixed(2)}x acima da EMA 9 - ALTA MODERADA)`;
            trend = 'rising';
        } else if (isVolumeFalling && volumeRatio < 0.8) {
            score = CCI_SETTINGS.volumeSensitivity.sellPoints;
            details = `Volume 1h: ${CCI_SETTINGS.volumeSensitivity.sellPoints} pontos (Volume ${volumeRatio.toFixed(2)}x abaixo da EMA 9 - FORTE BAIXA)`;
            trend = 'falling';
        } else if (isVolumeFalling) {
            score = -4; // Pontuação menor para baixa moderada
            details = `Volume 1h: -4 pontos (Volume ${volumeRatio.toFixed(2)}x abaixo da EMA 9 - BAIXA MODERADA)`;
            trend = 'falling';
        } else {
            details = `Volume 1h: 0 pontos (Volume ${volumeRatio.toFixed(2)}x - NEUTRO)`;
        }
        ...
        console.log(`⚠️ Erro análise volume sensível ${symbol}: ${error.message}`);
        return {
            score: 0,
            details: 'Volume 1h: Erro na análise',
            trend: 'neutral',
            data: null
        };
    }
...
// =====================================================================
// 🛡️ SISTEMA DE RISK LAYER AVANÇADO COM ANÁLISE DE CCI E FALLBACKS
// =====================================================================
...
class SophisticatedRiskLayer {
    constructor() {
        ...
        console.log('🛡️  Risk Layer Sofisticado inicializado com análise de CCI e fallbacks');
    }
    ...
    async assessSignalRisk(signal) {
        try {
            ...
            // NOVO: Analisar risco baseado em análise CCI
            const cciRisk = await this.analyzeCCIRisk(signal);
            riskAssessment.factors.push(cciRisk);
            riskAssessment.overallScore += cciRisk.score * this.riskFactors.CCI_ANALYSIS_RISK.weight;
    ...
    // NOVA FUNÇÃO: Analisar risco por uso de fallbacks
    ...
    // NOVA FUNÇÃO: Analisar risco baseado em análise CCI
    async analyzeCCIRisk(signal) {
        try {
            const cciAnalysis = signal.marketData.cciAnalysis;
            
            if (!cciAnalysis) {
                return { 
                    type: 'CCI_ANALYSIS', 
                    score: 0, 
                    message: 'Sem análise CCI disponível' 
                };
            }

            const totalScore = cciAnalysis.totalScore || 0;
            let score = 0;
            let message = '';
            let riskType = 'NEUTRO';

            // Analisar baseado no score total do CCI
            if (totalScore >= 12) {
                score = -1.0; // Reduz risco - CCI muito favorável
                riskType = 'BAIXO';
                message = `✅ CCI MUITO FAVORÁVEL: ${totalScore} pontos (1h, 12h, 1d alinhados)`;
            } else if (totalScore >= 6) {
                score = -0.5; // Reduz risco - CCI favorável
                riskType = 'BAIXO';
                message = `✅ CCI FAVORÁVEL: ${totalScore} pontos`;
            } else if (totalScore >= 0) {
                score = 0;
                riskType = 'NEUTRO';
                message = `⚪ CCI NEUTRO: ${totalScore} pontos`;
            } else if (totalScore >= -6) {
                score = 1.0;
                riskType = 'MEDIANO';
                message = `⚠️ CCI DESFAVORÁVEL: ${totalScore} pontos`;
            } else {
                score = 2.0;
                riskType = 'ALTO';
                message = `🔴 CCI MUITO DESFAVORÁVEL: ${totalScore} pontos`;
            }

            // Analisar consistência entre timeframes
            const results = cciAnalysis.results || {};
            ...
            if (totalTimeframes > 0) {
                ...
                if (alignmentRatio >= 0.8) {
                    score -= 0.5;
                    message += ` | ${alignedTimeframes}/${totalTimeframes} timeframes alinhados ✅`;
                } else if (alignmentRatio >= 0.5) {
                    message += ` | ${alignedTimeframes}/${totalTimeframes} timeframes alinhados ⚪`;
                } else {
                    score += 0.5;
                    message += ` | Apenas ${alignedTimeframes}/${totalTimeframes} timeframes alinhados ⚠️`;
                }
            }

            // Analisar volume sensível
            const volumeAnalysis = cciAnalysis.volumeAnalysis;
            if (volumeAnalysis) {
                const volumeScore = volumeAnalysis.score || 0;
                
                if (volumeScore >= 6) {
                    score -= 0.3;
                    message += ` | Volume 1h: FORTE ${volumeAnalysis.trend === 'rising' ? 'ALTA' : 'BAIXA'} ✅`;
                } else if (volumeScore <= -6) {
                    score += 0.3;
                    message += ` | Volume 1h: FORTE ${volumeAnalysis.trend === 'rising' ? 'ALTA' : 'BAIXA'} ⚠️`;
                }
            }
            ...
            return {
                ...
                data: {
                    ...
                    alignment: alignedTimeframes + '/' + totalTimeframes,
    ...
    generateRecommendations(riskAssessment) {
        ...
        // Recomendações baseadas em análise CCI
        riskAssessment.factors.forEach(factor => {
            if (factor.type === 'CCI_ANALYSIS') {
                if (factor.data.totalScore < 0) {
                    recommendations.push('⚠️ <i>CCI DESFAVORÁVEL</i>');
                    recommendations.push('• Score CCI negativo: considere evitar');
                    recommendations.push('• Aguarde alinhamento dos timeframes');
                } else if (factor.data.totalScore < 6) {
                    recommendations.push('🟡 <i>CCI NEUTRO</i>');
                    recommendations.push('• Score CCI baixo - cautela');
                    recommendations.push('• Aguarde confirmação adicional');
                } else if (factor.data.totalScore >= 12) {
                    recommendations.push('✅ <i>CCI MUITO FAVORÁVEL</i>');
                    recommendations.push('• Score CCI excelente - alta confiança');
                    recommendations.push('• Pode aumentar tamanho da posição');
                }
            }
        });
        ...
        riskAssessment.factors.forEach(factor => {
            if (factor.score >= 2) {
                switch (factor.type) {
                    case 'CCI_ANALYSIS':
                        recommendations.push(`• <b>CCI desfavorável:</b> Score ${factor.data.totalScore} - considere evitar`);
                        break;
                    case 'VOLATILITY':
    ...
    generateWarnings(riskAssessment) {
        ...
        // Warnings baseados em análise CCI
        riskAssessment.factors.forEach(factor => {
            if (factor.type === 'CCI_ANALYSIS' && factor.data.totalScore < 0) {
                warnings.push(`⚠️ CCI DESFAVORÁVEL: Score ${factor.data.totalScore} pontos`);
            }
        });
        ...
        riskAssessment.factors.forEach(factor => {
            if (factor.score >= 2.5 && factor.type !== 'STOCHASTIC_TREND' && factor.type !== 'CCI_ANALYSIS' && factor.type !== 'FALLBACK') {
                warnings.push(`⚠️ ${factor.message}`);
            } else if (factor.score >= 2 && factor.type !== 'STOCHASTIC_TREND' && factor.type !== 'CCI_ANALYSIS' && factor.type !== 'FALLBACK') {
    ...
    logRiskAssessment(symbol, assessment) {
        ...
        // Log específico para análise CCI
        assessment.factors.forEach(factor => {
            if (factor.type === 'CCI_ANALYSIS') {
                console.log(`   Análise CCI: ${factor.data.totalScore} pontos - ${factor.message}`);
            }
        });
...
// =====================================================================
// 📤 FUNÇÃO ATUALIZADA PARA ENVIAR ALERTAS (COM CCI)
// =====================================================================
...
async function sendSignalAlertWithRisk(signal) {
    try {
        ...
        // 🔹 NOVO: Informações de análise CCI
        const cciAnalysis = signal.marketData.cciAnalysis;
        let cciInfo = '';
        if (cciAnalysis) {
            cciInfo = `\n📊 <i>Análise CCI (20, EMA 5):</i> ${cciAnalysis.totalScore} pontos`;
            cciAnalysis.details.forEach(detail => {
                cciInfo += `\n   ${detail}`;
            });
            
            // Análise de volume sensível
            if (cciAnalysis.volumeAnalysis) {
                const vol = cciAnalysis.volumeAnalysis;
                cciInfo += `\n   ${vol.details} (${vol.trend === 'rising' ? '📈' : vol.trend === 'falling' ? '📉' : '➡️'})`;
            }
        }
        ...
        // 🔹 NOVO: Incluir análise CCI na análise
        let cciAnalysisText = '';
        if (cciAnalysis) {
            if (cciAnalysis.totalScore >= 12) {
                cciAnalysisText = ` | 📈 CCI MUITO FAVORÁVEL (${cciAnalysis.totalScore} pontos)`;
            } else if (cciAnalysis.totalScore >= 6) {
                cciAnalysisText = ` | 📊 CCI FAVORÁVEL (${cciAnalysis.totalScore} pontos)`;
            } else if (cciAnalysis.totalScore >= 0) {
                cciAnalysisText = ` | ⚪ CCI NEUTRO (${cciAnalysis.totalScore} pontos)`;
            } else {
                cciAnalysisText = ` | ⚠️ CCI DESFAVORÁVEL (${cciAnalysis.totalScore} pontos)`;
            }
        }
            ...
            if (signal.isBullish) {
                if (isNearPivot && pivotType === 'resistance') {
                    if (parseFloat(pivotDistance) < 0.3) {
                        analysisType = `Analisando...FALSO ROMPIMENTO (Pivot Bear ${pivotStrengthText})${cciAnalysisText}`;
                        analysisEmoji = '🟡⚠️';
                    } else {
                        analysisType = `Analisando...REVERSÃO (Pivot Bull ${pivotStrengthText})${cciAnalysisText}`;
                        analysisEmoji = '🟢🔍';
                    }
                } else if (rsiValue >= 25 && rsiValue <= RSI_BUY_MAX && stochValid && emaAlignment) {
                    if (isNearPivot && pivotType === 'Suporte') {
                        analysisType = `Analisando...COMPRA (Pivot Bull ${pivotStrengthText})${cciAnalysisText}`;
                        analysisEmoji = '🟢🔍';
                    } else {
                        analysisType = `Analisando...COMPRA${cciAnalysisText}`;
                        analysisEmoji = '🟢🔍';
                    }
                } else if (rsiValue > RSI_BUY_MAX && rsiValue <= 75) {
                    analysisType = `Analisando...CORREÇÃO${cciAnalysisText}`;
                    analysisEmoji = '🟡⚠️';
                } else {
                    analysisType = `Análise...NEUTRA${cciAnalysisText}`;
                    analysisEmoji = '🤖';
                }
            } else {
                if (isNearPivot && pivotType === 'Suporte') {
                    if (parseFloat(pivotDistance) < 0.3) {
                        ...
                        analysisType = `Analisando...EXAUSTÃO (Pivot Bear ${pivotStrengthText})${cciAnalysisText}`;
                        analysisEmoji = '🔴🔍';
                    }
                } else if (rsiValue >= RSI_SELL_MIN && rsiValue <= 75 && !stochValid && !emaAlignment) {
                    if (isNearPivot && pivotType === 'Resistência') {
                        analysisType = `Analisando...VENDA (Pivot Bear ${pivotStrengthText})${cciAnalysisText}`;
                        analysisEmoji = '🔴🔍';
                    } else {
                        analysisType = `Analisando...VENDA${cciAnalysisText}`;
                        analysisEmoji = '🔴🔍';
                    }
...
<i>✨Titanium by @J4Rviz✨</i>
        ...
        console.log(`   Análise CCI: ${cciAnalysis?.totalScore || 0} pontos`);
...
function calculateProbability(signal) {
    ...
    // 🔹 NOVO: Considerar análise CCI na probabilidade
    const cciAnalysis = signal.marketData.cciAnalysis;
    if (cciAnalysis) {
        const cciScore = cciAnalysis.totalScore || 0;
...
async function sendInitializationMessage(allSymbols) {
    try {
        ...
        const message = `
🚀 <b>TITANIUM ATIVADO COM SISTEMA DE FALLBACK ROBUSTO E ANÁLISE CCI!</b>

${brazilTime.full}
📊 Sistema aprimorado com:
• Sistema de Fallback Robusto para todos os componentes
• Análise CCI com EMA 5 (1h, 12h, 1d) - 6 pontos por timeframe
• Volume 1h sensível com EMA 9 - 8 pontos para alta/baixa
• Detecção Robusta de Volume (3m) com fallbacks
• Stochastic Tendência 12h/Diário
• Sistema de Risco Avançado com monitoramento de fallbacks
...
✨ by @J4Rviz
        ...
        if (!success) {
            ...
            console.log('\n' + '='.repeat(60));
            console.log('🚀 TITANIUM ATIVADO COM SISTEMA DE FALLBACK ROBUSTO E ANÁLISE CCI!');
...
// =====================================================================
// 📊 FUNÇÃO PARA CALCULAR QUALIDADE (COM ANÁLISE CCI E FALLBACKS)
// =====================================================================
...
async function calculateSignalQuality(symbol, isBullish, marketData) {
    ...
    // 🔹 NOVO: Pontuação para análise CCI
    if (marketData.cciAnalysis) {
        const cciAnalysis = marketData.cciAnalysis;
        let cciScore = 0;
        let cciDetail = '';
        ...
        if (totalScore >= 12) {
            cciScore = QUALITY_WEIGHTS.cciAnalysis;
            cciDetail = `${cciScore}/${QUALITY_WEIGHTS.cciAnalysis} (CCI EXCELENTE: ${totalScore} pontos - 3 timeframes alinhados)`;
        } else if (totalScore >= 6) {
            cciScore = QUALITY_WEIGHTS.cciAnalysis * 0.8;
            cciDetail = `${cciScore.toFixed(1)}/${QUALITY_WEIGHTS.cciAnalysis} (CCI BOM: ${totalScore} pontos)`;
        } else if (totalScore >= 0) {
            cciScore = QUALITY_WEIGHTS.cciAnalysis * 0.5;
            cciDetail = `${cciScore.toFixed(1)}/${QUALITY_WEIGHTS.cciAnalysis} (CCI NEUTRO: ${totalScore} pontos)`;
        } else if (totalScore >= -6) {
            cciScore = 0;
            cciDetail = `0/${QUALITY_WEIGHTS.cciAnalysis} (CCI DESFAVORÁVEL: ${totalScore} pontos)`;
            failedChecks.push(`CCI: Score negativo (${totalScore} pontos)`);
        } else {
            cciScore = -QUALITY_WEIGHTS.cciAnalysis * 0.3;
            cciDetail = `${cciScore.toFixed(1)}/${QUALITY_WEIGHTS.cciAnalysis} (CCI MUITO DESFAVORÁVEL: ${totalScore} pontos!)`;
            failedChecks.push(`CCI: Score muito negativo (${totalScore} pontos)`);
        }
        
        score += cciScore;
        details.push(` Análise CCI (1h,12h,1d): ${cciDetail}`);
        ...
        failedChecks.push(`CCI: Não analisado`);
    }
...
async function monitorSymbol(symbol) {
    try {
        ...
        // 🔹 NOVO: Analisar CCI para todos os timeframes
        const cciAnalysisData = await analyzeCCIForAllTimeframes(symbol, isBullish);
        ...
        const marketData = {
            ...
            cciAnalysis: cciAnalysisData // 🔹 NOVO: Adicionar dados de análise CCI
        };
        ...
        // 🔹 NOVO: Informações de análise CCI
        const cciAnalysis = cciAnalysisData;
        let cciInfo = 'N/A';
        if (cciAnalysis) {
            cciInfo = `${cciAnalysis.totalScore} pontos (${cciAnalysis.details.length} análises)`;
        }
        ...
        console.log(`   📊 Pivot: ${pivotType} ${pivotDistance}% (${pivotStrength} - ${pivotTimeframe})`);
        console.log(`   📊 Análise CCI: ${cciInfo}`); // 🔹 NOVO
...
// =====================================================================
// 🔄 LOOP PRINCIPAL DO BOT COM SUPORTE A FALLBACKS
// =====================================================================
...
async function mainBotLoop() {
    ...
    console.log(`\n TITANIUM ATIVADO COM SISTEMA DE FALLBACK ROBUSTO E ANÁLISE CCI!`);
    console.log(` ${allSymbols.length} ativos Binance Futures`);
    console.log(` Sistema aprimorado com fallbacks para todos os componentes`);
    console.log(` Análise CCI com EMA 5 (1h, 12h, 1d) - 6 pontos por timeframe`);
    console.log(` Volume 1h sensível com EMA 9 - 8 pontos para alta/baixa`);
...
<i>✨Titanium Risk Management with CCI Analysis by @J4Rviz✨</i>
        `;
...
async function startBot() {
    try {
        ...
        console.log(` Análise CCI com EMA 5 nos timeframes 1h, 12h, 1d`);
        console.log(` Volume 1h sensível com EMA 9 - 8 pontos para alta/baixa`);
        ...
        console.log('🛡️  Risk Layer Sofisticado com análise CCI e monitoramento de fallbacks ativado');
