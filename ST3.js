...
const config = {
  ...
  STOP_SWING_LOOKBACK: 30,
  STOP_ATR_BUFFER: 0.5,
  STOP_ATR_BUFFER_MIN: 1.0,
};
// Logger
...
// ================= DETECÇÃO DE SWING LOW / HIGH ================= //
function detectSwingLow(ohlcv, lookback = 30) {
  ...
  // Só aceita swing se tiver pelo menos 3 candles de distância do preço atual
  if (lookback - swingLowIndex - 1 > 3) {
    return swingLow;
  }
  return null;
}

function detectSwingHigh(ohlcv, lookback = 30) {
  if (!ohlcv || ohlcv.length < 5) return null;
  const recent = ohlcv.slice(-lookback);
  ...
  for (let i = 2; i < recent.length - 2; i++) {
    const candle = recent[i];
    if (candle.high > recent[i-2].high && 
  ...
  // Só aceita swing se tiver pelo menos 3 candles de distância do preço atual
  if (lookback - swingHighIndex - 1 > 3) {
    return swingHigh;
  }
  return null;
}
...
async function sendAlertStochasticCross(symbol, data) {
  const { price, rsi1h, lsr, fundingRate, estocastico4h, estocasticoD, ema13_3m_prev, ema34_3m_prev, ema55, vwap1h, adx1h, fvg, volumeData, atr, ohlcv15m } = data;
  ...
  if (signalStrengthBuy.level && stochOkBuy && rsiOkBuy && atrOk && lsrOkForLong && (volumeOkBuy || isReentryBuy)) {
    ...
    if (!foiAlertado || isReentryBuy) { // Permite reentry mesmo em cooldown (ajuste 8)
      ...
      // === NOVO STOP DINÂMICO BASEADO EM SWING + BUFFER ===
      const atrAtual = atr || 0;
      const swingLow = detectSwingLow(ohlcv15m, config.STOP_SWING_LOOKBACK);
      let stop = swingLow ? swingLow - (config.STOP_ATR_BUFFER * atrAtual) : zonas.suporte;  // fallback se não achar swing
      // Garante que o stop nunca fique "dentro" da entrada (segurança)
      if (stop >= entry) {
        stop = entry - (config.STOP_ATR_BUFFER_MIN * atrAtual); // força um stop válido
      }
      ...
      alertText = buildBuyAlertMessage(symbol, data, count, dataHora, format, tradingViewLink, classificacao, ratio, reward10x, targetPct, targetLong1Pct, targetLong2Pct, targetLong3Pct, buyEntryLow, targetBuy, targetBuyLong1, targetBuyLong2, targetBuyLong3, zonas, price, rsi1hEmoji, lsr, lsrSymbol, fundingRateText, vwap1hText, estocasticoD, stochDEmoji, direcaoD, estocastico4h, stoch4hEmoji, direcao4h, adx1h, volumeZScore, signalStrengthBuy, tag, stop);
    ...
    if (!foiAlertado || isReentrySell) { // Permite reentry (ajuste 8)
      ...
      // === NOVO STOP DINÂMICO BASEADO EM SWING + BUFFER ===
      const atrAtual = atr || 0;
      const swingHigh = detectSwingHigh(ohlcv15m, config.STOP_SWING_LOOKBACK);
      let stop = swingHigh ? swingHigh + (config.STOP_ATR_BUFFER * atrAtual) : zonas.resistencia;  // fallback se não achar swing
      // Garante que o stop nunca fique "dentro" da entrada (segurança)
      if (stop <= entry) {
        stop = entry + (config.STOP_ATR_BUFFER_MIN * atrAtual); // força um stop válido
      }
