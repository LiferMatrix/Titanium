...
// =====================================================================
// 🔄 CIRCUIT BREAKER CLASS (ADICIONADA)
// =====================================================================
...
class AdvancedLearningSystem {
    constructor() {
        ...
        this.openTrades = new Map(); // Trades abertos para tracking
        ...
        this.tradeTrackingHours = 24; // Horas para acompanhar trade
    ...
    async checkTradeOutcome(tradeId) {
        try {
            ...
            // Verificar se atingiu algum target
            ...
            // Se não atingiu target, verificar stop
            if (outcome === 'FAILURE') {
                ...
                if (stopHit) {
                    ...
                    // Se não atingiu nem target nem stop, usar preço atual
            ...
            // Analisar padrões após fechar trade
            await this.analyzePatterns();
    ...
    async analyzePatterns() {
        try {
            ...
            // Otimizar parâmetros se tiver dados suficientes
...
class AdaptiveRateLimiter {
    constructor() {
        ...
        this.adaptiveDelay = 100; // Delay base entre requests
        this.minDelay = 50;       // Delay mínimo
        this.maxDelay = 500;      // Delay máximo
        this.usageThreshold = 0.7; // Threshold para aumentar delay
    ...
    async processQueue() {
        ...
        try {
            while (this.queue.length > 0) {
                ...
                // Verificar limites com delay adaptativo
    ...
    checkLimits(weight) {
        ...
        // Retornar falso se próximo do limite
...
async function sendTelegramAlert(message, isDryRun = false) {
    try {
        if (isDryRun && !LIVE_MODE) {
            console.log(`${DRY_RUN_PREFIX}Alerta: ${message.substring(0, 100)}...`);
            return true;
        }
        ...
        logToFile(`📤 Alerta ${LIVE_MODE ? 'REAL' : 'DRY-RUN'} enviado`);
        return true;
    } catch (error) {
...
class AdaptiveSymbolGroupManager {
    constructor() {
        ...
        this.baseDelay = 8000; // Delay base entre grupos
        this.minDelay = 4000;  // Delay mínimo
        this.maxDelay = 15000; // Delay máximo
    ...
    getNextGroup() {
        ...
        if (this.currentGroupIndex === 0) {
            ...
            // Ajustar delay baseado no uso do rate limiter
            this.adjustDelayBasedOnUsage();
            
            // Pausa estratégica a cada 5 ciclos
    ...
    adjustDelayBasedOnUsage() {
        // Se muitos grupos sem sinais, reduzir delay
        ...
        // Resetar contador se encontrou sinais
...
async function monitorSymbol(symbol) {
    try {
        ...
        const marketData = {
            volume: volumeData,
            volatility: volatilityData,  // NOVO
            lsr: lsrData,                // NOVO
            rsi: rsiData,
            adx1h: adx1hData,
            stoch: stochData,
...
📊 <b>Configuração:</b>
• Ativos: ${allSymbols.length} pares USDT
• Modo: ${LIVE_MODE ? 'REAL' : 'DRY-RUN (TESTE)'}
• Aprendizado: ATIVO AVANÇADO
...
⏰ <b>Início:</b> ${getBrazilianDateTime().full}
🔔 by @J4Rviz.
    `;
    
    await sendTelegramAlert(initMessage, !LIVE_MODE);
    ...
    while (true) {
        try {
            ...
            // Relatório de performance a cada hora
