...
const config = {
  ...
  EMA_FAST: 4, // Período da EMA rápida
  EMA_SLOW: 10, // Período da EMA lenta
};
...
// Função para calcular CVD (Cumulative Volume Delta)
async function calculateCVD(symbol, timeframe = '3m', limit = 100) {
  const cacheKey = `cvd_${symbol}_${timeframe}`;
  const cached = getCachedData(cacheKey);
  if (cached) return cached;
  ...
  try {
    ...
    // Calcula CVD com base nos trades
    for (const trade of trades) {
      const { side, amount } = trade;
      if (!side || !amount || isNaN(amount)) continue;
      if (side === 'buy') cumulativeDelta += amount;
      else if (side === 'sell') cumulativeDelta -= amount;
    }

    // Para verificar se está subindo ou caindo, usamos trades anteriores
    const previousTrades = await withRetry(() => exchangeSpot.fetchTrades(symbol, undefined, limit, { until: trades[0].timestamp - 1 }));
    for (const trade of previousTrades) {
      const { side, amount } = trade;
      if (!side || !amount || isNaN(amount)) continue;
      if (side === 'buy') previousDelta += amount;
      else if (side === 'sell') previousDelta -= amount;
    }

    const result = {
      value: cumulativeDelta,
      isRising: cumulativeDelta > previousDelta,
      percentChange: previousDelta !== 0 ? ((cumulativeDelta - previousDelta) / Math.abs(previousDelta) * 100).toFixed(2) : '0.00'
    };

    setCachedData(cacheKey, result);
    logger.info(`CVD para ${symbol}: Valor=${cumulativeDelta}, IsRising=${result.isRising}, PercentChange=${result.percentChange}%`);
    return result;
  } catch (e) {
    logger.error(`Erro ao calcular CVD para ${symbol}: ${e.message}`);
    return getCachedData(cacheKey) || { value: 0, isRising: false, percentChange: '0.00' };
  }
}

// Função para calcular OBV (On-Balance Volume)
function calculateOBV(ohlcv) {
  if (!ohlcv || ohlcv.length < 2) return { value: 0, isRising: false, percentChange: '0.00' };
  ...
  for (let i = 1; i < ohlcv.length; i++) {
    ...
    if (isNaN(currentClose) || isNaN(previousClose) || isNaN(volume)) continue;
    ...
    // Se currentClose === previousClose, OBV permanece o mesmo
  }

  // Para verificar se está subindo ou caindo, calculamos OBV para o período anterior
  const previousOhlcv = ohlcv.slice(0, -1);
  for (let i = 1; i < previousOhlcv.length; i++) {
    const currentClose = previousOhlcv[i].close;
    const previousClose = previousOhlcv[i - 1].close;
    const volume = previousOhlcv[i].volume;
  ...
  const percentChange = previousObv !== 0 ? ((obv - previousObv) / Math.abs(previousObv) * 100).toFixed(2) : '0.00';
  const result = {
    value: obv,
    isRising: obv > previousObv,
    percentChange
  };

  logger.info(`OBV calculado: Valor=${obv}, IsRising=${result.isRising}, PercentChange=${percentChange}%`);
  return result;
}
...
async function sendAlertEMATrend(symbol, data) {
  ...
  if (!emaCrossover) {
    logger.warn(`Cruzamento EMA 4/10 inválido para ${symbol}`);
    return;
  }
  ...
  // Log para depuração
  logger.info(`EMA 4/10 (3m) para ${symbol}: Bullish=${isBullishCrossover}, Bearish=${isBearishCrossover}, EMA4=${emaCrossover.emaFast}, EMA10=${emaCrossover.emaSlow}`);

  // Calcular CVD e OBV
  const cvd = await calculateCVD(symbol, '3m');
  const ohlcv3mRaw = getCachedData(`ohlcv_${symbol}_3m`) || await withRetry(() => exchangeSpot.fetchOHLCV(symbol, '3m', undefined, config.EMA_SLOW + 1));
  const ohlcv3m = normalizeOHLCV(ohlcv3mRaw);
  const obv = calculateOBV(ohlcv3m);
  ...
  const emaCrossoverText = isBullishCrossover ? `🔘EMA: Bullish ✅` : isBearishCrossover ? `🔘EMA: Bearish 🛑` : `🔘EMA: 🔘Sem Confir.`;
  ...
  const cvdText = cvd ? `${cvd.isRising ? '📈' : '📉'} CVD: ${cvd.percentChange}%` : '🔹 Indisp.';
  const obvText = obv ? `${obv.isRising ? '📈' : '📉'} OBV: ${obv.percentChange}%` : '🔹 Indisp.';
  ...
  let alertText = '';
  // Condições para compra: EMA 4 cruza acima da EMA 10 (3m), RSI 1h < 55, LSR < 1.8, Delta >= 15%, CVD subindo, OBV subindo
  ...
  // Condições para venda: EMA 4 cruza abaixo da EMA 10 (3m), RSI 1h > 60, LSR > 2.8, Delta <= -15%, CVD caindo, OBV caindo
  ...
  // Log das condições
  logger.info(`Condições para ${symbol}: BuySignal=${isBuySignal}, SellSignal=${isSellSignal}, EMA4/10(3m)=${isBullishCrossover ? 'Bullish' : isBearishCrossover ? 'Bearish' : 'Neutro'}, RSI1h=${rsi1h}, OI5m=${oi5m.isRising}, OI15m=${oi15m.isRising}, LSR=${lsr.value}, Delta=${aggressiveDelta.deltaPercent}, CVD=${cvd.isRising}, OBV=${obv.isRising}, Volatility=${volatility.toFixed(4)}`);
  ...
  if (isBuySignal) {
    ...
    if (!foiAlertado) {
      ...
      logger.info(`Sinal de compra detectado para ${symbol}: Preço=${format(price)}, EMA 4/10(3m)=Bullish, RSI 1h=${rsi1h.toFixed(2)}, OI 5m=${oi5m.percentChange}%, OI 15m=${oi15m.percentChange}%, LSR=${lsr.value ? lsr.value.toFixed(2) : 'N/A'}, Delta=${aggressiveDelta.deltaPercent}%, CVD=${cvd.isRising}, OBV=${obv.isRising}, NearBuyZone=${isNearBuyZone}, Volatility=${volatility.toFixed(4)}`);
    }
    ...
    if (!foiAlertado) {
      ...
      state.ultimoAlertaPorAtivo[symbol].historico = state.ultimoAlertaPorAtivo[symbol].historico.slice(-config.MAX_HISTORICO_ALERTAS);
      logger.info(`Sinal de venda detectado para ${symbol}: Preço=${format(price)}, EMA 4/10(3m)=Bearish, RSI 1h=${rsi1h.toFixed(2)}, OI 5m=${oi5m.percentChange}%, OI 15m=${oi15m.percentChange}%, LSR=${lsr.value ? lsr.value.toFixed(2) : 'N/A'}, Delta=${aggressiveDelta.deltaPercent}%, CVD=${cvd.isRising}, OBV=${obv.isRising}, NearSellZone=${isNearSellZone}, Volatility=${volatility.toFixed(4)}`);
    }
  }
  ...
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      if (ohlcv3m.length < config.EMA_SLOW + 1) {
        logger.warn(`Candles insuficientes para EMA 4/10 (3m) em ${symbol}: ${ohlcv3m.length}`);
        return;
      }
...
async function main() {
  ...
  try {
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium Scalp 💹Start...'));
