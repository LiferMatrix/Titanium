...
async function sendSingleAlert(alert) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            log(`${penaltyIcon} ALERTA ENVIADO: ${alert.direction} ${alert.symbol} [${alert.timeframe}] - Score: ${alert.confirmationScore} | Pivô: +${pivotBonus} | Volume: ${alert.volume1h.direction} (${alert.volume1h.percentage.toFixed(1)}%) | Penalidades: ${alert.totalPenalty}`, 'success');
        }
        return true;
    } catch (error) {
...
async function sendGroupedAlerts(alerts, priority) {
    ...
    if (buyAlerts.length > 0) {
        ...
        buyAlerts.slice(0, 5).forEach(a => {
            ...
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} ${pivotIcon} | Vol: ${a.volume1h.direction} (${a.volume1h.percentage.toFixed(1)}%) | Penal: ${a.totalPenalty}\n`;
        });
    ...
    if (sellAlerts.length > 0) {
        ...
        sellAlerts.slice(0, 5).forEach(a => {
            const pivotIcon = a.pivotDivergenceDetected ? '🎯' : '';
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} ${pivotIcon} | Vol: ${(100 - a.volume1h.percentage).toFixed(1)}% vendedor | Penal: ${a.totalPenalty}\n`;
        });
...
// =====================================================================
// === ANALISAR TIMEFRAME (VERSÃO ATUALIZADA SEM CVD) ===
// =====================================================================
...
// =====================================================================
// === FUNÇÃO: ANÁLISE DE DIVERGÊNCIA COM PIVÔ (SEM CVD) ===
// =====================================================================
...
async function startScanner() {
    ...
    log(`🎯 Pivô Divergence: ATIVADO | Bônus: ${CONFIG.ALERTS.PIVOT_DIVERGENCE.SCORE_BONUS}`, 'success');
