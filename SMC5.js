...
const QUALITY_WEIGHTS = {
    ...
    marketStructure: 20, // 🔵 NOVO: Peso da estrutura de mercado
    adx1h: 5,        // Peso do ADX 1h
    adx15m: 5,       // Peso do ADX 15m
    bosChoch: 15     // 🔵 NOVO: Peso do BOS/CHOCH
};

// === ANÁLISE DE MARKET STRUCTURE ===
const USE_MARKET_STRUCTURE = true;
const MS_TIMEFRAME = '1h';
const MS_LOOKBACK = 200;
const MS_SWING_LEFT_BARS = 3;
const MS_SWING_RIGHT_BARS = 3;

// Configurações de Order Blocks
const ORDER_BLOCK_SETTINGS = {
    lookback: 50,
    reversalBars: 3,
    minSizePercent: 0.5,
    recentThreshold: 10 // candles atrás
};

// Configurações de Fair Value Gaps
const FVG_SETTINGS = {
    minSizePercent: 0.1,
    minBodySizePercent: 0.05
};

// Configurações de Liquidity
const LIQUIDITY_SETTINGS = {
    swingLookback: 20,
    tolerancePercent: 0.5,
    recentHighLowLookback: 20
};
...
const ADX_15M_SETTINGS = {
    period: 14,
    timeframe: '15m',
    strongTrendThreshold: 25
};

// 🔵 CONFIGURAÇÕES BOS/CHOCH
const BOS_CHOCH_SETTINGS = {
    timeframe: '1h',
    lookback: 50, // Velas para análise
    recentBars: 10, // Últimas N velas para verificar BOS
    minSwingPoints: 3 // Mínimo de swing points para análise válida
};

// 🔵 NOVA FUNÇÃO: Verificar Break of Structure (BOS) ou Change of Character (CHOCH)
async function checkBOSorCHOCH(symbol, isBullishSweep) {
    try {
        // Buscar candles do timeframe 1h
        const candles = await getCandlesCached(symbol, BOS_CHOCH_SETTINGS.timeframe, BOS_CHOCH_SETTINGS.lookback);
        
        if (candles.length < BOS_CHOCH_SETTINGS.lookback) {
            return {
                isValid: true, // Se não temos dados suficientes, não bloqueamos
                type: 'N/A',
                message: 'BOS/CHOCH: ⚪ Dados insuficientes',
                score: 0
            };
        }
        
        // Identificar swing points
        const { swingHighs, swingLows } = identifySwingPoints(
            candles, 
            MS_SWING_LEFT_BARS, 
            MS_SWING_RIGHT_BARS
        );
        
        if (swingHighs.length < BOS_CHOCH_SETTINGS.minSwingPoints || 
            swingLows.length < BOS_CHOCH_SETTINGS.minSwingPoints) {
            return {
                isValid: true,
                type: 'N/A',
                message: 'BOS/CHOCH: ⚪ Swing points insuficientes',
                score: 0
            };
        }
        
        // Ordenar swing points por tempo (mais recentes primeiro)
        const recentSwingHighs = swingHighs.slice(-BOS_CHOCH_SETTINGS.minSwingPoints);
        const recentSwingLows = swingLows.slice(-BOS_CHOCH_SETTINGS.minSwingPoints);
        
        // Determinar estrutura atual
        let structure = 'neutral';
        let lastHigherHigh = null;
        let lastHigherLow = null;
        let lastLowerHigh = null;
        let lastLowerLow = null;
        
        // Verificar se temos higher highs e higher lows (estrutura bullish)
        if (recentSwingHighs.length >= 2 && recentSwingLows.length >= 2) {
            ...
            if (lastHigh.price > prevHigh.price && lastLow.price > prevLow.price) {
                ...
                structure = 'bearish';
                lastLowerHigh = lastHigh;
                lastLowerLow = lastLow;
            }
        }
        
        // Verificar BOS/CHOCH baseado no tipo de sweep
        let bosChochType = 'none';
        let isValid = false;
        let score = 0;
        let message = '';
        
        const currentPrice = candles[candles.length - 1].close;
        const recentCandles = candles.slice(-BOS_CHOCH_SETTINGS.recentBars);
        
        if (isBullishSweep) {
            // Para sweep BULLISH (reversão de baixa para alta)
            // Precisamos de CHOCH (Change of Character) bearish -> bullish
            // Ou BOS bullish claro
            
            // Verificar CHOCH: quebra do último lower low seguido de higher low
            if (lastLowerLow) {
                // Verificar se o preço quebrou o último lower low recentemente
                const brokeLowerLow = recentCandles.some(c => c.close < lastLowerLow.price);
                const formedHigherLow = recentCandles.some(c => c.close > lastLowerLow.price * 1.005);
                
                if (brokeLowerLow && formedHigherLow) {
                    bosChochType = 'CHOCH_bearish_to_bullish';
                    isValid = true;
                    score = QUALITY_WEIGHTS.bosChoch;
                    message = '✅ BOS/CHOCH: CHOCH bearish→bullish detectado';
                }
            }
            
            // Verificar BOS bullish: quebra do último higher high
            if (!isValid && lastHigherHigh) {
                const brokeHigherHigh = recentCandles.some(c => c.close > lastHigherHigh.price);
                if (brokeHigherHigh) {
                    bosChochType = 'BOS_bullish';
                    isValid = true;
                    score = QUALITY_WEIGHTS.bosChoch * 0.8;
                    message = '✅ BOS/CHOCH: BOS bullish detectado';
                }
            }
            
            // Se não encontrou BOS/CHOCH específico
            if (!isValid) {
                message = '❌ BOS/CHOCH: Nenhum BOS/CHOCH bullish detectado antes do sweep';
            }
            
        } else {
            // Para sweep BEARISH (reversão de alta para baixa)
            // Precisamos de CHOCH (Change of Character) bullish -> bearish
            // Ou BOS bearish claro
            
            // Verificar CHOCH: quebra do último higher high seguido de lower high
            if (lastHigherHigh) {
                // Verificar se o preço quebrou o último higher high recentemente
                const brokeHigherHigh = recentCandles.some(c => c.close > lastHigherHigh.price);
                const formedLowerHigh = recentCandles.some(c => c.close < lastHigherHigh.price * 0.995);
                
                if (brokeHigherHigh && formedLowerHigh) {
                    bosChochType = 'CHOCH_bullish_to_bearish';
                    isValid = true;
                    score = QUALITY_WEIGHTS.bosChoch;
                    message = '✅ BOS/CHOCH: CHOCH bullish→bearish detectado';
                }
            }
            
            // Verificar BOS bearish: quebra do último lower low
            if (!isValid && lastLowerLow) {
                const brokeLowerLow = recentCandles.some(c => c.close < lastLowerLow.price);
                if (brokeLowerLow) {
                    bosChochType = 'BOS_bearish';
                    isValid = true;
                    score = QUALITY_WEIGHTS.bosChoch * 0.8;
                    message = '✅ BOS/CHOCH: BOS bearish detectado';
                }
            }
            
            // Se não encontrou BOS/CHOCH específico
            if (!isValid) {
                message = '❌ BOS/CHOCH: Nenhum BOS/CHOCH bearish detectado antes do sweep';
            }
        }
        ...
        return {
            ...
            score: score,
            structure: structure,
            lastHigherHigh: lastHigherHigh?.price,
            lastLowerLow: lastLowerLow?.price,
            currentPrice: currentPrice
        };
        
    } catch (error) {
        logToFile(`⚠️ Erro ao verificar BOS/CHOCH(${symbol}): ${error.message}`);
        return {
            isValid: true, // Não bloquear em caso de erro
            type: 'error',
            message: 'BOS/CHOCH: ⚪ Erro na análise',
            score: 0
        };
    }
}

// 🔵 FUNÇÃO MODIFICADA: Calcular qualidade do sinal (agora inclui BOS/CHOCH)
async function calculateSignalQuality(symbol, isBullish, volumeCheck, oiCheck, volatilityCheck, lsrCheck, rsi1h, emas3mData, bosChochCheck) {
    let score = 0;
    let details = [];
    ...
    // 🔵 7. BOS/CHOCH (15 pontos - NOVO)
    if (bosChochCheck) {
        let bosChochScore = bosChochCheck.score || 0;
        score += bosChochScore;
        details.push(`🏗️ BOS/CHOCH: ${bosChochScore}/${QUALITY_WEIGHTS.bosChoch} ${bosChochCheck.message}`);
    } else {
        details.push(`🏗️ BOS/CHOCH: 0/${QUALITY_WEIGHTS.bosChoch} (não analisado)`);
    }
    
    // 🔵 8. Market Structure (20 pontos)
    if (USE_MARKET_STRUCTURE) {
        const msAnalysis = await analyzeMarketStructure(symbol);
        
        if (msAnalysis.hasValidData) {
            let msScore = 0;
            
            if (isBullish && msAnalysis.bias === 'bullish') {
                msScore = QUALITY_WEIGHTS.marketStructure;
                details.push(`🏛️ MS: ${msScore}/${QUALITY_WEIGHTS.marketStructure} (confluence bullish alinhado)`);
            } else if (!isBullish && msAnalysis.bias === 'bearish') {
                msScore = QUALITY_WEIGHTS.marketStructure;
                details.push(`🏛️ MS: ${msScore}/${QUALITY_WEIGHTS.marketStructure} (confluence bearish alinhado)`);
            } else if (msAnalysis.bias === 'neutral') {
                msScore = QUALITY_WEIGHTS.marketStructure * 0.5;
                details.push(`🏛️ MS: ${msScore}/${QUALITY_WEIGHTS.marketStructure} (confluence neutro)`);
            } else {
                msScore = 0;
                details.push(`🏛️ MS: 0/${QUALITY_WEIGHTS.marketStructure} (confluence oposto ao sinal)`);
            }
            
            score += msScore;
        } else {
            details.push(`🏛️ MS: 0/${QUALITY_WEIGHTS.marketStructure} (dados insuficientes)`);
        }
    } else {
        details.push(`🏛️ MS: N/A (desativado)`);
    }
    ...
    if (adx1h.raw !== null) {
        ...
        if (adx1h.isStrongTrend) {
            adx1hScore = QUALITY_WEIGHTS.adx1h;
            details.push(`📊 ADX 1h: ${adx1hScore}/${QUALITY_WEIGHTS.adx1h} (${adx1h.adx} > ${ADX_1H_SETTINGS.strongTrendThreshold})`);
        } else {
            adx1hScore = QUALITY_WEIGHTS.adx1h * 0.3;
            details.push(`📊 ADX 1h: ${adx1hScore}/${QUALITY_WEIGHTS.adx1h} (${adx1h.adx} ≤ ${ADX_1H_SETTINGS.strongTrendThreshold})`);
        }
        
        score += adx1hScore;
    } else {
        details.push(`📊 ADX 1h: 0/${QUALITY_WEIGHTS.adx1h} (dados indisponíveis)`);
    }
    
    // 🔵 10. ADX 15m (5 pontos)
    const adx15m = await getADX15m(symbol);
    if (adx15m.raw !== null) {
        let adx15mScore = 0;
        
        if (adx15m.isStrongTrend) {
            adx15mScore = QUALITY_WEIGHTS.adx15m;
            details.push(`📊 ADX 15m: ${adx15mScore}/${QUALITY_WEIGHTS.adx15m} (${adx15m.adx} > ${ADX_15M_SETTINGS.strongTrendThreshold})`);
        } else {
            adx15mScore = QUALITY_WEIGHTS.adx15m * 0.3;
            details.push(`📊 ADX 15m: ${adx15mScore}/${QUALITY_WEIGHTS.adx15m} (${adx15m.adx} ≤ ${ADX_15M_SETTINGS.strongTrendThreshold})`);
        }
        
        score += adx15mScore;
    } else {
        details.push(`📊 ADX 15m: 0/${QUALITY_WEIGHTS.adx15m} (dados indisponíveis)`);
    }
...
// 🔵 FUNÇÃO MODIFICADA: Detect sweeps com filtro BOS/CHOCH
async function detectSweeps(symbol) {
    try {
        ...
        if (buySweepDetected || sellSweepDetected) {
            // 🔵 NOVO: Verificar BOS/CHOCH antes de armazenar o sweep
            const bosChochCheck = await checkBOSorCHOCH(symbol, buySweepDetected);
            
            // Se não passar no critério BOS/CHOCH, não armazenar
            if (!bosChochCheck.isValid) {
                logToFile(`❌ Sweep ${buySweepDetected ? 'Bullish' : 'Bearish'} rejeitado por falta de BOS/CHOCH em ${symbol}`);
                return null;
            }
            ...
            if (buySweepDetected) {
                ...
                recentSweeps[symbol].lastBuyBOS = bosChochCheck;
            } else if (sellSweepDetected) {
                recentSweeps[symbol].lastSellSweep = now;
                recentSweeps[symbol].sellSweepPrice = price;
                recentSweeps[symbol].lastSellBOS = bosChochCheck;
            }
            
            logToFile(`✅ Sweep ${buySweepDetected ? 'Bullish' : 'Bearish'} detectado e armazenado com BOS/CHOCH válido em ${symbol}: ${bosChochCheck.type}`);
...
// 🔵 FUNÇÃO ATUALIZADA: Monitorar confirmações (agora inclui BOS/CHOCH no quality score)
async function monitorConfirmation(symbol) {
    try {
        ...
        // 🔵 ADICIONAR ANÁLISE DE MARKET STRUCTURE
        const msAnalysis = USE_MARKET_STRUCTURE ? await analyzeMarketStructure(symbol) : null;
        
        // 🔵 ADICIONAR ADX 1h e 15m
        const [adx1h, adx15m] = await Promise.all([
            getADX1h(symbol),
            getADX15m(symbol)
        ]);
        ...
        if (hadBuySweep && emas3mData.isAboveEMA55 && emas3mData.isEMA13CrossingUp) {
            ...
            // 🔵 OBTER BOS/CHOCH do sweep armazenado
            const bosChochCheck = recentSweeps[symbol].lastBuyBOS;
            
            // 🔵 CALCULAR qualidade do sinal (agora inclui BOS/CHOCH)
            const qualityScore = await calculateSignalQuality(
                symbol, true, volumeCheck, oiCheck, volatilityCheck, lsrCheck, rsi1h, emas3mData, bosChochCheck
            );
            ...
            if (now - alertsCooldown[symbol].lastBuyConfirmation > COOLDOWN) {
                ...
                // 🔵 ATUALIZAR FUNÇÃO buildAlertMessage (agora inclui BOS/CHOCH)
        ...
        if (hadSellSweep && emas3mData.isBelowEMA55 && emas3mData.isEMA13CrossingDown) {
            ...
            // 🔵 OBTER BOS/CHOCH do sweep armazenado
            const bosChochCheck = recentSweeps[symbol].lastSellBOS;
            
            // 🔵 CALCULAR qualidade do sinal (agora inclui BOS/CHOCH)
            const qualityScore = await calculateSignalQuality(
                symbol, false, volumeCheck, oiCheck, volatilityCheck, lsrCheck, rsi1h, emas3mData, bosChochCheck
            );
...
// 🔵 FUNÇÃO ATUALIZADA: Construir mensagem de alerta (agora inclui BOS/CHOCH)
function buildAlertMessage(isBullish, symbol, priceFormatted, brDateTime, targetsAndStop, 
                          ...
                          qualityScore, msAnalysis = null, bosChochCheck = null) {
    ...
    // 🔵 ADICIONAR BOS/CHOCH SE DISPONÍVEL
    if (bosChochCheck) {
        message += `🏗️ ${bosChochCheck.message}\n`;
    }
    ...
    message += ` #ADX 1h: <b>${adx1h?.adx || 'N/A'}</b> ${adx1h?.isStrongTrend ? '✅' : '⚠️'}\n`;
    message += ` #ADX 15m: <b>${adx15m?.adx || 'N/A'}</b> ${adx15m?.isStrongTrend ? '✅' : '⚠️'}\n`;
...
// 🔵 FUNÇÃO MODIFICADA: Inicializar recentSweeps com campos BOS
function initAlertsCooldown(symbols) {
    symbols.forEach(symbol => {
        ...
        recentSweeps[symbol] = {
            ...
            buySweepPrice: 0,
            sellSweepPrice: 0,
            lastBuyBOS: null,    // 🔵 NOVO: Armazenar análise BOS para sweep de compra
            lastSellBOS: null    // 🔵 NOVO: Armazenar análise BOS para sweep de venda
        };
    });
}

// 🔵 FUNÇÃO ATUALIZADA: Main bot loop com logging de BOS/CHOCH
async function mainBotLoop() {
    ...
    const initMsg = '\n' +
        ...
        ` 🏛️ MARKET STRUCTURE: ${USE_MARKET_STRUCTURE ? 'ATIVADA' : 'DESATIVADA'} (${MS_TIMEFRAME})\n` +
        ` 📊 ADX 1h: Período ${ADX_1H_SETTINGS.period} (limite ${ADX_1H_SETTINGS.strongTrendThreshold})\n` +
        ` 📊 ADX 15m: Período ${ADX_15M_SETTINGS.period} (limite ${ADX_15M_SETTINGS.strongTrendThreshold})\n` +
        ` 🏗️ FILTRO BOS/CHOCH: ATIVADO (requer BOS claro antes do sweep)\n` +
        ` 📊 FILTRO DE QUALIDADE: Score mínimo ${QUALITY_THRESHOLD}/100 (inclui BOS/CHOCH, Market Structure e ADX)\n` +
        '='.repeat(70) + '\n';
    ...
    const brDateTime = getBrazilianDateTime();
    await sendAlert(`🤖 <b>SMC Confirmation Bot com BOS/CHOCH, Market Structure e ADX (Todos os pares Binance Futures)</b>\n` +
                    `📍 <b>Horário Brasil (BRT):</b> ${brDateTime.full}\n` +
                    `📊 <b>Ativos monitorados:</b> ${SYMBOLS.length} pares USDT\n` +
                    `🏛️ <b>Market Structure:</b> ${USE_MARKET_STRUCTURE ? 'ATIVADA' : 'DESATIVADA'} (${MS_TIMEFRAME})\n` +
                    `📊 <b>ADX 1h:</b> Período ${ADX_1H_SETTINGS.period} (limite ${ADX_1H_SETTINGS.strongTrendThreshold})\n` +
                    `📊 <b>ADX 15m:</b> Período ${ADX_15M_SETTINGS.period} (limite ${ADX_15M_SETTINGS.strongTrendThreshold})\n` +
                    `🏗️ <b>Filtro BOS/CHOCH:</b> ATIVADO (requer BOS claro no 1h antes do sweep)\n` +
                    `📈 <b>Filtro de qualidade:</b> ${QUALITY_THRESHOLD}/100 (inclui BOS/CHOCH, Market Structure e ADX)\n` +
                    `⚠️ <b>ATENÇÃO:</b> Sweeps sem BOS/CHOCH serão ignorados\n` +
                    `by @J4Rviz.`);
    ...
    while (true) {
        try {
            ...
            // 🔵 PROCESSAR DETECÇÃO DE SWEEPS (COM FILTRO BOS/CHOCH)
            console.log('🔍 Detectando sweeps (com filtro BOS/CHOCH)...');
            for (let i = 0; i < SYMBOLS.length; i += BATCH_SIZE) {
                ...
                // Contar sweeps com BOS válido
                sweepsWithBOS += batchResults.filter(r => r.bosChoch && r.bosChoch.isValid).length;
            ...
            for (let i = 0; i < SYMBOLS.length; i += BATCH_SIZE) {
                ...
                for (const alert of batchAlerts) {
                    ...
                    console.log(`🏗️ BOS/CHOCH: ${alert.bosChochCheck?.type || 'N/A'}`);
                    console.log(`📊 ADX 1h: ${alert.adx1h?.adx || 'N/A'} (${alert.adx1h?.isStrongTrend ? 'Forte' : 'Fraca'})`);
                    console.log(`📊 ADX 15m: ${alert.adx15m?.adx || 'N/A'} (${alert.adx15m?.isStrongTrend ? 'Forte' : 'Fraca'})`);
                    if (alert.msAnalysis && alert.msAnalysis.hasValidData) {
                        console.log(`🏛️ MS: ${alert.msAnalysis.confluenceMessage}`);
                    }
                    
                    logToFile(`ALERTA CONFIRMAÇÃO ${alert.signal} - ${alert.symbol} - Preço: $${alert.price} - Score: ${alert.qualityScore.score} - BOS/CHOCH: ${alert.bosChochCheck?.type || 'N/A'} - ADX 1h: ${alert.adx1h?.adx} - ADX 15m: ${alert.adx15m?.adx} - MS: ${alert.msAnalysis ? alert.msAnalysis.confluenceMessage : 'N/A'}`);
            ...
            if (sweepsDetected > 0) {
                console.log(`\n🔍 ${sweepsDetected} sweep(s) detectado(s)`);
                console.log(`🏗️ ${sweepsWithBOS} sweep(s) com BOS/CHOCH válido(s)`);
            }
...
// 🔵 NOVA FUNÇÃO: Calcular ADX
async function getADX(symbol, timeframe = '1h', period = 14) {
    try {
        // Precisamos de mais candles para o ADX calcular corretamente
        const candles = await getCandlesCached(symbol, timeframe, period * 3 + 10);
        ...
        if (!adxValues || adxValues.length === 0) {
            return {
                adx: "N/A",
                isStrongTrend: false,
                message: `ADX ${timeframe}: ⚪ Erro no cálculo`,
                raw: null
            };
        }

        // Acessar .adx do último objeto
        ...
        if (typeof currentADX !== 'number' || isNaN(currentADX) || currentADX < 0) {
            return {
                adx: "N/A",
                isStrongTrend: false,
                message: `ADX ${timeframe}: ⚪ Valor inválido`,
                raw: null
            };
        }

        const threshold = timeframe === '1h' ? ADX_1H_SETTINGS.strongTrendThreshold : ADX_15M_SETTINGS.strongTrendThreshold;
        const isStrongTrend = currentADX > threshold;
        ...
        return {
            ...
            message: isStrongTrend ?
                `✅ ADX ${timeframe}: ${currentADX.toFixed(2)} (tendência forte)` :
                `⚠️ ADX ${timeframe}: ${currentADX.toFixed(2)} (tendência fraca)`
        };

    } catch (error) {
        logToFile(`⚠️ Erro ao calcular ADX ${timeframe}(${symbol}): ${error.message}`);
        return {
            adx: "N/A",
            isStrongTrend: false,
            message: `ADX ${timeframe}: ⚪ Erro na análise`,
            raw: null
        };
    }
}

// 🔵 FUNÇÕES ESPECÍFICAS PARA ADX 1h E 15m
async function getADX1h(symbol) {
    return await getADX(symbol, ADX_1H_SETTINGS.timeframe, ADX_1H_SETTINGS.period);
}

async function getADX15m(symbol) {
    return await getADX(symbol, ADX_15M_SETTINGS.timeframe, ADX_15M_SETTINGS.period);
}
...
function cleanupCaches() {
    ...
    Object.keys(recentSweeps).forEach(symbol => {
        if (recentSweeps[symbol].lastBuySweep && 
            ...
            recentSweeps[symbol].lastBuyBOS = null;
        }
        ...
        if (recentSweeps[symbol].lastSellSweep && 
            ...
            recentSweeps[symbol].lastSellBOS = null;
        }
    });
...
// === NOVAS FUNÇÕES DE MARKET STRUCTURE ===

// 🔵 FUNÇÃO: Identificar swing highs e lows
function identifySwingPoints(candles, leftBars = MS_SWING_LEFT_BARS, rightBars = MS_SWING_RIGHT_BARS) {
...
// 🔵 FUNÇÃO: Identificar estruturas de mercado (MSS - Market Structure Shift)
function identifyMarketStructure(candles) {
    const { swingHighs, swingLows } = identifySwingPoints(candles, 3, 3);
    ...
    if (swingHighs.length >= 3 && swingLows.length >= 3) {
        // Últimos 3 swing points
        const recentHighs = swingHighs.slice(-3);
        const recentLows = swingLows.slice(-3);
        
        // Verificar Higher Highs e Higher Lows (Bullish)
        const isHigherHighs = recentHighs[2] && recentHighs[1] && recentHighs[2].price > recentHighs[1].price;
        const isHigherLows = recentLows[2] && recentLows[1] && recentLows[2].price > recentLows[1].price;
        
        // Verificar Lower Highs e Lower Lows (Bearish)
        const isLowerHighs = recentHighs[2] && recentHighs[1] && recentHighs[2].price < recentHighs[1].price;
        const isLowerLows = recentLows[2] && recentLows[1] && recentLows[2].price < recentLows[1].price;
        
        if (isHigherHighs && isHigherLows) {
            structure = 'bullish';
            lastHigherHigh = recentHighs[2];
            lastHigherLow = recentLows[2];
        } else if (isLowerHighs && isLowerLows) {
            structure = 'bearish';
            lastLowerHigh = recentHighs[2];
            lastLowerLow = recentLows[2];
        } else if (isHigherHighs && !isHigherLows) {
            structure = 'possible_bullish_break';
        } else if (!isLowerHighs && isLowerLows) {
            structure = 'possible_bearish_break';
        }
    }
    ...
    return {
        ...
        swingHighs,
        swingLows,
        hasValidStructure: structure !== 'sideways'
    };
}

// 🔵 FUNÇÃO: Identificar Order Blocks
function identifyOrderBlocks(candles, lookback = ORDER_BLOCK_SETTINGS.lookback) {
    const blocks = [];
    ...
    for (let i = 3; i < candles.length; i++) {
        // Bullish Order Block: Uma candle com corpo pequeno seguida por uma grande candle de baixa
        if (i >= lookback) {
            const current = candles[i];
            const previous = candles[i-1];
            const twoBack = candles[i-2];
            
            // Bullish OB: Doji ou pequena candle seguida por grande candle baixa
            const previousBodySize = Math.abs(previous.close - previous.open);
            const previousRange = previous.high - previous.low;
            const previousBodyRatio = previousBodySize / previousRange;
            
            const currentBodySize = Math.abs(current.close - current.open);
            const currentRange = current.high - current.low;
            const currentBodyRatio = currentBodySize / currentRange;
            
            // Bullish OB criteria
            if (previousBodyRatio < 0.3 && current.close < current.open && 
                currentBodyRatio > 0.7 && current.close < previous.low) {
                ...
                blocks.push({
                    ...
                    index: i-1,
                    time: previous.time,
                    isRecent: i >= candles.length - ORDER_BLOCK_SETTINGS.recentThreshold
                });
            }
            
            // Bearish OB criteria
            if (previousBodyRatio < 0.3 && current.close > current.open && 
                currentBodyRatio > 0.7 && current.close > previous.high) {
    ...
    // Filtrar apenas os mais recentes e significativos
    const recentBlocks = blocks.filter(b => b.isRecent);
    const significantBlocks = recentBlocks.filter(b => {
        const sizePercent = ((b.high - b.low) / b.low) * 100;
        return sizePercent >= ORDER_BLOCK_SETTINGS.minSizePercent;
    });
    
    return {
        allBlocks: blocks,
        recentBlocks: recentBlocks,
        significantBlocks: significantBlocks,
        bullishBlocks: significantBlocks.filter(b => b.type === 'bullish'),
        bearishBlocks: significantBlocks.filter(b => b.type === 'bearish')
    };
}

// 🔵 FUNÇÃO: Identificar Fair Value Gaps (FVG)
function identifyFairValueGaps(candles) {
    ...
    for (let i = 2; i < candles.length; i++) {
        ...
        // Bullish FVG: A candle anterior tem sombra superior acima da máxima da candle dois períodos atrás
        if (previous.high > twoBack.high && current.low > previous.low) {
            const gapTop = Math.min(previous.high, current.low);
            const gapBottom = Math.max(twoBack.high, previous.low);
            
            if (gapTop > gapBottom) {
                const gapSize = ((gapTop - gapBottom) / gapBottom) * 100;
                const bodySize = Math.abs(previous.close - previous.open);
                const bodyPercent = (bodySize / (previous.high - previous.low)) * 100;
                
                if (gapSize >= FVG_SETTINGS.minSizePercent && 
                    bodyPercent >= FVG_SETTINGS.minBodySizePercent) {
                    ...
                    gaps.push({
                        ...
                        index: i-1,
                        time: previous.time,
                        isRecent: i >= candles.length - 10
                    });
                }
            }
        }
        
        // Bearish FVG: A candle anterior tem sombra inferior abaixo da mínima da candle dois períodos atrás
        if (previous.low < twoBack.low && current.high < previous.high) {
            const gapTop = Math.min(twoBack.low, previous.high);
            const gapBottom = Math.max(previous.low, current.high);
    ...
    return {
        allGaps: gaps,
        recentGaps: gaps.filter(g => g.isRecent),
        bullishGaps: gaps.filter(g => g.type === 'bullish'),
        bearishGaps: gaps.filter(g => g.type === 'bearish')
    };
}

// 🔵 FUNÇÃO: Identificar áreas de liquidez
function identifyLiquidityAreas(candles, lookback = LIQUIDITY_SETTINGS.swingLookback) {
    const liquidityAreas = [];
    
    if (candles.length < lookback + 1) {
        return liquidityAreas;
    }
    ...
    // Áreas de liquidez acima das máximas (stop losses de venda)
    liquidityAreas.push({
        type: 'above_high',
        price: maxHigh,
        description: 'Liquidez acima da máxima recente',
        distancePercent: 0
    });
    
    // Áreas de liquidez abaixo das mínimas (stop losses de compra)
    liquidityAreas.push({
        type: 'below_low',
        price: minLow,
        description: 'Liquidez abaixo da mínima recente',
        distancePercent: 0
    });
    
    // Verificar últimos 20 candles para identificar clusters de preço
    const priceClusters = {};
    const clusterTolerance = (maxHigh - minLow) * (LIQUIDITY_SETTINGS.tolerancePercent / 100);
    
    recentCandles.forEach(candle => {
        const clusterKey = Math.round(candle.close / clusterTolerance);
        
        if (!priceClusters[clusterKey]) {
            priceClusters[clusterKey] = {
                price: candle.close,
                count: 1,
                totalVolume: candle.volume
            };
        } else {
            priceClusters[clusterKey].count++;
            priceClusters[clusterKey].totalVolume += candle.volume;
        }
    });
    ...
    significantClusters.forEach((cluster, index) => {
        if (index < 3) { // Top 3 clusters
            liquidityAreas.push({
                type: 'volume_cluster',
                price: cluster.price,
                description: `Cluster de volume (${cluster.count} candles)`,
...
// 🔵 FUNÇÃO: Analisar market structure completa
async function analyzeMarketStructure(symbol) {
    try {
        if (!USE_MARKET_STRUCTURE) {
            return {
                enabled: false,
                message: "Análise de Market Structure desativada"
            };
        }
        
        const candles = await getCandlesCached(symbol, MS_TIMEFRAME, MS_LOOKBACK);
        
        if (candles.length < 50) {
            return {
                enabled: true,
                message: "Dados insuficientes para análise de Market Structure",
                hasValidData: false
            };
        }
        
        // Executar todas as análises
        const marketStructure = identifyMarketStructure(candles);
        const orderBlocks = identifyOrderBlocks(candles);
        const fairValueGaps = identifyFairValueGaps(candles);
        const liquidityAreas = identifyLiquidityAreas(candles);
        
        // Determinar confluence
        let confluenceScore = 0;
        let confluenceDetails = [];
        
        // Pontuar estrutura de mercado
        if (marketStructure.hasValidStructure) {
            confluenceScore += 25;
            confluenceDetails.push(`📊 Estrutura: ${marketStructure.structure} (+25)`);
        }
        
        // Pontuar order blocks relevantes
        if (orderBlocks.significantBlocks.length > 0) {
            confluenceScore += 20;
            confluenceDetails.push(`🧱 Order Blocks: ${orderBlocks.significantBlocks.length} significativos (+20)`);
        }
        
        // Pontuar FVGs recentes
        if (fairValueGaps.recentGaps.length > 0) {
            confluenceScore += 15;
            confluenceDetails.push(`📈 FVGs: ${fairValueGaps.recentGaps.length} recentes (+15)`);
        }
        
        // Pontuar áreas de liquidez identificadas
        if (liquidityAreas.significantClusters.length > 0) {
            confluenceScore += 10;
            confluenceDetails.push(`💧 Liquidez: ${liquidityAreas.significantClusters.length} clusters (+10)`);
        }
        
        // Determinar se há confluence bullish ou bearish
        let bias = 'neutral';
        ...
        return {
            ...
            bias,
            confluenceMessage,
            summary: `${bias === 'bullish' ? '🟢' : bias === 'bearish' ? '🔴' : '⚪'} MS: ${confluenceMessage} (Score: ${confluenceScore}/70)`
        };
        
    } catch (error) {
        logToFile(`⚠️ Erro na análise de Market Structure(${symbol}): ${error.message}`);
        return {
            enabled: true,
            hasValidData: false,
            message: "Erro na análise de Market Structure",
            confluenceScore: 0,
            bias: 'neutral',
            confluenceMessage: 'Erro na análise'
        };
    }
}
...
// Iniciar o bot
console.log('\n' + '='.repeat(80));
console.log('🤖 BOT DE CONFIRMAÇÕES SMC 1H COM BOS/CHOCH, MARKET STRUCTURE E ADX (TODOS OS PARES BINANCE FUTURES)');
