...
// =====================================================================
// === FUNÇÃO CHECK BOLLINGER TOUCH COM CONFIRMAÇÃO DE CANDLE ===
// =====================================================================
async function checkBollingerTouch(symbol, type) {
    try {
        ...
        if (!candles || candles.length < CONFIG.MONITOR.BOLLINGER.PERIOD + 1) return false;
        ...
        if (type === 'buy') {
            // Verifica se tocou na banda inferior
            const touchedLowerBand = currentPrice <= bollinger.lower;
            
            if (!touchedLowerBand) return false;
            
            // CONFIRMAÇÃO DE CANDLE PARA COMPRA: candle de alta (close > low anterior)
            const candleConfirmed = currentCandle.close > previousCandle.low;
            
            if (candleConfirmed) {
                console.log(`✅ Bollinger COMPRA confirmado para ${symbol}: preço ${formatPrice(currentPrice)} ≤ banda inferior ${formatPrice(bollinger.lower)} | candle confirmado (close ${formatPrice(currentCandle.close)} > low anterior ${formatPrice(previousCandle.low)})`);
                return true;
            } else {
                console.log(`❌ Bollinger COMPRA bloqueado para ${symbol}: tocou banda inferior mas candle NÃO confirmado (close ${formatPrice(currentCandle.close)} ≤ low anterior ${formatPrice(previousCandle.low)})`);
                return false;
            }
            
        } else {
            // Verifica se tocou na banda superior
            const touchedUpperBand = currentPrice >= bollinger.upper;
            
            if (!touchedUpperBand) return false;
            
            // CONFIRMAÇÃO DE CANDLE PARA VENDA: candle de baixa (close < high anterior)
            const candleConfirmed = currentCandle.close < previousCandle.high;
            
            if (candleConfirmed) {
                console.log(`✅ Bollinger VENDA confirmado para ${symbol}: preço ${formatPrice(currentPrice)} ≥ banda superior ${formatPrice(bollinger.upper)} | candle confirmado (close ${formatPrice(currentCandle.close)} < high anterior ${formatPrice(previousCandle.high)})`);
                return true;
            } else {
                console.log(`❌ Bollinger VENDA bloqueado para ${symbol}: tocou banda superior mas candle NÃO confirmado (close ${formatPrice(currentCandle.close)} ≥ high anterior ${formatPrice(previousCandle.high)})`);
                return false;
            }
        }
...
// =====================================================================
// === MONITOR CVD+RSI ALERTS (SEM DOMINÂNCIA) ===
// =====================================================================
async function monitorCVDRSIAlerts() {
    ...
    for (const item of [...watched.positive, ...watched.negative]) {
        ...
        if (isUp && rsi.value < CONFIG.MONITOR.RSI.CVD_RSI_BUY_THRESHOLD) {
            ...
            if (!bollingerTouch) {
                console.log(`🚫 Alerta CVD+RSI COMPRA bloqueado para ${item.symbol}: Sem toque na banda inferior do Bollinger (com confirmação de candle)`);
                continue;
            }
        ...
        if (isDown && rsi.value > CONFIG.MONITOR.RSI.CVD_RSI_SELL_THRESHOLD) {
            ...
            if (!bollingerTouch) {
                console.log(`🚫 Alerta CVD+RSI VENDA bloqueado para ${item.symbol}: Sem toque na banda superior do Bollinger (com confirmação de candle)`);
                continue;
            }
...
// =====================================================================
// === MONITOR CVD AND RSI (ALERTAS COMPLETOS - SEM DOMINÂNCIA) ===
// =====================================================================
...
// =====================================================================
// === FUNÇÕES DE FORMATAÇÃO DAS MENSAGENS ===
// =====================================================================
...
async function startMonitor() {
    ...
    console.log('✅ Confirmação de candle ATIVADA:');
    console.log('   - COMPRA: candle de alta (close > low anterior)');
    console.log('   - VENDA: candle de baixa (close < high anterior)');
    console.log('='.repeat(70));
