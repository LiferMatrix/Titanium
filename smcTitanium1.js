...
async function analyzeCCI(symbol, timeframe = '4h') {
    try {
        ...
        const prevEma = cciValues.length > 5 ? calculateEMAForCCI(cciValues.slice(0, -1), 5) : cciEma;
        
        let direction = '';
        let directionEmoji = '';
        
        // Determina direção baseado na posição em relação à EMA
...
async function sendSMCAlert(analysis) {
    ...
    message += `<b>#Entrada_Fibo_OTE:</b> <i>${analysis.oteText}</i>\n`;
    ...
    message += `<i>✨ Titanium Prime by @J4Rviz</i>`;
