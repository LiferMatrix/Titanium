...
function checkCandleRejection(candles, entryPrice, tradeType) {
    ...
    // Para COMPRA: rejeição de baixa (wick inferior longo)
    if (tradeType === 'BUY') {
    ...
    // Para VENDA: rejeição de alta (wick superior longo)
    if (tradeType === 'SELL') {
...
const CONFIG = {
    ...
    MONITOR: {
        ...
        MIN_SCORE_ACCEPT: 50,  // SCORE MÍNIMO OBRIGATÓRIO
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        // Adicionar mensagem de rejeição no retorno
...
async function sendInitMessage() {
    const msg = `<i>⚡ Titanium ⚡</i>\n\n<i> Monitorando com REJEIÇÃO DE PREÇO OBRIGATÓRIA (wick ≥ 2x corpo)</i>`;
