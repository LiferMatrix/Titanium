...
async function sendAlertRSI(symbol, price, rsi5m, rsi15m, rsi1h, rsi4h, rsi1d, lsr, fundingRate, support, resistance, vwap1h, stoch4h, stoch1d, stoch4hPrevious, stoch1dPrevious, volume3m) {
  ...
  let volume3mText = 'Vol 3m Estável';
  if (volume3m.status !== '🔹 ESTÁVEL' && volume3m.status !== '🔹 Indisp.') {
    const multiplier = (parseFloat(volume3m.percentChange) / 100 + 1).toFixed(1);
    const direction = volume3m.status.includes('COMPRADOR') ? 'Comprador' : 'Vendedor';
    volume3mText = `Vol 3m ${direction} ${multiplier}x`;
  }
