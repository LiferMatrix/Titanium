...
async function sendFVGBullAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData) {
    ...
    let fvgDetailsText = '';
    for (const fvg of fvgData.bullish) {
        fvgDetailsText += `\n   └─ ${fvg.timeframe}: ${formatPrice(fvg.bottom)} → ${formatPrice(fvg.top)} (gap: ${fvg.gapSize.toFixed(2)}%)`;
    }
    ...
    const message = `<b>🟢 ${symbolShort} FVG Bull Confirmado</b>\n` +
                    `<i>${dt.date.slice(0,5)} ${dt.time.slice(0,5)}hs</i> ${tvLinkText}\n` +
                    `<b> FVG Bullish + Volume 3min Comprador</b>\n` +
                    `<b> Preço atual:</b> ${formatPrice(currentPrice)}\n` +
                    `<b> Timeframes com FVG:</b> ${fvgTimeframesText}\n` +
                    `<b> Zonas FVG Bullish:</b>${fvgDetailsText}\n` +
                    `<b> Volume 3min:</b> ${volumeData.volumeRatio}x média (${volumeData.direction})\n` +
                    `<i>${divergenciaText}</i>\n` +
                    `<i>${macdText}</i>\n` +
                    `<i>${chochText}</i>\n` +
...
async function sendFVGBearAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData) {
    ...
    const message = `<b>🔴 ${symbolShort} FVG Bear Confirmado</b>\n` +
                    `<i>${dt.date.slice(0,5)} ${dt.time.slice(0,5)}hs</i> ${tvLinkText}\n` +
                    
                    `<b> FVG Bearish + Volume 3min Vendedor</b>\n` +
                   
                    `<b> Preço atual:</b> ${formatPrice(currentPrice)}\n` +
                    `<b> Timeframes com FVG:</b> ${fvgTimeframesText}\n` +
                    `<b> Zonas FVG Bearish:</b>${fvgDetailsText}\n` +
                    `<b> Volume 3min:</b> ${volumeData.volumeRatio}x média (${volumeData.direction})\n` +
                    `<i>${divergenciaText}</i>\n` +
                    `<i>${macdText}</i>\n` +
                    `<i>${chochText}</i>\n` +
...
async function startMonitorWithReconnect() {
    ...
    console.log(`📊 Alertas FVG + Volume 3min ATIVOS com dados completos`);
    console.log('='.repeat(70));
