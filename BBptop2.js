...
const CONFIG = {
    ...
    // =================================================================
    // CONFIGURAÇÕES DO CVD (FLUXO DE ORDENS) - TIMEFRAME 15 MINUTOS
    // =================================================================
    CVD: {
        ENABLED: true,
        TIMEFRAME: '15m',
...
// =====================================================================
// === FUNÇÕES CVD (CUMULATIVE VOLUME DELTA) - TIMEFRAME 15 MINUTOS ===
// =====================================================================

async function getCVDData(symbol) {
    try {
        const interval = CONFIG.CVD.TIMEFRAME;
        const limit = CONFIG.CVD.LOOKBACK_CANDLES + 10;
        const candles = await getCandles(symbol, interval, limit);
        
        if (!candles || candles.length < CONFIG.CVD.LOOKBACK_CANDLES) return null;
        
        // Calcula o CVD simulando: cada candle tem volume e direção baseada no close vs open
        let cumulativeDelta = 0;
        const deltas = [];
        
        for (let i = 0; i < candles.length; i++) {
            const candle = candles[i];
            // Direção: se close > open = compra agressiva, se close < open = venda agressiva
        ...
        return {
            ...
            previousCandle: deltas[deltas.length - 2]
        };
    } catch (error) {
        log(`Erro ao buscar CVD para ${symbol}: ${error.message}`, 'error');
        return null;
    }
}

function detectCVDDivergence(cvdData, candles) {
    if (!cvdData || !cvdData.deltas || cvdData.deltas.length < 20) return null;
    
    const deltas = cvdData.deltas;
    const prices = candles.map(c => c.close);
    const recentDeltas = deltas.slice(-CONFIG.CVD.LOOKBACK_CANDLES);
    const recentPrices = prices.slice(-CONFIG.CVD.LOOKBACK_CANDLES);
    
    // Encontra máximas e mínimas do preço e CVD
    const priceHighs = findLocalExtremes(recentPrices, 'high');
    const priceLows = findLocalExtremes(recentPrices, 'low');
    const cvdHighs = findLocalExtremes(recentDeltas.map(d => d.cumulative), 'high');
    const cvdLows = findLocalExtremes(recentDeltas.map(d => d.cumulative), 'low');
    
    let divergence = null;
    
    // Divergência de Alta (Bullish): preço faz nova mínima, CVD faz mínima mais alta
    if (priceLows.length >= 2 && cvdLows.length >= 2) {
        ...
        if (lastPriceLow.value < prevPriceLow.value && lastCvdLow.value > prevCvdLow.value) {
            divergence = {
                ...
                strength: Math.abs(lastPriceLow.value - prevPriceLow.value) / prevPriceLow.value * 100,
                score: CONFIG.CVD.SCORE_BONUS.BULLISH_DIVERGENCE
            };
        }
    }
    
    // Divergência de Baixa (Bearish): preço faz nova máxima, CVD faz máxima mais baixa
    if (priceHighs.length >= 2 && cvdHighs.length >= 2) {
        ...
        if (lastPriceHigh.value > prevPriceHigh.value && lastCvdHigh.value < prevCvdHigh.value) {
            divergence = {
                ...
                strength: Math.abs(lastPriceHigh.value - prevPriceHigh.value) / prevPriceHigh.value * 100,
...
async function analyzeCVD(symbol, currentPrice, isGreenAlert) {
    if (!CONFIG.CVD.ENABLED) return { cvdScore: 0, cvdSignal: null, cvdConfirmed: false };
    
    try {
        const candles = await getCandles(symbol, CONFIG.CVD.TIMEFRAME, 30);
        const cvdData = await getCVDData(symbol);
        
        if (!cvdData || !candles || candles.length < 20) {
            return { cvdScore: 0, cvdSignal: null, cvdConfirmed: false };
        }
        
        const cvdDivergence = detectCVDDivergence(cvdData, candles);
        ...
        if (cvdDivergence) {
            ...
            if (isAligned) {
                log(`✅ ${symbol} - CVD divergência alinhada (${cvdDivergence.type}) +${cvdDivergence.score}`, 'success');
                return {
                ...
                log(`⚠️ ${symbol} - CVD divergência ${cvdDivergence.type} não alinhada com direção ${isGreenAlert ? 'COMPRA' : 'VENDA'}`, 'warning');
                return { cvdScore: 0, cvdSignal: cvdDivergence, cvdConfirmed: false };
            }
        }
        ...
        log(`Erro na análise CVD para ${symbol}: ${error.message}`, 'error');
        return { cvdScore: 0, cvdSignal: null, cvdConfirmed: false };
    }
}
...
async function sendSingleAlert(alert) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            log(`${penaltyIcon} ALERTA ENVIADO: ${alert.direction} ${alert.symbol} [${alert.timeframe}] - Score: ${alert.confirmationScore} | CVD: ${alert.cvdConfirmed ? '✅' : '❌'} | Volume: ${alert.volume1h.direction} (${alert.volume1h.percentage.toFixed(1)}%) | Penalidades: ${alert.totalPenalty}`, 'success');
        }
        return true;
    } catch (error) {
...
async function sendGroupedAlerts(alerts, priority) {
    ...
    if (buyAlerts.length > 0) {
        ...
        buyAlerts.slice(0, 5).forEach(a => {
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} | CVD: ${a.cvdConfirmed ? '✅' : '❌'} | Vol: ${a.volume1h.direction} (${a.volume1h.percentage.toFixed(1)}%) | Penal: ${a.totalPenalty}\n`;
        });
    ...
    if (sellAlerts.length > 0) {
        ...
        sellAlerts.slice(0, 5).forEach(a => {
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} | CVD: ${a.cvdConfirmed ? '✅' : '❌'} | Vol: ${(100 - a.volume1h.percentage).toFixed(1)}% vendedor | Penal: ${a.totalPenalty}\n`;
        });
...
async function analyzeSymbol(symbol) {
    try {
        ...
        // =================================================================
        // ANÁLISE DO CVD (FLUXO DE ORDENS) - TIMEFRAME 15 MINUTOS
        // =================================================================
        ...
        if (cvdAnalysis.cvdConfirmed) {
            ...
            log(`✅ ${symbol} - CVD confirmado! Bônus +${cvdAnalysis.cvdScore} | Score total: ${maxScore.toFixed(1)}`, 'success');
        }
...
function formatAlert(data) {
    ...
    // Linha do CVD
    const cvdLine = data.cvdConfirmed ? ` #Vol #Delta : ${data.cvdSignal?.type === 'bullish' ? '💹BULL' : '🔻BEAR'}  [+${data.cvdScore}]` : `Vol Delta: ❌ Indefinido`;
...
Titanium Prime v2.0 (CVD 15m) by @J4Rviz</i>`;
}
...
async function startScanner() {
    ...
    try {
        const token = CONFIG.TELEGRAM.BOT_TOKEN;
        const chatId = CONFIG.TELEGRAM.CHAT_ID;
        const initMessage = `🚀 Titanium Prime v2.0 Ativado\n` +
