...
const config = {
  ...
  EMA_LOOKBACK: 100, // Candles para calcular EMAs no 3m (deve cobrir EMA_SLOW_PERIOD + margem)
...
async function sendAlertRSI(symbol, price, rsi5m, rsi15m, rsi1h, rsi4h, rsi1d, lsr, fundingRate, oi5m, oi15m, support, resistance, vwap1h, stoch4h, stoch1d, stoch4hPrevious, stoch1dPrevious, alertType, emoji) {
  const agora = Date.now();
  if (!state.ultimoRSIAlert[symbol]) state.ultimoRSIAlert[symbol] = { historico: [], visitedOversold: false, visitedOverbought: false, lastEmaFast: null, lastEmaSlow: null };
  ...
  // Montar texto do alerta SEM volume
...
async function monitorRSI() {
  try {
    ...
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      // Buscar dados (aumentar lookback para 3m para EMAs)
      let ohlcv3mRaw = getCachedData(`${cacheKeyPrefix}_3m`) || await withRetry(() => exchangeSpot.fetchOHLCV(symbolWithSlash, '3m', undefined, config.EMA_LOOKBACK));
      ...
      // Removido volume3m da chamada

      // Verificar gatilhos de alerta com memória + cruzamento EMA
      let alertType = null;
      let emoji = '';

      if (state.ultimoRSIAlert[symbol].visitedOversold && bullCross) {
        // Verifica qual zona RSI estava quando cruzou (usa RSI atual, assumindo que não saiu da zona antes do cruzamento)
      ...
      if (state.ultimoRSIAlert[symbol].visitedOverbought && bearCross) {
