...
const CONFIG = {
    ...
    ALERTS: {
        ...
        CONFIRMATION: {
            FALLBACK_INTERVALS: ['3m', '1m', '5m', '15m'],
            VOLUME_THRESHOLDS: {
                '3m': 1.5,
                '1m': 1.8,
                '5m': 1.3,
...
// =====================================================================
// === FUNÇÃO PARA BUSCAR VOLUME RATIO DE QUALQUER TIMEFRAME ===
// =====================================================================
async function getVolumeRatioForInterval(symbol, interval) {
    try {
        const candles = await getCandles(symbol, interval, 30);
        if (!candles || candles.length < 21) return 1.0;
        ...
        log(`Erro ao buscar volume ratio para ${symbol} ${interval}: ${error.message}`, 'warning');
        return 1.0;
    }
}
...
// =====================================================================
// === ANALISAR TIMEFRAME (COM ANÁLISE DE REVERSÃO EM TIMEFRAMES MENORES COM FALLBACK) ===
// =====================================================================
async function analyzeTimeframe(symbol, candles, timeframe) {
    try {
        ...
        // === FALLBACK EM CASCATA PARA TIMEFRAMES MENORES ===
        let usedTimeframe = timeframe;
        let reversalCandles = candles.slice(-10);
        let fullConfirmationCandles = null;
        let confirmationStatus = 'pending';
        let usedInterval = null;
        
        // Aplica fallback apenas para timeframes maiores (1h, 4h, 1d)
        if (CONFIG.ALERTS.CONFIRMATION.REQUIRE_CONFIRMATION && 
            (timeframe === '1h' || timeframe === '4h' || timeframe === '1d')) {
            
            const fallbackIntervals = CONFIG.ALERTS.CONFIRMATION.FALLBACK_INTERVALS;
            let candlesFound = null;
            
            for (const interval of fallbackIntervals) {
                try {
                    const candlesTemp = await getCandles(symbol, interval, 30);
                    if (candlesTemp && candlesTemp.length >= 15) {
                        candlesFound = candlesTemp;
                        usedInterval = interval;
                        break;
                    }
                } catch (error) {
                    log(`${symbol} - Falha ao buscar ${interval}, tentando próximo...`, 'warning');
                    await new Promise(r => setTimeout(r, 200));
                }
            }
            
            if (candlesFound && usedInterval) {
                fullConfirmationCandles = candlesFound;
                reversalCandles = candlesFound.slice(-15);
                usedTimeframe = usedInterval;
                
                if (usedInterval !== '3m') {
                    log(`${symbol} [${timeframe}] - Usando fallback ${usedInterval} (3m indisponível)`, 'warning');
                    confirmationStatus = 'fallback';
                } else {
                    confirmationStatus = 'confirmed';
                    log(`${symbol} [${timeframe}] - Análise de reversão em ${usedInterval} para entrada antecipada`, 'info');
                }
            } else {
                log(`${symbol} [${timeframe}] - Sem dados de timeframe menor, alerta bloqueado por segurança`, 'warning');
                return null;
            }
        } else {
            // Se não requer confirmação ou timeframe é menor, usa o próprio timeframe
        ...
        // === CONFIRMAÇÃO NO TIMEFRAME MENOR COM THRESHOLDS ADAPTATIVOS ===
        if (fullConfirmationCandles && fullConfirmationCandles.length >= 2 && usedInterval) {
            const prevCandle = fullConfirmationCandles[fullConfirmationCandles.length - 2];
            const lastCandleConf = fullConfirmationCandles[fullConfirmationCandles.length - 1];
            
            // Obter volume ratio para o intervalo usado
            let volumeRatio = 1.0;
            if (usedInterval === '3m') {
                volumeRatio = additional.volume3m.ratio;
            } else {
                volumeRatio = await getVolumeRatioForInterval(symbol, usedInterval);
            }
            
            // Thresholds adaptativos baseado no timeframe usado
            const volumeThreshold = CONFIG.ALERTS.CONFIRMATION.VOLUME_THRESHOLDS[usedInterval] || 1.5;
            const openCloseTolerance = CONFIG.ALERTS.CONFIRMATION.OPEN_CLOSE_TOLERANCE[usedInterval] || 0;
            
            const volumeSpikeRequired = volumeRatio > volumeThreshold;
            
            if (touchedLower) {
                const touchedBand = prevCandle.low <= bb.lower;
                const closedAboveBand = lastCandleConf.close > bb.lower;
                const openAbovePrevClose = lastCandleConf.open > prevCandle.close * (1 + openCloseTolerance);
                
                if (!touchedBand) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} anterior NÃO tocou a banda inferior`, 'warning');
                    return null;
                }
                if (!closedAboveBand) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} atual não fechou acima da banda inferior`, 'warning');
                    return null;
                }
                if (!openAbovePrevClose) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} atual não abriu acima do fechamento anterior`, 'warning');
                    return null;
                }
                if (!volumeSpikeRequired) {
                    log(`${symbol} [${timeframe}] - Volume ${usedInterval} insuficiente (${volumeRatio.toFixed(2)}x < ${volumeThreshold}x)`, 'warning');
                    return null;
                }
                
                log(`${symbol} [${timeframe}] - Confirmação ${usedInterval} OK: Toque + fechamento acima + abertura > anterior + volume ${volumeRatio.toFixed(2)}x`, 'success');
                
            } else if (touchedUpper) {
                const touchedBand = prevCandle.high >= bb.upper;
                const closedBelowBand = lastCandleConf.close < bb.upper;
                const openBelowPrevClose = lastCandleConf.open < prevCandle.close * (1 - openCloseTolerance);
                
                if (!touchedBand) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} anterior NÃO tocou a banda superior`, 'warning');
                    return null;
                }
                if (!closedBelowBand) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} atual não fechou abaixo da banda superior`, 'warning');
                    return null;
                }
                if (!openBelowPrevClose) {
                    log(`${symbol} [${timeframe}] - Candle de ${usedInterval} atual não abriu abaixo do fechamento anterior`, 'warning');
                    return null;
                }
                if (!volumeSpikeRequired) {
                    log(`${symbol} [${timeframe}] - Volume ${usedInterval} insuficiente (${volumeRatio.toFixed(2)}x < ${volumeThreshold}x)`, 'warning');
                ...
                log(`${symbol} [${timeframe}] - Confirmação ${usedInterval} OK: Toque + fechamento abaixo + abertura < anterior + volume ${volumeRatio.toFixed(2)}x`, 'success');
            }
        } else if (CONFIG.ALERTS.CONFIRMATION.REQUIRE_CONFIRMATION && 
                   (timeframe === '1h' || timeframe === '4h' || timeframe === '1d')) {
            log(`${symbol} [${timeframe}] - Sem candles de confirmação suficientes, alerta bloqueado`, 'warning');
            return null;
        }
        ...
        const confirmationInfo = confirmationStatus === 'confirmed' ? `(3m)` : 
                                 (confirmationStatus === 'fallback' ? `(${usedInterval} fallback)` : '(original)');
        
        log(`🎯 ${symbol} [${timeframe}] - ${direction} CONFIRMADA! Score: ${confirmationScore} (reversão em ${usedTimeframe}${confirmationInfo}) | RSI: ${additional.rsi} | Bandwidth: ${bandwidth.toFixed(2)}% | Volume: ${additional.volume1h.direction} (${additional.volume1h.percentage.toFixed(1)}%) | Stop: ${formatPrice(stopLoss)} | Penalidades: ${totalPenalty}`, 'success');
        ...
        const alert = {
            ...
            bandwidth: bandwidth,
            usedTimeframe: `${usedTimeframe}${confirmationStatus !== 'pending' && usedInterval ? ` (${usedInterval})` : ''}`,
            ...additional
        };
