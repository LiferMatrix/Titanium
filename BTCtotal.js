...
// =====================================================================
// === FUNÇÃO PARA VERIFICAR VOLUME DE 3 MINUTOS (MELHORADA) ===
// =====================================================================
async function check3MinVolumeEnhanced(symbol, direction) {
    try {
        const candles = await getCandles(symbol, '3m', 20);
        if (!candles || candles.length < 10) return { confirmed: false, error: 'candles insuficientes' };
        
        const lastCandle = candles[candles.length - 1];
        const secondLastCandle = candles[candles.length - 2];
        
        // 1. Volume ratio (média dos últimos 10 vs atual)
        const avgVolume = candles.slice(-11, -1).reduce((a, b) => a + b.volume, 0) / 10;
        const volumeRatio = lastCandle.volume / avgVolume;
        
        // 2. Volume explosivo (candle atual vs anterior)
        const volumeSpikeRatio = lastCandle.volume / secondLastCandle.volume;
        
        // 3. Tendência de volume (últimos 3 candles crescendo?)
        const last3Volumes = candles.slice(-3).map(c => c.volume);
        const increasingVolume = last3Volumes[0] < last3Volumes[1] && last3Volumes[1] < last3Volumes[2];
        
        // 4. Range do candle (volatilidade)
        const candleRange = Math.abs(lastCandle.high - lastCandle.low);
        const avgRange = candles.slice(-10).reduce((a,b) => a + (b.high - b.low), 0) / 10;
        const rangeRatio = candleRange / avgRange;
        
        // Critérios de confirmação combinados
        let confirmations = 0;
        let reasons = [];
        
        // Volume ratio mínimo 1.5x
        if (volumeRatio >= 1.5) {
            confirmations++;
            reasons.push(`volume ${volumeRatio.toFixed(1)}x média`);
            
            if (volumeRatio >= 2.5) {
                confirmations++;
                reasons.push(`volume EXPLOSIVO (${volumeRatio.toFixed(1)}x)`);
            }
        }
        
        // Spike de volume (candle 2x maior que anterior)
        if (volumeSpikeRatio >= 1.8) {
            confirmations++;
            reasons.push(`spike de volume (${volumeSpikeRatio.toFixed(1)}x anterior)`);
        }
        
        // Tendência de volume crescendo
        if (increasingVolume) {
            confirmations++;
            reasons.push(`volume crescente 3 candles`);
        }
        
        // Range aumentado (volatilidade)
        if (rangeRatio >= 1.3) {
            confirmations++;
            reasons.push(`range expandido (${rangeRatio.toFixed(1)}x média)`);
        }
        
        // Verifica direção do candle
        const isBullishCandle = lastCandle.close > lastCandle.open;
        const isBearishCandle = lastCandle.close < lastCandle.open;
        
        const directionMatch = (direction === 'BUY' && isBullishCandle) || 
                               (direction === 'SELL' && isBearishCandle);
        
        if (!directionMatch) {
            return { 
                confirmed: false, 
                reason: `candle na direção oposta (${isBullishCandle ? 'alta' : 'baixa'})`,
                volumeRatio: volumeRatio.toFixed(1)
            };
        }
        
        // Pelo menos 2 confirmações para considerar válido
        const confirmed = confirmations >= 2;
        
        let strength = 'FRACO';
        if (confirmations >= 3) strength = '🔥 FORTE';
        else if (confirmations >= 2) strength = '✅ MÉDIO';
        
        return { 
            confirmed, 
            volumeRatio: volumeRatio.toFixed(1), 
            direction: direction === 'BUY' ? 'comprador' : 'vendedor',
            confirmations,
            strength,
...
// =====================================================================
// === FUNÇÃO PARA AVALIAR QUALIDADE DO FVG ===
// =====================================================================
function evaluateFVGQuality(fvg, candles, currentPrice) {
    let score = 0;
    let reasons = [];
    
    // 1. Gap size (maior gap = mais relevante)
    if (fvg.gapSize >= 1.0) {
        score += 30;
        reasons.push(`gap grande (${fvg.gapSize.toFixed(1)}%)`);
    } else if (fvg.gapSize >= 0.5) {
        score += 20;
        reasons.push(`gap médio (${fvg.gapSize.toFixed(1)}%)`);
    } else if (fvg.gapSize >= 0.2) {
        score += 10;
        reasons.push(`gap pequeno (${fvg.gapSize.toFixed(1)}%)`);
    } else {
        score += 5;
        reasons.push(`gap muito pequeno (${fvg.gapSize.toFixed(1)}%)`);
    }
    
    // 2. Idade do FVG (FVGs recentes são mais relevantes)
    const fvgAge = Date.now() - fvg.timestamp;
    const ageHours = fvgAge / (1000 * 60 * 60);
    
    if (ageHours < 1) {
        score += 25;
        reasons.push(`FVG muito recente (<1h)`);
    } else if (ageHours < 4) {
        score += 15;
        reasons.push(`FVG recente (<4h)`);
    } else if (ageHours < 24) {
        score += 5;
        reasons.push(`FVG do dia (<24h)`);
    } else {
        score -= 10;
        reasons.push(`FVG antigo (>24h) - menos relevante`);
    }
    
    // 3. Liquidez ao redor do FVG (volume nos candles próximos)
    const nearbyCandles = candles.filter(c => {
        const candleTime = new Date(c.time).getTime();
        const fvgTime = new Date(fvg.timestamp).getTime();
        return Math.abs(candleTime - fvgTime) < 3600000; // 1 hora
    });
    
    if (nearbyCandles.length > 0) {
        const avgNearbyVolume = nearbyCandles.reduce((a,b) => a + b.volume, 0) / nearbyCandles.length;
        const totalAvgVolume = candles.slice(-50).reduce((a,b) => a + b.volume, 0) / 50;
        
        if (avgNearbyVolume > totalAvgVolume * 1.5) {
            score += 20;
            reasons.push(`volume alto na formação (+50%)`);
        } else if (avgNearbyVolume > totalAvgVolume * 1.2) {
            score += 10;
            reasons.push(`volume acima da média`);
        }
    }
    
    // 4. Quanto do gap já foi preenchido
    if (fvg.status === 'dentro') {
        let fillPercent = 0;
        if (fvg.isBullish) {
            fillPercent = ((currentPrice - fvg.bottom) / (fvg.top - fvg.bottom)) * 100;
        } else {
            fillPercent = ((fvg.top - currentPrice) / (fvg.top - fvg.bottom)) * 100;
        }
        
        if (fillPercent > 70) {
            score += 15;
            reasons.push(`gap 70%+ preenchido - atenção para reversão`);
        } else if (fillPercent > 40) {
            score += 5;
            reasons.push(`gap parcialmente preenchido (${fillPercent.toFixed(0)}%)`);
        }
    }
    
    // Classificação final
    let classification = '';
    if (score >= 60) classification = '🚀 ALTA QUALIDADE';
    else if (score >= 40) classification = '✅ QUALIDADE MÉDIA';
    else if (score >= 20) classification = '⚠️ BAIXA QUALIDADE';
    else classification = '❌ FRACO - IGNORAR';
...
// =====================================================================
// === FUNÇÃO PARA VERIFICAR ALINHAMENTO COM TENDÊNCIA ===
// =====================================================================
async function analyzeFVGAlignment(symbol, fvgType, currentPrice, timeframe) {
    try {
        // Busca candles de timeframe maior para contexto
        const higherTF = timeframe === '15m' ? '1h' : (timeframe === '1h' ? '4h' : '1d');
        const higherCandles = await getCandles(symbol, higherTF, 50);
        
        if (!higherCandles || higherCandles.length < 20) return { aligned: true, confidence: 50, reason: 'dados insuficientes', higherTF };
        
        const ema20 = calculateEMA(higherCandles.map(c => c.close), 20);
        const ema50 = calculateEMA(higherCandles.map(c => c.close), 50);
        const currentHigherPrice = higherCandles[higherCandles.length - 1].close;
        ...
        if (fvgType === 'bullish') {
            // FVG Bullish em tendência de baixa = menos relevante
            if (currentHigherPrice > ema20 && currentHigherPrice > ema50) {
                aligned = true;
                confidence = 85;
                reason = `alinhado com tendência de ALTA em ${higherTF}`;
            } else if (currentHigherPrice < ema20 && currentHigherPrice < ema50) {
                aligned = false;
                confidence = 25;
                reason = `contra tendência de BAIXA em ${higherTF} - risco alto`;
            } else {
                aligned = true;
                confidence = 55;
                reason = `tendência lateral em ${higherTF}`;
            }
        } else {
            // FVG Bearish em tendência de alta = menos relevante
            if (currentHigherPrice < ema20 && currentHigherPrice < ema50) {
                aligned = true;
                confidence = 85;
                reason = `alinhado com tendência de BAIXA em ${higherTF}`;
            } else if (currentHigherPrice > ema20 && currentHigherPrice > ema50) {
                aligned = false;
                confidence = 25;
                reason = `contra tendência de ALTA em ${higherTF} - risco alto`;
            } else {
                aligned = true;
                confidence = 55;
                reason = `tendência lateral em ${higherTF}`;
        ...
        return { aligned, confidence, reason, higherTF };
        
    } catch (error) {
        return { aligned: true, confidence: 50, reason: 'erro na análise', higherTF: 'N/A' };
    }
}

// =====================================================================
// === FUNÇÃO PARA DETECTAR STACKED FVGs ===
// =====================================================================
function detectStackedFVGs(fvgs) {
    const stacked = [];
    
    // Agrupa FVGs por timeframe e ordena por preço
    const fvgsByTF = {};
    for (const fvg of fvgs) {
        if (!fvgsByTF[fvg.timeframe]) fvgsByTF[fvg.timeframe] = [];
        fvgsByTF[fvg.timeframe].push(fvg);
    }
    
    for (const [tf, tfFvgs] of Object.entries(fvgsByTF)) {
        // Ordena FVGs por preço
        const sortedBullish = tfFvgs.filter(f => f.isBullish).sort((a,b) => a.bottom - b.bottom);
        const sortedBearish = tfFvgs.filter(f => f.isBearish).sort((a,b) => b.top - a.top);
        
        // Detecta gaps consecutivos (stacked)
        if (sortedBullish.length >= 2) {
            const gap1 = sortedBullish[0];
            const gap2 = sortedBullish[1];
            const distance = Math.abs(gap2.bottom - gap1.top);
            
            if (distance / gap1.top < 0.01) { // Menos de 1% de distância
                stacked.push({
                    type: 'bullish',
                    timeframe: tf,
                    count: sortedBullish.length,
                    totalRange: gap2.top - gap1.bottom,
                    strength: sortedBullish.length >= 3 ? 'EXTREMA' : 'ALTA',
                    description: `${sortedBullish.length} FVGs Bullish empilhados em ${tf}`
                });
            }
        }
        
        if (sortedBearish.length >= 2) {
            const gap1 = sortedBearish[0];
            const gap2 = sortedBearish[1];
            const distance = Math.abs(gap1.bottom - gap2.top);
            
            if (distance / gap1.top < 0.01) {
                stacked.push({
                    type: 'bearish',
                    timeframe: tf,
                    count: sortedBearish.length,
                    totalRange: gap1.top - gap2.bottom,
                    strength: sortedBearish.length >= 3 ? 'EXTREMA' : 'ALTA',
                    description: `${sortedBearish.length} FVGs Bearish empilhados em ${tf}`
                });
            }
        }
    }
...
// =====================================================================
// === FUNÇÃO PARA CLASSIFICAR PRIORIDADE DO ALERTA FVG ===
// =====================================================================
function classifyFVGAlerPriority(fvgQuality, volumeCheck, stackedFVGs, trendAlignment) {
    let priority = 'LOW';
    let priorityScore = 0;
    let reasons = [];
    
    // 1. Qualidade do FVG
    if (fvgQuality.score >= 60) {
        priorityScore += 30;
        reasons.push(`FVG ${fvgQuality.classification}`);
    } else if (fvgQuality.score >= 40) {
        priorityScore += 15;
        reasons.push(`FVG qualidade média`);
    } else {
        priorityScore += 5;
        reasons.push(`FVG qualidade baixa`);
    }
    
    // 2. Volume confirmation
    if (volumeCheck.confirmed && volumeCheck.strength === '🔥 FORTE') {
        priorityScore += 35;
        reasons.push(`volume FORTE (${volumeCheck.reasons})`);
    } else if (volumeCheck.confirmed) {
        priorityScore += 20;
        reasons.push(`volume confirmado`);
    }
    
    // 3. Stacked FVGs
    if (stackedFVGs.length > 0) {
        const strongest = stackedFVGs.reduce((a,b) => a.strength === 'EXTREMA' ? a : b);
        priorityScore += 25;
        reasons.push(strongest.description);
    }
    
    // 4. Alinhamento com tendência
    if (trendAlignment.aligned && trendAlignment.confidence >= 70) {
        priorityScore += 15;
        reasons.push(trendAlignment.reason);
    } else if (!trendAlignment.aligned) {
        priorityScore -= 20;
        reasons.push(`⚠️ ${trendAlignment.reason}`);
    }
    ...
    return { priority, priorityScore, reasons: reasons.join(' | ') };
}

// =====================================================================
// === FUNÇÃO PARA DETECTAR FVG EM MÚLTIPLOS TIMEFRAMES ===
// =====================================================================
    ...
    for (let i = 0; i < candles.length - 2; i++) {
        ...
        if (candle1.low > candle3.high) {
            bullishFVGs.push({ 
                top: candle1.low, 
                bottom: candle3.high, 
                gapSize: ((candle1.low - candle3.high) / candle3.high) * 100, 
                type: 'bullish', 
                isBullish: true,
        ...
        if (candle1.high < candle3.low) {
            bearishFVGs.push({ 
                top: candle3.low, 
                bottom: candle1.high, 
                gapSize: ((candle3.low - candle1.high) / candle1.high) * 100, 
                type: 'bearish', 
                isBearish: true,
                timestamp: candle3.time
            });
...
async function checkFVGOnTimeframe(symbol, timeframe) {
    try {
        const candles = await getCandles(symbol, timeframe, 100);
        if (!candles || candles.length < 30) return { bullish: [], bearish: [] };
        
        const fvgs = detectFVG(candles);
        const currentPrice = candles[candles.length - 1].close;
        
        const validBullishFVGs = [];
        const validBearishFVGs = [];
        
        for (const fvg of fvgs.bullish) {
            if (currentPrice <= fvg.top && currentPrice >= fvg.bottom) {
                validBullishFVGs.push({
                    ...fvg,
                    timeframe: timeframe,
                    distanceToPrice: 0,
                    status: 'dentro'
                ...
                const distance = ((currentPrice - fvg.top) / currentPrice) * 100;
                if (distance <= 1.0) {
                    validBullishFVGs.push({
                        ...fvg,
                        timeframe: timeframe,
        ...
        for (const fvg of fvgs.bearish) {
            if (currentPrice >= fvg.bottom && currentPrice <= fvg.top) {
                ...
                const distance = ((fvg.bottom - currentPrice) / currentPrice) * 100;
                if (distance <= 1.0) {
                    validBearishFVGs.push({
                        ...fvg,
                        timeframe: timeframe,
        ...
        return { bullish: validBullishFVGs, bearish: validBearishFVGs };
    } catch (error) {
        return { bullish: [], bearish: [] };
    }
}

async function checkAllTimeframesFVG(symbol) {
    ...
    for (const tf of timeframes) {
        const fvgData = await checkFVGOnTimeframe(symbol, tf);
        
        for (const fvg of fvgData.bullish) {
            results.bullish.push(fvg);
            results.bullishTimeframes.push(tf);
        }
        
        for (const fvg of fvgData.bearish) {
            results.bearish.push(fvg);
            results.bearishTimeframes.push(tf);
        }
    }
    
    results.confirmedBullish = results.bullish.length > 0;
    results.confirmedBearish = results.bearish.length > 0;
    
    return results;
}

// =====================================================================
// === FUNÇÃO PARA GERAR INTERPRETAÇÃO DOS CENÁRIOS ===
// =====================================================================
function gerarInterpretacaoBearish(symbol, currentPrice, fvgData, additionalData, enhancedData = null) {
    let interpretacao = "";
    
    // Encontra o gap mais próximo abaixo do preço (suporte imediato)
    let gapProximoAbaixo = null;
    let gapProximoAcima = null;
    
    for (const fvg of fvgData.bearish) {
        if (fvg.bottom < currentPrice && (!gapProximoAbaixo || fvg.bottom > gapProximoAbaixo.bottom)) {
            gapProximoAbaixo = fvg;
        }
        if (fvg.top > currentPrice && (!gapProximoAcima || fvg.top < gapProximoAcima.top)) {
            gapProximoAcima = fvg;
        }
    }
    
    // Encontra o gap semanal (maior timeframe)
    let gapSemanal = fvgData.bearish.find(f => f.timeframe === '1w');
    
    // Adiciona informações de qualidade e prioridade se disponíveis
    if (enhancedData) {
        interpretacao += `📊 QUALIDADE DO SETUP:\n`;
        interpretacao += `• ${enhancedData.quality.classification} (${enhancedData.quality.score} pts)\n`;
        interpretacao += `• ${enhancedData.priority.priority} (${enhancedData.priority.priorityScore} pts)\n`;
        if (enhancedData.stacked.length > 0) {
            interpretacao += `• 🔥 STACKED FVG: ${enhancedData.stacked[0].description}\n`;
        }
        interpretacao += `• ${enhancedData.trendAlignment.reason}\n\n`;
    }
    
    // Cenário 1: Pullback possível (Stoch sobrevendido)
    if (additionalData?.stoch4h && parseInt(additionalData.stoch4h.k) < 25 && additionalData.stoch4h.kTrend === '↑') {
        interpretacao += `🔥STOCH 4h Muito Baixo 🟢 (K${additionalData.stoch4h.k}↑)\n`;
        interpretacao += `→ Podemos ter uma PEQUENA ALTA, para testar resistências\n`;
        if (gapProximoAcima) {
            interpretacao += `→ Principal resistência: ${formatPrice(gapProximoAcima.top)} (${gapProximoAcima.timeframe})\n`;
        }
        interpretacao += `→ Isso seria um "PULLBACK" antes de continuar corrigindo\n\n`;
    }
    
    // Cenário 2: Rompimento baixista
    if (gapProximoAbaixo) {
        interpretacao += `🔻 CENÁRIO CORREÇÃO! (Do alerta atual):\n`;
        interpretacao += `Se romper ${formatPrice(gapProximoAbaixo.bottom)} (fundo do gap ${gapProximoAbaixo.timeframe}):\n`;
        
        // Encontra próximos suportes
        const suportes = [];
        if (additionalData?.clusters?.supports) {
            for (const sup of additionalData.clusters.supports) {
                if (sup.avgPrice < currentPrice) {
                    suportes.push(formatPrice(sup.avgPrice));
                }
            }
        }
        
        if (suportes.length >= 2) {
            interpretacao += `→ Próximos suportes: ${suportes[0]}, ${suportes[1]}\n`;
        } else if (suportes.length === 1) {
            interpretacao += `→ Próximo suporte: ${suportes[0]}\n`;
        }
        
        if (gapSemanal) {
            interpretacao += `→ Alvo máximo: ${formatPrice(gapSemanal.bottom)} (gap semanal)\n`;
        }
    }
    
    // Cenário 3: Se já está dentro do gap
    let dentroGap = false;
    for (const fvg of fvgData.bearish) {
        if (currentPrice >= fvg.bottom && currentPrice <= fvg.top) {
            dentroGap = true;
    ...
    if (dentroGap) {
        interpretacao += `\n⚠️ PREÇO DENTRO DO GAP DE ${fvgData.bearish[0]?.timeframe || '4h'}! Momento de decisão crítico.\n`;
    }
    
    return interpretacao;
}

function gerarInterpretacaoBullish(symbol, currentPrice, fvgData, additionalData, enhancedData = null) {
    let interpretacao = "";
    
    // Encontra o gap mais próximo acima do preço (resistência imediata)
    let gapProximoAcima = null;
    let gapProximoAbaixo = null;
    ...
    // Encontra o gap semanal (maior timeframe)
    let gapSemanal = fvgData.bullish.find(f => f.timeframe === '1w');
    ...
    // Cenário 1: Pullback possível (Stoch sobrecomprado)
    if (additionalData?.stoch4h && parseInt(additionalData.stoch4h.k) > 75 && additionalData.stoch4h.kTrend === '↓') {
        interpretacao += `🔥STOCH 4h Muito Alto🔴 (K${additionalData.stoch4h.k}↓)\n`;
        interpretacao += `→ Pode haver uma PEQUENA CORREÇÃO, para testar suportes\n`;
        if (gapProximoAbaixo) {
            interpretacao += `→ Principal suporte: ${formatPrice(gapProximoAbaixo.bottom)} (${gapProximoAbaixo.timeframe})\n`;
        }
        interpretacao += `→ Isso seria um "PULLBACK", antes de continuar subindo\n\n`;
    }
    
    // Cenário 2: Rompimento altista
    if (gapProximoAcima) {
        interpretacao += `💹 CENÁRIO DE ALTA! (Do alerta atual):\n`;
        interpretacao += `Se romper ${formatPrice(gapProximoAcima.top)} (topo do gap ${gapProximoAcima.timeframe}):\n`;
        
        // Encontra próximas resistências
        const resistencias = [];
        if (additionalData?.clusters?.resistances) {
            for (const res of additionalData.clusters.resistances) {
                if (res.avgPrice > currentPrice) {
                    resistencias.push(formatPrice(res.avgPrice));
                }
            }
        }
        
        if (resistencias.length >= 2) {
            interpretacao += `→ Próximas resistências: ${resistencias[0]}, ${resistencias[1]}\n`;
        } else if (resistencias.length === 1) {
            interpretacao += `→ Próxima resistência: ${resistencias[0]}\n`;
        }
        
        if (gapSemanal) {
            interpretacao += `→ Alvo máximo: ${formatPrice(gapSemanal.top)} (gap semanal)\n`;
        }
    }
    ...
    if (dentroGap) {
        interpretacao += `\n⚠️ PREÇO DENTRO DO GAP DE ${fvgData.bullish[0]?.timeframe || '4h'}! Momento de decisão crítico.\n`;
    }
...
// =====================================================================
// === FUNÇÃO PARA RESUMIR ZONAS FVG (COMPACTO) ===
// =====================================================================
function resumirZonasFVG(fvgData, type) {
    const fvgs = type === 'bearish' ? fvgData.bearish : fvgData.bullish;
    if (fvgs.length === 0) return "Nenhuma zona detectada";
    
    // Agrupa por timeframe
    const grouped = {};
    for (const fvg of fvgs) {
        if (!grouped[fvg.timeframe]) {
            grouped[fvg.timeframe] = [];
        }
        grouped[fvg.timeframe].push(fvg);
    }
    
    let resumo = "";
    const timeframesOrder = ['15m', '1h', '4h', '1d', '1w'];
    
    for (const tf of timeframesOrder) {
        if (grouped[tf]) {
            const gaps = grouped[tf];
            if (gaps.length === 1) {
                resumo += `└─ ${tf}: ${formatPrice(gaps[0].bottom)} → ${formatPrice(gaps[0].top)} (${gaps[0].gapSize.toFixed(2)}%)\n`;
            } else {
                // Se tem múltiplos gaps no mesmo timeframe, mostra apenas o range total
                const minBottom = Math.min(...gaps.map(g => g.bottom));
                const maxTop = Math.max(...gaps.map(g => g.top));
                resumo += `└─ ${tf}: ${formatPrice(minBottom)} → ${formatPrice(maxTop)} (${gaps.length} gaps, até ${Math.max(...gaps.map(g => g.gapSize)).toFixed(2)}%)\n`;
            }
        }
    }
...
// =====================================================================
// === MONITOR ESPECÍFICO PARA BTC E ETH COM FVG + VOLUME 3min MELHORADO ===
// =====================================================================
const fvgAlertedCache = new Map();

async function monitorBTCAndETHFVG() {
    const symbolsToMonitor = ['BTCUSDT','ETHUSDT'];
    
    console.log('\n' + '='.repeat(70));
    console.log('🎯 MONITOR FVG + VOLUME 3min - BTC e ETH (VERSÃO MELHORADA)');
    console.log('='.repeat(70));
    console.log(`📊 Timeframes monitorados: 15min, 1h, 4h, Diário, Semanal`);
    console.log(`📊 Volume requerido: Múltiplos critérios (mínimo 1.5x média + confirmações adicionais)`);
    console.log(`📊 Score mínimo para alerta: 40 pontos`);
    console.log('='.repeat(70));
    ...
    while (true) {
        for (const symbol of symbolsToMonitor) {
            try {
                ...
                const fvgMultiTimeframe = await checkAllTimeframesFVG(symbol);
                const currentPrice = await getCurrentPrice(symbol) || await getPriceFromTicker(symbol);
                
                // Busca dados adicionais para enriquecer o alerta
                const additionalData = await getAdditionalAlertData(symbol, currentPrice);
                
                // Busca candles para análise de qualidade
                const allCandles = await getCandles(symbol, '1h', 150);
                
                // Verifica FVG Bull Confirmado (FVG Bull + Volume Comprador 3min)
                if (fvgMultiTimeframe.confirmedBullish) {
                    const volumeCheck = await check3MinVolumeEnhanced(symbol, 'BUY');
                    
                    if (volumeCheck && volumeCheck.confirmed) {
                        // Avalia qualidade de CADA FVG detectado
                        const fvgQualityAssessments = [];
                        for (const fvg of fvgMultiTimeframe.bullish) {
                            const candlesForFVG = await getCandles(symbol, fvg.timeframe, 100);
                            const quality = evaluateFVGQuality(fvg, candlesForFVG, currentPrice);
                            const alignment = await analyzeFVGAlignment(symbol, 'bullish', currentPrice, fvg.timeframe);
                            fvgQualityAssessments.push({ ...quality, alignment, fvg });
                        }
                        
                        // Detecta stacked FVGs
                        const stacked = detectStackedFVGs(fvgMultiTimeframe.bullish);
                        
                        // Melhor qualidade entre todos FVGs
                        const bestQuality = fvgQualityAssessments.reduce((best, curr) => 
                            curr.score > best.score ? curr : best, fvgQualityAssessments[0]);
                        
                        // Classifica prioridade do alerta
                        const priority = classifyFVGAlerPriority(bestQuality, volumeCheck, stacked, bestQuality.alignment);
                        
                        // Só envia alerta se prioridade for HIGH ou MEDIUM e qualidade mínima
                        if (priority.priority !== '⚠️ LOW' && bestQuality.score >= 40) {
                            const enhancedData = {
                                quality: bestQuality,
                                stacked: stacked,
                                priority: priority,
                                trendAlignment: bestQuality.alignment
                            };
                            
                            const cacheKey = `${symbol}_FVG_BULL_CONFIRMADO`;
                            const lastAlert = fvgAlertedCache.get(cacheKey);
                            const cooldownMinutes = priority.priority === '🔥 HIGH' ? 120 : 60;
                            
                            if (!lastAlert || (Date.now() - lastAlert) > cooldownMinutes * 60 * 1000) {
                                await sendFVGBullAlertCompleto(symbol, currentPrice, fvgMultiTimeframe, volumeCheck, additionalData, enhancedData);
                                fvgAlertedCache.set(cacheKey, Date.now());
                                console.log(`✅ ${symbol} - FVG BULL CONFIRMADO enviado! (Prioridade: ${priority.priority})`);
                            } else {
                                console.log(`⏰ ${symbol} - FVG BULL em cooldown`);
                            }
                        } else {
                            console.log(`⏸️ ${symbol} - FVG Bull ignorado (prioridade baixa: ${priority.priorityScore} pts, qualidade: ${bestQuality.score})`);
                        }
                    } else {
                        console.log(`📉 ${symbol} - FVG Bull detectado mas volume 3min não confirmou`);
                    }
                }
                
                // Verifica FVG Bear Confirmado (FVG Bear + Volume Vendedor 3min)
                if (fvgMultiTimeframe.confirmedBearish) {
                    const volumeCheck = await check3MinVolumeEnhanced(symbol, 'SELL');
                    
                    if (volumeCheck && volumeCheck.confirmed) {
                        // Avalia qualidade de CADA FVG detectado
                        const fvgQualityAssessments = [];
                        for (const fvg of fvgMultiTimeframe.bearish) {
                            const candlesForFVG = await getCandles(symbol, fvg.timeframe, 100);
                            const quality = evaluateFVGQuality(fvg, candlesForFVG, currentPrice);
                            const alignment = await analyzeFVGAlignment(symbol, 'bearish', currentPrice, fvg.timeframe);
                            fvgQualityAssessments.push({ ...quality, alignment, fvg });
                        }
                        
                        // Detecta stacked FVGs
                        const stacked = detectStackedFVGs(fvgMultiTimeframe.bearish);
                        ...
                        if (priority.priority !== '⚠️ LOW' && bestQuality.score >= 40) {
                            ...
                            const cacheKey = `${symbol}_FVG_BEAR_CONFIRMADO`;
                            const lastAlert = fvgAlertedCache.get(cacheKey);
                            const cooldownMinutes = priority.priority === '🔥 HIGH' ? 120 : 60;
                            
                            if (!lastAlert || (Date.now() - lastAlert) > cooldownMinutes * 60 * 1000) {
                                await sendFVGBearAlertCompleto(symbol, currentPrice, fvgMultiTimeframe, volumeCheck, additionalData, enhancedData);
                                fvgAlertedCache.set(cacheKey, Date.now());
                                console.log(`✅ ${symbol} - FVG BEAR CONFIRMADO enviado! (Prioridade: ${priority.priority})`);
                            } else {
                                console.log(`⏰ ${symbol} - FVG BEAR em cooldown`);
                            }
                        } else {
                            console.log(`⏸️ ${symbol} - FVG Bear ignorado (prioridade baixa: ${priority.priorityScore} pts, qualidade: ${bestQuality.score})`);
                        }
                    } else {
                        console.log(`📈 ${symbol} - FVG Bear detectado mas volume 3min não confirmou`);
                    }
                }
                
                if (!fvgMultiTimeframe.confirmedBullish && !fvgMultiTimeframe.confirmedBearish) {
                    console.log(`⚪ ${symbol} - Nenhum FVG válido detectado nos timeframes`);
                }
                
                await delay(2000);
                
            } catch (error) {
                console.log(`⚠️ Erro ao monitorar ${symbol}: ${error.message}`);
            }
        }
        
        console.log(`\n⏳ Aguardando 60 segundos para próximo scan...\n`);
        await delay(60000);
    }
}

async function getAdditionalAlertData(symbol, currentPrice) {
    try {
        const symbolClean = symbol.replace('USDT', '');
        
        // Busca todos os dados adicionais em paralelo
        const [
            divergences,
            macdDivergence,
            structureBreaks,
            ...
            analyzeDivergences(symbol),
            analyzeMACDDivergences(symbol, null),
            getStructureBreaks(symbol),
            analyzeCCI(symbol, '4h'),
            analyzeCCIDaily(symbol),
            getRSI(symbol, '15m'),
            calculateStochastic(symbol, '4h', 5, 3, 3),
            calculateStochastic(symbol, '1d', 5, 3, 3),
            getLSRData([symbol]),
            getFundingRates([symbol]),
            getClusterLevels(symbol, currentPrice)
        ]);
        ...
        console.log(`⚠️ Erro ao buscar dados adicionais: ${error.message}`);
        return null;
    }
}

async function getStructureBreaks(symbol) {
    try {
...
async function getClusterLevels(symbol, currentPrice) {
    try {
        ...
        return findClusterLevels(candles, currentPrice, 0.5, 2);
    } catch (error) {
        return { supports: [], resistances: [] };
    }
}

async function sendFVGBullAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData, enhancedData = null) {
    ...
    let fvgTimeframesText = '';
    const uniqueTimeframes = [...new Set(fvgData.bullishTimeframes)];
    fvgTimeframesText = uniqueTimeframes.join(', ');
    
    // Resumo compacto das zonas FVG
    let fvgResumo = resumirZonasFVG(fvgData, 'bullish');
    
    // Gera interpretação dos cenários com dados melhorados
    let interpretacao = gerarInterpretacaoBullish(symbolShort, currentPrice, fvgData, additionalData, enhancedData);
    ...
    if (enhancedData) {
        let stackedText = '';
        if (enhancedData.stacked.length > 0) {
            stackedText = `\n<i>• 🔥 STACKED FVG: ${enhancedData.stacked[0].description}</i>`;
        }
        
        advancedSection = `
<b>📊 Análise Avançada FVG:</b>
<i>• Qualidade: ${enhancedData.quality.classification} (${enhancedData.quality.score} pts)</i>
<i>• ${enhancedData.quality.reasons}</i>
<i>• Idade: ${enhancedData.quality.ageHours}h</i>
<i>• ${enhancedData.trendAlignment.reason}</i>
<i>• Prioridade: ${enhancedData.priority.priority} (${enhancedData.priority.priorityScore} pts)</i>${stackedText}
`;
    }
    
    // Processa dados adicionais
    let divergenciaText = '';
    if (additionalData?.divergences?.hasBullish) {
        const tfs = additionalData.divergences.bullish.map(d => d.timeframe).join(', ');
        divergenciaText = `• Divergência RSI de ALTA (${tfs})`;
    } else {
        divergenciaText = `• Divergência RSI: Nenhuma detectada`;
    }
    
    let macdText = '';
    if (additionalData?.macdDivergence?.hasBullish) {
        macdText = `• 🟢 MACD Bullish em: ${additionalData.macdDivergence.bullish.join(', ')}`;
    } else {
        macdText = `• MACD: Neutro`;
    }
    
    let chochText = '';
    if (additionalData?.structureBreaks?.bullish) {
        chochText = `• ${additionalData.structureBreaks.description}`;
    } else if (additionalData?.structureBreaks?.bearish) {
        chochText = `• ${additionalData.structureBreaks.description}`;
    } else {
        chochText = `• CHoCH: Nenhuma quebra recente`;
    }
    
    let cci4hText = additionalData?.cci4h?.display || '➡️ Neutro';
    let cciDailyText = additionalData?.cciDaily?.display || '➡️ Neutro';
    
    let rsiText = '';
    if (additionalData?.rsi) {
        let rsiStatus = '';
        if (additionalData.rsi.value < 35) rsiStatus = 'sobrevendido';
        else if (additionalData.rsi.value > 70) rsiStatus = 'sobrecomprado';
        else rsiStatus = 'neutro';
        rsiText = `• RSI ${additionalData.rsi.value.toFixed(0)} (${rsiStatus}) ${additionalData.rsi.emoji || ''}`;
    } else {
        rsiText = `• RSI N/A`;
    }
    
    let stochText = '';
    if (additionalData?.stoch4h && additionalData?.stochDaily) {
        stochText = `• Stoch 4h: K${additionalData.stoch4h.k}${additionalData.stoch4h.kTrend} D${additionalData.stoch4h.d} | Diário: K${additionalData.stochDaily.k}${additionalData.stochDaily.kTrend} D${additionalData.stochDaily.d}`;
    } else {
        stochText = `• Stoch: N/A`;
    }
    ...
    let lsrText = '';
    let lsrCheck = '';
    if (additionalData?.lsr) {
        const lsrValue = additionalData.lsr.value;
        lsrText = `• #LSR ${lsrValue.toFixed(2)}`;
        lsrCheck = lsrValue < 2.5 ? '✅' : '';
    } else {
        lsrText = `• #LSR N/A`;
        lsrCheck = '';
    }
    ...
    if (additionalData?.funding !== undefined) {
        const fundingValue = (additionalData.funding * 100).toFixed(3);
        const fundingStatus = additionalData.funding < 0 ? 'negativo' : 'positivo';
        fundingText = `• Funding ${fundingValue}% (${fundingStatus})`;
        fundingCheck = additionalData.funding < 0 ? '✅' : '';
    } else {
    ...
    let resistenciaText = 'N/A';
    let suporteText = 'N/A';
    if (additionalData?.clusters) {
        if (additionalData.clusters.resistances && additionalData.clusters.resistances.length > 0) {
            resistenciaText = formatPrice(additionalData.clusters.resistances[0].avgPrice);
        }
        if (additionalData.clusters.supports && additionalData.clusters.supports.length > 0) {
            suporteText = formatPrice(additionalData.clusters.supports[0].avgPrice);
        }
    }
    
    let volumeStrengthText = '';
    if (volumeData.strength) {
        volumeStrengthText = `${volumeData.reasons ? ` (${volumeData.reasons})` : ''}`;
    }
    
    const message = `<b>🟢 ${symbolShort} FVG Bull💹✔︎</b>\n` +
                    `<i>${dt.date.slice(0,5)} ${dt.time.slice(0,5)}hs</i> ${tvLinkText}\n` +
                    `<b> Preço atual:</b> ${formatPrice(currentPrice)}\n` +
                    `<b> Timeframes com FVG:</b> ${fvgTimeframesText}\n` +
                    `<b> Zonas FVG Bullish:</b>\n${fvgResumo}` +
                    `<b> Vol 3min:</b> ${volumeData.volumeRatio}x média (${volumeData.direction})${volumeStrengthText}\n` +
                    (advancedSection ? advancedSection + `\n` : '') +
                    `<i>${divergenciaText}</i>\n` +
                    `<i>${macdText}</i>\n` +
                    `<i>${chochText}</i>\n` +
                    `<i>• CCI 4h: ${cci4hText}</i>\n` +
                    `<i>• CCI 1D: ${cciDailyText}</i>\n` +
                    `<b>🔺 RESISTÊNCIA:</b> <i>${resistenciaText}</i>\n` +
                    `<b>🔻 SUPORTE:</b> <i>${suporteText}</i>\n` +
                    `<b> Indicadores:</b>\n` +
                    `<i>${rsiText}</i>\n` +
                    `<i>${stochText}</i>\n` +
                    `<i>${lsrText} ${lsrCheck}</i>\n` +
                    `<i>${fundingText} ${fundingCheck}</i>\n` +
                    `<b>🔍 Análise 🤖 IA dos Cenários:</b>\n` +
                    `<i>${interpretacao}</i>\n` +
...
async function sendFVGBearAlertCompleto(symbol, currentPrice, fvgData, volumeData, additionalData, enhancedData = null) {
    const dt = getBrazilianDateTime();
    const symbolShort = symbol.replace('USDT', '');
    const tvLink = `https://www.tradingview.com/chart/?symbol=BINANCE%3A${symbol}`;
    const tvLinkText = `<a href="${tvLink}"> 🔍Ver_Gráfico</a>`;
    ...
    let fvgTimeframesText = '';
    const uniqueTimeframes = [...new Set(fvgData.bearishTimeframes)];
    fvgTimeframesText = uniqueTimeframes.join(', ');
    
    // Resumo compacto das zonas FVG
    let fvgResumo = resumirZonasFVG(fvgData, 'bearish');
    
    // Gera interpretação dos cenários com dados melhorados
    let interpretacao = gerarInterpretacaoBearish(symbolShort, currentPrice, fvgData, additionalData, enhancedData);
...
`;
    ...
    // Processa dados adicionais
    let divergenciaText = '';
    if (additionalData?.divergences?.hasBearish) {
        const tfs = additionalData.divergences.bearish.map(d => d.timeframe).join(', ');
        divergenciaText = `• Divergência RSI de BAIXA (${tfs})`;
    } else {
        divergenciaText = `• Divergência RSI: Nenhuma detectada`;
    }
    
    let macdText = '';
    if (additionalData?.macdDivergence?.hasBearish) {
        macdText = `• 🔴 MACD Bearish em: ${additionalData.macdDivergence.bearish.join(', ')}`;
    } else {
        macdText = `• MACD: Neutro`;
    }
    
    let chochText = '';
    if (additionalData?.structureBreaks?.bearish) {
        chochText = `• ${additionalData.structureBreaks.description}`;
    } else if (additionalData?.structureBreaks?.bullish) {
        chochText = `• ${additionalData.structureBreaks.description}`;
    } else {
        chochText = `• CHoCH: Nenhuma quebra recente`;
    }
    ...
    if (additionalData?.funding !== undefined) {
        const fundingValue = (additionalData.funding * 100).toFixed(3);
        const fundingStatus = additionalData.funding > 0 ? 'positivo' : 'negativo';
        fundingText = `• Funding ${fundingValue}% (${fundingStatus})`;
        fundingCheck = additionalData.funding > 0 ? '✅' : '';
    } else {
        fundingText = `• Funding N/A`;
    ...
    const message = `<b>🔴 ${symbolShort} FVG Bear⤵️✔︎</b>\n` +
                    `<i>${dt.date.slice(0,5)} ${dt.time.slice(0,5)}hs</i> ${tvLinkText}\n` +
                    `<b> Preço atual:</b> ${formatPrice(currentPrice)}\n` +
                    `<b> Timeframes com FVG:</b> ${fvgTimeframesText}\n` +
                    `<b> Zonas FVG Bearish:</b>\n${fvgResumo}` +
                    `<b> Vol 3min:</b> ${volumeData.volumeRatio}x média (${volumeData.direction})${volumeStrengthText}\n` +
                    (advancedSection ? advancedSection + `\n` : '') +
                    `<i>${divergenciaText}</i>\n` +
                    `<i>${macdText}</i>\n` +
...
async function getPriceFromTicker(symbol) {
    try {
...
// =====================================================================
// === REJEIÇÃO DE PREÇO (WICK GRANDE) - OBRIGATÓRIO ===
// =====================================================================
function checkCandleRejection(candles, entryPrice, tradeType) {
    if (!candles || candles.length < 2) return { passed: false, reason: 'Sem candles suficientes' };
    
    const lastCandle = candles[candles.length - 1];
    const bodySize = Math.abs(lastCandle.close - lastCandle.open);
    
    if (bodySize === 0) return { passed: false, reason: 'Corpo do candle é zero (doji)' };
    ...
    if (tradeType === 'BUY') {
        const hasRejection = lowerWick >= (bodySize * 2);
        
        if (!hasRejection) {
            return { 
                passed: false, 
                reason: `❌ Sem rejeição de baixa | Wick inferior ${(lowerWick/bodySize).toFixed(1)}x corpo (mínimo 2x)`
            };
        }
        ...
        if (!touchedLevel) {
            return { 
                passed: false, 
                reason: `❌ Não tocou no nível de entrada`
            };
        }
        
        return { 
            passed: true, 
            reason: `✅ Rejeição de baixa confirmada | Wick ${(lowerWick/bodySize).toFixed(1)}x corpo`,
            strength: lowerWick >= (bodySize * 3) ? 'EXTREMA' : 'FORTE'
        };
    }
    
    if (tradeType === 'SELL') {
        const hasRejection = upperWick >= (bodySize * 2);
        
        if (!hasRejection) {
            return { 
                passed: false, 
                reason: `❌ Sem rejeição de alta | Wick superior ${(upperWick/bodySize).toFixed(1)}x corpo (mínimo 2x)`
            };
        }
        ...
        return { 
            passed: true, 
            reason: `✅ Rejeição de alta confirmada | Wick ${(upperWick/bodySize).toFixed(1)}x corpo`,
            strength: upperWick >= (bodySize * 3) ? 'EXTREMA' : 'FORTE'
        };
    }
    
    return { passed: false, reason: 'Tipo de trade inválido' };
}
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        // =====================================================================
        // VERIFICAÇÃO OBRIGATÓRIA DE REJEIÇÃO DE PREÇO (WICK GRANDE)
        // =====================================================================
        const latestCandles = await getCandles(fullSymbol, '15m', 5);
        const rejectionCheck = checkCandleRejection(latestCandles, entryPrice, tradeSignal);
        
        if (!rejectionCheck.passed) {
            console.log(`❌ ${symbol} | REJEIÇÃO NEGADA: ${rejectionCheck.reason}`);
            return null;
        }
        
        console.log(`✅ ${symbol} | ${rejectionCheck.reason}`);
        ...
        // SÓ CONTINUA SE O SCORE FOR MAIOR OU IGUAL AO MÍNIMO
        ...
        if (rejectionCheck.passed) {
            rejectionMessage = `🕯️ ${tradeSignal === 'BUY' ? 'REJEIÇÃO DE BAIXA (martelo)' : 'REJEIÇÃO DE ALTA (estrela)'} | ${rejectionCheck.reason}`;
        }
...
// =====================================================================
// === ENVIAR ALERTA COMPLETO (MANTIDO ORIGINAL) ===
// =====================================================================
...
// =====================================================================
// === MONITOR PRINCIPAL - APENAS BTC E ETH ===
// =====================================================================
...
async function scanAndAlert() {
    ...
    try {
        // Filtra apenas BTC e ETH
        const allSymbols = await getAllSymbols();
        const symbols = allSymbols.filter(s => s.symbol === 'BTCUSDT' || s.symbol === 'ETHUSDT');
        
        if (!symbols.length) {
            console.log(`⚠️ BTC e ETH não encontrados`);
            return;
        }
       
        console.log(`🔍 Escaneando ${symbols.length} símbolos (BTC e ETH)...`);
        console.log(`📊 SCORE MÍNIMO OBRIGATÓRIO: ${CONFIG.MONITOR.MIN_SCORE_ACCEPT}`);
        console.log(`🕯️ REJEIÇÃO DE PREÇO (WICK ≥ 2x CORPO) - OBRIGATÓRIA`);
        let alertsSent = 0;
...
async function sendInitMessage() {
    const msg = `<i>⚡ Titanium ⚡</i>\n\n<i> 🤖 Olá... espero que estejam bem...monitoramento especial no BTC, ETH... sou o agente IA do @J4rviz... </i>\n<i> bons trades a todos...!</i>`;
...
async function startMonitorWithReconnect() {
    ...
    console.log('\n' + '='.repeat(70));
    console.log(' Titanium - Monitor BTC e ETH ');
    console.log('='.repeat(70));
    console.log(`✅ Monitor 15m ativo | SCORE MÍNIMO: ${CONFIG.MONITOR.MIN_SCORE_ACCEPT}`);
    console.log(`🕯️ REJEIÇÃO DE PREÇO OBRIGATÓRIA (wick ≥ 2x corpo)`);
    console.log(`📊 Alertas FVG + Volume 3min ATIVOS com interpretação de cenários`);
    console.log('='.repeat(70));
    
    // Inicia também o monitor de FVG + Volume 3min
    monitorBTCAndETHFVG().catch(console.error);
