...
function analyzeTimeframeConflict(emaAnalysis) {
    ...
    if (sum === 3) {
        return { level: 'ALINHADO', message: '✅ TODOS TIMEFRAMES ALINHADOS', score: 100 };
    } else if (sum === 2) {
...
function analyzeScoreQuality(score) {
    if (score >= 80) {
        return { emoji: '🏆', message: 'Setup EXCEPCIONAL - Alta confiança', recommendation: 'Entrada normal' };
    } else if (score >= 65) {
        ...
        return { emoji: '🔴', message: 'Setup INVÁLIDO - Não operar', recommendation: 'Aguardar melhor setup' };
    }
}
    ...
    for (const fvg of fvgsToCheck) {
        ...
        if (distance < minDistance) {
            ...
            closest = {
                ...
                status,
                qualityScore,
                displayDistance: distance === 0 ? 'DENTRO DO FVG' : `${distance.toFixed(2)}% ${status === 'abaixo' ? '📈 para chegar' : '📉 para chegar'}`
            };
        }
    }
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        if (dailyEMA.trend === 'BAIXA' && !dailyEMA.isAboveEMA55 && !dailyEMA.isAboveEMA233) {
            ...
            if (!isOversold) {
                let score = 0, reasons = [];
                if (bearishFVG) { score += bearishFVG.qualityScore === 3 ? 30 : (bearishFVG.qualityScore === 2 ? 25 : 20); reasons.push(`FVG Bearish ${bearishFVG.status === 'dentro' ? 'DENTRO' : `à ${bearishFVG.distancePercent.toFixed(2)}%`}`); }
                ...
                if (score >= CONFIG.MONITOR.SMC.MIN_SCORE) {
                    tradeSignal = 'SELL';
        ...
        if (dailyEMA.trend === 'ALTA' && dailyEMA.isAboveEMA55 && dailyEMA.isAboveEMA233) {
            ...
            if (!isOverbought) {
                let score = 0, reasons = [];
                if (bullishFVG) { score += bullishFVG.qualityScore === 3 ? 30 : (bullishFVG.qualityScore === 2 ? 25 : 20); reasons.push(`FVG Bullish ${bullishFVG.status === 'dentro' ? 'DENTRO' : `à ${bullishFVG.distancePercent.toFixed(2)}%`}`); }
...
async function sendSMCAlert(analysis) {
    ...
    let message = `<i>${tradeEmoji} ${tradeText} - ${analysis.symbol}USDT</i>\n`;
    ...
    if (analysis.bullishFVG) {
        message += `<i>📍FVG Bullish: ${formatPrice(analysis.bullishFVG.targetPrice)} (${analysis.bullishFVG.displayDistance}) | Gap: ${analysis.bullishFVG.gapSize.toFixed(2)}%</i>\n`;
    }
    if (analysis.bearishFVG) {
        message += `<i>📍FVG Bearish: ${formatPrice(analysis.bearishFVG.targetPrice)} (${analysis.bearishFVG.displayDistance}) | Gap: ${analysis.bearishFVG.gapSize.toFixed(2)}%</i>\n`;
    }
    ...
    try {
        ...
        for (const symbolData of symbols) {
            try {
                ...
                if (!analysis) {
                    // Sem sinal, apenas continua
                    continue;
                }
                
                // Verificar se pode alertar
