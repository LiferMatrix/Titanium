...
async function sendAlertStochasticCross(symbol, data) {
  ...
  // Garante que o objeto exista (agora com validação extra)
  ...
  const tempoDesdeUltimoAlerta = agora - (ativo.ultimoAlerta || 0); // Força 0 se undefined
  ...
  const reentryForcada = precoPertoDaUltimaEntrada && volumeZScore > config.VOLUME_Z_THRESHOLD + 1.5; // Threshold mais alto: 3.5

  // ==================== COOLDOWN CONSERVADOR 1H POR MOEDA ====================
  if (tempoDesdeUltimoAlerta < config.TEMPO_COOLDOWN_MS && !reentryForcada) {
    ...
    return; // SAI FORA IMEDIATAMENTE - NÃO MANDA NADA
  }
  // ===========================================================================
  ...
  if (signalStrengthBuy.level && stochOkBuy && rsiOkBuy && atrOk && lsrOkForLong && (volumeOkBuy || isReentryBuy)) {
    ...
    ativo.ultimoAlerta = agora;  // ← TRAVA AQUI por 60min
