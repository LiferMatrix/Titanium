...
// Arquivo de persistência para listagens
const symbolsFile = 'initialSymbols.json';

// Armazena símbolos iniciais (persistente)
let initialSymbols = new Set();
let isFirstRun = true; // Flag para bloquear alertas na primeira execução
...
async function checkListings() {
  ...
  if (isFirstRun) {
    ...
    isFirstRun = false; // Desativa flag após salvar
    await logMessage(`📊 ${initialSymbols.size} pares USDT carregados e salvos inicialmente. Nenhum alerta enviado na inicialização.`);
    return;
  }

  // === EXECUÇÕES SUBSEQUENTES: compara e alerta ===
  ...
  // Atualiza conjunto inicial com os atuais (caso haja remoções ou adições)
  initialSymbols = new Set(currentSymbols);
  await saveInitialSymbols();
}
