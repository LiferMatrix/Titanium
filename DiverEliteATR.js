...
async function fetchVWAP1h(symbol) {
  ...
  if (cached) return cached;
  try {
    const raw = await withRetry(() => exchangeSpot.fetchOHLCV(symbol, '1h', undefined, 48)); // 48h pra ter boa base
    const ohlcv = normalizeOHLCV(raw);
    const input = {
      high: ohlcv.map(c => c.high),
      low: ohlcv.map(c => c.low),
...
async function sendAlertRSIDivergence(symbol, timeframe, price, rsiValue, divergence, lsr, rsi1hValue, volumeData, adx15m, adx1h, vwap1h) {
  ...
  if (isBullish) {
    lsrOk = lsr.value <= 2.0;
    rsiOk = rsi1hValue < 60;
    volOk = volumeData.totalVolume > 2.3 * volumeData.avgVolume && volumeData.buyVolume > volumeData.sellVolume;
