...
const RETRACEMENT_SETTINGS = {
    minPercentage: 0.25,    // 25% - Retração mínima conservadora
    maxPercentage: 0.50,    // 50% - Retração máxima (limite)
    useDynamicATR: true,    // Usar ATR para retrações dinâmicas
    atrMultiplierMin: 0.5,  // Multiplicador ATR para retração mínima
    atrMultiplierMax: 1.0,  // Multiplicador ATR para retração máxima
    volatilityAdjustment: {
        low: 1.0,           // Ajuste para baixa volatilidade
        medium: 1.2,        // Ajuste para média volatilidade
        high: 1.5           // Ajuste para alta volatilidade
    }
};

// === CONFIGURAÇÕES DE STOP DINÂMICO ===
const DYNAMIC_STOP_SETTINGS = {
    baseATRMultiplier: 3.5,  // Multiplicador base do ATR para stop
    minStopPercentage: 2.0,  // Stop mínimo em porcentagem
    maxStopPercentage: 8.0,  // Stop máximo em porcentagem
    volatilityBased: true,   // Ajustar stop baseado na volatilidade
    volatilityMultipliers: {
        low: 0.8,            // Multiplicador para baixa volatilidade
        medium: 1.0,         // Multiplicador para volatilidade média
        high: 1.3            // Multiplicador para alta volatilidade
    }
};

// === CONFIGURAÇÕES PARA ANÁLISE DE SUPORTE/RESISTÊNCIA ===
const SUPPORT_RESISTANCE_SETTINGS = {
    lookbackPeriod: 50,      // Período para análise de S/R
    timeframe: '15m',        // Timeframe para S/R
    minTouchPoints: 2,       // Mínimo de toques para considerar S/R
    proximityThreshold: 1.5, // % de proximidade para alerta (1.5%)
    breakoutThreshold: 0.8,  // % para considerar rompimento (0.8%)
    strongLevelThreshold: 3, // Mínimo de toques para nível forte
    recentPeriod: 20         // Período recente para análise
};

// === CONFIGURAÇÕES PARA RISCO DE ROMPIMENTO ===
const BREAKOUT_RISK_SETTINGS = {
    highRiskDistance: 0.5,   // Alto risco se < 0.5% do nível
    mediumRiskDistance: 1.0, // Médio risco se < 1.0% do nível
    lowRiskDistance: 2.0,    // Baixo risco se < 2.0% do nível
    safeDistance: 3.0        // Seguro se > 3.0% do nível
};
...
class AdvancedLearningSystem {
    ...
    extractPatterns(trade) {
        ...
        // NOVOS PADRÕES: SUPORTE/RESISTÊNCIA
    ...
    async optimizeParameters(closedTrades) {
        try {
            ...
            // NOVO: Otimizar parâmetros de risco de rompimento
...
• Stop Dinâmico: <b>ATR ${DYNAMIC_STOP_SETTINGS.baseATRMultiplier.toFixed(1)}x</b>
• Risco Rompimento: <b>Ativo</b>
• Suporte/Resistência: <b>Ativo</b>
...
async function sendTelegramAlert(message) {
    try {
        // SEMPRE envia para o Telegram (modo REAL)
...
async function getATRData(symbol, timeframe = '15m', period = 14) {
    try {
        ...
        // Calcular volatilidade baseada no ATR
...
async function analyzeSupportResistance(symbol, currentPrice, isBullish) {
    try {
        ...
        // Encontrar níveis de suporte (mínimos locais)
        const supportLevels = findSupportLevels(lows, candles);
        
        // Encontrar níveis de resistência (máximos locais)
        ...
        // Determinar risco de rompimento
...
function findSupportLevels(lows, candles) {
    const levels = [];
    const window = 5; // Janela para encontrar mínimos locais
    ...
    for (let i = window; i < lows.length - window; i++) {
        ...
        if (isLocalMin) {
            // Verificar se já existe nível similar
            const existingLevel = levels.find(level => 
                Math.abs(level.price - currentLow) / currentLow < 0.005 // 0.5% de tolerância
            );
...
function filterRelevantLevels(levels, currentPrice, isSupport) {
    return levels
        ...
        .filter(level => {
            if (isSupport) {
                return level.price < currentPrice; // Suportes abaixo do preço
            } else {
                return level.price > currentPrice; // Resistências acima do preço
            }
        })
        .sort((a, b) => {
            if (isSupport) {
                return b.price - a.price; // Suportes: do mais alto para o mais baixo
            } else {
                return a.price - b.price; // Resistências: do mais baixo para o mais alto
            }
        });
}
...
function calculateBreakoutRisk(currentPrice, nearestSupport, nearestResistance, isBullish, recentBreakouts) {
    ...
    if (isBullish) {
        // Para compra: risco de resistência acima
        ...
        // Para venda: risco de suporte abaixo
        if (nearestSupport) {
    ...
    // Verificar se houve rompimentos recentes
...
✅ Stop Dinâmico baseado em ATR e volatilidade
✅ Retrações calculadas com ATR dinâmico
✅ Análise de Suporte/Resistência
✅ Detecção de Risco de Rompimento
✅ Sistema de aprendizado com tracking de trades
✅ Delay adaptativo entre grupos
✅ Otimização automática de parâmetros
✅ Relatórios de performance
✅ Entradas otimizadas por volatilidade

⚙️ <b>Configurações de Stop:</b>
• Base: ${DYNAMIC_STOP_SETTINGS.baseATRMultiplier.toFixed(1)}x ATR
• Mínimo: ${DYNAMIC_STOP_SETTINGS.minStopPercentage}%
• Máximo: ${DYNAMIC_STOP_SETTINGS.maxStopPercentage}%
• Ajuste por volatilidade: ${DYNAMIC_STOP_SETTINGS.volatilityBased ? 'ATIVO' : 'INATIVO'}

🎯 <b>Configurações de Retração:</b>
• Base: ${RETRACEMENT_SETTINGS.atrMultiplierMin.toFixed(1)}-${RETRACEMENT_SETTINGS.atrMultiplierMax.toFixed(1)}x ATR
• Método: ${RETRACEMENT_SETTINGS.useDynamicATR ? 'Dinâmico (ATR)' : 'Fixo'}

📊 <b>Análise S/R:</b>
• Período: ${SUPPORT_RESISTANCE_SETTINGS.lookbackPeriod} candles
• Proximidade: ${SUPPORT_RESISTANCE_SETTINGS.proximityThreshold}%
• Toques mínimos: ${SUPPORT_RESISTANCE_SETTINGS.minTouchPoints}
...
// =====================================================================
// 📊 FUNÇÃO ATUALIZADA PARA CALCULAR QUALIDADE COM SUPORTE/RESISTÊNCIA
// =====================================================================
...
async function calculateSignalQuality(symbol, isBullish, marketData) {
    ...
    // 11. Funding Rate (5 pontos extra)
    ...
    // 12. RISCO DE ROMPIMENTO (10 pontos) - NOVO
    ...
    // 13. SUPORTE/RESISTÊNCIA (10 pontos) - NOVO
...
async function monitorSymbol(symbol) {
    try {
        ...
        console.log(`   📊 S/R: ${srDistance}% | Risco: ${breakoutRisk} | Stop: ${targetsData.stopPercentage}%`);
...
// =====================================================================
// 📤 FUNÇÃO ATUALIZADA PARA ENVIAR ALERTAS COM ANÁLISE S/R
// =====================================================================
...
async function sendSignalAlert(signal) {
    try {
        ...
        const lsrValue = signal.marketData.lsr?.lsrRatio?.toFixed(2) || '0.00';
        const lsrThreshold = signal.isBullish ? LSR_BUY_THRESHOLD : LSR_SELL_THRESHOLD;
        const lsrInfo = `LSR 15m: ${lsrValue} (Limite: ${lsrThreshold})`;
        
        // Informações de volatilidade
        const volatilityLevel = signal.targetsData.atrData?.volatilityLevel || 'medium';
        const atrValue = signal.targetsData.atrData?.value?.toFixed(6) || '0.000000';
        const atrPercentage = signal.targetsData.atrData?.percentage?.toFixed(2) || '0.00';
        ...
        if (srData && !srData.error) {
            if (srData.nearestSupport) {
                srAnalysis += ` <b>Suporte mais próximo:</b> $${srData.nearestSupport.price.toFixed(6)}\n`;
                srAnalysis += `   • Força: ${srData.nearestSupport.strength}\n`;
                srAnalysis += `   • Distância: ${srData.nearestSupport.distancePercent.toFixed(2)}%\n`;
                srAnalysis += `   • ${srData.nearestSupport.isNear ? '⚠️ PRÓXIMO!' : '✅ Distância segura'}\n\n`;
            }
            
            if (srData.nearestResistance) {
                srAnalysis += ` <b>Resistência mais próxima:</b> $${srData.nearestResistance.price.toFixed(6)}\n`;
                srAnalysis += `   • Força: ${srData.nearestResistance.strength}\n`;
                srAnalysis += `   • Distância: ${srData.nearestResistance.distancePercent.toFixed(2)}%\n`;
                srAnalysis += `   • ${srData.nearestResistance.isNear ? '⚠️ PRÓXIMO!' : '✅ Distância segura'}\n\n`;
            }
            ...
            if (breakoutRisk) {
                ...
                else if (breakoutRisk.level === 'low') riskEmoji = '🟢';
                else if (breakoutRisk.level === 'very_low') riskEmoji = '✅';
                
                riskAnalysis = `${riskEmoji} <b>Risco de Rompimento:</b> ${breakoutRisk.level.toUpperCase()}\n`;
                riskAnalysis += `   • ${breakoutRisk.reason}\n`;
                riskAnalysis += `   • Distância: ${breakoutRisk.distancePercent?.toFixed(2) || 'N/A'}%\n`;
            }
            
            // Informações do nível mais relevante
            const relevantLevel = signal.isBullish ? srData.nearestResistance : srData.nearestSupport;
            if (relevantLevel) {
                const levelType = signal.isBullish ? 'resistência' : 'suporte';
                levelInfo = `🎯 <b>${levelType.toUpperCase()} ALVO:</b> $${relevantLevel.price.toFixed(6)}\n`;
                levelInfo += `   • Potencial: ${relevantLevel.distancePercent.toFixed(2)}%\n`;
                levelInfo += `   • Força: ${relevantLevel.strength}\n`;
            }
        }
        
        const message = `
${directionEmoji} <b>🤖 IA Análise ${direction} </b>
<b>Operação:</b> ${getBrazilianDateTime().full}
<b> #ATIVO - ${signal.symbol}</b>
 <b>Preço Atual:</b> $${signal.price.toFixed(6)}
 <b>SCORE:</b> ${signal.qualityScore.grade} (${signal.qualityScore.score}/100)
 <b>Indicadores/Sentimento:</b>
${signal.qualityScore.details.join('\n')}
• ${lsrInfo}

${riskAnalysis ? riskAnalysis + '\n' : ''}
 <b>Volatilidade:</b>
• Nível: <b>${volatilityLevel.toUpperCase()}</b>
• ATR: $${atrValue} (${atrPercentage}%)
• Stop: ${signal.targetsData.stopPercentage}%

${srAnalysis ? '<b>Suporte/Resistência:</b>\n' + srAnalysis : ''}
${levelInfo ? levelInfo + '\n' : ''}
🛑<b>Stop :</b> $${stopPrice.toFixed(6)} (${signal.targetsData.stopPercentage}%)
<b>Risco/Retorno:</b> ${signal.targetsData.bestRiskReward}:1

 <b>Regiões de Entrada:</b>
• <b>Entrada Ideal:</b> $${signal.price.toFixed(6)}
• <b>Retração Mínima:</b> $${signal.targetsData.retracementData.minRetracementPrice.toFixed(6)} (${signal.targetsData.retracementData.minRetracementPercentage.toFixed(2)}%)
• <b>Retração Máxima:</b> $${signal.targetsData.retracementData.maxRetracementPrice.toFixed(6)} (${signal.targetsData.retracementData.maxRetracementPercentage.toFixed(2)}%)
• <b>NÃO ultrapassar retração máxima!</b>

 <b>Alvos Sugeridos:</b>
${signal.targetsData.targets.slice(0, 4).map((target, index) => 
    `• Target ${index + 1}: $${target.price} (${target.target}%) RR:${target.riskReward}x`
).join('\n')}
• <b>Melhor Alvo:</b> $${signal.targetsData.bestTarget.price} (${signal.targetsData.bestTarget.target}%) RR:${signal.targetsData.bestTarget.riskReward}x

💡 <b>Estratégia Recomendada:</b>
1. Aguardar pullback até zona de retração mínima
2. Entrada escalonada: 20%/30% at 50% na máxima
3. Stop único em $${stopPrice.toFixed(6)}
4. Take profit escalonado nos alvos acima
${relevantLevel ? `5. ${levelInfo.split('\n')[0]}` : ''}
<b>System Titanium - by @J4Rviz.</b>
        ...
        console.log(`📤 Alerta REAL enviado: ${signal.symbol} ${direction}`);
        console.log(`📊 S/R: ${srData?.nearestSupport?.distancePercent?.toFixed(2) || 'N/A'}% suporte | ${srData?.nearestResistance?.distancePercent?.toFixed(2) || 'N/A'}% resistência`);
        console.log(`🎯 Risco: ${breakoutRisk?.level || 'N/A'} | ${breakoutRisk?.reason || ''}`);
...
async function startBot() {
    try {
        ...
        console.log('\n' + '='.repeat(80));
        console.log('🚀 BINANCE FUTURES BOT - VERSÃO ANÁLISE S/R AVANÇADA');
