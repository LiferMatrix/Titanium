...
// =====================================================================
const TELEGRAM_BOT_TOKEN = '7633398974:AAHaVFsA';
...
function gerarInterpretacaoBearish(symbol, currentPrice, fvgData, additionalData) {
    ...
    if (additionalData?.stoch4h && parseInt(additionalData.stoch4h.k) < 25 && additionalData.stoch4h.kTrend === '↑') {
        interpretacao += `⚠️ STOCH 4h ESTÁ SOBREVENDIDO (K${additionalData.stoch4h.k}↑)\n`;
        interpretacao += `→ Pode ter PEQUENA ALTA para testar resistências\n`;
        if (gapProximoAcima) {
            interpretacao += `→ Principal resistência: ${formatPrice(gapProximoAcima.top)} (${gapProximoAcima.timeframe})\n`;
        }
        interpretacao += `→ Isso seria um "PULLBACK" antes de continuar caindo\n\n`;
    }
    
    // Cenário 2: Rompimento baixista
    if (gapProximoAbaixo) {
        interpretacao += `📉 CENÁRIO BAIXISTA (SEU SINAL):\n`;
...
function gerarInterpretacaoBullish(symbol, currentPrice, fvgData, additionalData) {
    ...
    if (additionalData?.stoch4h && parseInt(additionalData.stoch4h.k) > 75 && additionalData.stoch4h.kTrend === '↓') {
        interpretacao += `⚠️ STOCH 4h ESTÁ SOBRECOMPRADO (K${additionalData.stoch4h.k}↓)\n`;
        interpretacao += `→ Pode ter PEQUENA QUEDA para testar suportes\n`;
        if (gapProximoAbaixo) {
    ...
    if (gapProximoAcima) {
        interpretacao += `📈 CENÁRIO ALTISTA (SEU SINAL):\n`;
...
async function sendInitMessage() {
    const msg = `<i>⚡ Titanium ⚡</i>\n\n<i> Monitorando APENAS BTC e ETH com REJEIÇÃO DE PREÇO OBRIGATÓRIA (wick ≥ 2x corpo)</i>\n<i> Alertas FVG + Volume 3min ativos com interpretação de cenários!</i>`;
