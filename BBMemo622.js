...
const CONFIG = {
    ...
    ALERTS: {
        ...
        VOLUME_DIRECTION: {
            ...
            DYNAMIC_THRESHOLDS: {
                ...
                MIN_ACCEPTABLE_BUY: 25,
                MIN_ACCEPTABLE_SELL: 75
            }
        },
        ...
        WICK_REJECTION: {
            ENABLED: true,
            SCORE_BONUS: 2,
            MIN_BODY_TO_WICK_RATIO: 0.3,
            CHECK_TIMEFRAMES: ['15m', '30m', '1h', '4h'],
            MIN_WICK_PERCENT: 0.3,
            MAX_BODY_PERCENT: 0.7
        }
    },
    VOLUME: { EMA_PERIOD: 9 },
    RSI: {
    ...
    LSR_PENALTY: {
        ...
        DYNAMIC_THRESHOLDS: {
            ...
            PIVOT_BOOST: 0.3,
            MAX_TIGHTEN_BUY: 1.5,
            MIN_TIGHTEN_SELL: 1.8,
            RELAX_FOR_WEAK_SIGNALS: true
        }
    },
    FUNDING_PENALTY: {
        ENABLED: true,
        LEVELS: {
            POSITIVE: {
                ...
                MEDIUM: { THRESHOLD: 0.05, POINTS: -10 },
                HIGH: { THRESHOLD: 0.1, POINTS: -12 }
            },
            NEGATIVE: {
                VERY_LOW: { THRESHOLD: -0.01, POINTS: -2 },
                LOW: { THRESHOLD: -0.02, POINTS: -3 },
...
// =====================================================================
// === FUNÇÃO PARA DETECTAR REJEIÇÃO DE WICK (CORRIGIDA) ===
// === AGORA RESPEITA CORRETAMENTE A DIREÇÃO DO ALERTA ===
// =====================================================================
function detectWickRejection(candles, isGreenAlert) {
    if (!CONFIG.ALERTS.WICK_REJECTION.ENABLED) {
        return { hasRejection: false, scoreBonus: 0, message: '', timeframe: null };
    }
    ...
    try {
        ...
        const timeframes = CONFIG.ALERTS.WICK_REJECTION.CHECK_TIMEFRAMES;
        const minBodyToWickRatio = CONFIG.ALERTS.WICK_REJECTION.MIN_BODY_TO_WICK_RATIO;
        const minWickPercent = CONFIG.ALERTS.WICK_REJECTION.MIN_WICK_PERCENT;
        const maxBodyPercent = CONFIG.ALERTS.WICK_REJECTION.MAX_BODY_PERCENT;
        ...
        for (const tf of timeframes) {
            ...
            // =============================================================
            // CORREÇÃO: DIREÇÃO CORRETA DO WICK
            // =============================================================
            // COMPRA (isGreenAlert = true) → Procura rejeição de BAIXA (wick inferior)
            // VENDA (isGreenAlert = false) → Procura rejeição de ALTA (wick superior)
            // =============================================================
            
            if (isGreenAlert) {
                // COMPRA: procurar rejeição de BAIXA (wick inferior longo)
                const hasLowerRejection = lowerWickPercent >= minWickPercent && 
                                          bodyPercent <= maxBodyPercent &&
                                          lowerWick > bodySize * minBodyToWickRatio;
                
                if (hasLowerRejection) {
                    const scoreBonus = CONFIG.ALERTS.WICK_REJECTION.SCORE_BONUS;
                    log(`✅🕯️ ${Object.keys(candles)[0]} [${tf}] - REJEIÇÃO DE BAIXA detectada para COMPRA (wick inferior ${lowerWickPercent.toFixed(1)}%) | Bônus: +${scoreBonus}`, 'success');
                    return {
                        hasRejection: true,
                        scoreBonus: scoreBonus,
                        message: `🕯️ Rejeição de BAIXA em ${tf} (wick inferior ${lowerWickPercent.toFixed(1)}%)`,
                        timeframe: tf,
                        rejectionType: 'lower'
                    };
                }
            } else {
                // VENDA: procurar rejeição de ALTA (wick superior longo)
                const hasUpperRejection = upperWickPercent >= minWickPercent && 
                                          bodyPercent <= maxBodyPercent &&
                                          upperWick > bodySize * minBodyToWickRatio;
                
                if (hasUpperRejection) {
                    const scoreBonus = CONFIG.ALERTS.WICK_REJECTION.SCORE_BONUS;
                    log(`✅🕯️ ${Object.keys(candles)[0]} [${tf}] - REJEIÇÃO DE ALTA detectada para VENDA (wick superior ${upperWickPercent.toFixed(1)}%) | Bônus: +${scoreBonus}`, 'success');
                    return {
                        hasRejection: true,
                        scoreBonus: scoreBonus,
                        message: `🕯️ Rejeição de ALTA em ${tf} (wick superior ${upperWickPercent.toFixed(1)}%)`,
                        timeframe: tf,
                        rejectionType: 'upper'
                    };
                }
        ...
        log(`Erro ao detectar rejeição de wick: ${error.message}`, 'error');
        return { hasRejection: false, scoreBonus: 0, message: '', timeframe: null };
    }
}
...
// =====================================================================
// === FUNÇÃO DINÂMICA DE VERIFICAÇÃO DE DIREÇÃO DO VOLUME ===
// =====================================================================
function checkVolumeDirection(volume1h, isGreenAlert, divergenceScore = 0, timeframe = '1h') {
    if (!CONFIG.ALERTS.VOLUME_DIRECTION.REQUIRE_VOLUME_DIRECTION) {
        return { passed: true, message: '', usedThreshold: null, relaxed: false };
    }
    
    const buyerPercentage = volume1h.percentage;
    const sellerPercentage = 100 - buyerPercentage;
    
    const dynamicConfig = CONFIG.ALERTS.VOLUME_DIRECTION.DYNAMIC_THRESHOLDS;
    
    if (!dynamicConfig.ENABLED) {
        if (isGreenAlert) {
            const minBuyer = CONFIG.ALERTS.VOLUME_DIRECTION.BUY_MIN_PERCENTAGE;
            if (buyerPercentage >= minBuyer) {
                return { passed: true, message: `Volume comprador ${buyerPercentage.toFixed(1)}% ≥ ${minBuyer}%`, usedThreshold: minBuyer, relaxed: false };
            } else {
                return { passed: false, message: `❌ Volume comprador insuficiente: ${buyerPercentage.toFixed(1)}% < ${minBuyer}%`, usedThreshold: minBuyer, relaxed: false };
            }
        } else {
            const maxBuyer = CONFIG.ALERTS.VOLUME_DIRECTION.SELL_MAX_PERCENTAGE;
            if (buyerPercentage <= maxBuyer) {
                return { passed: true, message: `Volume vendedor ${sellerPercentage.toFixed(1)}% ≥ ${100-maxBuyer}%`, usedThreshold: maxBuyer, relaxed: false };
            } else {
                return { passed: false, message: `❌ Volume vendedor insuficiente: ${sellerPercentage.toFixed(1)}% < ${100-maxBuyer}%`, usedThreshold: maxBuyer, relaxed: false };
            }
        }
    }
    ...
    let baseThreshold;
    if (isGreenAlert) {
        baseThreshold = dynamicConfig.BASE_BUY;
    } else {
        baseThreshold = dynamicConfig.BASE_SELL;
    }
    ...
    if (divergenceScore >= dynamicConfig.SCORE_BOOST.EXTREME_SCORE_THRESHOLD) {
        totalRelax += dynamicConfig.SCORE_BOOST.EXTREME_RELAX;
        relaxationReason.push(`Score extremo (${divergenceScore.toFixed(1)})`);
    } else if (divergenceScore >= dynamicConfig.SCORE_BOOST.HIGH_SCORE_THRESHOLD) {
        totalRelax += dynamicConfig.SCORE_BOOST.BONUS_RELAX;
        relaxationReason.push(`Score alto (${divergenceScore.toFixed(1)})`);
    }
    
    const timeframeBoost = dynamicConfig.TIMEFRAME_BOOST[timeframe] || 0;
    if (timeframeBoost > 0) {
        totalRelax += timeframeBoost;
        relaxationReason.push(`Timeframe ${timeframe}`);
    }
    
    if (isGreenAlert) {
        adjustedThreshold = Math.max(dynamicConfig.MIN_ACCEPTABLE_BUY, baseThreshold - totalRelax);
    } else {
        adjustedThreshold = Math.min(dynamicConfig.MIN_ACCEPTABLE_SELL, baseThreshold + totalRelax);
    }
    
    let passed = false;
    let message = '';
    let relaxed = totalRelax > 0;
    
    if (isGreenAlert) {
        if (buyerPercentage >= adjustedThreshold) {
            passed = true;
            message = `Volume comprador ${buyerPercentage.toFixed(1)}% ≥ ${adjustedThreshold}%`;
            if (relaxed) message += ` (relaxado por: ${relaxationReason.join(', ')})`;
        } else if (buyerPercentage >= dynamicConfig.MIN_ACCEPTABLE_BUY && !CONFIG.ALERTS.VOLUME_DIRECTION.STRICT_MODE) {
            passed = true;
            message = `⚠️ Volume comprador ${buyerPercentage.toFixed(1)}% (abaixo do ideal ${adjustedThreshold}%, mas aceito)`;
            if (relaxed) message += ` | Relax: ${relaxationReason.join(', ')}`;
            log(message, 'warning');
        } else {
            passed = false;
            message = `❌ Volume comprador insuficiente: ${buyerPercentage.toFixed(1)}% < ${adjustedThreshold}%`;
        }
    } else {
        if (buyerPercentage <= adjustedThreshold) {
            passed = true;
            message = `Volume vendedor ${sellerPercentage.toFixed(1)}% ≥ ${100-adjustedThreshold}%`;
            if (relaxed) message += ` (relaxado por: ${relaxationReason.join(', ')})`;
        } else if (buyerPercentage <= dynamicConfig.MIN_ACCEPTABLE_SELL && !CONFIG.ALERTS.VOLUME_DIRECTION.STRICT_MODE) {
            passed = true;
            message = `⚠️ Volume vendedor ${sellerPercentage.toFixed(1)}% (abaixo do ideal, mas aceito)`;
            if (relaxed) message += ` | Relax: ${relaxationReason.join(', ')}`;
            log(message, 'warning');
        } else {
            passed = false;
            message = `❌ Volume vendedor insuficiente: ${sellerPercentage.toFixed(1)}% < ${100-adjustedThreshold}%`;
        }
    }
    ...
    return {
        passed,
        message,
        usedThreshold: adjustedThreshold,
        relaxed,
        relaxationReason: relaxationReason.join(', '),
...
// =====================================================================
// === FUNÇÃO DINÂMICA DE VERIFICAÇÃO DE LSR ===
// =====================================================================
function checkLSRPenalty(lsr, isGreenAlert, divergenceScore = 0, timeframe = '1h', hasWickRejection = false, hasPivotDivergence = false) {
    if (!lsr) return { hasPenalty: false, points: 0, message: '', usedThreshold: null, tightened: false };
    
    const dynamicConfig = CONFIG.LSR_PENALTY.DYNAMIC_THRESHOLDS;
    ...
    if (!dynamicConfig.ENABLED) {
        if (isGreenAlert && lsr > CONFIG.LSR_PENALTY.BUY_MAX_RATIO) {
            return { 
                ...
                message: `LSR ${lsr.toFixed(2)} > ${CONFIG.LSR_PENALTY.BUY_MAX_RATIO}`,
                usedThreshold: CONFIG.LSR_PENALTY.BUY_MAX_RATIO,
                tightened: false
            };
        }
        if (!isGreenAlert && lsr < CONFIG.LSR_PENALTY.SELL_MIN_RATIO) {
            return { 
                hasPenalty: true, 
                points: CONFIG.LSR_PENALTY.PENALTY_POINTS,
                message: `LSR ${lsr.toFixed(2)} < ${CONFIG.LSR_PENALTY.SELL_MIN_RATIO}`,
                usedThreshold: CONFIG.LSR_PENALTY.SELL_MIN_RATIO,
                tightened: false
            };
        }
        return { hasPenalty: false, points: 0, message: '', usedThreshold: null, tightened: false };
    }
    
    let baseThreshold;
    if (isGreenAlert) {
        baseThreshold = dynamicConfig.BASE_BUY_MAX;
    } else {
        baseThreshold = dynamicConfig.BASE_SELL_MIN;
    }
    
    let adjustedThreshold = baseThreshold;
    let totalTighten = 0;
    let tighteningReason = [];
    
    if (divergenceScore >= dynamicConfig.SCORE_BOOST.EXTREME_SCORE_THRESHOLD) {
        totalTighten += dynamicConfig.SCORE_BOOST.EXTREME_TIGHTEN;
        tighteningReason.push(`Score extremo (${divergenceScore.toFixed(1)})`);
    } else if (divergenceScore >= dynamicConfig.SCORE_BOOST.HIGH_SCORE_THRESHOLD) {
        totalTighten += dynamicConfig.SCORE_BOOST.BONUS_TIGHTEN;
        tighteningReason.push(`Score alto (${divergenceScore.toFixed(1)})`);
    } else if (dynamicConfig.RELAX_FOR_WEAK_SIGNALS && divergenceScore < 5) {
        totalTighten -= 0.3;
        tighteningReason.push(`Score baixo (${divergenceScore.toFixed(1)}) - relaxado`);
    }
    
    const timeframeTighten = dynamicConfig.TIMEFRAME_BOOST[timeframe] || 0;
    if (timeframeTighten > 0) {
        totalTighten += timeframeTighten;
        tighteningReason.push(`Timeframe ${timeframe}`);
    }
    
    if (hasWickRejection) {
        totalTighten += dynamicConfig.WICK_BOOST;
        tighteningReason.push(`Rejeição de wick`);
    }
    
    if (hasPivotDivergence) {
        totalTighten += dynamicConfig.PIVOT_BOOST;
        tighteningReason.push(`Pivô divergente`);
    }
    
    if (isGreenAlert) {
        adjustedThreshold = Math.max(dynamicConfig.MAX_TIGHTEN_BUY, baseThreshold - totalTighten);
    } else {
        adjustedThreshold = Math.min(dynamicConfig.MIN_TIGHTEN_SELL, baseThreshold + totalTighten);
    }
    
    let hasPenalty = false;
    let message = '';
    let tightened = totalTighten > 0;
    
    if (isGreenAlert) {
        if (lsr > adjustedThreshold) {
            hasPenalty = true;
            message = `LSR ${lsr.toFixed(2)} > ${adjustedThreshold.toFixed(1)}`;
            if (tightened) message += ` (rigor aumentado por: ${tighteningReason.join(', ')})`;
        } else {
            message = `LSR ${lsr.toFixed(2)} ≤ ${adjustedThreshold.toFixed(1)} ✅`;
            if (tightened) message += ` (rigor aumentado: ${tighteningReason.join(', ')})`;
        }
    } else {
        if (lsr < adjustedThreshold) {
            hasPenalty = true;
    ...
    if (hasPenalty) {
        log(`⚠️ LSR Dinâmico - ${message}`, 'warning');
        return {
            hasPenalty: true,
            points: CONFIG.LSR_PENALTY.PENALTY_POINTS,
            message: message,
            usedThreshold: adjustedThreshold,
            tightened: tightened,
            tighteningReason: tighteningReason.join(', ')
        };
    }
...
function checkFundingPenalty(funding, isGreenAlert) {
    ...
    if (isGreenAlert && funding > 0 && CONFIG.FUNDING_PENALTY.BUY_PENALTY_FOR_POSITIVE) {
        ...
        const levels = CONFIG.FUNDING_PENALTY.LEVELS.POSITIVE;
        
        if (absFunding >= levels.HIGH.THRESHOLD) {
            totalPoints += levels.HIGH.POINTS;
            penaltyMessage = `Fund +${fundingPercent.toFixed(2)}% (MUITO positivo)`;
        } else if (absFunding >= levels.MEDIUM.THRESHOLD) {
            totalPoints += levels.MEDIUM.POINTS;
            penaltyMessage = `Fund +${fundingPercent.toFixed(2)}% (forte positivo)`;
        } else if (absFunding >= levels.LOW.THRESHOLD) {
            totalPoints += levels.LOW.POINTS;
            penaltyMessage = `Fund +${fundingPercent.toFixed(2)}% (positivo)`;
        } else if (absFunding >= levels.VERY_LOW.THRESHOLD) {
            totalPoints += levels.VERY_LOW.POINTS;
    ...
    if (!isGreenAlert && funding < 0 && CONFIG.FUNDING_PENALTY.SELL_PENALTY_FOR_NEGATIVE) {
        ...
        const levels = CONFIG.FUNDING_PENALTY.LEVELS.NEGATIVE;
        
        if (absFunding >= Math.abs(levels.HIGH.THRESHOLD)) {
            totalPoints += levels.HIGH.POINTS;
            penaltyMessage = `Fund ${fundingPercent.toFixed(2)}% (MUITO negativo)`;
        } else if (absFunding >= Math.abs(levels.MEDIUM.THRESHOLD)) {
            totalPoints += levels.MEDIUM.POINTS;
            penaltyMessage = `Fund ${fundingPercent.toFixed(2)}% (forte negativo)`;
        } else if (absFunding >= Math.abs(levels.LOW.THRESHOLD)) {
            totalPoints += levels.LOW.POINTS;
            penaltyMessage = `Fund ${fundingPercent.toFixed(2)}% (negativo)`;
        } else if (absFunding >= Math.abs(levels.VERY_LOW.THRESHOLD)) {
            totalPoints += levels.VERY_LOW.POINTS;
            penaltyMessage = `Fund ${fundingPercent.toFixed(2)}%`;
        }
    }
    
    if (CONFIG.FUNDING_PENALTY.FAVORABLE_BONUS.ENABLED) {
        const minBonusThreshold = CONFIG.FUNDING_PENALTY.FAVORABLE_BONUS.MIN_THRESHOLD;
        
        if (isGreenAlert && funding < 0 && Math.abs(funding) >= minBonusThreshold) {
            const bonusPoints = CONFIG.FUNDING_PENALTY.FAVORABLE_BONUS.BUY_NEGATIVE_BONUS;
            totalPoints += bonusPoints;
            bonusMessage = `🎯 Fund ${fundingPercent.toFixed(2)}% (negativo = BÔNUS COMPRA +${bonusPoints})`;
        }
        
        if (!isGreenAlert && funding > 0 && funding >= minBonusThreshold) {
            const bonusPoints = CONFIG.FUNDING_PENALTY.FAVORABLE_BONUS.SELL_POSITIVE_BONUS;
            totalPoints += bonusPoints;
            bonusMessage = `🎯 Fund +${fundingPercent.toFixed(2)}% (positivo = BÔNUS VENDA +${bonusPoints})`;
        }
    }
    
    if (totalPoints < CONFIG.FUNDING_PENALTY.MAX_TOTAL_PENALTY) {
        totalPoints = CONFIG.FUNDING_PENALTY.MAX_TOTAL_PENALTY;
    }
    
    const finalMessage = penaltyMessage || bonusMessage;
    
    return {
        hasPenalty: totalPoints < 0,
        points: totalPoints,
        message: finalMessage,
        isBonus: totalPoints > 0
    };
}

// =====================================================================
// === FUNÇÃO DE SUPORTE E RESISTÊNCIA ===
// =====================================================================
...
// =====================================================================
// === DADOS ADICIONAIS E ANÁLISE TÉCNICA ===
// =====================================================================
...
// =====================================================================
// === VERIFICAÇÕES DE COOLDOWN, LIMITE DIÁRIO, SCORE MÍNIMO ===
// =====================================================================
...
async function sendSingleAlert(alert) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const pivotBonus = alert.pivotDivergenceBonus || 0;
            log(`${penaltyIcon} ALERTA ENVIADO: ${alert.direction} ${alert.symbol} [${alert.timeframe}] - Score: ${alert.confirmationScore} | Pivô: +${pivotBonus} | Wick: ${alert.wickBonus || 0} | Volume: ${alert.volume1h.direction} (${alert.volume1h.percentage.toFixed(1)}%) | Penalidades: ${alert.totalPenalty}`, 'success');
        }
        return true;
    } catch (error) {
...
async function sendGroupedAlerts(alerts, priority) {
    ...
    if (buyAlerts.length > 0) {
        ...
        buyAlerts.slice(0, 5).forEach(a => {
            const pivotIcon = a.pivotDivergenceDetected ? '🎯' : '';
            const wickIcon = a.wickDetected ? '🕯️' : '';
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} ${pivotIcon} ${wickIcon} | Vol: ${a.volume1h.direction} (${a.volume1h.percentage.toFixed(1)}%) | Penal: ${a.totalPenalty}\n`;
        });
    ...
    if (sellAlerts.length > 0) {
        ...
        sellAlerts.slice(0, 5).forEach(a => {
            const pivotIcon = a.pivotDivergenceDetected ? '🎯' : '';
            const wickIcon = a.wickDetected ? '🕯️' : '';
            message += `• ${a.symbol.replace('USDT', '')} [${a.timeframe}] Score: ${a.confirmationScore} ${pivotIcon} ${wickIcon} | Vol: ${(100 - a.volume1h.percentage).toFixed(1)}% vendedor | Penal: ${a.totalPenalty}\n`;
        });
...
// =====================================================================
// === FUNÇÃO DE ANÁLISE DE PIVÔ ===
// =====================================================================
async function analyzePivotDivergence(symbol, timeframe, candles, isGreenAlert, currentPrice) {
    if (!CONFIG.ALERTS.PIVOT_DIVERGENCE.ENABLED) {
        return { hasPivotDivergence: false, scoreBonus: 0, pivotInfo: null, message: '' };
    }
    ...
    try {
        if (!candles || candles.length < 50) return { hasPivotDivergence: false, scoreBonus: 0, pivotInfo: null, message: '' };
        ...
        for (let i = 3; i < candles.length - 3; i++) {
            ...
            if (isPivotHigh) {
                ...
                for (let k = 0; k < candles.length; k++) {
                    if (Math.abs(k - i) > 5 && Math.abs(highs[k] - pivotPrice) / pivotPrice < 0.005) {
                        touches++;
                    }
                }
                if (touches >= minStrength) {
                    pivotHighs.push({ price: pivotPrice, index: i, touches, type: 'resistance' });
                }
            }
            ...
            if (isPivotLow) {
                let touches = 1;
                const pivotPrice = lows[i];
                for (let k = 0; k < candles.length; k++) {
                    if (Math.abs(k - i) > 5 && Math.abs(lows[k] - pivotPrice) / pivotPrice < 0.005) {
                        touches++;
                    }
                }
                if (touches >= minStrength) {
                    pivotLows.push({ price: pivotPrice, index: i, touches, type: 'support' });
                }
            }
        }
        ...
        if (isGreenAlert) {
            ...
            if (resistancesAbove.length > 0) {
                ...
                relevantPivot = resistancesAbove[0];
            }
        }
        
        if (!relevantPivot) {
            return { hasPivotDivergence: false, scoreBonus: 0, pivotInfo: null, message: 'Nenhum pivô significativo encontrado' };
        }
        ...
        if (rsiValues.length > relevantPivot.index) {
            const rsiAtPivot = rsiValues[relevantPivot.index - CONFIG.RSI.PERIOD] || 50;
            ...
            if (isGreenAlert) {
                ...
                } else if (priceAtPivot < currentPrice && rsiAtPivot > currentRSI) {
                    hasDivergence = true;
                ...
                if (priceAtPivot > currentPrice && rsiAtPivot > currentRSI) {
                    ...
                    hasDivergence = true;
                    divergenceType = 'hidden_bearish_pivot';
                }
            }
        }
        
        if (!hasDivergence) {
            return { hasPivotDivergence: false, scoreBonus: 0, pivotInfo: relevantPivot, message: `Pivô ${relevantPivot.type} em ${formatPrice(relevantPivot.price)} (${relevantPivot.touches}x) sem divergência` };
        }
        
        let scoreBonus = CONFIG.ALERTS.PIVOT_DIVERGENCE.SCORE_BONUS;
        
        const pivotStrengthText = relevantPivot.touches >= 5 ? '🔥 FORTE' : (relevantPivot.touches >= 3 ? '⚡ MÉDIO' : '⚠️ FRACO');
        
        return {
            hasPivotDivergence: true,
            scoreBonus: scoreBonus,
            pivotInfo: {
                ...relevantPivot,
                strengthText: pivotStrengthText,
                divergenceType: divergenceType
            },
            message: `🎯 Pivô ${relevantPivot.type} em ${formatPrice(relevantPivot.price)} (${relevantPivot.touches}x ${pivotStrengthText}) com divergência ${divergenceType} | Bônus: +${scoreBonus}`
        };
        
    } catch (error) {
        log(`Erro em analyzePivotDivergence ${symbol}: ${error.message}`, 'error');
        return { hasPivotDivergence: false, scoreBonus: 0, pivotInfo: null, message: `Erro: ${error.message}` };
    }
}
...
async function analyzeDivergenceTimeframe(symbol, candles, timeframe, allCandles) {
    try {
        ...
        const wickRejection = detectWickRejection(allCandles, isGreenAlert);
        let wickBonus = 0;
        if (wickRejection.hasRejection) {
            wickBonus = wickRejection.scoreBonus;
            log(`✅🕯️ ${symbol} [${timeframe}] - ${wickRejection.message} | Bônus: +${wickBonus}`, 'success');
        }
        ...
        if (CONFIG.ALERTS.PIVOT_DIVERGENCE.ENABLED) {
            pivotDivergenceResult = await analyzePivotDivergence(symbol, timeframe, candles, isGreenAlert, currentPrice);
            if (pivotDivergenceResult.hasPivotDivergence) {
                pivotScoreBonus = pivotDivergenceResult.scoreBonus;
                log(`🎯 ${symbol} [${timeframe}] - ${pivotDivergenceResult.message}`, 'success');
            }
        }

        let confirmationScore = bestDiv.score + wickBonus + pivotScoreBonus;

        const lsrPenalty = checkLSRPenalty(
            additional.lsr, 
            isGreenAlert, 
            confirmationScore, 
            timeframe, 
            wickRejection.hasRejection, 
            pivotDivergenceResult?.hasPivotDivergence || false
        );
        ...
        } else if (fundingPenalty.points > 0) {
            confirmationScore += fundingPenalty.points;
            if (fundingPenalty.message) penaltyMessages.push(fundingPenalty.message);
        }
        ...
        return {
            ...
            wickDetected: wickRejection.hasRejection,
            wickBonus: wickBonus,
            wickMessage: wickRejection.message,
            wickTimeframe: wickRejection.timeframe,
            lsrDynamicThreshold: lsrPenalty.usedThreshold,
            lsrTightened: lsrPenalty.tightened
        };
    } catch (error) {
...
async function getTopSymbols() {
    try {
        ...
        return [
            ...
            'AAVEUSDT','SUSHIUSDT','COMPUSDT','SNXUSDT','YFIUSDT','CRVUSDT','BALUSDT','1INCHUSDT','ENJUSDT','ZILUSDT'
        ];
    }
}
...
async function analyzeSymbol(symbol) {
    try {
        ...
        const volumeCheck = checkVolumeDirection(
            bestDivergence.volume1h, 
            isGreenAlert, 
            maxScore, 
            bestDivergence.usedTimeframe || '1h'
        );
        
        if (!volumeCheck.passed) {
            log(`❌ ${symbol} - ${finalDirection} bloqueado: ${volumeCheck.message}`, 'warning');
            return [];
        }
        
        if (volumeCheck.relaxed) {
            log(`📊 ${symbol} - Volume DINÂMICO: ${volumeCheck.buyerPercentage?.toFixed(1) || volumeCheck.sellerPercentage?.toFixed(1)}% (relaxado por: ${volumeCheck.relaxationReason})`, 'info');
        }
        ...
        if (existingStudy) {
            ...
            if (emaConfirmation.confirmed) {
                ...
                const allPenaltyMessages = [...(bestDivergence.penaltyMessages || [])];
                
                let pivotInfoText = '';
                if (bestDivergence.pivotDivergenceDetected && bestDivergence.pivotInfo) {
                    const pivot = bestDivergence.pivotInfo;
                    pivotInfoText = `🎯 Pivô ${pivot.type} em ${formatPrice(pivot.price)} (${pivot.touches}x ${pivot.strengthText}) | Bônus: +${bestDivergence.pivotDivergenceBonus}`;
                }
                
                let wickInfoText = '';
                if (bestDivergence.wickDetected && bestDivergence.wickMessage) {
                    wickInfoText = ` ${bestDivergence.wickMessage}\n   +${bestDivergence.wickBonus}`;
                }
                
                let lsrInfoText = '';
                if (bestDivergence.lsrTightened && bestDivergence.lsrDynamicThreshold) {
                    lsrInfoText = `📊 LSR Dinâmico: ${bestDivergence.lsrValue?.toFixed(2)} ≤ ${bestDivergence.lsrDynamicThreshold.toFixed(1)} ✅`;
                }
                ...
                const consolidated = {
                    ...
                    pivotDivergenceDetected: bestDivergence.pivotDivergenceDetected,
                    pivotDivergenceBonus: bestDivergence.pivotDivergenceBonus,
                    pivotInfo: bestDivergence.pivotInfo,
                    pivotInfoText: pivotInfoText,
                    wickDetected: bestDivergence.wickDetected,
                    wickBonus: bestDivergence.wickBonus,
                    wickMessage: bestDivergence.wickMessage,
                    wickInfoText: wickInfoText,
                    lsrInfoText: lsrInfoText,
                    volumeRelaxed: volumeCheck.relaxed,
                    volumeRelaxReason: volumeCheck.relaxationReason,
                    volumeUsedThreshold: volumeCheck.usedThreshold,
                    divergencesList: divergences.map(d => ({
                        ...
                        pivotBonus: d.pivotDivergenceBonus || 0,
                        wickBonus: d.wickBonus || 0
                    })),
                    bullishCount,
                    bearishCount,
                    isNearSupport,
        ...
        if (bollingerCheck.shouldStudy && CONFIG.ALERTS.MEMORY.ENABLED) {
            const studyData = {
                ...
                divergences: divergences.map(d => ({ timeframe: d.timeframe, type: d.direction, subtype: d.divergenceType, score: d.confirmationScore, pivotBonus: d.pivotDivergenceBonus || 0, wickBonus: d.wickBonus || 0 })),
                bullishCount,
                bearishCount,
                isNearSupport,
                isNearResistance,
...
function formatAlert(data) {
    ...
    if (data.divergencesList && data.divergencesList.length > 0) {
        const tfList = data.divergencesList.map(d => `${d.timeframe}${d.isRecent ? '✅' : '⚠️'}${d.pivotBonus > 0 ? '🎯' : ''}${d.wickBonus > 0 ? '🕯️' : ''}`).join(',');
    ...
    if (data.pivotDivergenceDetected && data.pivotInfo) {
        ...
        const divergenceIcon = pivot.divergenceType === 'bullish_pivot' ? '📈' : 
                               (pivot.divergenceType === 'bearish_pivot' ? '📉' : '🔮');
        pivotDivergenceLine = `🎯 Pivô ${pivot.type} em ${formatPrice(pivot.price)} (${pivot.touches}x ${pivot.strengthText}) ${divergenceIcon}\n   Bônus: +${data.pivotDivergenceBonus}`;
    }
    
    let wickRejectionLine = '';
    if (data.wickDetected && data.wickMessage) {
...
async function startScanner() {
    ...
    log(`📚 Memória ATIVADA | Expira em ${CONFIG.ALERTS.MEMORY.EXPIRY_HOURS}h`, 'success');
    ...
    try {
        ...
        const initMessage = ` Titanium Prime X\n` +
            ...
            `📊 Volume Dinâmico: Buy ${CONFIG.ALERTS.VOLUME_DIRECTION.DYNAMIC_THRESHOLDS.BASE_BUY}% / Sell ${CONFIG.ALERTS.VOLUME_DIRECTION.DYNAMIC_THRESHOLDS.BASE_SELL}%\n` +
            `📊 LSR Dinâmico: Buy ≤ ${CONFIG.LSR_PENALTY.DYNAMIC_THRESHOLDS.BASE_BUY_MAX} / Sell ≥ ${CONFIG.LSR_PENALTY.DYNAMIC_THRESHOLDS.BASE_SELL_MIN}\n` +
            `🕯️ Wick: COMPRA=wick inferior / VENDA=wick superior\n` +
