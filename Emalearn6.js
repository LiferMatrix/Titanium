...
const QUALITY_WEIGHTS = {
    ...
    stoch4h: 8,          
    cci4h: 10,
    divergence15m: 10,    // NOVO: Divergência 15m
    breakoutRisk: 8,      // Reduzido para dar espaço à divergência
    supportResistance: 8  // Reduzido para dar espaço à divergência
};
...
const DIVERGENCE_SETTINGS = {
    timeframe: '15m',
    lookbackPeriod: 20,      // Período para análise de divergência
    rsiPeriod: 14,
    minCandleDistance: 3,    // Mínimo de candles entre máximos/mínimos
    confirmationCandles: 2   // Candles de confirmação após divergência
};
...
class AdvancedLearningSystem {
    ...
    async optimizeParameters(closedTrades) {
        try {
            ...
            // NOVO: Otimizar parâmetros de divergência
...
async function sendTelegramAlert(message) {
    try {
        ...
        // Configuração com timeout mais robusta
        const controller = new AbortController();
...
async function checkDivergence15m(symbol, isBullish) {
    try {
        ...
        // Encontrar máximos e mínimos locais no preço
        ...
        // Encontrar máximos e mínimos locais no RSI
        ...
        // Verificar confirmação (candles após a divergência)
        let confirmed = false;
        if (bestDivergence) {
...
function findRegularDivergence(priceHighs, priceLows, rsiHighs, rsiLows, candles) {
    // Verificar divergência bullish regular (preço faz lower low, RSI faz higher low)
    ...
    // Verificar divergência bearish regular (preço faz higher high, RSI faz lower high)
...
function findHiddenDivergence(priceHighs, priceLows, rsiHighs, rsiLows, candles) {
    // Verificar divergência bullish oculta (preço faz higher low, RSI faz lower low)
    if (priceLows.length >= 2 && rsiLows.length >= 2) {
        const lastPriceLow = priceLows[priceLows.length - 1];
        const prevPriceLow = priceLows[priceLows.length - 2];
        const lastRSILow = rsiLows[rsiLows.length - 1];
    ...
    // Verificar divergência bearish oculta (preço faz lower high, RSI faz higher high)
    if (priceHighs.length >= 2 && rsiHighs.length >= 2) {
        const lastPriceHigh = priceHighs[priceHighs.length - 1];
        const prevPriceHigh = priceHighs[priceHighs.length - 2];
        const lastRSIHigh = rsiHighs[rsiHighs.length - 1];
...
function checkDivergenceConfirmation(divergence, candles, isBullish) {
    ...
    // Requer pelo menos 2 candles após a divergência para confirmação
    if (candlesAfterDivergence < DIVERGENCE_SETTINGS.confirmationCandles) {
        return false;
    }
    
    // Para divergência bullish, verificar se o preço subiu após a divergência
    ...
    // Para divergência bearish, verificar se o preço caiu após a divergência
...
🔔 by @J4Rviz.
        ...
        // Tentativa mais robusta com retry
        let success = false;
        let attempts = 0;
        const maxAttempts = 3;
...
// =====================================================================
// 📊 FUNÇÃO ATUALIZADA PARA CALCULAR QUALIDADE COM DIVERGÊNCIA
// =====================================================================
...
async function calculateSignalQuality(symbol, isBullish, marketData) {
    ...
    // 12. DIVERGÊNCIA 15M (10 pontos) - NOVO
    if (marketData.divergence15m && marketData.divergence15m.hasDivergence) {
        ...
        // Verificar se a divergência está alinhada com a direção
        ...
        if (isAligned) {
            ...
            // Divergência contra a direção - penalizar
            divergenceScore = 0;
    ...
    // 13. RISCO DE ROMPIMENTO (8 pontos)
    ...
    // 14. SUPORTE/RESISTÊNCIA (8 pontos)
...
async function monitorSymbol(symbol) {
    try {
        ...
        // Adicionar análise de divergência 15m
        ...
        // Log com informações de divergência e S/R
...
// =====================================================================
// 📤 FUNÇÃO ATUALIZADA PARA ENVIAR ALERTAS COM DIVERGÊNCIA
// =====================================================================
...
async function sendSignalAlert(signal) {
    try {
        ...
        // Classificação do volume 3 minutos
        ...
        // Probabilidade de seguir o movimento
        const buyProbability = signal.isBullish ? calculateProbability(signal) : 0;
        const sellProbability = !signal.isBullish ? calculateProbability(signal) : 0;
...
<b>📊Volume 3m:</b> ${volumeClassification} (${volumeRatio.toFixed(2)}x, ${volumeAboveBelow}% ${volumeRatio >= 1 ? 'acima' : 'abaixo'} da média)
<b>🎯Probabilidade:</b> Compra ${buyProbability}% | Venda ${sellProbability}%
<b>📈Distância S/R:</b> ${distancePercent}% ${signal.isBullish ? 'da resistência' : 'do suporte'}
...
function calculateProbability(signal) {
    ...
    baseProbability += (signal.qualityScore.score - 70) * 0.5;
    
    // Ajustar baseado no volume
    const volumeRatio = signal.marketData.volume?.rawRatio || 0;
    if (volumeRatio >= 2.0) baseProbability += 10;
    else if (volumeRatio >= 1.5) baseProbability += 5;
    
    // Ajustar baseado na distância S/R
    const srData = signal.marketData.supportResistance;
    const nearestLevel = signal.isBullish ? 
        srData?.nearestResistance : srData?.nearestSupport;
    const distance = nearestLevel?.distancePercent || 0;
    ...
    else if (distance >= 2.0) baseProbability += 5;
    else if (distance >= 1.0) baseProbability += 2;
    else if (distance < 0.5) baseProbability -= 10;
    
    // Ajustar baseado na divergência
...
async function startBot() {
    try {
        ...
        console.log('\n' + '='.repeat(80));
        console.log('🚀 TITANIUM - ANÁLISE AVANÇADA COM DIVERGÊNCIA 15M');
