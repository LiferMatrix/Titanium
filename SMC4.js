...
// 🔵 FUNÇÃO MELHORADA: Filtro de Qualidade de Sinal (agora com Market Structure)
async function calculateSignalQuality(symbol, isBullish, volumeCheck, oiCheck, volatilityCheck, lsrCheck, rsi1h, emas3mData) {
    ...
    // 🔵 7. Market Structure (20 pontos - NOVO)
    if (USE_MARKET_STRUCTURE) {
...
// 🔵 FUNÇÃO ATUALIZADA: Construir mensagem de alerta (SEM POSITION SIZING)
...
// 🔵 FUNÇÃO ATUALIZADA: Monitorar confirmações de reversão (SEM POSITION SIZING)
async function monitorConfirmation(symbol) {
    try {
        ...
        if (hadBuySweep && emas3mData.isAboveEMA55 && emas3mData.isEMA13CrossingUp) {
            ...
            // 🔵 NOVO: Calcular qualidade do sinal (agora inclui Market Structure)
            ...
            if (now - alertsCooldown[symbol].lastBuyConfirmation > COOLDOWN) {
                ...
                // 🔵 ATUALIZAR FUNÇÃO buildAlertMessage (agora inclui Market Structure)
...
async function mainBotLoop() {
    ...
    const initMsg = '\n' +
        ...
        ` 📊 FILTRO DE QUALIDADE: Score mínimo ${QUALITY_THRESHOLD}/100 (com Market Structure)\n` +
        '='.repeat(70) + '\n';
    ...
    const brDateTime = getBrazilianDateTime();
    await sendAlert(`🤖 <b>SMC Confirmation Bot com Market Structure (Todos os pares Binance Futures)</b>\n` +
                    ...
                    `📈 <b>Filtro de qualidade:</b> ${QUALITY_THRESHOLD}/100 (inclui Market Structure)\n` +
        ...
        try {
            ...
            for (let i = 0; i < SYMBOLS.length; i += BATCH_SIZE) {
                ...
                for (const alert of batchAlerts) {
                    ...
                    logToFile(`ALERTA CONFIRMAÇÃO ${alert.signal} - ${alert.symbol} - Preço: $${alert.price} - Score: ${alert.qualityScore.score} - MS: ${alert.msAnalysis ? alert.msAnalysis.confluenceMessage : 'N/A'}`);
...
// Iniciar o bot
console.log('\n' + '='.repeat(80));
console.log('🤖 BOT DE CONFIRMAÇÕES SMC 1H COM MARKET STRUCTURE (TODOS OS PARES BINANCE FUTURES)');
