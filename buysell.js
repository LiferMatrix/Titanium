...
class CleanupManager {
    constructor() {
        this.maxCacheSize = 500; // Número máximo de itens por cache
        this.maxHistorySize = 500; // Número máximo de histórico por símbolo
        this.cleanupInterval = 60 * 60 * 1000; // Limpeza a cada 1 hora
    ...
    startCleanupScheduler() {
        ...
        // Também executa limpeza inicial após 5 minutos
        setTimeout(() => {
            this.performCleanup();
        }, 5 * 60 * 1000);
    }
    ...
    performCleanup() {
        console.log('🧹 Iniciando limpeza automática de memória e cache...');
        
        // Limpar caches antigos
        this.cleanupMapCache(alertCooldown);
        this.cleanupMapCache(divergenceCache);
        this.cleanupMapCache(bollingerCache);
        this.cleanupMapCache(rsiCache);
        
        // Limpar dados CVD e preços
        if (globalCVDData) {
            this.cleanupMapData(globalCVDData);
        }
        if (globalPriceData) {
            this.cleanupMapData(globalPriceData);
        }
        ...
        if (global.gc) {
            global.gc();
            console.log('🗑️ Garbage collector executado');
        }
        
        console.log('✅ Limpeza automática concluída');
    }

    cleanupMapCache(mapCache) {
        if (!mapCache || typeof mapCache.size === 'undefined') return;
        ...
        // Remover por tamanho
        if (mapCache.size > this.maxCacheSize) {
            const entriesToRemove = mapCache.size - this.maxCacheSize;
            const entries = Array.from(mapCache.entries());
            for (let i = 0; i < entriesToRemove; i++) {
                mapCache.delete(entries[i][0]);
                removedCount++;
            }
        }
        
        if (removedCount > 0) {
            console.log(`🧹 Limpos ${removedCount} itens de cache`);
        }
    }

    cleanupMapData(mapData) {
        if (!mapData || typeof mapData.size === 'undefined') return;
        
        let removedCount = 0;
        
        for (const [symbol, data] of mapData.entries()) {
            // Limpar histórico se existir
            if (data && data.history && data.history.length > this.maxHistorySize) {
                const toRemove = data.history.length - this.maxHistorySize;
                data.history.splice(0, toRemove);
                removedCount += toRemove;
            }
            
            // Remover dados muito antigos
            if (data && data.lastUpdate && (Date.now() - data.lastUpdate) > 2 * 60 * 60 * 1000) {
                mapData.delete(symbol);
                removedCount++;
            }
        }
        
        if (removedCount > 0) {
            console.log(`🧹 Limpos ${removedCount} registros de dados`);
        }
    }
    ...
    cleanupLogFiles() {
        ...
        if (fs.existsSync(logDir)) {
            try {
                ...
                // Ignora erro se não conseguir ler logs
            }
        }
    }
}
...
// Dados globais para CVD e preços
let globalCVDData = new Map();
let globalPriceData = new Map();
...
function setLatestVolumeProfile(symbol, profile) {
    ...
    // Limitar tamanho do objeto globalVolumeProfile
    const keys = Object.keys(globalVolumeProfile);
    if (keys.length > 500) {
        const toRemove = keys.slice(0, keys.length - 500);
        toRemove.forEach(key => delete globalVolumeProfile[key]);
    }
}
...
class RealtimeAlertManager {
    constructor() {
        ...
        this.cvdData = globalCVDData;
        this.priceData = globalPriceData;
    ...
    subscribeToSymbol(symbol) {
        ...
        this.cvdData.set(symbol, { value: 0, buyVolume: 0, sellVolume: 0, history: [], lastUpdate: Date.now() });
        this.priceData.set(symbol, { price: 0, timestamp: 0, lastUpdate: Date.now() });
    ...
    handleCandle(symbol, rawData) {
        try {
            ...
            if (candle.x) {
                ...
                this.priceData.set(symbol, { price: price, timestamp: Date.now(), lastUpdate: Date.now() });
    ...
    async checkPendingAlerts(symbol, currentPrice) {
        ...
        // Verificar se o alerta pendente expirou (mais de 1 hora)
        if (Date.now() - pending.timestamp > 3600000) {
            this.pendingAlerts.delete(pendingKey);
            console.log(`⏰ Alerta pendente expirado para ${symbol}`);
            return;
        }
...
async function start() {
    ...
    console.log('🧹 COM SISTEMA DE LIMPEZA AUTOMÁTICA');
    ...
    console.log('🧹 Limpeza automática ativa (a cada 1 hora)');
}
