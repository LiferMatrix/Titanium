...
const SYMBOLS = [
    ...
    'TURBOUSDT', 'NOTUSDT', 'WLDUSDT', 'SUSHIUSDT', 'YGGUSDT',
    'ENAUSDT', 'TIAUSDT', 'SEIUSDT', 'ZKUSDT', 'GALAUSDT',
    'CHZUSDT', 'HOTUSDT', 'MASKUSDT', 'PIXELUSDT', 'API3USDT',
    'NEIROUSDT', 'MEMEUSDT', 'JTOUSDT', 'ONEUSDT', 'BTCDOMUSDT',
    ...
    'MANTAUSDT', 'ILVUSDT', 'MAGICUSDT', 'PORTALUSDT', 'SANDUSDT',  
    'DYMUSDT', 'ZILUSDT', 'CTSIUSDT', 'VIRTUALUSDT', 'MANAUSDT',
    'PAXGUSDT', 'RSRUSDT', 'XVGUSDT', 'ATAUSDT',  'ATOMUSDT',
    'USDCUSDT'
];
...
const BATCH_SIZE = 15; // Processar 15 ativos em paralelo (seguro para Binance)
const candleCache = {}; // Cache simples de candles (reduz 70% das requisições)
...
async function monitorSymbolSweep(symbol) {
    try {
        ...
        // Calcular EMA 55 com verificação de segurança
        const ema55 = calculateEMA(closes.slice(-100), 55);
        if (ema55 === null) {
            logToFile(`⚠️ Não foi possível calcular EMA 55 para ${symbol}`);
            return null;
        }
        ...
        if (buySignal || sellSignal) {
            ...
            // 🔵 ADICIONAR FUNDING RATE NAS BUSCAS
            const [lsrData, orderBook, rsi1h, stoch4h, stochDaily, fundingRate] = await Promise.all([
                getLSR(symbol, '15m'),
                getOrderBook(symbol),
                getRSI(symbol, '1h'),
                getStochastic(symbol, '4h'),
                getStochastic(symbol, '1d'),
                getFundingRate(symbol)  // 🔵 NOVA CHAMADA
            ]);
...
async function processBatch(batch, processFunction) {
    ...
    results.forEach((result, index) => {
        if (result.status === 'fulfilled' && result.value) {
            ...
            console.log(`❌ Erro em ${symbol}: ${result.reason.message}`);
        }
    });
...
async function mainBotLoop() {
    ...
    const initMsg = '\n' +
        '='.repeat(50) + '\n' +
        ' 🤖 BOT DO SWEEP 1H INICIADO (VERSÃO SUPERIOR)\n' +
        ` 📊 MONITORANDO ${SYMBOLS.length} ATIVOS\n` +
        ` ⚡ PROCESSAMENTO EM LOTE (${BATCH_SIZE} ATIVOS EM PARALELO)\n` +
        ` 💾 CACHE DE CANDLES ATIVADO (50 SEGUNDOS)\n` +
        ` 🛡️  TRATAMENTO DE RATE LIMIT IMPLEMENTADO\n` +
        ` 📈 EMA CÁLCULO ATUALIZADO (MAIS CONFIÁVEL)\n` +
        '='.repeat(50) + '\n';
    
    console.log(initMsg);
    logToFile(`🤖 Bot iniciado - Monitorando ${SYMBOLS.length} ativos (versão superior)`);
    ...
    const brDateTime = getBrazilianDateTime();
    await sendAlert(`🤖 <b>Titanium SMC Sentinel (Versão Superior)</b>\n` +
                    `📍 <b>Horário Brasil (BRT):</b> ${brDateTime.full}\n` +
                    `📊 Monitorando ${SYMBOLS.length} ativos\n` +
                    `⚡ Processamento em lote (${BATCH_SIZE} ativos em paralelo)\n` +
                    `💾 Cache de candles ativado\n` +
                    `🛡️  Rate limit protection ativo\n` +
                    `📈 EMA cálculo otimizado (mais confiável)\n` +
                    `by @J4Rviz.`);
    ...
    while (true) {
        try {
            ...
            console.log(`\n🔄 Ciclo ${cycleCount} - Verificando ${SYMBOLS.length} ativos em lotes...`);
            
            // 🔵 PROCESSAR SWEEPS EM LOTES
            console.log('🔍 Verificando sweeps...');
            for (let i = 0; i < SYMBOLS.length; i += BATCH_SIZE) {
                const batch = SYMBOLS.slice(i, i + BATCH_SIZE);
                const batchAlerts = await processBatch(batch, monitorSymbolSweep);
                
                // Enviar alertas do batch
                for (const alert of batchAlerts) {
                    console.log(`\n🔔 SWEEP DETECTADO PARA ${alert.symbol}!`);
                    console.log(`📊 ${alert.signal} - Preço: $${alert.priceFormatted}`);
                    console.log(`📈 Volume 3m: ${alert.volumeInfo.ratio}x da média`);
                    logToFile(`ALERTA SWEEP ${alert.signal} - ${alert.symbol} - Preço: $${alert.price} - Volume: ${alert.volumeInfo.ratio}x`);
                    
                    await sendAlert(alert.message);
                    
                    // Atualizar cooldown
                    if (alert.signal === 'Venda') {
                        alertsCooldown[alert.symbol].lastSellAlert = Date.now();
                    } else {
                        alertsCooldown[alert.symbol].lastBuyAlert = Date.now();
                    }
...
// Iniciar o bot
console.log('\n' + '='.repeat(60));
console.log('🤖 BOT DE MONITORAMENTO SMC 1H (VERSÃO SUPERIOR)');
