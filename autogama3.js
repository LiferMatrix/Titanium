...
async function getAIPredictive(symbol, data) {
  try {
    ...
    if (probLong > 78) {
      const stage = probLong > 92 ? 'ALERTA QUENTE' : 'PRE-ALERTA';
      const msg = `
${stage} IA [${symbol}] LONGO
Probabilidade: ${probLong.toFixed(1)}% em ${timeEst.toFixed(0)}min
Put Wall alvo: ${data.putWall.toFixed(2)} USDT
Funding: ${data.fundingRate.toFixed(4)}% → ${fundingProj.toFixed(4)}%
Volume +${volChange.toFixed(0)}% | Baleias ${whaleRatioBuy.toFixed(1)}x mais COMPRA
${data.rsi1h.bullish_divergence ? 'Divergência BULLISH' : 'RSI subindo'}
${stage === 'ALERTA QUENTE' ? 'ENTRE COM 50-70% AGORA' : 'PREPARAR POSIÇÃO LONGO'}
      `.trim();
    ...
    if (probShort > 78) {
      const stage = probShort > 92 ? 'ALERTA QUENTE SHORT' : 'PRE-ALERTA SHORT';
      const msg = `
${stage} IA [${symbol}] SHORT
Probabilidade: ${probShort.toFixed(1)}% em ${timeEst.toFixed(0)}min
Call Wall alvo: ${data.callWall.toFixed(2)} USDT
Funding: ${data.fundingRate.toFixed(4)}% → ${fundingProj.toFixed(4)}%
Volume +${volChange.toFixed(0)}% | Baleias ${whaleRatioSell.toFixed(1)}x mais VENDA
${data.rsi1h.bearish_divergence ? 'Divergência BEARISH' : 'RSI caindo'}
${stage.includes('QUENTE') ? 'ENTRE COM 50-70% SHORT AGORA' : 'PREPARAR POSIÇÃO SHORT'}
      `.trim();
      await sendTelegramMessage(msg);
    }
    ...
    await logMessage(`Erro IA preditiva dual: ${e.message}`);
...
async function checkAlerts() {
  const promises = symbols.map(async (symbol) => {
    ...
    await getAIPredictive(symbol, data);
