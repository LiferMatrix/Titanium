    ...
    try {
        ...
        if (isBuyTrendValid) {
            ...
            if (!isOverbought && hasRequiredDivergence) {
                ...
                if (divergences.hasBullish) { score += 15; reasons.push(`${divergences.bullish[0]?.emoji || '🟢'} Divergência RSI Bullish`); hasDivergence = true; }
        ...
        if (isSellTrendValid) {
            ...
            if (!isOversold && hasRequiredDivergence) {
                ...
                if (divergences.hasBearish) { score += 15; reasons.push(`${divergences.bearish[0]?.emoji || '🔴'} Divergência RSI Bearish`); hasDivergence = true; }
...
async function sendSMCAlert(analysis) {
    ...
    message += `<b>🛑 Stop:</b> <i>${formatPrice(analysis.stopPrice)} (${analysis.stopPercent.toFixed(0)}%)</i>\n`;
