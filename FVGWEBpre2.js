...
const SETUP_80_FILE = path.join(__dirname, 'setup80.json');

// =====================================================================
// === CONFIGURAÇÃO OTIMIZADA PARA 550 MOEDAS ===
// =====================================================================
const CONFIG = {
    TELEGRAM: {
    ...
    MONITOR: {
        // OTIMIZADO: Scan mais lento para 550 moedas
        SCAN_INTERVAL_SECONDS: 120, // 2 minutos (era 60)
        
        // FILTROS AJUSTADOS
        MIN_VOLUME_USDT: 300000, // Reduzido para 300k (pegar mais moedas)
        MAX_SYMBOLS: 550, // AGORA: todas as moedas!
        EXCLUDE_SYMBOLS: ['USDCUSDT', 'BUSDUSDT', 'TUSDUSDT', 'USDPUSDT'],
        
        // NOVOS FILTROS PARA OTIMIZAÇÃO
        ENABLE_LAYERED_SCAN: true,
        TOP_TIER_COUNT: 50,      // Top 50: scan a cada 60s + CVD
        MID_TIER_COUNT: 150,     // Next 150: scan a cada 120s + CVD
        LOW_TIER_COUNT: 350,     // Restante: scan a cada 300s, SEM CVD
        
        LSRS_PERIOD: '5m',
        LSR_15M_PERIOD: '15m',
        LSR_TREND_THRESHOLD: 0.05,
        ALERT_COOLDOWN_MINUTES: 30,
        SETUP_80_COOLDOWN_MINUTES: 60,
        ...
        // OTIMIZAÇÃO: Cache mais agressivo
        CACHE_TTL: {
            CANDLES: 60000,      // 1 minuto
            INDICATORS: 120000,  // 2 minutos
            LSR: 300000,         // 5 minutos
            FUNDING: 60000       // 1 minuto
        },
        
        // BATCH PROCESSING
        BATCH_SIZE: 30,          // Processar 30 moedas por vez
        BATCH_DELAY_MS: 3000,    // 3 segundos entre batches
        ...
        CVD: {
            ...
            ENABLE_FOR_TOP_ONLY: true,  // CVD só para TOP 200
            TOP_SYMBOLS_FOR_CVD: 200
        },
        FVG: {
...
class RateLimiter {
    ...
    async acquire() {
        ...
        if (this.isBanned) {
            const now = Date.now();
            if (now - this.lastBanCheck > 60000) {
                this.isBanned = false;
                console.log(`🔓 Ban resetado, retomando requisições...`);
            } else {
    ...
    async processQueue() {
        ...
        while (this.pendingRequests.length > 0) {
            ...
            if (this.requests.length >= this.maxRequests) {
                ...
                if (waitTime > 0) {
                    console.log(`⏳ Rate limit: ${this.requests.length}/${this.maxRequests}, aguardando ${Math.ceil(waitTime / 1000)}s...`);
            ...
            resolve();
            await this.delay(100); // Delay entre requisições
        }
        this.isProcessing = false;
    }
    ...
    markBan() {
        this.isBanned = true;
        this.lastBanCheck = Date.now();
        console.log(`🚨 Possível ban detectado! Aguardando 60 segundos...`);
    }
...
// Caches otimizados
const rsiCache = new LRUCache(600);
const divergenceCache = new LRUCache(400);
const convergenceCache = new LRUCache(400);
const pivotCache = new LRUCache(400);
const lsrTrendCache = new LRUCache(600);
const stochCache = new LRUCache(600);
const cciCache = new LRUCache(600);
const alertedSymbols = new Map();
const atrCache = new LRUCache(600);
const candlesCache = new LRUCache(800);
const fundingCache = new LRUCache(600);
const lsrCache = new LRUCache(600);

// =====================================================================
// === WEB SOCKET OTIMIZADO (STREAM COMBINADO) ===
// =====================================================================
class OptimizedCVDManager {
    constructor() {
    ...
    async subscribeToSymbols(symbols) {
        // Limitar a TOP 200 para CVD (como configurado)
        const topSymbols = symbols.slice(0, CONFIG.MONITOR.CVD.TOP_SYMBOLS_FOR_CVD);
        this.subscribedSymbols = topSymbols;
        
        console.log(`🔌 Inicializando CVD para ${topSymbols.length} símbolos (stream combinado)`);
    ...
    connectWebSocket() {
        ...
        // Criar stream combinado (até 200 símbolos por conexão)
        const streams = this.subscribedSymbols.map(s => 
            `${s.toLowerCase()}@aggTrade`
        ).join('/');
        
        const wsUrl = `wss://fstream.binance.com/stream?streams=${streams}`;
        console.log(`🔌 Conectando WebSocket combinado com ${this.subscribedSymbols.length} streams...`);
        
        this.mainWs = new WebSocket(wsUrl);
        
        this.mainWs.on('open', () => {
            console.log(`✅ WebSocket combinado conectado!`);
            this.isConnected = true;
            this.reconnectAttempts = 0;
            
            // Processar trades pendentes
            for (const trade of this.pendingTrades) {
                this.processTrade(trade.symbol, trade.data);
            }
            this.pendingTrades = [];
        });
        ...
        this.mainWs.on('message', (data) => {
            try {
                ...
                if (stream && tradeData) {
                    ...
                    this.processTrade(symbol, tradeData);
                }
            } catch (error) {}
        });
    ...
    processTrade(symbol, trade) {
        if (!this.cvdData.has(symbol)) return;
        ...
        while (data.history.length > 500) data.history.shift();
    ...
    handleDisconnect() {
        if (this.reconnectAttempts < this.maxReconnectAttempts) {
            const delayTime = Math.min(5000 * Math.pow(1.5, this.reconnectAttempts), 60000);
            this.reconnectAttempts++;
            console.log(`🔄 Tentando reconectar em ${delayTime / 1000}s (tentativa ${this.reconnectAttempts})...`);
            setTimeout(() => this.connectWebSocket(), delayTime);
        } else {
            console.log(`❌ Falha na reconexão após ${this.maxReconnectAttempts} tentativas`);
        }
    }
    
    getCVD(symbol) {
        // Se o símbolo não está no CVD (fora do TOP), retorna neutro
        if (!this.cvdData.has(symbol)) return { direction: '' };
        
        const data = this.cvdData.get(symbol);
        if (!data) return { direction: '' };
...
async function analyzeDivergencesWithPivots(symbol, currentPrice) {
    ...
    divergenceCache.set(cacheKey, result, CONFIG.MONITOR.CACHE_TTL.INDICATORS);
    return result;
}
...
// =====================================================================
// === FUNÇÕES PARA CÁLCULOS TÉCNICOS ===
// =====================================================================
async function calculateATR(symbol, timeframe = '1h', period = 14) {
    ...
    try {
        ...
        atrCache.set(cacheKey, result, CONFIG.MONITOR.CACHE_TTL.INDICATORS);
        return result;
    } catch (error) {
        return null;
    }
...
async function sendSetup80Alert(asset, type, setup) {
    ...
    if (type === 'BULL') {
        message = `<i>🟡 PRÉ-ALERTA - 80% DO SETUP 🟡\n`;
        message += ` ATIVO: ${asset.symbol}\n`;
        message += ` ${dt.full} hs\n`;
        message += ` Preço atual: ${formatPrice(currentPrice)} USDT\n`;
        message += `\n`;
        message += `📊 PROGRESSO DO SETUP DE COMPRA:\n`;
        message += ` ▸ ${atendidos}/4 critérios atendidos (${percentual.toFixed(0)}%)\n`;
        message += `\n`;
        message += `✅ CRITÉRIOS ATENDIDOS:\n`;
        
        if (setup.volumeOk) message += ` ✅ Volume 3m COMPRA anômalo\n`;
        if (setup.divergenciaOk) message += ` ✅ Divergência Bullish (15m + outro TF)\n`;
        if (setup.convergenciaOk) message += ` ✅ Convergência Bullish\n`;
        if (setup.estruturaOk) message += ` ✅ Estrutura de COMPRA\n`;
        ...
        if (divergencias.bestBullishDivergence) {
            ...
            message += `\n🔍 Divergência detectada: ${div.timeframe} (força: ${div.strength.toFixed(1)}%)\n`;
        }
        ...
        message += `\n🔔 Quando todos os critérios forem atendidos, o alerta de COMPRA será disparado!\n`;
        ...
        message += ` Preço atual: ${formatPrice(currentPrice)} USDT\n`;
        message += `\n`;
        message += `📊 PROGRESSO DO SETUP DE VENDA:\n`;
        message += ` ▸ ${atendidos}/4 critérios atendidos (${percentual.toFixed(0)}%)\n`;
        message += `\n`;
        message += `✅ CRITÉRIOS ATENDIDOS:\n`;
        
        if (setup.volumeOk) message += ` ✅ Volume 3m VENDA anômalo\n`;
        if (setup.divergenciaOk) message += ` ✅ Divergência Bearish (15m + outro TF)\n`;
        if (setup.convergenciaOk) message += ` ✅ Convergência Bearish\n`;
        if (setup.estruturaOk) message += ` ✅ Estrutura de VENDA\n`;
        ...
        message += `\n🔔 Quando todos os critérios forem atendidos, o alerta de VENDA será disparado!\n`;
        message += ` Titanium Prime X by @J4Rviz</i>`;
    }
...
async function calculateStochastic(symbol, timeframe) {
    ...
    try {
        ...
        stochCache.set(cacheKey, result, CONFIG.MONITOR.CACHE_TTL.INDICATORS);
        return result;
    } catch (error) {
        return null;
    }
...
async function calculateCCI(symbol, timeframe) {
    ...
    try {
        ...
        cciCache.set(cacheKey, result, CONFIG.MONITOR.CACHE_TTL.INDICATORS);
        return result;
    } catch (error) {
        return null;
    }
...
async function getCandles(symbol, interval, limit = 100) {
    ...
    try {
        ...
        candlesCache.set(cacheKey, candles, CONFIG.MONITOR.CACHE_TTL.CANDLES);
        return candles;
    } catch (error) {
        return [];
    }
...
async function getRSIForSymbol(symbol) {
    ...
    try {
        ...
        rsiCache.set(cacheKey, rsiValue, CONFIG.MONITOR.CACHE_TTL.INDICATORS);
        return rsiValue;
    } catch (error) {
        return null;
    }
...
async function getAllSymbols() {
    try {
        ...
        // Agora pega TODOS (sem limite) ou até MAX_SYMBOLS=550
        const allSymbols = sortedByVolume.slice(0, CONFIG.MONITOR.MAX_SYMBOLS);
        
        console.log(`📊 Total de símbolos monitorados: ${allSymbols.length}`);
        
        return allSymbols.map(i => ({ 
            symbol: i.symbol, 
    ...
    try {
        ...
        for (let i = 0; i < symbols.length; i += batchSize) {
            ...
            const promises = batch.map(async s => {
                const cacheKey = `lsr_${s}`;
                const cached = lsrCache.get(cacheKey);
                if (cached) return { symbol: s, lsr: cached };
                ...
                try {
                    ...
                    if (data && data.length && data[0] && data[0].longShortRatio) {
                        ...
                        lsrCache.set(cacheKey, lsr, CONFIG.MONITOR.CACHE_TTL.LSR);
                        return { symbol: s, lsr };
                    }
...
async function getLSRTrend(symbol) {
    const cacheKey = `lsr_trend_${symbol}`;
    const cached = lsrTrendCache.get(cacheKey);
    if (cached) return cached;
    ...
    try {
        ...
        lsrTrendCache.set(cacheKey, result, CONFIG.MONITOR.CACHE_TTL.LSR);
        return result;
    } catch (error) {
...
async function checkAndAlert(symbolData, cvdManager) {
    ...
    try {
        ...
        if (setupProgress.compra.total >= 3 && !canAlert(fullSymbol, 'SETUP80_BUY') && canSendSetup80(fullSymbol, 'BUY')) {
            ...
            markSetup80Sent(fullSymbol, 'BUY');
        }
        
        if (setupProgress.venda.total >= 3 && !canAlert(fullSymbol, 'SETUP80_SELL') && canSendSetup80(fullSymbol, 'SELL')) {
            await sendSetup80Alert({
                symbol,
                fullSymbol,
                price,
            ...
            markSetup80Sent(fullSymbol, 'SELL');
        }
...
// =====================================================================
// === SCAN OTIMIZADO EM CAMADAS ===
// =====================================================================
const cvdManager = new OptimizedCVDManager();
...
async function scanAndAlert() {
    ...
    try {
        ...
        // Inicializar CVD com TOP símbolos
        if (cvdManager.subscribedSymbols.length === 0) {
            const topSymbols = allSymbols.slice(0, CONFIG.MONITOR.CVD.TOP_SYMBOLS_FOR_CVD).map(s => s.symbol);
            await cvdManager.subscribeToSymbols(topSymbols);
        }
        
        log(`🔍 Escaneando ${allSymbols.length} símbolos em batches...`, 'info');
        ...
        for (let i = 0; i < allSymbols.length; i += batchSize) {
            ...
            // Processar batch em paralelo com limite
            await Promise.all(batch.map(symbolData => checkAndAlert(symbolData, cvdManager)));
            
            totalProcessed += batch.length;
            log(`📊 Progresso: ${totalProcessed}/${allSymbols.length} símbolos processados`, 'info');
            
            // Pausa entre batches para não sobrecarregar
            if (i + batchSize < allSymbols.length) {
                await delay(CONFIG.MONITOR.BATCH_DELAY_MS);
            }
        }
        
        log(`✅ Scan completo. ${totalProcessed} ativos verificados.`, 'success');
    } catch (error) {
