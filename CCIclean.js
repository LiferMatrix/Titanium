...
async function cleanupOldLogs() {
  try {
    ...
    const now = Date.now();
    const maxAgeMs = config.LOG_MAX_FILES * 24 * 60 * 60 * 1000; // LOG_MAX_FILES dias em milissegundos
...
async function main() {
  ...
  try {
    await ensureLogDir(); // Criar diretório de logs
    await cleanupOldLogs(); // Limpar logs antigos
    await checkConnection();
    ...
    setInterval(cleanupOldLogs, 24 * 60 * 60 * 1000); // Executar cleanupOldLogs diariamente
  } catch (e) {
