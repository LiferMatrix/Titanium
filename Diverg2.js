...
async function sendAlertRSIDivergence(symbol, data) {
  ...
  const { ohlcv15m, ohlcv3m, price, rsi15m, rsiDivergence, lsr, fundingRate, aggressiveDelta, oi5m, oi15m } = data;
  ...
  let alertText = '';
  // Condições para compra: Divergência de alta RSI (15m), volume anômalo no 3m, OI 5m e 15m subindo
  const isBuySignal = isBullishDivergence && isAnomalousVolume3m && oi5m.isRising && oi15m.isRising;
  
  // Condições para venda: Divergência de baixa RSI (15m), volume anômalo no 3m, OI 5m e 15m caindo
  const isSellSignal = isBearishDivergence && isAnomalousVolume3m && !oi5m.isRising && !oi15m.isRising;

  logger.info(`Condições para ${symbol}: BuySignal=${isBuySignal}, SellSignal=${isSellSignal}, RSI Divergence(15m)=${isBullishDivergence ? 'Bullish' : isBearishDivergence ? 'Bearish' : 'Neutro'}, RSI15m=${rsi15m}, AnomalousVolume3m=${isAnomalousVolume3m}, OI5mRising=${oi5m.isRising}, OI15mRising=${oi15m.isRising}`);
  ...
  if (isBuySignal) {
    ...
    if (!foiAlertado) {
      ...
      logger.info(`Sinal de compra detectado para ${symbol}: Preço=${format(price)}, RSI Divergence(15m)=Bullish, RSI 15m=${rsi15m.toFixed(2)}, AnomalousVolume3m=${isAnomalousVolume3m}, OI5mRising=${oi5m.isRising}, OI15mRising=${oi15m.isRising}`);
    }
    ...
    if (!foiAlertado) {
      ...
      state.ultimoAlertaPorAtivo[symbol].historico = state.ultimoAlertaPorAtivo[symbol].historico.slice(-config.MAX_HISTORICO_ALERTAS);
      logger.info(`Sinal de venda detectado para ${symbol}: Preço=${format(price)}, RSI Divergence(15m)=Bearish, RSI 15m=${rsi15m.toFixed(2)}, AnomalousVolume3m=${isAnomalousVolume3m}, OI5mRising=${oi5m.isRising}, OI15mRising=${oi15m.isRising}`);
    }
  }
...
async function checkConditions() {
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      if (!rsi15mValues.length || !rsiDivergence) {
