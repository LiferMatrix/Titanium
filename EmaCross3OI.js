...
const config = {
  ...
  EMA_SLOW: 20, // Período da EMA lenta
};
...
async function sendAlertEMATrend(symbol, data) {
  ...
  if (!emaCrossover) {
    logger.warn(`Cruzamento EMA 9/20 inválido para ${symbol}`);
    return;
  }
  ...
  // Log para depuração
  logger.info(`EMA 9/20 (15m) para ${symbol}: Bullish=${isBullishCrossover}, Bearish=${isBearishCrossover}, EMA9=${emaCrossover.emaFast}, EMA20=${emaCrossover.emaSlow}`);
  ...
  const emaCrossoverText = isBullishCrossover ? `EMA 9/20 (15m): Bullish 🟢` : isBearishCrossover ? `EMA 9/20 (15m): Bearish 🔴` : `EMA 4/10 (15m): Neutro`;
  ...
  let alertText = '';
  // Condições para compra: OI 5m e 15m subindo, EMA 9 cruza acima da EMA 20 (15m), RSI 1h < 60, LSR < 2.7, Delta >= 20%, Volatilidade >= 0.5%
  ...
  // Condições para venda: OI 5m e 15m caindo, EMA 9 cruza abaixo da EMA 20 (15m), RSI 1h > 60, LSR > 2.8, Delta <= -20%, Volatilidade >= 0.5%
  ...
  // Log das condições
  logger.info(`Condições para ${symbol}: BuySignal=${isBuySignal}, SellSignal=${isSellSignal}, EMA9/20(15m)=${isBullishCrossover ? 'Bullish' : isBearishCrossover ? 'Bearish' : 'Neutro'}, RSI1h=${rsi1h}, OI5m=${oi5m.isRising}, OI15m=${oi15m.isRising}, LSR=${lsr.value}, Delta=${aggressiveDelta.deltaPercent}, Volatility=${volatility.toFixed(4)}`);
  ...
  if (isBuySignal) {
    ...
    if (!foiAlertado) {
      ...
      logger.info(`Sinal de compra detectado para ${symbol}: Preço=${format(price)}, EMA 9/20(15m)=Bullish, RSI 1h=${rsi1h.toFixed(2)}, OI 5m=${oi5m.percentChange}%, OI 15m=${oi15m.percentChange}%, LSR=${lsr.value ? lsr.value.toFixed(2) : 'N/A'}, Delta=${aggressiveDelta.deltaPercent}%, NearBuyZone=${isNearBuyZone}, Volatility=${volatility.toFixed(4)}`);
    }
    ...
    if (!foiAlertado) {
      ...
      state.ultimoAlertaPorAtivo[symbol].historico = state.ultimoAlertaPorAtivo[symbol].historico.slice(-config.MAX_HISTORICO_ALERTAS);
      logger.info(`Sinal de venda detectado para ${symbol}: Preço=${format(price)}, EMA 9/20(15m)=Bearish, RSI 1h=${rsi1h.toFixed(2)}, OI 5m=${oi5m.percentChange}%, OI 15m=${oi15m.percentChange}%, LSR=${lsr.value ? lsr.value.toFixed(2) : 'N/A'}, Delta=${aggressiveDelta.deltaPercent}%, NearSellZone=${isNearSellZone}, Volatility=${volatility.toFixed(4)}`);
    }
  }
...
async function checkConditions() {
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      const ohlcv15mRaw = getCachedData(`${cacheKeyPrefix}_15m`) || await withRetry(() => exchangeSpot.fetchOHLCV(symbol, '15m', undefined, config.EMA_SLOW + 1));
      ...
      if (ohlcv15m.length < config.EMA_SLOW + 1) {
        logger.warn(`Candles insuficientes para EMA 9/20 (15m) em ${symbol}: ${ohlcv15m.length}`);
        return;
      }

      const rsi1hValues = calculateRSI(ohlcv1h);
      const emaCrossover = calculateEMACrossover(ohlcv15m);
