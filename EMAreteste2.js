...
// =====================================================================
// === ESTRATÉGIAS DE RETESTE EXISTENTES ===
// =====================================================================
...
// =====================================================================
// === NOVAS ESTRATÉGIAS: RETESTE MA50 E MA200 ===
// =====================================================================
...
function checkMA50Retest(candles) {
    ...
    // Calcular Média Móvel Simples 50
    const ma50 = calculateSMA(closes, 50);
    
    // Calcular EMA55 para comparação
    const ema55 = calculateEMA(closes, 55);
    
    const tolerance = lastCandle.close * CONFIG.ALERTS.RETEST.TOUCH_TOLERANCE;
    
    // Verificar alinhamento MA50 vs EMA55 (diferença < 1%)
    const alinhado = Math.abs(ma50 - ema55) / ma50 < 0.01;
    
    // Reteste de ALTA (preço acima, toca MA50 como suporte)
    ...
    // Reteste de BAIXA (preço abaixo, toca MA50 como resistência)
    if (lastCandle.close < ma50) {
...
function checkMA200Retest(candles) {
    ...
    // Calcular Média Móvel Simples 200
    ...
    // Contar quantas EMAs estão alinhadas com a MA200
    let emasAlinhadas = 0;
    ...
    // Reteste de ALTA (tocando MA200 como suporte)
    ...
    // Reteste de BAIXA (tocando MA200 como resistência)
    if (lastCandle.close < ma200) {
...
async function analyzeSymbol(symbol) {
    try {
        ...
        for (const timeframe of CONFIG.TIMEFRAMES) {
            ...
            if (candles.length < 250) continue; // Precisa de candles suficientes para MA200
            ...
            // Calcular CCI apenas para timeframes 1h (referência)
            if (timeframe === '1h') {
                const candles1h = await getCandles(symbol, '1h', 100, 'high');
                ...
                // Para outros timeframes, usar CCI do timeframe atual como referência
                const cciAnalysis = calculateCCIWithEMA(candles, 20, 5);
                cciDirection = cciAnalysis.direction;
                cciEmoji = cciAnalysis.emoji;
            }
            
            // ===== ESTRATÉGIAS EXISTENTES (1-6) =====
...
async function sendAlert(data) {
 ...
 Titanium Prime by @J4Rviz</i>`;
    ...
    // RETESTE MA50
        ...
        const resist1 = formatPrice(data.ma50 * 1.1);
        const resist2 = formatPrice(data.ma50 * 1.05);
        const supt1 = formatPrice(data.ma50 * 0.95);
        const supt2 = formatPrice(data.ma50 * 0.9);
 ...
 Titanium Prime by @J4Rviz</i>`;
    ...
    // RETESTE MA200
        ...
        const resist1 = formatPrice(data.ma200 * 1.1);
        const resist2 = formatPrice(data.ma200 * 1.05);
        const supt1 = formatPrice(data.ma200 * 0.95);
        const supt2 = formatPrice(data.ma200 * 0.9);
...
async function startScanner() {
    ...
    console.log(`   7. 🔷 RETESTE MA50 (NOVO)`);
    console.log(`   8. 🏦 RETESTE MA200 (NOVO) - INSTITUCIONAL`);
    console.log('='.repeat(60) + '\n');
