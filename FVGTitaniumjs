...
const CONFIG = {
    ...
    MONITOR: {
        ...
        STOP_CONFIG: {
            ...
            ATR_MULTIPLIER_LOW_VOL: 1.2,
            MIN_STOP_PERCENT: 0.8,
            MAX_STOP_PERCENT: 3.5,
            STRUCTURE_BUFFER: 0.3,
            TRAILING_PERCENT: 0.8,
            VOL_HIGH_THRESHOLD: 0.02,
...
// =====================================================================
// === CACHE PARA DIVERGÊNCIA DO STOCHASTIC ===
// =====================================================================
const stochDivergenceCache = new Map();
...
class TrailingStopManager {
    ...
    startTrailing(symbol, entryPrice, tradeType, trailingPercent = CONFIG.MONITOR.STOP_CONFIG.TRAILING_PERCENT) {
    ...
    updateTrailingStop(symbol, currentPrice, initialStopPrice) {
        const active = this.activeStops.get(symbol);
        if (!active) return initialStopPrice;
        
        let newStopPrice = initialStopPrice;
        const now = Date.now();
        
        if (now - active.lastUpdate < 10000) return initialStopPrice;
        active.lastUpdate = now;
        
        if (active.tradeType === 'BUY') {
            if (currentPrice > active.highestPrice) {
                active.highestPrice = currentPrice;
                const potentialStop = currentPrice * (1 - active.trailingPercent / 100);
                newStopPrice = Math.max(potentialStop, initialStopPrice);
                if (newStopPrice > initialStopPrice) {
                    console.log(`  📈 Trailing Stop ${symbol} atualizado: ${newStopPrice.toFixed(4)}`);
                }
            }
        } else {
            if (currentPrice < active.lowestPrice) {
                active.lowestPrice = currentPrice;
                const potentialStop = currentPrice * (1 + active.trailingPercent / 100);
                newStopPrice = Math.min(potentialStop, initialStopPrice);
                if (newStopPrice < initialStopPrice) {
                    console.log(`  📉 Trailing Stop ${symbol} atualizado: ${newStopPrice.toFixed(4)}`);
                }
            }
        ...
        this.activeStops.set(symbol, active);
        return newStopPrice;
    }
    ...
    getActiveStop(symbol) {
        return this.activeStops.get(symbol);
    }
}
...
// =====================================================================
async function calculateVolatility(symbol, lookback = 50) {
    try {
        const candles = await getCandles(symbol, '15m', lookback);
        if (!candles || candles.length < 20) return { avgVolatility: 0.01, maxVolatility: 0.02 };
        ...
        const maxVolatility = Math.max(...returns);
        
        return { avgVolatility, maxVolatility };
    } catch (error) {
        return { avgVolatility: 0.01, maxVolatility: 0.02 };
    }
}
...
// =====================================================================
async function calculateSmartStopV2(symbol, entryPrice, tradeType, structureData, currentPrice) {
    const atrData = await calculateATR(symbol, '15m', CONFIG.MONITOR.SMC.ATR_PERIOD);
    const candles = await getCandles(symbol, '15m', 50);
    const volatility = await calculateVolatility(symbol, 50);
    const { avgVolatility } = volatility;
    
    let finalStopPercent = 1.0;
    let stopType = "ATR";
    let stopReasons = [];
    
    // 1. STOP BASEADO EM ATR
    let atrPercent = atrData ? (atrData.atr / entryPrice) * 100 : 1.5;
    
    let multiplier = CONFIG.MONITOR.STOP_CONFIG.ATR_MULTIPLIER_BASE;
    if (avgVolatility > CONFIG.MONITOR.STOP_CONFIG.VOL_HIGH_THRESHOLD) {
        multiplier = CONFIG.MONITOR.STOP_CONFIG.ATR_MULTIPLIER_HIGH_VOL;
        stopReasons.push(`alta volatilidade (${(avgVolatility*100).toFixed(2)}%)`);
    } else if (avgVolatility < CONFIG.MONITOR.STOP_CONFIG.VOL_LOW_THRESHOLD) {
        multiplier = CONFIG.MONITOR.STOP_CONFIG.ATR_MULTIPLIER_LOW_VOL;
        stopReasons.push(`baixa volatilidade (${(avgVolatility*100).toFixed(2)}%)`);
    }
    
    let atrStopPercent = atrPercent * multiplier;
    atrStopPercent = Math.min(Math.max(atrStopPercent, CONFIG.MONITOR.STOP_CONFIG.MIN_STOP_PERCENT), CONFIG.MONITOR.STOP_CONFIG.MAX_STOP_PERCENT);
    finalStopPercent = atrStopPercent;
    
    // 2. STOP BASEADO EM SUPORTE/RESISTÊNCIA
    const clusters = findClusterLevels(candles, entryPrice, 0.5, 1);
    let srStopPrice = null;
    let srStopPercent = null;
    
    if (tradeType === 'BUY' && clusters.supports && clusters.supports.length > 0) {
        const nearestSupport = clusters.supports[0];
        if (nearestSupport && nearestSupport.avgPrice < entryPrice) {
            const supportDistance = ((entryPrice - nearestSupport.avgPrice) / entryPrice) * 100;
            if (supportDistance < finalStopPercent) {
                srStopPercent = supportDistance + CONFIG.MONITOR.STOP_CONFIG.SR_BUFFER;
                srStopPrice = nearestSupport.avgPrice * (1 - CONFIG.MONITOR.STOP_CONFIG.SR_BUFFER / 100);
                stopReasons.push(`suporte em ${formatPrice(nearestSupport.avgPrice)}`);
            }
        }
    } else if (tradeType === 'SELL' && clusters.resistances && clusters.resistances.length > 0) {
        const nearestResistance = clusters.resistances[0];
        if (nearestResistance && nearestResistance.avgPrice > entryPrice) {
            const resistanceDistance = ((nearestResistance.avgPrice - entryPrice) / entryPrice) * 100;
            if (resistanceDistance < finalStopPercent) {
                srStopPercent = resistanceDistance + CONFIG.MONITOR.STOP_CONFIG.SR_BUFFER;
                srStopPrice = nearestResistance.avgPrice * (1 + CONFIG.MONITOR.STOP_CONFIG.SR_BUFFER / 100);
                stopReasons.push(`resistência em ${formatPrice(nearestResistance.avgPrice)}`);
            }
        }
    }
    
    if (srStopPercent && srStopPercent < finalStopPercent) {
        finalStopPercent = Math.min(srStopPercent, CONFIG.MONITOR.STOP_CONFIG.MAX_STOP_PERCENT);
        stopType = "S_R";
    }
    
    // 3. STOP BASEADO EM ESTRUTURA
    let structureStopPercent = null;
    let structureStopPrice = null;
    
    if (structureData) {
        const structurePrice = structureData.sweepPrice || structureData.obPrice;
        if (tradeType === 'BUY' && structurePrice && structurePrice < entryPrice) {
            structureStopPercent = ((entryPrice - structurePrice) / entryPrice) * 100;
            structureStopPrice = structurePrice * 0.998;
        } else if (tradeType === 'SELL' && structurePrice && structurePrice > entryPrice) {
            structureStopPercent = ((structurePrice - entryPrice) / entryPrice) * 100;
            structureStopPrice = structurePrice * 1.002;
        }
        
        if (structureStopPercent && structureStopPercent > 0.5 && structureStopPercent < finalStopPercent) {
            finalStopPercent = Math.min(structureStopPercent + CONFIG.MONITOR.STOP_CONFIG.STRUCTURE_BUFFER, CONFIG.MONITOR.STOP_CONFIG.MAX_STOP_PERCENT);
            stopType = "ESTRUTURA";
            stopReasons.push(`estrutura em ${formatPrice(structurePrice)}`);
        }
    }
    
    // 4. AJUSTE POR TIMEFRAME
    try {
        const emaAnalysis = await analyzeMultiTimeframeEMAs(symbol);
        if (emaAnalysis) {
            if (tradeType === 'BUY' && emaAnalysis['4h']?.trend === 'ALTA') {
                finalStopPercent = Math.min(finalStopPercent * 1.1, CONFIG.MONITOR.STOP_CONFIG.MAX_STOP_PERCENT);
                stopReasons.push(`4h em ALTA (+10%)`);
            } else if (tradeType === 'BUY' && emaAnalysis['4h']?.trend === 'BAIXA') {
                finalStopPercent = Math.max(finalStopPercent * 0.9, CONFIG.MONITOR.STOP_CONFIG.MIN_STOP_PERCENT);
                stopReasons.push(`4h em BAIXA (-10%)`);
            } else if (tradeType === 'SELL' && emaAnalysis['4h']?.trend === 'BAIXA') {
                finalStopPercent = Math.min(finalStopPercent * 1.1, CONFIG.MONITOR.STOP_CONFIG.MAX_STOP_PERCENT);
                stopReasons.push(`4h em BAIXA (+10%)`);
            } else if (tradeType === 'SELL' && emaAnalysis['4h']?.trend === 'ALTA') {
                finalStopPercent = Math.max(finalStopPercent * 0.9, CONFIG.MONITOR.STOP_CONFIG.MIN_STOP_PERCENT);
                stopReasons.push(`4h em ALTA (-10%)`);
            }
        }
    ...
    finalStopPercent = Math.max(finalStopPercent, CONFIG.MONITOR.STOP_CONFIG.MIN_STOP_PERCENT);
    
    let stopPrice;
    if (srStopPrice && stopType === "S_R") {
        stopPrice = srStopPrice;
    } else if (structureStopPrice && stopType === "ESTRUTURA") {
        stopPrice = structureStopPrice;
    } else {
        stopPrice = tradeType === 'SELL' 
    ...
    return { stopPrice, stopPercent: finalStopPercent, stopType, stopReasons };
}
...
function detectMACDDivergence(prices, macdHistogram, macdValues, timeframe) {
    ...
    return {
        ...
        summary: bullish ? `🟢 MACD Bullish (${detectedTimeframes.join(', ')})` : (bearish ? `🔴 MACD Bearish (${detectedTimeframes.join(', ')})` : '')
    };
}
...
async function analyzeMACDDivergences(symbol, tradeSignal) {
    try {
        ...
        return { bullish: [], bearish: [], hasBullish: false, hasBearish: false, summary: '❌ Sem divergência MACD', timeframesList: [] };
    }
}

// =====================================================================
// === DIVERGÊNCIA DO STOCHASTIC ===
// =====================================================================
function findStochPivots(values, lookback = 3) {
    const pivots = { highs: [], lows: [] };
...
function detectStochasticDivergence(prices, stochKValues, stochDValues, timeframe) {
    if (prices.length < 30 || stochKValues.length < 30) {
        return { bullish: false, bearish: false, hasDivergence: false, isQualified: false, timeframes: [] };
    }
    
    const pricePivots = findPivots(prices, 5);
    const stochPivots = findStochPivots(stochKValues, 5);
    ...
    const currentK = stochKValues[stochKValues.length - 1];
    const currentD = stochDValues[stochDValues.length - 1];
    const prevK = stochKValues.length > 1 ? stochKValues[stochKValues.length - 2] : currentK;
    const prevD = stochDValues.length > 1 ? stochDValues[stochDValues.length - 2] : currentD;
    
    const kCrossoverUp = prevK <= prevD && currentK > currentD;
    const kCrossoverDown = prevK >= prevD && currentK < currentD;
    ...
    for (let i = 0; i < pricePivots.lows.length; i++) {
        for (let j = i + 1; j < pricePivots.lows.length; j++) {
            ...
            if (priceLower && priceDistance >= 5) {
                const stochLow1 = stochPivots.lows.find(s => Math.abs(s.index - priceLow1.index) <= 5);
                const stochLow2 = stochPivots.lows.find(s => Math.abs(s.index - priceLow2.index) <= 5);
                
                if (stochLow1 && stochLow2 && stochLow2.value > stochLow1.value) {
                    bullish = true;
                    detectedTimeframes.push(timeframe);
                    isQualified = kCrossoverUp;
                }
            }
        }
    }
    ...
    for (let i = 0; i < pricePivots.highs.length; i++) {
        for (let j = i + 1; j < pricePivots.highs.length; j++) {
            ...
            if (priceHigher && priceDistance >= 5) {
                const stochHigh1 = stochPivots.highs.find(s => Math.abs(s.index - priceHigh1.index) <= 5);
                const stochHigh2 = stochPivots.highs.find(s => Math.abs(s.index - priceHigh2.index) <= 5);
                
                if (stochHigh1 && stochHigh2 && stochHigh2.value < stochHigh1.value) {
                    bearish = true;
                    detectedTimeframes.push(timeframe);
                    isQualified = kCrossoverDown;
                }
            }
        }
    }
    ...
    return {
        bullish: bullish,
        bearish: bearish,
        hasDivergence: bullish || bearish,
        isQualified: isQualified,
        kCrossoverUp: kCrossoverUp,
        kCrossoverDown: kCrossoverDown,
        currentK: currentK,
        currentD: currentD,
        timeframes: detectedTimeframes,
        summary: bullish ? `🟢 Stochastic Bullish (${detectedTimeframes.join(', ')})${kCrossoverUp ? ' ✅ K > D' : ' ⚠️ aguardar K > D'}` : 
                  (bearish ? `🔴 Stochastic Bearish (${detectedTimeframes.join(', ')})${kCrossoverDown ? ' ✅ K < D' : ' ⚠️ aguardar K < D'}` : '')
    };
}

async function checkStochasticDivergenceOnTimeframe(symbol, timeframe) {
    try {
        const cacheKey = `${symbol}_stoch_divergence_${timeframe}`;
        const now = Date.now();
        const cached = stochDivergenceCache.get(cacheKey);
        if (cached && (now - cached.timestamp) < 300000) {
            return cached.data;
        }
        
        const candles = await getCandles(symbol, timeframe, CONFIG.MONITOR.SMC.DIVERGENCE_LOOKBACK);
        if (!candles || candles.length < 50) return { bullish: false, bearish: false, hasDivergence: false, isQualified: false, timeframes: [] };
        ...
        for (let i = period; i <= prices.length; i++) {
            ...
            const rawK = ((currentPrice - low) / (high - low)) * 100;
            
            if (kValues.length >= slowing - 1) {
                const kSlice = [...kValues.slice(-(slowing - 1)), rawK];
                const smoothedK = kSlice.reduce((a, b) => a + b, 0) / slowing;
            ...
            if (kValues.length >= 3) {
                const dValue = (kValues[kValues.length - 1] + kValues[kValues.length - 2] + kValues[kValues.length - 3]) / 3;
                dValues.push(dValue);
            } else {
                dValues.push(rawK);
            }
        ...
        if (kValues.length < 30 || dValues.length < 30) return { bullish: false, bearish: false, hasDivergence: false, isQualified: false, timeframes: [] };
        
        const divergence = detectStochasticDivergence(prices, kValues, dValues, timeframe);
        
        addToCache(stochDivergenceCache, cacheKey, { data: divergence, timestamp: now });
...
async function checkStochasticDivergence4h(symbol) {
    return await checkStochasticDivergenceOnTimeframe(symbol, '4h');
}

async function checkStochasticDivergence12h(symbol) {
    return await checkStochasticDivergenceOnTimeframe(symbol, '12h');
}

async function checkStochasticDivergenceBothTF(symbol) {
    const div4h = await checkStochasticDivergence4h(symbol);
    const div12h = await checkStochasticDivergence12h(symbol);
    
    return {
        has4h: div4h.hasDivergence,
        has12h: div12h.hasDivergence,
        isQualified4h: div4h.isQualified,
        isQualified12h: div12h.isQualified,
        bullish4h: div4h.bullish,
        bullish12h: div12h.bullish,
        bearish4h: div4h.bearish,
        bearish12h: div12h.bearish,
        hasAny: div4h.hasDivergence || div12h.hasDivergence,
        isAnyQualified: (div4h.hasDivergence && div4h.isQualified) || (div12h.hasDivergence && div12h.isQualified),
        count: (div4h.hasDivergence ? 1 : 0) + (div12h.hasDivergence ? 1 : 0),
        summary: (div4h.hasDivergence ? div4h.summary + ' ' : '') + (div12h.hasDivergence ? div12h.summary : '')
    };
}
...
async function analyzeCCI(symbol, timeframe = '4h') {
    try {
        ...
        const candles = await getCandles(symbol, timeframe, 60);
        ...
        const cciValues = calculateCCI(highs, lows, closes, CONFIG.MONITOR.SMC.CCI_PERIOD);
        if (!cciValues || cciValues.length < 10) return null;
        
        const cciEma = calculateEMAForCCI(cciValues, CONFIG.MONITOR.SMC.CCI_EMA_PERIOD);
        const currentCCI = cciValues[cciValues.length - 1];
        const prevCCI = cciValues.length > 1 ? cciValues[cciValues.length - 2] : currentCCI;
        const prevEma = cciValues.length > 5 ? calculateEMAForCCI(cciValues.slice(0, -1), CONFIG.MONITOR.SMC.CCI_EMA_PERIOD) : cciEma;
        
        let crossoverUp = false;
        let crossoverDown = false;
        
        if (prevCCI <= prevEma && currentCCI > cciEma) {
            crossoverUp = true;
        }
        if (prevCCI >= prevEma && currentCCI < cciEma) {
            crossoverDown = true;
        ...
        const result = {
            ...
            crossoverUp,
            crossoverDown,
            display: `CCI:${currentCCI.toFixed(0)} EMA:${cciEma ? cciEma.toFixed(0) : '--'}`
        };
            ...
            if (priceLower && priceDistance >= 5) {
                ...
                if (rsiLow1 && rsiLow2 && rsiLow2.value > rsiLow1.value) {
                    ...
                    if (strength >= CONFIG.MONITOR.SMC.DIVERGENCE_MIN_STRENGTH) {
                        divergences.bullish.push({ type: 'bullish', timeframe: timeframe, strength: strength, emoji: '🎯#Pivô⤴️🟢' });
                    }
                }
            }
        }
        ...
        for (let j = i + 1; j < pricePivots.highs.length; j++) {
            ...
            if (priceHigher && priceDistance >= 5) {
                ...
                if (rsiHigh1 && rsiHigh2 && rsiHigh2.value < rsiHigh1.value) {
                    ...
                    if (strength >= CONFIG.MONITOR.SMC.DIVERGENCE_MIN_STRENGTH) {
                        divergences.bearish.push({ type: 'bearish', timeframe: timeframe, strength: strength, emoji: '🎯#Pivô⤵️🔻' });
                    }
                }
            }
        }
...
async function analyzeDivergences(symbol) {
    try {
        ...
        const timeframes = ['15m', '30m', '1h', '2h', '4h', '12h', '1d', '3d', '1w'];
        const result = { bullish: [], bearish: [], hasBullish: false, hasBearish: false, summary: ' ❌ Sem divergências RSI' };
        for (const tf of timeframes) {
            try {
                const candles = await getCandles(symbol, tf, CONFIG.MONITOR.SMC.DIVERGENCE_LOOKBACK);
                if (!candles || candles.length < 50) continue;
        ...
        return { bullish: [], bearish: [], hasBullish: false, hasBearish: false, summary: '❌ Sem divergências RSI' };
    }
}
...
function detectStructureBreaks(candles) {
    ...
    return { bullish: hasRecentBullishBreak, bearish: hasRecentBearishBreak, description: hasRecentBullishBreak ? '🟰CHoCH ALTA recente' : (hasRecentBearishBreak ? '➖CHoCH BAIXA recente' : '❌Sem quebra recente') };
}
...
function isExtremeOversold(rsi, stoch4h) {
    return (rsi && rsi.value <= CONFIG.MONITOR.SMC.EXTREME_OVERSOLD_RSI) || (stoch4h && stoch4h.k <= CONFIG.MONITOR.SMC.EXTREME_OVERSOLD_STOCH);
}

function isExtremeOverbought(rsi, stoch4h) {
    return (rsi && rsi.value >= CONFIG.MONITOR.SMC.EXTREME_OVERBOUGHT_RSI) || (stoch4h && stoch4h.k >= CONFIG.MONITOR.SMC.EXTREME_OVERBOUGHT_STOCH);
}
...
function calculateWeightedScore(analysis) {
    ...
    if (analysis.hasStochDivergence && analysis.stochDivergenceQualified) {
        totalScore += weights.stochDivergence;
    }
    maxPossible += weights.stochDivergence;
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        const stochDivergenceAnalysis = await checkStochasticDivergenceBothTF(fullSymbol);
        ...
        let hasStochDivergence = false;
        let stochDivergenceQualified = false;
        let macdDivergenceSummary = '';
        let stochDivergenceSummary = '';
        ...
        if (isBuyTrendValid) {
            ...
            if (stochDivergenceAnalysis.bullish4h || stochDivergenceAnalysis.bullish12h) {
                hasStochDivergence = true;
                stochDivergenceQualified = (stochDivergenceAnalysis.bullish4h && stochDivergenceAnalysis.isQualified4h) || 
                                          (stochDivergenceAnalysis.bullish12h && stochDivergenceAnalysis.isQualified12h);
                const timeframes = [];
                if (stochDivergenceAnalysis.bullish4h && stochDivergenceAnalysis.isQualified4h) timeframes.push('4h');
                if (stochDivergenceAnalysis.bullish12h && stochDivergenceAnalysis.isQualified12h) timeframes.push('12h');
                stochDivergenceSummary = timeframes.length > 0 ? 
                    `🟢 Stochastic Bullish (${timeframes.join('+')}) ✅ K > D` : 
                    (stochDivergenceAnalysis.bullish4h || stochDivergenceAnalysis.bullish12h ? 
                        `🟢 Stochastic Bullish (${stochDivergenceAnalysis.bullish4h ? '4h' : ''}${stochDivergenceAnalysis.bullish4h && stochDivergenceAnalysis.bullish12h ? '+' : ''}${stochDivergenceAnalysis.bullish12h ? '12h' : ''}) ⚠️ aguardar K > D` : '');
            }
            ...
            if (!isOverbought && hasRequiredDivergence) {
                ...
                if (structureBreaks.bullish) { reasons.push(structureBreaks.description); }
                if (liquiditySweep && liquiditySweep.direction === 'BUY') { reasons.push('Liquidity Sweep'); hasLiquiditySweep = true; }
                if (bullishMitigation.mitigated) { reasons.push(`Mitigação OB (${bullishMitigation.quality || 'MÉDIA'})`); hasOBMitigation = true; }
                if (oiAnalysis && (oiAnalysis.direction === 'subindo' || oiAnalysis.direction === 'subindo forte')) { reasons.push('OI em alta'); }
                if (emaAnalysis['1h']?.trend === 'ALTA') { reasons.push('EMA1h em alta'); }
                if (emaAnalysis['4h']?.trend === 'ALTA') { reasons.push('EMA4h em alta'); }
                if (divergences.hasBullish) { reasons.push(`${divergences.bullish[0]?.emoji || '🟢'} Divergência RSI Bullish`); hasDivergence = true; }
                
                const volumeConf = await analyzeVolumeConfirmation(fullSymbol, bullishFVG?.targetPrice || price, 0);
                if (volumeConf.confirmed) { reasons.push(`Volume ${volumeConf.strength} (${volumeConf.volumeRatio}x média)`); volumeConfirmed = true; }
                ...
                if (hasCCICrossover) reasons.push(`📈 CCI Bullish Crossover (4h)`);
                if (lsrConfirmed) reasons.push(`📊 LSR favorável: ${lsr.value.toFixed(2)}`);
                if (fundingConfirmed) reasons.push(`💰 Funding negativo: ${(funding * 100).toFixed(3)}%`);
                if (hasMACDDivergence) reasons.push(`📊 MACD Bullish confirmado`);
                if (hasStochDivergence && stochDivergenceQualified) reasons.push(`📊 ${stochDivergenceSummary}`);
                
                timeframesAligned = conflictAnalysis.level === 'ALINHADO';
                if (timeframesAligned) reasons.push(`Timeframes alinhados (${conflictAnalysis.level})`);
                ...
                const weightedScore = calculateWeightedScore({
                    ...
                    fundingConfirmed,
                    hasMACDDivergence,
                    hasStochDivergence: hasStochDivergence && stochDivergenceQualified,
                    stochDivergenceQualified: stochDivergenceQualified
                });
                
                if (weightedScore >= CONFIG.MONITOR.MIN_SCORE_ACCEPT && bullishFVG && isFVGValid) {
                    tradeSignal = 'BUY';
                    ...
                    setupDescription = `${reasons.join(' + ')} (Score: ${Math.round(weightedScore)})`;
                    
                    const stopData = await calculateSmartStopV2(fullSymbol, entryPrice, tradeSignal, structureData, price);
                    ...
                    const divTimeframesList = buyDivTimeframes;
                    const divInfo = `🟢 Divergência RSI Bullish detectada em: ${divTimeframesList.join(', ')}`;
                    const macdDivInfo = hasMACDDivergence ? macdDivergenceSummary : '';
                    const oteText = `\n<i>📍 OTE Entry (${oteSuggested.level}): ${formatPrice(oteSuggested.entry)}</i>`;
                    const pdText = `\n<i>${premiumDiscount.emoji} ${premiumDiscount.zone}: ${premiumDiscount.recommendation}</i>`;
                    let breakerText = '';
                    if (breakerBlocks.length > 0) {
                        const breaker = breakerBlocks[0];
                        breakerText = `\n<i>🔗 Breaker Block: ${formatPrice(breaker.price)} (${breaker.type})</i>`;
                    }
                    let cciText = '';
                    if (cciAnalysis) {
                        const cciStatus = cciAnalysis.crossoverUp ? '⤴️' : 'CCI Neutro';
                        cciText = `\n<i> CCI 4h: ${cciAnalysis.display} | ${cciStatus}</i>`;
                    }
                    let lsrFundText = '';
                    if (lsr) {
                        const lsrStatus = lsr.value < 2.5 ? '✅' : '⚠️';
                        lsrFundText += `\n<i>📊 LSR: ${lsr.value.toFixed(2)} ${lsr.emoji} ${lsrStatus}</i>`;
                    }
                    if (funding) {
                        const fundingPercent = (funding * 100).toFixed(3);
                        const fundingStatus = funding < 0 ? '✅' : '⚠️';
                        lsrFundText += `\n<i>💰 Funding: ${fundingPercent}% ${fundingStatus}</i>`;
                    }
                    ...
                    return {
                        ...
                        setupDescription, oiAnalysis, funding: funding ? funding * 100 : null, lsr, rsi, stoch4h, stochDaily,
                        emaAnalysis, bullishFVG, bearishFVG, structureBreaks, divergences,
                        conflictAnalysis, scoreQuality,
                        score: Math.round(weightedScore).toString(),
                        supportsText: supportsText || '',
                        resistancesText: resistancesText || '',
                        higherTimeframe,
                        premiumDiscount: pdText,
                        breakerText,
                        oteText,
                        volumeConfirmed,
                        fvgValidation: bullishFVGValidation,
                        ...
                        hasMACDDivergence,
                        hasStochDivergence: hasStochDivergence && stochDivergenceQualified,
                        stochDivergenceSummary: stochDivergenceSummary,
                        stopReasons: stopData.stopReasons
                    };
                }
            }
        }
        ...
        if (isSellTrendValid) {
            ...
            if (stochDivergenceAnalysis.bearish4h || stochDivergenceAnalysis.bearish12h) {
                hasStochDivergence = true;
                stochDivergenceQualified = (stochDivergenceAnalysis.bearish4h && stochDivergenceAnalysis.isQualified4h) || 
                                          (stochDivergenceAnalysis.bearish12h && stochDivergenceAnalysis.isQualified12h);
                const timeframes = [];
                if (stochDivergenceAnalysis.bearish4h && stochDivergenceAnalysis.isQualified4h) timeframes.push('4h');
                if (stochDivergenceAnalysis.bearish12h && stochDivergenceAnalysis.isQualified12h) timeframes.push('12h');
                stochDivergenceSummary = timeframes.length > 0 ? 
                    `🔴 Stochastic Bearish (${timeframes.join('+')}) ✅ K < D` : 
                    (stochDivergenceAnalysis.bearish4h || stochDivergenceAnalysis.bearish12h ? 
                        `🔴 Stochastic Bearish (${stochDivergenceAnalysis.bearish4h ? '4h' : ''}${stochDivergenceAnalysis.bearish4h && stochDivergenceAnalysis.bearish12h ? '+' : ''}${stochDivergenceAnalysis.bearish12h ? '12h' : ''}) ⚠️ aguardar K < D` : '');
            }
            ...
            if (!isOversold && hasRequiredDivergence) {
                ...
                if (structureBreaks.bearish) { reasons.push(structureBreaks.description); }
                if (liquiditySweep && liquiditySweep.direction === 'SELL') { reasons.push('Liquidity Sweep'); hasLiquiditySweep = true; }
                if (bearishMitigation.mitigated) { reasons.push(`Mitigação OB (${bearishMitigation.quality || 'MÉDIA'})`); hasOBMitigation = true; }
                if (oiAnalysis && (oiAnalysis.direction === 'subindo' || oiAnalysis.direction === 'subindo forte')) { reasons.push('OI em alta'); }
                if (emaAnalysis['1h']?.trend === 'BAIXA') { reasons.push('EMA1h em baixa'); }
                if (emaAnalysis['4h']?.trend === 'BAIXA') { reasons.push('EMA4h em baixa'); }
                if (divergences.hasBearish) { reasons.push(`${divergences.bearish[0]?.emoji || '🔴'} Divergência RSI Bearish`); hasDivergence = true; }
                ...
                if (hasCCICrossover) reasons.push(`📉 CCI Bearish Crossover (4h)`);
                if (lsrConfirmed) reasons.push(`📊 LSR favorável: ${lsr.value.toFixed(2)}`);
                if (fundingConfirmed) reasons.push(`💰 Funding positivo: ${(funding * 100).toFixed(3)}%`);
                if (hasMACDDivergence) reasons.push(`📊 MACD Bearish confirmado`);
                if (hasStochDivergence && stochDivergenceQualified) reasons.push(`📊 ${stochDivergenceSummary}`);
                ...
                if (weightedScore >= CONFIG.MONITOR.MIN_SCORE_ACCEPT && bearishFVG && isFVGValid) {
                    tradeSignal = 'SELL';
                    ...
                    const scoreQuality = analyzeScoreQuality(weightedScore);
                    const divTimeframesList = sellDivTimeframes;
                    const divInfo = `🔴 Divergência RSI Bearish detectada em: ${divTimeframesList.join(', ')}`;
                    const macdDivInfo = hasMACDDivergence ? macdDivergenceSummary : '';
                    const oteText = `\n<i>📍 OTE Entry (${oteSuggested.level}): ${formatPrice(oteSuggested.entry)}</i>`;
                    const pdText = `\n<i>${premiumDiscount.emoji} ${premiumDiscount.zone}: ${premiumDiscount.recommendation}</i>`;
                    let breakerText = '';
                    ...
                    let cciText = '';
                    if (cciAnalysis) {
                        const cciStatus = cciAnalysis.crossoverDown ? '⤵️' : 'CCI Neutro';
                        cciText = `\n<i> CCI 4h: ${cciAnalysis.display} | ${cciStatus}</i>`;
                    }
                    let lsrFundText = '';
                    if (lsr) {
                    ...
                    return {
                        ...
                        oteText,
                        volumeConfirmed,
                        fvgValidation: bearishFVGValidation,
                        divergenceInfo: divInfo,
                        minScoreRequired: CONFIG.MONITOR.MIN_SCORE_ACCEPT,
                        cciText,
                        lsrFundText,
...
async function sendSMCAlert(analysis) {
    ...
    if (analysis.tradeSignal === 'SELL') {
        if (analysis.bearishFVG) {
            fvgText = formatPrice(analysis.bearishFVG.targetPrice);
            fvgDistance = analysis.bearishFVG.displayDistance || '';
        }
        if (analysis.structureBreaks && analysis.structureBreaks.bearish) {
            chochText = '✅';
        }
        zoneText = analysis.premiumDiscount.includes('PRÊMIO') ? 'PRÊMIO 🔴' : (analysis.premiumDiscount.includes('DESCONTO') ? 'DESCONTO 🟢' : 'JUSTO ➡️');
        oteText = analysis.oteText.match(/OTE Entry \(([^)]+)\): ([\d.]+)/);
        if (oteText) {
            oteLevel = oteText[1];
            oteText = oteText[2];
        }
        ...
        if (analysis.bullishFVG) {
            ...
            fvgDistance = analysis.bullishFVG.displayDistance || '';
        }
        if (analysis.structureBreaks && analysis.structureBreaks.bullish) {
            chochText = '✅';
        }
        zoneText = analysis.premiumDiscount.includes('DESCONTO') ? 'DESCONTO 🟢' : (analysis.premiumDiscount.includes('PRÊMIO') ? 'PRÊMIO 🔴' : 'JUSTO ➡️');
        oteText = analysis.oteText.match(/OTE Entry \(([^)]+)\): ([\d.]+)/);
        if (oteText) {
            oteLevel = oteText[1];
            oteText = oteText[2];
    ...
    if (analysis.volumeConfirmed) {
        ...
        if (volMatch) {
            volumeText = `${volMatch[1]} (${volMatch[2]}x média)`;
        } else {
            volumeText = 'Confirmado';
        }
    }
    ...
    const fundingStatus = analysis.tradeSignal === 'SELL' ? 
        (analysis.funding > 0 ? 'positivo' : 'negativo') : 
    ...
    if (analysis.rsi) {
        ...
        else if (analysis.rsi.value > 70) rsiStatus = 'sobrecomprado';
    ...
    let stopInfo = '';
    if (analysis.stopPercent && analysis.stopType) {
        stopInfo = ` (${analysis.stopType}${analysis.stopReasons ? `: ${analysis.stopReasons.join(', ')}` : ''})`;
    }
    ...
    if (analysis.hasStochDivergence && analysis.stochDivergenceSummary) {
        message += `<i>• ${analysis.stochDivergenceSummary}</i>\n`;
    }
    
    if (fvgText) message += `<i>• FVG ${analysis.tradeSignal === 'SELL' ? 'Bearish' : 'Bullish'} a ${fvgText} ${fvgDistance ? `(${fvgDistance})` : ''}</i>\n`;
    if (volumeText) message += `<i>• Volume ${volumeText.includes('EXPLOSIVO') ? '💥' : (volumeText.includes('ALTO') ? '🔥' : '✅')} ${volumeText}</i>\n`;
    if (chochText) message += `<i>• CHoCH de ${analysis.tradeSignal === 'SELL' ? 'BAIXA' : 'ALTA'} recente ${chochText}</i>\n`;
    if (cciText) message += `<i>• CCI 4h: ${cciText}</i>\n`;
    
    message += `<b> #Entrada:</b> <i>${formatPrice(analysis.entryPrice)}</i>\n`;
    message += `<b> Alvos:</b> <i>${targetsSimple}</i>\n`;
    message += `<b>🛑 Stop:</b> <i>${formatPrice(analysis.stopPrice)} (${analysis.stopPercent.toFixed(1)}%)${stopInfo}</i>\n`;
    
    if (oteText && oteLevel) {
        message += `<b>#Entrada_Fibo_OTE:</b> <i>${formatPrice(parseFloat(oteText))} (${oteLevel})</i>\n`;
    }
    message += `<b> Zona:</b> <i>${zoneText}</i>\n`;
    
    message += `<b> Indicadores:</b>\n`;
    message += `<i>• RSI ${analysis.rsi?.value?.toFixed(0) || 'N/A'} (${rsiStatus}) ${analysis.rsi?.emoji || ''}</i>\n`;
    if (stoch4hText) message += `<i>• Stoch 4h: ${stoch4hText} | Diário: ${stochDailyText}</i>\n`;
    message += `<i>• #LSR ${lsrValue}  ${lsrCheck}</i>\n`;
    message += `<i>• Funding ${fundingValue}% (${fundingStatus}) ${fundingCheck}</i>\n`;
    
    if (resistanceText !== 'N/A' || supportText !== 'N/A') {
        message += `<b>🔺 RESISTÊNCIA:</b> <i>${resistanceText}</i>\n`;
...
async function scanAndAlert() {
    ...
    try {
        ...
        console.log(`📊 DIVERGÊNCIAS: RSI | MACD | STOCHASTIC (4h/12h)`);
        let alertsSent = 0;
...
// =====================================================================
process.on('SIGINT', () => {
    console.log('🛑 Desligando Titanium...');
    process.exit(0);
});
