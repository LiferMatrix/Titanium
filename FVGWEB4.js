...
// Função para suavizar RSI (reduz ruído)
...
// Função para encontrar RSI por proximidade de preço
...
// Função para encontrar volume próximo ao preço
...
// Função para calcular pontuação de convergência
function calculateConvergenceScore(pivot1, pivot2, rsi1, rsi2, volumeRatio, type) {
    let score = 0;
    const reasons = [];
    
    // 1. Força do movimento de preço (0-40 pontos)
    ...
    // 2. Força do movimento do RSI (0-35 pontos)
    ...
    // 3. Confirmação de volume (0-15 pontos)
    if (volumeRatio) {
        if (volumeRatio >= 1.5) {
            score += 15;
    ...
    // 4. Precisão da correspondência RSI (0-10 pontos)
...
// Função principal para detectar convergências (versão melhorada)
function findConvergencesWithPivotsV2(prices, rsiValues, candles, pivotZones, currentPrice) {
    ...
    for (let i = 0; i < pivotZones.supports.length; i++) {
        for (let j = i + 1; j < pivotZones.supports.length; j++) {
            ...
            // Verifica se o segundo suporte é MAIS ALTO que o primeiro
    ...
    // Detectar convergência Bearish (resistências mais baixas)
    for (let i = 0; i < pivotZones.resistances.length; i++) {
        for (let j = i + 1; j < pivotZones.resistances.length; j++) {
            ...
            // Verifica se o segundo topo é MAIS BAIXO que o primeiro
...
// Função para detectar convergência em múltiplos timeframes
async function detectMultiTimeframeConvergence(symbol, currentPrice) {
    ...
    // Análise de alinhamento multi-timeframe
    ...
    // Encontrar a melhor convergência geral
...
// =====================================================================
// === SISTEMA DE DIVERGÊNCIAS (mantido original) ===
// =====================================================================
...
async function analyzeDivergencesWithPivots(symbol, currentPrice) {
    ...
    // Detectar convergências multi-timeframe
    ...
    // Analisar divergências por timeframe
...
async function analyzeVolumeAlert(symbol, currentPrice, divergencias, convergencias) {
    try {
        ...
        if (volume3mBuyer && volume3mBuyer.isAnomaly) {
            ...
            const temConvergenciaAlta = divergencias && (divergencias.hasBullishMultiTfConvergence || (divergencias.hasBullishConvergence && divergencias.convergenceScore >= 40));
        ...
        if (volume3mSeller && volume3mSeller.isAnomaly) {
            ...
            const temConvergenciaBaixa = divergencias && (divergencias.hasBearishMultiTfConvergence || (divergencias.hasBearishConvergence && divergencias.convergenceScore >= 40));
...
async function getAllSymbols() {
    try {
        ...
        // 🔥 NOVO: ORDENAR POR VOLUME (MAIOR PARA MENOR)
        const sortedByVolume = filtered.sort((a, b) => 
            parseFloat(b.quoteVolume) - parseFloat(a.quoteVolume)
        );
        
        // Pegar os TOP 280 por volume
...
async function sendFVGAlert(asset, type) {
    ...
    // Adicionar info de multi-timeframe
    let multiTfInfo = '';
...
async function startMonitor() {
    console.log('\n' + '='.repeat(70));
    console.log('🚀 TITANIUM PRIME X - v2.0 (Convergências Melhoradas)');
