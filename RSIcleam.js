...
const logger = winston.createLogger({
  level: 'info', // Mude para 'error' em produção para reduzir logs, se desejar
  ...
  transports: [
    new DailyRotateFile({
      ...
      level: 'error',
      maxSize: config.LOG_MAX_SIZE, // 100 MB por arquivo
      maxFiles: config.LOG_MAX_FILES, // Manter 7 dias
...
async function cleanupOldLogs() {
  try {
    ...
    const now = Date.now();
    const maxAgeMs = config.LOG_MAX_FILES * 24 * 60 * 60 * 1000; // 7 dias em milissegundos
...
async function main() {
  ...
  try {
    ...
    setInterval(cleanupOldLogs, 24 * 60 * 60 * 1000); // Limpar logs diariamente
    setInterval(cleanupCache, config.CACHE_TTL); // Limpar cache a cada 2 minutos
  } catch (e) {
