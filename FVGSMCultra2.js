...
async function analyzeDivergences(symbol) {
    try {
        ...
        const result = { bullish: [], bearish: [], hasBullish: false, hasBearish: false, summary: ' ❌ Sem divergências' };
        for (const tf of timeframes) {
            try {
...
// =====================================================================
// === DETECÇÃO DE CONVERGÊNCIAS DE RSI COM ALVOS PRECISOS ===
// =====================================================================

// Função melhorada para encontrar pivôs com validação de força
function findPivotsAdvanced(data, lookback = 5, minStrength = 0.5) {
    const pivots = { highs: [], lows: [] };
    ...
    for (let i = lookback; i < data.length - lookback; i++) {
        ...
        for (let j = 1; j <= lookback; j++) {
            ...
            if (data[i] > data[i - j]) {
                leftStrength += (data[i] - data[i - j]) / data[i] * 100;
            }
            if (data[i] > data[i + j]) {
                rightStrength += (data[i] - data[i + j]) / data[i] * 100;
            }
        }
        ...
        const totalStrength = (leftStrength + rightStrength) / 2;
        
        if (isHigh && totalStrength >= minStrength) {
            pivots.highs.push({ index: i, value: data[i], strength: totalStrength });
        }
        if (isLow && totalStrength >= minStrength) {
            pivots.lows.push({ index: i, value: data[i], strength: totalStrength });
        }
    }
...
// Função para encontrar resistência mais próxima
function findNearestResistance(prices, currentPrice, fromIndex) {
    const lookAhead = Math.min(50, prices.length - fromIndex);
    let nearestResistance = null;
    let minDistance = Infinity;
    
    for (let i = fromIndex + 5; i < Math.min(fromIndex + lookAhead, prices.length); i++) {
        let isLocalHigh = true;
        for (let j = 1; j <= 3; j++) {
            if (i - j >= 0 && prices[i] <= prices[i - j]) isLocalHigh = false;
            if (i + j < prices.length && prices[i] <= prices[i + j]) isLocalHigh = false;
        }
        if (isLocalHigh && prices[i] > currentPrice) {
            const distance = ((prices[i] - currentPrice) / currentPrice) * 100;
            if (distance < minDistance) {
                minDistance = distance;
                nearestResistance = prices[i];
            }
        }
    }
    return nearestResistance || currentPrice * 1.05;
}

// Função para encontrar suporte mais próximo
function findNearestSupport(prices, currentPrice, fromIndex) {
    const lookAhead = Math.min(50, prices.length - fromIndex);
    ...
    for (let i = fromIndex + 5; i < Math.min(fromIndex + lookAhead, prices.length); i++) {
        let isLocalLow = true;
        for (let j = 1; j <= 3; j++) {
            if (i - j >= 0 && prices[i] >= prices[i - j]) isLocalLow = false;
            if (i + j < prices.length && prices[i] >= prices[i + j]) isLocalLow = false;
        }
        if (isLocalLow && prices[i] < currentPrice) {
            const distance = ((currentPrice - prices[i]) / currentPrice) * 100;
            if (distance < minDistance) {
                minDistance = distance;
                nearestSupport = prices[i];
            }
        }
    }
    return nearestSupport || currentPrice * 0.95;
}

// Função melhorada para detectar convergências
function detectAdvancedConvergences(prices, rsiValues, timeframe) {
    if (prices.length < 50 || rsiValues.length < 50) return { bullish: [], bearish: [] };
    
    const pricePivots = findPivotsAdvanced(prices, 5, 0.8);
    const rsiPivots = findPivotsAdvanced(rsiValues, 5, 1.0);
    
    const convergences = { bullish: [], bearish: [] };
    
    // CONVERGÊNCIA BULLISH - Preço faz FUNDOS mais ALTOS, RSI faz FUNDOS mais BAIXOS
    for (let i = 0; i < pricePivots.lows.length; i++) {
        for (let j = i + 1; j < pricePivots.lows.length; j++) {
            const priceLow1 = pricePivots.lows[i];
            const priceLow2 = pricePivots.lows[j];
            ...
            if (priceHigher && priceDistance >= 8 && priceDistance <= 50) {
                const rsiLow1 = rsiPivots.lows.find(r => Math.abs(r.index - priceLow1.index) <= 3);
                const rsiLow2 = rsiPivots.lows.find(r => Math.abs(r.index - priceLow2.index) <= 3);
                
                if (rsiLow1 && rsiLow2) {
                    const rsiLower = rsiLow2.value < rsiLow1.value;
                    const rsiDivergenceStrength = Math.abs(rsiLow2.value - rsiLow1.value);
                    
                    if (rsiLower && rsiDivergenceStrength >= 3) {
                        const priceChangePercent = ((priceLow2.value - priceLow1.value) / priceLow1.value) * 100;
                        const rsiChangePoints = Math.abs(rsiLow2.value - rsiLow1.value);
                        const convergenceStrength = (priceChangePercent + rsiChangePoints) / 2;
                        
                        // CÁLCULO DOS ALVOS PRECISOS
                        const amplitude = priceLow2.value - priceLow1.value;
                        const lastLow = priceLow2.value;
                        
                        // Encontra resistência para ajuste dos alvos
                        let nearestResistance = findNearestResistance(prices, lastLow, priceLow2.index);
                        
                        // Alvos baseados em Fibonacci
                        const target1_raw = lastLow + amplitude * 0.618;
                        const target2_raw = lastLow + amplitude * 1.0;
                        const target3_raw = lastLow + amplitude * 1.618;
                        
                        // Ajusta alvos baseado em resistências reais
                        let target1 = target1_raw;
                        let target2 = target2_raw;
                        let target3 = target3_raw;
                        
                        if (nearestResistance) {
                            if (target1 > nearestResistance) target1 = nearestResistance * 0.998;
                            if (target2 > nearestResistance) target2 = nearestResistance * 0.995;
                            if (target3 > nearestResistance) target3 = nearestResistance * 0.99;
                        }
                        
                        const stopLoss = lastLow * 0.99;
                        const riskReward1 = (target1 - lastLow) / (lastLow - stopLoss);
                        const riskReward2 = (target2 - lastLow) / (lastLow - stopLoss);
                        const riskReward3 = (target3 - lastLow) / (lastLow - stopLoss);
                        
                        let quality = 'MÉDIA';
                        let qualityScore = 0;
                        
                        if (riskReward1 >= 2 && convergenceStrength >= 3) {
                            quality = 'EXCELENTE';
                            qualityScore = 3;
                        } else if (riskReward1 >= 1.5 && convergenceStrength >= 2) {
                            quality = 'BOA';
                            qualityScore = 2;
                        } else if (riskReward1 >= 1) {
                            quality = 'ACEITÁVEL';
                            qualityScore = 1;
                        }
                        ...
                        convergences.bullish.push({
                            ...
                            timeframe: timeframe,
                            strength: convergenceStrength.toFixed(1),
                            quality: quality,
                            qualityScore: qualityScore,
                            pivot1Price: priceLow1.value,
                            pivot1RSI: rsiLow1.value,
                            pivot2Price: priceLow2.value,
                            pivot2RSI: rsiLow2.value,
                            amplitude: amplitude,
                            lastLow: lastLow,
                            target1: target1,
                            target1Percent: ((target1 - lastLow) / lastLow * 100).toFixed(2),
                            target1RR: riskReward1.toFixed(1),
                            target2: target2,
                            target2Percent: ((target2 - lastLow) / lastLow * 100).toFixed(2),
                            target2RR: riskReward2.toFixed(1),
                            target3: target3,
                            target3Percent: ((target3 - lastLow) / lastLow * 100).toFixed(2),
                            target3RR: riskReward3.toFixed(1),
                            stopLoss: stopLoss,
                            stopPercent: ((lastLow - stopLoss) / lastLow * 100).toFixed(2),
                            nearestResistance: nearestResistance,
                            emoji: qualityScore >= 2 ? '🟢🎯' : '🟡'
                        });
                    }
                }
            }
    ...
    // CONVERGÊNCIA BEARISH - Preço faz TOPOS mais BAIXOS, RSI faz TOPOS mais ALTOS
    for (let i = 0; i < pricePivots.highs.length; i++) {
        for (let j = i + 1; j < pricePivots.highs.length; j++) {
            const priceHigh1 = pricePivots.highs[i];
            const priceHigh2 = pricePivots.highs[j];
            ...
            if (priceLower && priceDistance >= 8 && priceDistance <= 50) {
                const rsiHigh1 = rsiPivots.highs.find(r => Math.abs(r.index - priceHigh1.index) <= 3);
                const rsiHigh2 = rsiPivots.highs.find(r => Math.abs(r.index - priceHigh2.index) <= 3);
                
                if (rsiHigh1 && rsiHigh2) {
                    const rsiHigher = rsiHigh2.value > rsiHigh1.value;
                    const rsiDivergenceStrength = Math.abs(rsiHigh2.value - rsiHigh1.value);
                    
                    if (rsiHigher && rsiDivergenceStrength >= 3) {
                        const priceChangePercent = ((priceHigh1.value - priceHigh2.value) / priceHigh1.value) * 100;
                        const rsiChangePoints = Math.abs(rsiHigh2.value - rsiHigh1.value);
                        const convergenceStrength = (priceChangePercent + rsiChangePoints) / 2;
                        
                        // CÁLCULO DOS ALVOS PRECISOS PARA BAIXA
                        const amplitude = priceHigh1.value - priceHigh2.value;
                        const lastHigh = priceHigh2.value;
                        
                        let nearestSupport = findNearestSupport(prices, lastHigh, priceHigh2.index);
                        
                        const target1_raw = lastHigh - amplitude * 0.618;
                        const target2_raw = lastHigh - amplitude * 1.0;
                        const target3_raw = lastHigh - amplitude * 1.618;
                        ...
                        if (nearestSupport) {
                            if (target1 < nearestSupport) target1 = nearestSupport * 1.002;
                            if (target2 < nearestSupport) target2 = nearestSupport * 1.005;
                            if (target3 < nearestSupport) target3 = nearestSupport * 1.01;
                        }
                        
                        const stopLoss = lastHigh * 1.01;
                        const riskReward1 = (lastHigh - target1) / (stopLoss - lastHigh);
                        const riskReward2 = (lastHigh - target2) / (stopLoss - lastHigh);
                        const riskReward3 = (lastHigh - target3) / (stopLoss - lastHigh);
                        ...
                        convergences.bearish.push({
                            ...
                            quality: quality,
                            qualityScore: qualityScore,
                            pivot1Price: priceHigh1.value,
                            pivot1RSI: rsiHigh1.value,
                            pivot2Price: priceHigh2.value,
                            pivot2RSI: rsiHigh2.value,
                            amplitude: amplitude,
                            lastHigh: lastHigh,
                            target1: target1,
                            target1Percent: ((lastHigh - target1) / lastHigh * 100).toFixed(2),
                            target1RR: riskReward1.toFixed(1),
                            target2: target2,
                            target2Percent: ((lastHigh - target2) / lastHigh * 100).toFixed(2),
                            target2RR: riskReward2.toFixed(1),
                            target3: target3,
                            target3Percent: ((lastHigh - target3) / lastHigh * 100).toFixed(2),
                            target3RR: riskReward3.toFixed(1),
                            stopLoss: stopLoss,
                            stopPercent: ((stopLoss - lastHigh) / lastHigh * 100).toFixed(2),
    ...
    convergences.bullish.sort((a, b) => b.qualityScore - a.qualityScore);
    convergences.bearish.sort((a, b) => b.qualityScore - a.qualityScore);
    
    return convergences;
}

// Função principal de análise de convergências (ATUALIZADA)
async function analyzeConvergences(symbol) {
    try {
        const cacheKey = `${symbol}_convergences_v2`;
        const now = Date.now();
        ...
        for (const tf of timeframes) {
            ...
            try {
                ...
                if (rsiValues.length < 50) continue;
                
                const convergences = detectAdvancedConvergences(prices, rsiValues, tf);
                
                for (const conv of convergences.bullish) {
                    if (conv.qualityScore >= 1) {
                        result.bullish.push(conv);
                        result.hasBullish = true;
                    }
                }
                
                for (const conv of convergences.bearish) {
                    if (conv.qualityScore >= 1) {
                        result.bearish.push(conv);
                        result.hasBearish = true;
                    }
                }
            } catch (error) {}
        ...
        if (result.bullish.length > 0) {
            result.bullish.sort((a, b) => b.qualityScore - a.qualityScore);
            result.bestBullish = result.bullish[0];
        }
        
        if (result.bearish.length > 0) {
            result.bearish.sort((a, b) => b.qualityScore - a.qualityScore);
            result.bestBearish = result.bearish[0];
        }
        
        if (result.hasBullish && result.hasBearish) {
            result.summary = `🔄 Convergências MISTAS (${result.bullish.length} Bullish | ${result.bearish.length} Bearish)`;
        } else if (result.hasBullish && result.bestBullish) {
            const best = result.bestBullish;
            result.summary = `🟢 CONVERGÊNCIA BULLISH (${best.timeframe}) - Força: ${best.strength} | Qualidade: ${best.quality} | RR: ${best.target1RR}:1`;
        } else if (result.hasBearish && result.bestBearish) {
            const best = result.bestBearish;
            result.summary = `🔴 CONVERGÊNCIA BEARISH (${best.timeframe}) - Força: ${best.strength} | Qualidade: ${best.quality} | RR: ${best.target1RR}:1`;
        }
        ...
        console.log(`⚠️ Erro nas convergências para ${symbol}: ${error.message}`);
        return { 
            bullish: [], bearish: [], hasBullish: false, hasBearish: false, 
            bestBullish: null, bestBearish: null, summary: '❌ Erro na análise de convergências' 
        };
    }
}

// =====================================================================
// === FUNÇÃO PARA FORMATAR CONVERGÊNCIAS NO ALERTA ===
// =====================================================================
function formatConvergenceAlert(convergences, tradeSignal) {
    if (!convergences) return '';
    
    let result = '';
    
    if (tradeSignal === 'BUY' && convergences.hasBullish && convergences.bestBullish) {
        const c = convergences.bestBullish;
        result += `<i>#Convergência_de_Alta (${c.timeframe})</i>`;
        result += `\n<i> Força: ${c.strength} | Qualidade: ${c.quality}</i>`;
        result += `\n<i> Fundo: ${formatPrice(c.pivot2Price)} | RSI: ${c.pivot2RSI.toFixed(0)}</i>`;
        result += `\n<i> Alvo 1 (61.8%): ${formatPrice(c.target1)} (+${c.target1Percent}%) </i>`;
        result += `\n<i> Alvo 2 (100%): ${formatPrice(c.target2)} (+${c.target2Percent}%) </i>`;
        result += `\n<i> Alvo 3 (161.8%): ${formatPrice(c.target3)} (+${c.target3Percent}%) </i>`;
        result += `\n<i>🛑Stop: ${formatPrice(c.stopLoss)} (-${c.stopPercent}%)</i>`;
        if (c.nearestResistance) {
            result += `\n<i>   🔺Resistência prox: ${formatPrice(c.nearestResistance)}</i>`;
        }
    }
    
    if (tradeSignal === 'SELL' && convergences.hasBearish && convergences.bestBearish) {
        const c = convergences.bestBearish;
        result += `<i> #Convergência_de_Baixa (${c.timeframe})</i>`;
        result += `\n<i> Força: ${c.strength} | Qualidade: ${c.quality}</i>`;
        result += `\n<i> Topo: ${formatPrice(c.pivot2Price)} | RSI: ${c.pivot2RSI.toFixed(0)}</i>`;
        result += `\n<i> Alvo 1 (61.8%): ${formatPrice(c.target1)} (${c.target1Percent}%)</i>`;
        result += `\n<i> Alvo 2 (100%): ${formatPrice(c.target2)} (${c.target2Percent}%)</i>`;
        result += `\n<i> Alvo 3 (161.8%): ${formatPrice(c.target3)} (${c.target3Percent}%)</i>`;
        result += `\n<i>🛑Stop: ${formatPrice(c.stopLoss)} (+${c.stopPercent}%)</i>`;
        if (c.nearestSupport) {
            result += `\n<i>   🔻Suporte prox: ${formatPrice(c.nearestSupport)}</i>`;
        }
    }
...
function analyzeTimeframeConflict(emaAnalysis) {
    ...
    if (sum === 3) return { level: 'ALINHADO', message: '✔︎ TODOS TIMEFRAMES ALINHADOS', score: 100 };
...
function analyzeScoreQuality(score) {
    if (score >= 80) return { emoji: '🏆', message: 'Setup #TOP ', recommendation: '' };
    if (score >= 65) return { emoji: '🔥', message: 'Setup #MUITO #BOM ', recommendation: '' };
    if (score >= 50) return { emoji: '✅', message: 'Setup VÁLIDO ', recommendation: 'Cautela...' };
...
function findOrderBlocks(candles, lookback = 50) {
    const bullishOB = [], bearishOB = [];
    for (let i = 3; i < Math.min(candles.length, lookback); i++) {
        const candle = candles[i], candle1 = candles[i-1], candle2 = candles[i-2], candle3 = candles[i-3];
        if (candle2.close < candle3.close && candle1.close < candle2.close && candle.close > candle1.high && candle.volume > candle1.volume * 1.2) {
            bullishOB.push({ price: candle2.low });
        }
        if (candle2.close > candle3.close && candle1.close > candle2.close && candle.close < candle1.low && candle.volume > candle1.volume * 1.2) {
    ...
    try {
        ...
        if (dailyEMA.trend === 'BAIXA' && !dailyEMA.isAboveEMA55 && !dailyEMA.isAboveEMA233) {
            ...
            if (!isOversold) {
                ...
                if (convergences.hasBearish) { score += 20; reasons.push(`${convergences.bestBearish?.emoji || '🔴'} Convergência Bearish`); }
        ...
        if (dailyEMA.trend === 'ALTA' && dailyEMA.isAboveEMA55 && dailyEMA.isAboveEMA233) {
            ...
            if (!isOverbought) {
                ...
                if (convergences.hasBullish) { score += 20; reasons.push(`${convergences.bestBullish?.emoji || '🟢'} Convergência Bullish`); }
...
async function sendSMCAlert(analysis) {
    ...
    // ADICIONA CONVERGÊNCIAS COM ALVOS PRECISOS
    if (analysis.convergences && (analysis.convergences.hasBullish || analysis.convergences.hasBearish)) {
        message += formatConvergenceAlert(analysis.convergences, analysis.tradeSignal);
    }
    
    message += `<i>🔺#Resistência: ${analysis.resistancesText || 'N/A'}</i>\n`;
    message += `<i>🔻#Suporte: ${analysis.supportsText || 'N/A'}</i>\n`;
