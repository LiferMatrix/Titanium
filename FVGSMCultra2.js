            ...
            if (priceLower && priceDistance >= 8 && priceDistance <= 50) {
                ...
                if (rsiHigh1 && rsiHigh2) {
                    ...
                    if (rsiHigher && rsiDivergenceStrength >= 3) {
                        ...
                        // CORREÇÃO: ORDEM DOS ALVOS PARA BEARISH
                        // Alvo 1 = 161.8% (mais distante)
                        // Alvo 2 = 100% (médio)
                        // Alvo 3 = 61.8% (mais próximo)
                        convergences.bearish.push({
                            ...
                            lastHigh: lastHigh,
                            target1: target3,
                            target1Percent: ((lastHigh - target3) / lastHigh * 100).toFixed(2),
                            target1RR: riskReward3.toFixed(1),
                            target2: target2,
                            target2Percent: ((lastHigh - target2) / lastHigh * 100).toFixed(2),
                            target2RR: riskReward2.toFixed(1),
                            target3: target1,
                            target3Percent: ((lastHigh - target1) / lastHigh * 100).toFixed(2),
                            target3RR: riskReward1.toFixed(1),
                            stopLoss: stopLoss,
...
function formatConvergenceAlert(convergences, tradeSignal) {
    ...
    if (tradeSignal === 'BUY' && convergences.hasBullish && convergences.bestBullish) {
        ...
        if (c.nearestResistance) {
            //result += `\n<i>   🔺Resistência da Convergência: ${formatPrice(c.nearestResistance)}</i>`;
        }
    }
    ...
    if (tradeSignal === 'SELL' && convergences.hasBearish && convergences.bestBearish) {
        ...
        if (c.nearestSupport) {
            //result += `\n<i>   🔻Suporte da Convergência: ${formatPrice(c.nearestSupport)}</i>`;
        }
    }
...
async function sendSMCAlert(analysis) {
    ...
    message += `<i>\n#Resistência:${analysis.resistancesText || 'N/A'}</i>\n`;
