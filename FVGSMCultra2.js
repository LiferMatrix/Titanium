...
function findClusterLevels(candles, currentPrice, tolerancePercent = 0.5, maxClusters = 2) {
    ...
    for (const candle of candles) {
        ...
        // Agrupa suportes (abaixo do preço atual)
        if (low < currentPrice) {
        ...
        // Agrupa resistências (acima do preço atual)
    ...
    // Ordena por número de toques (maior primeiro)
...
// =====================================================================
// === FUNÇÃO PARA CLASSIFICAR FORÇA DO CLUSTER ===
// =====================================================================
...
async function analyzeSMC(symbolData) {
    ...
    try {
        ...
        // =============================================================
        // ANÁLISE DE CLUSTERS (Suportes e Resistências)
        // =============================================================
        const clusters = findClusterLevels(fifteenCandles, price, 0.5, 2);
        
        // Formata a string de suportes e resistências para o alerta
        let supportsText = '';
...
async function sendSMCAlert(analysis) {
    ...
    message += `<i>🔺#Resistência: ${analysis.resistancesText}</i>\n`;
    message += `<i>🔻#Suporte: ${analysis.supportsText}</i>\n`;
    
    // =============================================================
    // === NOVO: MOSTRA ALVOS DAS CONVERGÊNCIAS ===
    // =============================================================
    
    // Mostra alvo da convergência BEARISH (para trades de venda)
    if (analysis.convergences && analysis.convergences.bearish && analysis.convergences.bearish.length > 0 && analysis.tradeSignal === 'SELL') {
        const bearConv = analysis.convergences.bearish[0];
        if (bearConv.targetPrice) {
            const timeframeStr = bearConv.timeframe;
            const distanceToTarget = ((analysis.price - bearConv.targetPrice) / analysis.price) * 100;
            message += `<i>🎯 Alvo Convergência Bearish: ${formatPrice(bearConv.targetPrice)} (${timeframeStr}) - ${distanceToTarget.toFixed(1)}% abaixo</i>\n`;
        }
    }
    
    // Mostra alvo da convergência BULLISH (para trades de compra)
    if (analysis.convergences && analysis.convergences.bullish && analysis.convergences.bullish.length > 0 && analysis.tradeSignal === 'BUY') {
        const bullConv = analysis.convergences.bullish[0];
        if (bullConv.targetPrice) {
            const timeframeStr = bullConv.timeframe;
            const distanceToTarget = ((bullConv.targetPrice - analysis.price) / analysis.price) * 100;
            message += `<i>🎯 Alvo Convergência Bullish: ${formatPrice(bullConv.targetPrice)} (${timeframeStr}) - ${distanceToTarget.toFixed(1)}% acima</i>\n`;
        }
    }
    
    // Mostra todos os alvos de convergência (opcional - descomente se quiser mostrar múltiplos)
    /*
    if (analysis.convergences && analysis.convergences.bearish && analysis.convergences.bearish.length > 1) {
        message += `<i>📍 Outros alvos Bearish: ${analysis.convergences.bearish.slice(1).map(c => `${formatPrice(c.targetPrice)} (${c.timeframe})`).join(', ')}</i>\n`;
    }
    if (analysis.convergences && analysis.convergences.bullish && analysis.convergences.bullish.length > 1) {
        message += `<i>📍 Outros alvos Bullish: ${analysis.convergences.bullish.slice(1).map(c => `${formatPrice(c.targetPrice)} (${c.timeframe})`).join(', ')}</i>\n`;
    }
    */
    
    // =============================================================
    // === CONTINUA COM O RESTO DO ALERTA ===
    // =============================================================
...
// =====================================================================
// === MONITOR COM RECONEXÃO AUTOMÁTICA ===
// =====================================================================
async function startMonitorWithReconnect() {
    ...
    console.log('\n' + '='.repeat(70));
    console.log(' Titanium MONITOR com Reconexão Automática');
