...
const QUALITY_THRESHOLD = 65;       // ↑ de 70 (filtro mais rigoroso)
const QUALITY_WEIGHTS = {
    volume: 37,                    // ↑ de 42 (volume ainda mais crítico)
    oi: 1,
    volatility: 5,                 // ↑ de 7
    lsr: 8,                        // ↑ de 8
    rsi: 15,                       // ↑ de 18
...
// === NOVAS CONFIGURAÇÕES EMA 233/610 ===
const EMA_SETTINGS = {
    ema233Period: 233,
    ema610Period: 610,
    timeframe: '15m',
    requiredCandles: 700  // Para calcular EMA 610 precisamos de mais candles
};
...
// =====================================================================
// 🧠 SISTEMA DE APRENDIZADO REMOVIDO CONFORME SOLICITADO
// =====================================================================
...
async function sendSignalAlertWithRisk(signal) {
    try {
        ...
        // 🔹 MODIFICADO: Análise baseada apenas nas EMAs 233/610
        let analysisType = '';
        let analysisEmoji = signal.isBullish ? '💹🔍' : '🔍🚨';
        
        if (signal.emaType === '233') {
            analysisType = `IA analisando ${signal.isBullish ? '⤴️' : '⤵️'} EMA 233`;
        } else if (signal.emaType === '610') {
            analysisType = `IA analisando ${signal.isBullish ? '⤴️' : '⤵️'} EMA 610`;
        } else {
            analysisType = `IA analisando EMA`;
        }

        let alertTitle = '';
        
        if (signal.emaType === '233') {
            alertTitle = `${analysisEmoji} <i>${analysisType}: ${signal.symbol}</i>`;
        } else if (signal.emaType === '610') {
            alertTitle = `${analysisEmoji} <i>${analysisType}: ${signal.symbol}</i>`;
        } else {
            alertTitle = `${analysisEmoji} <i>IA analisando: ${signal.symbol}</i>`;
        }
...
<i>✨Titanium by @J4Rviz✨</i>
        ...
        console.log(`\n📤 Análise IA enviada: ${signal.symbol}`);
...
<i>✨🤖IA Titanium by @J4Rviz</i>
        ...
        console.log(`📤 Análise IA enviada: ${signal.symbol}`);
        console.log(`   Data/Hora: ${now.full} TradingView`);
        console.log(`   Volume: ${volumeRatio.toFixed(2)}x (Score: ${volumeScore.toFixed(2)} - ${volumeClassification})`);
        console.log(`   Volume Confirmado: ${isVolumeConfirmed ? '✅ SIM' : '❌ NÃO'}`);
        console.log(`   Tipo de Análise: ${analysisType}`);
...
• EMAs 233/610 (15m) como condição primária
✨ by @J4Rviz
        `;
...
// 🔹 NOVA FUNÇÃO PARA OBTER EMAs 233 e 610
async function getEMAs15m(symbol) {
    try {
        const candles = await getCandlesCached(symbol, EMA_SETTINGS.timeframe, EMA_SETTINGS.requiredCandles);
        if (candles.length < EMA_SETTINGS.ema610Period) {
            return null;
        }
        ...
        // Calcular EMA 233
        const ema233 = EMA.calculate({ period: EMA_SETTINGS.ema233Period, values: closes });
        const latestEma233 = ema233[ema233.length - 1];
        const previousEma233 = ema233[ema233.length - 2];

        // Calcular EMA 610
        const ema610 = EMA.calculate({ period: EMA_SETTINGS.ema610Period, values: closes });
        const latestEma610 = ema610[ema610.length - 1];
        const previousEma610 = ema610[ema610.length - 2];

        // Determinar se o preço fechou acima/abaixo das EMAs
        const closedAboveEMA233 = currentPrice > latestEma233;
        const closedAboveEMA610 = currentPrice > latestEma610;
        const closedBelowEMA233 = currentPrice < latestEma233;
        const closedBelowEMA610 = currentPrice < latestEma610;

        // Verificar cruzamento do preço com as EMAs
        const crossedAboveEMA233 = previousPrice <= previousEma233 && currentPrice > latestEma233;
        const crossedBelowEMA233 = previousPrice >= previousEma233 && currentPrice < latestEma233;
        const crossedAboveEMA610 = previousPrice <= previousEma610 && currentPrice > latestEma610;
        const crossedBelowEMA610 = previousPrice >= previousEma610 && currentPrice < latestEma610;
        ...
        return {
            ...
            ema233: latestEma233,
            ema610: latestEma610,
            closedAboveEMA233: closedAboveEMA233,
            closedAboveEMA610: closedAboveEMA610,
            closedBelowEMA233: closedBelowEMA233,
            closedBelowEMA610: closedBelowEMA610,
            crossedAboveEMA233: crossedAboveEMA233,
            crossedBelowEMA233: crossedBelowEMA233,
            crossedAboveEMA610: crossedAboveEMA610,
            crossedBelowEMA610: crossedBelowEMA610,
            priceDistanceFromEMA233: ((currentPrice - latestEma233) / latestEma233) * 100,
            priceDistanceFromEMA610: ((currentPrice - latestEma610) / latestEma610) * 100
        };
    } catch (error) {
        console.log(`⚠️ Erro cálculo EMAs 233/610 ${symbol}: ${error.message}`);
        return null;
    }
}
...
async function calculateSignalQuality(symbol, isBullish, marketData) {
    ...
    // 🔹 MODIFICADO: EMA Alignment agora baseado nas EMAs 233/610
    if (marketData.ema15m) {
        let emaScore = 0;
        let emaDetail = '';
        
        if (marketData.emaType === '233') {
            if (isBullish && marketData.ema15m.closedAboveEMA233) {
                emaScore = QUALITY_WEIGHTS.emaAlignment;
                emaDetail = `${emaScore}/${QUALITY_WEIGHTS.emaAlignment} (Preço acima EMA 233)`;
            } else if (!isBullish && marketData.ema15m.closedBelowEMA233) {
                emaScore = QUALITY_WEIGHTS.emaAlignment;
                emaDetail = `${emaScore}/${QUALITY_WEIGHTS.emaAlignment} (Preço abaixo EMA 233)`;
            } else {
                failedChecks.push(`EMA 233: Preço não ${isBullish ? 'acima' : 'abaixo'} da EMA`);
            }
        } else if (marketData.emaType === '610') {
            if (isBullish && marketData.ema15m.closedAboveEMA610) {
                emaScore = QUALITY_WEIGHTS.emaAlignment;
                emaDetail = `${emaScore}/${QUALITY_WEIGHTS.emaAlignment} (Preço acima EMA 610)`;
            } else if (!isBullish && marketData.ema15m.closedBelowEMA610) {
                emaScore = QUALITY_WEIGHTS.emaAlignment;
                emaDetail = `${emaScore}/${QUALITY_WEIGHTS.emaAlignment} (Preço abaixo EMA 610)`;
            } else {
                failedChecks.push(`EMA 610: Preço não ${isBullish ? 'acima' : 'abaixo'} da EMA`);
            }
        }
        
        score += emaScore;
        if (emaDetail) details.push(` EMA ${marketData.emaType}: ${emaDetail}`);
    }
...
async function monitorSymbol(symbol) {
    try {
        // 🔹 MODIFICADO: Usar EMAs 233/610 em 15m como condição primária
        const emaData = await getEMAs15m(symbol);
        if (!emaData) return null;

        const rsiData = await getRSI1h(symbol);
        if (!rsiData) return null;

        // Verificar condições para EMA 233
        let isBullish233 = false;
        let isBearish233 = false;
        let emaType233 = null;

        if (emaData.closedAboveEMA233) {
            isBullish233 = true;
            emaType233 = '233';
        } else if (emaData.closedBelowEMA233) {
            isBearish233 = true;
            emaType233 = '233';
        }

        // Verificar condições para EMA 610
        let isBullish610 = false;
        let isBearish610 = false;
        let emaType610 = null;

        if (emaData.closedAboveEMA610) {
            isBullish610 = true;
            emaType610 = '610';
        } else if (emaData.closedBelowEMA610) {
            isBearish610 = true;
            emaType610 = '610';
        }

        // Gerar sinais separados para cada EMA
        const signals = [];

        // Sinal para EMA 233
        if (isBullish233 || isBearish233) {
            const isBullish = isBullish233;
            
            if (isBullish && rsiData.value > RSI_BUY_MAX) {
                console.log(`❌ ${symbol}: RSI alto para análise EMA 233 (${rsiData.value.toFixed(1)} > ${RSI_BUY_MAX})`);
            } else if (!isBullish && rsiData.value < RSI_SELL_MIN) {
                console.log(`❌ ${symbol}: RSI baixo para análise EMA 233 (${rsiData.value.toFixed(1)} < ${RSI_SELL_MIN})`);
            } else {
                signals.push({
                    isBullish: isBullish,
                    emaType: '233',
        ...
        // Sinal para EMA 610
        if (isBullish610 || isBearish610) {
            const isBullish = isBullish610;
            
            if (isBullish && rsiData.value > RSI_BUY_MAX) {
                console.log(`❌ ${symbol}: RSI alto para análise EMA 610 (${rsiData.value.toFixed(1)} > ${RSI_BUY_MAX})`);
            } else if (!isBullish && rsiData.value < RSI_SELL_MIN) {
                console.log(`❌ ${symbol}: RSI baixo para análise EMA 610 (${rsiData.value.toFixed(1)} < ${RSI_SELL_MIN})`);
            } else {
                signals.push({
                    isBullish: isBullish,
                    emaType: '610',
        ...
        // Processar cada sinal separadamente
        const processedSignals = [];
        
        for (const signal of signals) {
            const isBullish = signal.isBullish;
            const emaType = signal.emaType;
            ...
            if (!lsrData.isValid) continue;
            
            if (!stoch4hData.isValid) {
                console.log(`❌ ${symbol}: Stochastic 4h não confirmado para EMA ${emaType} ${isBullish ? 'bullish' : 'bearish'}`);
                continue;
            }
            ...
            if (!qualityScore.isAcceptable) continue;
        ...
        return processedSignals.length > 0 ? processedSignals : null;
...
async function processSymbolGroup(symbols) {
    ...
    for (const symbol of symbols) {
        try {
            ...
            const signals = await monitorSymbol(symbol);
            if (signals) {
...
async function mainBotLoop() {
    ...
    console.log(` Sistema de análise IA com EMAs 233/610 (15m)`);
    ...
    while (true) {
        try {
            ...
            console.log(`✅ ${((endTime - startTime) / 1000).toFixed(1)}s | Análises: ${signals.length} (Total: ${totalSignals})`);
            ...
            console.log(`📊 Progresso: ${status.consecutiveNoSignals} grupos sem análises | Total: ${totalAnalysis}`);
...
async function startBot() {
    try {
        ...
        console.log('\n' + '='.repeat(80));
        console.log(' TITANIUM - ATIVADO COM ANÁLISE DE EMAs 233/610');
        ...
        console.log(` Bot configurado e pronto para análise`);
        ...
        console.log('✅ Tudo pronto! Iniciando análise...');
