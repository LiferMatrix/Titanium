...
function calculateRSI(data) {
  if (!data || data.length < config.RSI_PERIOD + 1) return null;
...
function calculateATR(data) {
  if (!data || data.length < config.ATR_PERIOD + 1) return null;
  ...
  try {
    ...
    const coinsData = await limitConcurrency(usdtPairs, async (symbol) => {
      try {
        ...
        return { symbol, price, rsi, rsi1h, atr, lsr, funding, delta, oi5m, oi15m, volume, volumeSpike, fundingAnomaly, anomalyDetected };
      } catch (e) {
...
async function main() {
  ...
  try {
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium Optmus Prime  Start...!'));
    await checkCoins();
