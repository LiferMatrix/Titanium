...
async function sendAlert1h2h(symbol, data) {
  const { ohlcv15m, ohlcv3m, ohlcv1h, ohlcvDiario, ohlcv4h, price, wpr2h, wpr1h, rsi1h, lsr, isOIRising5m, estocasticoD, estocastico4h, fundingRate, oi15m, ema34_3m, ema89_3m, previousEma34_3m, previousEma89_3m } = data;
  ...
  const isBuySignal = state.wprTriggerState[symbol]['1h_2h'].buyTriggered && 
                      isOIRising5m && 
                      ema34_3m > ema89_3m && 
                      previousEma34_3m <= previousEma89_3m &&
  ...
  if (isBuySignal) {
    try {
      await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, `🟢*WPR -✳️Compra/Reversão✳️*\n\n${alertText}`, {
        parse_mode: 'Markdown',
    ...
    try {
      await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, `🔴*WPR -🔻Correção/Exaustão🔻*\n\n${alertText}`, {
        parse_mode: 'Markdown',
        disable_web_page_preview: true
      }));
      if (!state.ultimoAlertaPorAtivo[symbol]) state.ultimoAlertaPorAtivo[symbol] = {};
...
async function checkConditions() {
  try {
    await limitConcurrency(config.PARES_MONITORADOS, async (symbol) => {
      ...
      const ohlcv3mRawFutures = getCachedData(`${cacheKeyPrefix}_3m`) || await withRetry(() => exchangeFutures.fetchOHLCV(symbol, '3m', undefined, config.EMA_89_PERIOD + 1));
      ...
      if (!wpr2hValues.length || !wpr1hValues.length || !rsi1hValues.length || !ema34_3mValues.length || !ema89_3mValues.length) {
      ...
      await sendAlert1h2h(symbol, {
        ...
        previousEma89_3m: ema89_3mValues[ema89_3mValues.length - 2] || 0
      });
    }, 5);
  } catch (e) {
...
async function main() {
  ...
  try {
    ...
    await withRetry(() => bot.api.sendMessage(config.TELEGRAM_CHAT_ID, '🤖 Titanium RESET WPR2 💹Start...'));
