...
const config = {
  ...
  RSI_OVERSOLD_THRESHOLD: 40, // Ajustado para teste
...
async function sendMonitorAlert(coins) {
  ...
  // Funding Rate mais negativos com OI 5m e 15m subindo
  const topLowFundingWithRisingOI = coins
    .filter(c => c.funding.current !== null && c.funding.current < 0 && c.oi5m.isRising && c.oi15m.isRising)
    .sort((a, b) => a.funding.current - b.funding.current)
    .slice(0, 3);

  // Funding Rate mais positivos com OI 5m e 15m caindo
  const topHighFundingWithFallingOI = coins
    .filter(c => c.funding.current !== null && c.funding.current > 0 && !c.oi5m.isRising && !c.oi15m.isRising)
    .sort((a, b) => b.funding.current - a.funding.current)
    .slice(0, 2);
  ...
  logger.info(`Funding negativo com OI subindo: ${topLowFundingWithRisingOI.length}, Funding positivo com OI caindo: ${topHighFundingWithFallingOI.length}, RSI baixo com OI subindo: ${topLowRsiWithRisingOI.length}, RSI alto com OI caindo: ${topHighRsiWithFallingOI.length}`);

  // Alerta para Funding Rate mais negativos com OI subindo
  if (topLowFundingWithRisingOI.length > 0) {
    let fundingLowAlertText = `🟢*Funding Rate Negativo com OI Subindo 🚀*\n\n`;
    fundingLowAlertText += await Promise.all(topLowFundingWithRisingOI.map(async (coin, i) => {
    ...
    logger.info(`Tamanho da mensagem de Funding Negativo com OI subindo: ${fundingLowAlertText.length} caracteres`);
    await sendTelegramMessage(fundingLowAlertText);
    logger.info('Alerta de moedas com Funding Negativo e OI subindo enviado com sucesso');
  }

  // Alerta para Funding Rate mais positivos com OI caindo
  if (topHighFundingWithFallingOI.length > 0) {
    let fundingHighAlertText = `🔴*Funding Rate Positivo com OI Caindo 📉*\n\n`;
    fundingHighAlertText += await Promise.all(topHighFundingWithFallingOI.map(async (coin, i) => {
      const tradingViewLink = `https://www.tradingview.com/chart/?symbol=BINANCE:${coin.symbol.replace('/', '')}&interval=15`;
      const deltaText = coin.delta.isBuyPressure ? `💹${format(coin.delta.deltaPercent)}%` : `⭕${format(coin.delta.deltaPercent)}%`;
      let lsrSymbol = '';
      if (coin.lsr !== null && !isNaN(coin.lsr)) {
    ...
    fundingHighAlertText += `\n☑︎ 🤖 Monitor - @J4Rviz`;

    logger.info(`Tamanho da mensagem de Funding Positivo com OI caindo: ${fundingHighAlertText.length} caracteres`);
    await sendTelegramMessage(fundingHighAlertText);
    logger.info('Alerta de moedas com Funding Positivo e OI caindo enviado com sucesso');
  }
  ...
  if (topLowFundingWithRisingOI.length === 0 && topHighFundingWithFallingOI.length === 0 && topLowRsiWithRisingOI.length === 0 && topHighRsiWithFallingOI.length === 0) {
