...
const TELEGRAM_BOT_TOKEN = '7633398974:AAH0A';
...
const QUALITY_WEIGHTS = {
    ...
    candlePatterns: 8             // NOVO: Peso para padrões de candles
};
...
// 6. FALLBACK PARA PADRÕES DE CANDLES
async function analyzeCandlePatternsWithFallback(symbol, isBullish, currentPrice) {
    try {
        const patterns = await analyzeCandlePatterns(symbol, isBullish, currentPrice);
        
        if (patterns.patterns.length > 0 || patterns.strongestPattern) {
            return patterns;
        }

        // FALLBACK: Análise simplificada do candle atual
        const candles = await getCandlesWithFallback(symbol, '15m', 5);
        if (candles.length >= 1) {
            ...
            let patternName = 'NEUTRO';
            let strength = 'FRACO';
            
            if (isBullish && isBullishCandle && bodySize > totalRange * 0.7) {
                patternName = 'CANDLE_FORTE_BULLISH';
                strength = 'MODERADO';
            } else if (!isBullish && !isBullishCandle && bodySize > totalRange * 0.7) {
                patternName = 'CANDLE_FORTE_BEARISH';
                strength = 'MODERADO';
            }
            
            // Usar EMA de volume como fallback
            const volumeHistory = candles.slice(-10).map(c => c.volume);
            ...
            fallbackSystem.recordFallback(
                'CandlePatterns', 
                'SIMPLE_ANALYSIS', 
                { symbol, patternName, volumeRatio }
            );
            ...
            return {
                patterns: [{
                    ...
                    strength: strength,
                    description: 'Análise simplificada do candle atual'
                }],
                strongestPattern: {
                    name: patternName,
                    type: isBullish ? 'BULLISH' : 'BEARISH',
                ...
                combinedScore: strength === 'MODERADO' ? 40 : 20,
                hasStrongPattern: strength === 'MODERADO',
                confirmation: volumeRatio > 1.0,
                volumeAnalysis: {
                    confirmed: volumeRatio > 1.0,
        ...
        // FALLBACK FINAL
        fallbackSystem.recordFallback('CandlePatterns', 'NO_PATTERNS', { symbol });
        ...
        return {
            ...
            confirmation: false,
            isFallback: true,
            fallbackType: 'NO_PATTERNS'
        };

    } catch (error) {
        console.log(`⚠️ Fallback CandlePatterns ${symbol}: ${error.message}`);
...
// 7. FALLBACK PARA FORÇA RELATIVA BTC
...
// =====================================================================
// 🛡️ SISTEMA DE RISK LAYER AVANÇADO COM ANÁLISE DE CANDLES E FALLBACKS
// =====================================================================
...
class SophisticatedRiskLayer {
    constructor() {
        ...
        this.riskFactors = {
            ...
            CANDLE_PATTERN_RISK: { weight: 1.3 },
            FALLBACK_RISK: { weight: 1.7 } // NOVO: Risco por uso de fallbacks
        };

        this.riskHistory = new Map();
        this.maxHistorySize = 100;

        console.log('🛡️  Risk Layer Sofisticado inicializado com análise de fallbacks');
    }
    ...
    generateWarnings(riskAssessment) {
        ...
        riskAssessment.factors.forEach(factor => {
            if (factor.score >= 2.5 && factor.type !== 'STOCHASTIC_TREND' && factor.type !== 'CANDLE_PATTERN' && factor.type !== 'FALLBACK') {
                warnings.push(`⚠️ ${factor.message}`);
            } else if (factor.score >= 2 && factor.type !== 'STOCHASTIC_TREND' && factor.type !== 'CANDLE_PATTERN' && factor.type !== 'FALLBACK') {
                warnings.push(`🔶 ${factor.message}`);
            }
        });
...
// =====================================================================
// 📤 FUNÇÃO ATUALIZADA PARA ENVIAR ALERTAS (COM FALLBACKS)
// =====================================================================
...
async function sendInitializationMessage(allSymbols) {
    try {
        ...
        const message = `
🚀 <b>TITANIUM ATIVADO COM SISTEMA DE FALLBACK ROBUSTO!</b>
...
// =====================================================================
// 📊 FUNÇÃO PARA CALCULAR QUALIDADE (COM PADRÕES DE CANDLES E FALLBACKS)
// =====================================================================
...
async function monitorSymbol(symbol) {
    try {
        ...
        // 🔹 NOVO: Analisar padrões de candles com fallback
        const candlePatternsData = await analyzeCandlePatternsWithFallback(symbol, isBullish, emaData.currentPrice);
...
async function mainBotLoop() {
    ...
    console.log(`\n TITANIUM ATIVADO COM SISTEMA DE FALLBACK ROBUSTO!`);
