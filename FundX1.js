...
// =====================================================================
// === CONFIGURAÇÃO COM STOP LOSS E TARGETS ===
// =====================================================================
const CONFIG = {
    TELEGRAM: {
    ...
    MONITOR: {
        ...
        RSI: {
            ...
            CVD_LSR_MAX_BUY: 1.6,
            CVD_LSR_MIN_SELL: 2.5
        },
        BOLLINGER: {
            PERIOD: 20,
            STD_DEVIATION: 2.2,
            TIMEFRAME: '15m',
            APPROACH_PERCENTAGE: 0.6
        },
        VOLATILITY: {
            MIN_ATR_PERCENT: 0.4,
            ATR_PERIOD: 14,
...
// =====================================================================
// === FUNÇÃO PARA CALCULAR SCORE CVD (COMPLETO) ===
// =====================================================================
function calculateCVDScore(cvdChangePercent, buySellRatio, lsr, divergenceCount, isBuy) {
    let score = 0;
    const absChange = Math.abs(cvdChangePercent);
    
    // 1. CVD INTENSIDADE (máx 30 pts)
    if (absChange >= 50) score += 30;
    else if (absChange >= 30) score += 25;
    else if (absChange >= 15) score += 20;
    else if (absChange >= 5) score += 10;
    
    // 2. BUY/SELL RATIO (máx 25 pts)
    if (isBuy) {
        if (buySellRatio < 0.8) score += 25;
        else if (buySellRatio < 1.0) score += 20;
        else if (buySellRatio < 1.2) score += 10;
        else score += 0;
    } else {
        if (buySellRatio > 1.5) score += 25;
        else if (buySellRatio > 1.3) score += 20;
    ...
    if (isBuy) {
        ...
        else if (lsr < 1.2) score += 10;
        else score += 0;
    } else {
        if (lsr > 2.5) score += 25;
        else if (lsr > 2.0) score += 20;
        else if (lsr > 1.5) score += 10;
        else score += 0;
    }
    
    // 4. DIVERGÊNCIAS (máx 20 pts)
    if (divergenceCount >= 5) score += 20;
    else if (divergenceCount >= 3) score += 15;
    else if (divergenceCount >= 2) score += 10;
    else score += 0;
    
    return score;
}

function getScoreEmojiAndText(score) {
    if (score >= 80) return { emoji: '🏆', text: 'EXCELENTE' };
    if (score >= 70) return { emoji: '✅', text: 'BOM' };
    if (score >= 60) return { emoji: '🟡', text: 'REGULAR' };
    if (score >= 50) return { emoji: '⚠️', text: 'FRACO' };
    return { emoji: '❌', text: 'RUIM' };
}

// =====================================================================
// === FUNÇÃO PARA VERIFICAR DOMINÂNCIA DE COMPRAS/VENDAS ===
// =====================================================================
function checkBuySellDominance(buyVolume, sellVolume, isBuySignal) {
    if (isBuySignal) {
        // Para COMPRA: precisa ter MAIS COMPRAS que VENDAS
        const isValid = buyVolume > sellVolume;
        const difference = buyVolume - sellVolume;
        const differencePercent = sellVolume > 0 ? (difference / sellVolume) * 100 : 100;
        return { isValid, difference, differencePercent, buyVolume, sellVolume };
    } else {
        // Para VENDA: precisa ter MAIS VENDAS que COMPRAS
        const isValid = sellVolume > buyVolume;
        const difference = sellVolume - buyVolume;
        const differencePercent = buyVolume > 0 ? (difference / buyVolume) * 100 : 100;
        return { isValid, difference, differencePercent, buyVolume, sellVolume };
    }
}

// =====================================================================
// === FUNÇÃO PARA VERIFICAR VOLUME DIRECIONAL ANORMAL (3 MINUTOS) ===
// =====================================================================
async function checkDirectionalVolume(symbol, isBuySignal) {
    try {
        const candles = await getCandles(symbol, CONFIG.MONITOR.VOLUME_3M.TIMEFRAME, CONFIG.MONITOR.VOLUME_3M.LOOKBACK_CANDLES + 10);
        if (!candles || candles.length < CONFIG.MONITOR.VOLUME_3M.VOLUME_MA_PERIOD + 5) {
            console.log(`⚠️ Dados insuficientes para volume 3m em ${symbol}`);
            return { isValid: false, volumeRatio: 0, direction: null, volumeAnomaly: false };
        }
        
        const currentCandle = candles[candles.length - 1];
        const previousCandles = candles.slice(-CONFIG.MONITOR.VOLUME_3M.LOOKBACK_CANDLES);
        
        // Calcular média de volume
        const volumes = previousCandles.map(c => c.volume);
        const avgVolume = volumes.slice(-CONFIG.MONITOR.VOLUME_3M.VOLUME_MA_PERIOD).reduce((a, b) => a + b, 0) / CONFIG.MONITOR.VOLUME_3M.VOLUME_MA_PERIOD;
        const currentVolume = currentCandle.volume;
        const volumeRatio = currentVolume / avgVolume;
        
        // Determinar direção do volume baseado no preço
        const priceChange = ((currentCandle.close - currentCandle.open) / currentCandle.open) * 100;
        const isBullishVolume = priceChange > 0 && volumeRatio >= CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER;
        const isBearishVolume = priceChange < 0 && volumeRatio >= CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER;
        
        let isValid = false;
        let direction = null;
        
        if (isBuySignal) {
            // Para COMPRA: precisa de volume ANORMAL COMPRADOR (bullish)
            isValid = isBullishVolume && volumeRatio >= CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER;
            direction = isValid ? 'COMPRADOR' : null;
        } else {
            // Para VENDA: precisa de volume ANORMAL VENDEDOR (bearish)
            isValid = isBearishVolume && volumeRatio >= CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER;
            direction = isValid ? 'VENDEDOR' : null;
        }
        
        const volumeAnomaly = volumeRatio >= CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER;
        
        if (!isValid) {
            const requiredDir = isBuySignal ? 'COMPRADOR' : 'VENDEDOR';
            console.log(`🚫 Volume 3m bloqueado para ${symbol}: ${volumeRatio.toFixed(2)}x (precisa ${requiredDir} e ≥ ${CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER}x)`);
        } else {
            console.log(`✅ Volume 3m aprovado para ${symbol}: ${volumeRatio.toFixed(2)}x (${direction})`);
        }
        ...
        log(`Erro ao verificar volume 3m para ${symbol}: ${error.message}`, 'error');
        return { isValid: false, volumeRatio: 0, direction: null, volumeAnomaly: false };
    }
}

// =====================================================================
// === FUNÇÃO STOP LOSS E TARGETS INTELIGENTE ===
// =====================================================================
function computeStopAndTargets(currentPrice, isGreenAlert, timeframe, candles) {
    const atrMultiplier = CONFIG.MONITOR.STOP_LOSS.ATR_MULTIPLIER[timeframe] || 3.0;
    const minStopPercent = CONFIG.MONITOR.STOP_LOSS.MIN_STOP_DISTANCE_PERCENT[timeframe] || 2.0;
    const maxStopPercent = CONFIG.MONITOR.STOP_LOSS.MAX_STOP_DISTANCE_PERCENT[timeframe] || 7.0;
    ...
    if (!candles || candles.length < 20) {
        ...
        let tp1, tp2, tp3;
        if (isGreenAlert) {
            tp1 = currentPrice + atr * CONFIG.MONITOR.TARGETS.TP1_MULTIPLIER;
            tp2 = currentPrice + atr * CONFIG.MONITOR.TARGETS.TP2_MULTIPLIER;
            tp3 = currentPrice + atr * CONFIG.MONITOR.TARGETS.TP3_MULTIPLIER;
        } else {
            tp1 = currentPrice - atr * CONFIG.MONITOR.TARGETS.TP1_MULTIPLIER;
            tp2 = currentPrice - atr * CONFIG.MONITOR.TARGETS.TP2_MULTIPLIER;
            tp3 = currentPrice - atr * CONFIG.MONITOR.TARGETS.TP3_MULTIPLIER;
        ...
        if (stopDistance < minStopPercent) {
            ...
            stopDistance = maxStopPercent;
        }
        
        const stopPercent = stopDistance;
        
        return { stop: finalStop, tp1, tp2, tp3, stopPercent };
    }
    ...
    if (isGreenAlert) {
        ...
        stopDistance = (stopPrice - currentPrice) / currentPrice * 100;
        if (stopDistance < minStopPercent) {
            stopPrice = currentPrice * (1 + minStopPercent / 100);
            stopDistance = minStopPercent;
        } else if (stopDistance > maxStopPercent) {
    ...
    return { stop: stopPrice, tp1, tp2, tp3, stopPercent: stopDistance };
}

// =====================================================================
// === FUNÇÃO PARA CALCULAR ATR PERCENTUAL (VOLATILIDADE) ===
// =====================================================================
async function calculateATRPercent(symbol, timeframe = '1h', period = 14) {
    try {
        ...
        log(`Erro ao calcular ATR para ${symbol}: ${error.message}`, 'error');
        return null;
    }
}
...
class RealCVDManager {
    ...
    getConnectedSymbols() {
        return Array.from(this.subscribedSymbols).filter(s => this.isConnected(s));
    }
...
// =====================================================================
// === MONITOR DE CONEXÃO E RECUPERAÇÃO AUTOMÁTICA ===
// =====================================================================
class ConnectionMonitor {
    constructor(cvdManagerInstance, fundingMemoryInstance) {
        this.cvdManager = cvdManagerInstance;
        this.fundingMemory = fundingMemoryInstance;
    ...
    startMonitoring() {
        ...
        console.log('🔍 Monitor de conexão iniciado (verifica a cada 30s)');
    }

    async checkConnection() {
        if (this.isRecovering) {
            console.log('⏳ Recuperação em andamento, aguardando...');
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                if (!this.isConnected) {
                    console.log('✅ Internet/API recuperada! Iniciando recuperação total...');
            ...
            if (this.isConnected) {
                console.log(`⚠️ Conexão perdida: ${error.message}`);
                this.isConnected = false;
            }
            
            this.recoveryAttempts++;
            
            if (this.recoveryAttempts >= this.maxRecoveryAttempts && !this.isRecovering) {
                console.log(`🔄 Tentativa ${this.recoveryAttempts} de recuperação...`);
    ...
    async attemptRecovery() {
        ...
        try {
            const symbols = Array.from(this.cvdManager.subscribedSymbols);
            console.log(`📡 Reconectando ${symbols.length} símbolos...`);
            
            for (const symbol of symbols) {
                this.cvdManager.unsubscribeFromSymbol(symbol);
            }
            
            await new Promise(resolve => setTimeout(resolve, 5000));
            
            const watched = this.fundingMemory.getWatchedSymbols();
            
            for (const item of [...watched.positive, ...watched.negative]) {
                this.cvdManager.subscribeToSymbol(item.fullSymbol);
            }
            
            rateLimiter.consecutiveErrors = 0;
            rateLimiter.currentDelay = rateLimiter.baseDelayMs;
    ...
    async fullRecovery() {
        ...
        try {
            ...
            if (result) {
                ...
                console.log('✅ Sistema totalmente recuperado!');
            }
        } catch (error) {
            console.log(`❌ Falha na recuperação completa: ${error.message}`);
        } finally {
            this.isRecovering = false;
        }
    }
    ...
    getStatus() {
        const connectedSymbols = this.cvdManager.getConnectedSymbols();
        return {
            isConnected: this.isConnected,
            connectedSymbols: connectedSymbols.length,
            totalSymbols: this.cvdManager.subscribedSymbols.size,
...
async function monitorCVDRSIAlerts() {
    ...
    for (const item of [...watched.positive, ...watched.negative]) {
        ...
        // =====================================================================
        // === VERIFICAÇÃO DE DOMINÂNCIA DE COMPRAS/VENDAS ===
        // =====================================================================
        const dominanceCheck = checkBuySellDominance(cvd.buyVolume, cvd.sellVolume, isUp);
        if (!dominanceCheck.isValid) {
            const required = isUp ? 'COMPRAS' : 'VENDAS';
            console.log(`🚫 Alerta CVD+RSI bloqueado para ${item.symbol}: ${required} não dominam (Compras: ${(cvd.buyVolume/1000000).toFixed(2)}M, Vendas: ${(cvd.sellVolume/1000000).toFixed(2)}M)`);
            continue;
        }
        
        // =====================================================================
        // === VERIFICAÇÃO DE VOLATILIDADE MÍNIMA (ATR PERCENTUAL) ===
        // =====================================================================
        const volatilityData = await calculateATRPercent(item.fullSymbol, CONFIG.MONITOR.VOLATILITY.TIMEFRAME, CONFIG.MONITOR.VOLATILITY.ATR_PERIOD);
        const minVolatility = CONFIG.MONITOR.VOLATILITY.MIN_ATR_PERCENT;
        
        if (!volatilityData || volatilityData.atrPercent < minVolatility) {
            const volMsg = volatilityData ? `${volatilityData.atrPercent.toFixed(2)}%` : 'indisponível';
            console.log(`🚫 Alerta CVD+RSI bloqueado para ${item.symbol}: Volatilidade ${volMsg} < ${minVolatility}% (mínimo exigido)`);
            continue;
        }
        
        // BUSCAR CLUSTERS PARA ESTE SYMBOL
        const clusters = await getClusterLevels(item.fullSymbol, item.price);
        
        // BUSCAR CANDLES PARA CALCULAR STOP LOSS E TARGETS
        const candles = await getCandles(item.fullSymbol, '1h', 100);
        ...
        if (isUp && rsi.value < CONFIG.MONITOR.RSI.CVD_RSI_BUY_THRESHOLD) {
            ...
            const bollingerTouch = await checkBollingerTouch(item.fullSymbol, 'buy');
            if (!bollingerTouch) {
                console.log(`🚫 Alerta CVD+RSI COMPRA bloqueado para ${item.symbol}: Sem toque na banda inferior do Bollinger`);
                continue;
            }
            
            // =====================================================================
            // === VERIFICAÇÃO DE VOLUME DIRECIONAL (3 MINUTOS) PARA COMPRA ===
            // =====================================================================
            const volumeCheck = await checkDirectionalVolume(item.fullSymbol, true);
            if (!volumeCheck.isValid) {
                console.log(`🚫 Alerta CVD+RSI COMPRA bloqueado para ${item.symbol}: Volume 3m ${volumeCheck.volumeRatio.toFixed(2)}x (precisa COMPRADOR ≥ ${CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER}x)`);
                continue;
            }
            ...
            if (fundingMemory.shouldAlertCVDRSI(item.fullSymbol, 'buy')) {
                ...
                // CALCULAR STOP LOSS E TARGETS PARA COMPRA (isGreenAlert = true)
                const stopData = computeStopAndTargets(item.price, true, '1h', candles);
                
                // CALCULAR SCORE CVD
                const divergenceCount = fundingMemory.getRSIDivergenceCount(item.fullSymbol).bullish;
                const score = calculateCVDScore(capped, cvd.buySellRatio, item.lsr, divergenceCount, true);
                const scoreInfo = getScoreEmojiAndText(score);
                
                alerts.push({ 
                    ...item, 
                    type: 'cvd_rsi_buy', 
                    action: 'COMPRA (CVD + RSI)', 
                    cvdChange: { changePercent: capped },
                    rsi1h: rsi.value, 
                    buySellRatio: cvd.buySellRatio, 
                    buyVolume: cvd.buyVolume, 
                    sellVolume: cvd.sellVolume,
                    totalTrades: cvd.totalTrades, 
                    lastPrice: cvd.lastPrice, 
                    alertNumber: alertNumber, 
                    bollingerTouch: 'inferior',
                    clusters: clusters,
                    stopLoss: stopData.stop,
                    stopLossPercent: stopData.stopPercent,
                    targets: { tp1: stopData.tp1, tp2: stopData.tp2, tp3: stopData.tp3 },
                    volatility: { atrPercent: volatilityData.atrPercent },
                    volume3m: { ratio: volumeCheck.volumeRatio, direction: volumeCheck.direction, priceChange: volumeCheck.priceChange },
                    dominance: { buyVolume: cvd.buyVolume, sellVolume: cvd.sellVolume, buyDominant: cvd.buyVolume > cvd.sellVolume },
                    score: score,
        ...
        if (isDown && rsi.value > CONFIG.MONITOR.RSI.CVD_RSI_SELL_THRESHOLD) {
            ...
            const bollingerTouch = await checkBollingerTouch(item.fullSymbol, 'sell');
            if (!bollingerTouch) {
                console.log(`🚫 Alerta CVD+RSI VENDA bloqueado para ${item.symbol}: Sem toque na banda superior do Bollinger`);
                continue;
            }
            
            // =====================================================================
            // === VERIFICAÇÃO DE VOLUME DIRECIONAL (3 MINUTOS) PARA VENDA ===
            // =====================================================================
            const volumeCheck = await checkDirectionalVolume(item.fullSymbol, false);
            if (!volumeCheck.isValid) {
                console.log(`🚫 Alerta CVD+RSI VENDA bloqueado para ${item.symbol}: Volume 3m ${volumeCheck.volumeRatio.toFixed(2)}x (precisa VENDEDOR ≥ ${CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER}x)`);
                continue;
            }
            ...
            if (fundingMemory.shouldAlertCVDRSI(item.fullSymbol, 'sell')) {
                ...
                // CALCULAR STOP LOSS E TARGETS PARA VENDA (isGreenAlert = false)
                const stopData = computeStopAndTargets(item.price, false, '1h', candles);
                
                // CALCULAR SCORE CVD
                const divergenceCount = fundingMemory.getRSIDivergenceCount(item.fullSymbol).bearish;
                const score = calculateCVDScore(capped, cvd.buySellRatio, item.lsr, divergenceCount, false);
                const scoreInfo = getScoreEmojiAndText(score);
                
                alerts.push({ 
                    ...item, 
                    type: 'cvd_rsi_sell', 
                    action: 'VENDA (CVD + RSI)', 
                    cvdChange: { changePercent: capped },
                    rsi1h: rsi.value, 
                    buySellRatio: cvd.buySellRatio, 
                    buyVolume: cvd.buyVolume, 
...
async function monitorCVDAndRSI() {
    ...
    for (const item of watched.positive) {
        ...
        if (cvd.direction === 'SELL') {
            ...
            // =====================================================================
            // === VERIFICAÇÃO DE DOMINÂNCIA DE COMPRAS/VENDAS PARA VENDA ===
            // =====================================================================
            const dominanceCheck = checkBuySellDominance(cvd.buyVolume, cvd.sellVolume, false);
            if (!dominanceCheck.isValid) {
                console.log(`🚫 Alerta COMPLETO VENDA bloqueado para ${item.symbol}: VENDAS não dominam (Compras: ${(cvd.buyVolume/1000000).toFixed(2)}M, Vendas: ${(cvd.sellVolume/1000000).toFixed(2)}M)`);
                continue;
            }
            ...
            if (bollinger && hasDiv && fundingMemory.shouldAlertCVD(item.fullSymbol, 'positive', cvd.cvdValue) && fundingMemory.shouldAlertBollinger(item.fullSymbol, 'sell')) {
                ...
                const volumeCheck = await checkDirectionalVolume(item.fullSymbol, false);
                if (!volumeCheck.isValid) {
                    console.log(`🚫 Alerta COMPLETO VENDA bloqueado para ${item.symbol}: Volume 3m ${volumeCheck.volumeRatio.toFixed(2)}x (precisa VENDEDOR ≥ ${CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER}x)`);
                    continue;
                }
                
                // CALCULAR SCORE CVD
                const score = calculateCVDScore(cvd.changePercent, cvd.buySellRatio, item.lsr, rsiData.totalBearishDivergences, false);
                const scoreInfo = getScoreEmojiAndText(score);
    ...
    for (const item of watched.negative) {
        ...
        if (cvd.direction === 'BUY') {
            ...
            // =====================================================================
            // === VERIFICAÇÃO DE DOMINÂNCIA DE COMPRAS/VENDAS PARA COMPRA ===
            // =====================================================================
            const dominanceCheck = checkBuySellDominance(cvd.buyVolume, cvd.sellVolume, true);
            if (!dominanceCheck.isValid) {
                console.log(`🚫 Alerta COMPLETO COMPRA bloqueado para ${item.symbol}: COMPRAS não dominam (Compras: ${(cvd.buyVolume/1000000).toFixed(2)}M, Vendas: ${(cvd.sellVolume/1000000).toFixed(2)}M)`);
                continue;
            }
            ...
            if (bollinger && hasDiv && fundingMemory.shouldAlertCVD(item.fullSymbol, 'negative', cvd.cvdValue) && fundingMemory.shouldAlertBollinger(item.fullSymbol, 'buy')) {
                ...
                const volumeCheck = await checkDirectionalVolume(item.fullSymbol, true);
                if (!volumeCheck.isValid) {
                    console.log(`🚫 Alerta COMPLETO COMPRA bloqueado para ${item.symbol}: Volume 3m ${volumeCheck.volumeRatio.toFixed(2)}x (precisa COMPRADOR ≥ ${CONFIG.MONITOR.VOLUME_3M.MIN_VOLUME_MULTIPLIER}x)`);
                    continue;
                }
                
                // CALCULAR SCORE CVD
                const score = calculateCVDScore(cvd.changePercent, cvd.buySellRatio, item.lsr, rsiData.totalBullishDivergences, true);
                const scoreInfo = getScoreEmojiAndText(score);
...
function formatCVDRSIAlert(alert) {
    ...
    const volatilityText = alert.volatility ? `📊 Vol: ${alert.volatility.atrPercent.toFixed(2)}%` : '';
    const volumeText = alert.volume3m ? `📈 Vol3m: ${alert.volume3m.ratio.toFixed(2)}x ${alert.volume3m.direction}` : '';
    
    let msg = `<i>${emoji} ${alert.symbol} - Preço: ${formatPrice(alert.lastPrice || alert.price)} </i>\n`;
    msg += `<i>🔹Alerta: ${alert.alertNumber} - ${dt.full}hs</i>\n`;
    msg += `<i>✨#Score: ${alert.scoreEmoji} ${alert.score}/100 (${alert.scoreText})</i>\n`;
    msg += `<i>LSR: ${alert.lsr.toFixed(2)} | Fund: ${alert.funding >= 0 ? '+' : ''}${alert.fundingPercent.toFixed(4)}%</i>\n`;
    msg += `<i>RSI 1h: ${alert.rsi1h.toFixed(1)} ${rsiStatus} | Trades: ${alert.totalTrades}</i>\n`;
    ...
    if (volatilityText) msg += `<i>${volatilityText}</i>\n`;
    if (volumeText) msg += `<i>${volumeText}</i>\n`;
    
    // STOP LOSS E TARGETS
    if (alert.stopLoss) {
        const stopPercent = alert.stopLossPercent ? alert.stopLossPercent.toFixed(2) : '?';
        msg += `<i>Alvo 1: ${formatPrice(alert.targets.tp1)}</i> | `;
        msg += `<i>Alvo 2: ${formatPrice(alert.targets.tp2)}</i> | `;
        msg += `<i>Alvo 3: ${formatPrice(alert.targets.tp3)}</i> | `;
        msg += `<i>⛔Stop : ${formatPrice(alert.stopLoss)} (${stopPercent}%)</i>\n`;
    }
    ...
    if (clusters.supports && clusters.supports.length > 0) {
        for (let i = 0; i < clusters.supports.length; i++) {
            ...
            suporteText += `${formatPrice(s.price)} (${s.count}x) ${forca}`;
    ...
    if (clusters.resistances && clusters.resistances.length > 0) {
        for (let i = 0; i < clusters.resistances.length; i++) {
            ...
            resistenciaText += `${formatPrice(r.price)} (${r.count}x) ${forca}`;
...
function formatCompleteAlert(alert) {
    ...
    msg += `<i>✨#Score: ${alert.scoreEmoji} ${alert.score}/100 (${alert.scoreText})</i>\n`;
    msg += `<i>LSR: ${alert.lsr.toFixed(2)} | Fund: ${alert.funding >= 0 ? '+' : ''}${alert.fundingPercent.toFixed(4)}%</i>\n`;
    msg += `<i>RSI 1h: ${rsiValue ? rsiValue.toFixed(1) : '-'} ${rsiStatus} | Trades: ${alert.cvd.totalTrades}</i>\n`;
    ...
    if (isBuy) {
        if (alert.rsiDivergences.bullishCount > 0) {
            msg += `<i>📈 Alta: ${alert.rsiDivergences.bullishCount}/${CONFIG.MONITOR.RSI.MIN_DIVERGENCES_REQUIRED} ${alert.rsiDivergences.bullishTimeframes.join(', ') || ''}</i>\n`;
        } else {
            msg += `<i>📈 Alta: 0/${CONFIG.MONITOR.RSI.MIN_DIVERGENCES_REQUIRED} divergências em Nenhum</i>\n`;
        }
    } else {
        if (alert.rsiDivergences.bearishCount > 0) {
            msg += `<i>📉 Baixa: ${alert.rsiDivergences.bearishCount}/${CONFIG.MONITOR.RSI.MIN_DIVERGENCES_REQUIRED} ${alert.rsiDivergences.bearishTimeframes.join(', ') || ''}</i>\n`;
        } else {
            msg += `<i>📉 Baixa: 0/${CONFIG.MONITOR.RSI.MIN_DIVERGENCES_REQUIRED} </i>\n`;
        }
    }
    msg += `<i>"Não é recomendação de investimento"</i>\n`;
    msg += `<i>🤖 Titanium Prime X by  @J4Rviz</i>`;
...
function formatListMessage(positive, negative) {
    ...
    let msg = `\n<i> 🎯Scanner: ${dt.full}hs...by @J4Rviz Technology  </i>\n`;
    msg += `<i>🔴 Titanium Setup Bear  </i>\n`;
...
async function startMonitor() {
    ...
    const connectionMonitor = new ConnectionMonitor(cvdManager, fundingMemory);
    
    // Mostrar status a cada hora
    setInterval(() => {
        const status = connectionMonitor.getStatus();
        if (!status.isConnected || status.connectedSymbols < status.totalSymbols) {
            console.log(`📡 Status conexão: Conectado=${status.isConnected}, Símbolos=${status.connectedSymbols}/${status.totalSymbols}, Recuperando=${status.isRecovering}`);
        }
    }, 3600000);
